...
- **cannot complete the full advertised co-op campaign flow** (the dev and packaged WebSocket transport now dispatch through the server host, but co-op create / proposal / launch route wiring is still staged),
...
| C-4 | product | **Resolved 2026-06-21 by `close-campaign-economic-loop`: repair tickets now advance to completion.** `repairProgressProcessor` is registered after the queue builder, consumes daily tech-hours, blocks on missing parts, drains `partsInventory`, restores recorded armor/structure against `unitMaxStates`, recomputes readiness, and keeps completed tickets idempotent. Evidence: `repairProgressProcessor.test.ts`, `repairQueueBuilderProcessor.test.ts`, focused campaign economy Jest set.                   | `src/lib/campaign/processors/repairProgressProcessor.ts` · `src/lib/campaign/processors/processorRegistration.ts` · `mekhq Part.succeed/Campaign.fixPart`                                                                           |
| C-5 | product | **Closed for transport dispatch; co-op wiring staged.** Dev WebSocket transport now binds authenticated sockets through `bindMultiplayerSocketConnection`, `MatchHostRegistry`, and `ServerMatchHost`; unknown-match/binding failures close with typed terminal envelopes instead of the Wave-2 stub path. Co-op create/proposal/launch wiring remains staged in tasks 5.1-5.4.                                                                                                                                      | `server.js` · `src/lib/multiplayer/server/bindMultiplayerSocketConnection.ts` · `openspec/changes/reconcile-multiplayer-coop-reality/tasks.md`                                                                                      |
| C-6 | product | **Closed for local packaged start.** `npm run build` hydrates `.next/standalone/server.js` with the multiplayer-aware custom server, removes the shadowing Next socket API route, and `npm run validate:multiplayer:packaged-socket` proves `npm run start` accepts a WebSocket upgrade and streams replay. Docker/Electron use the same hydration path and should keep deploy/runtime smoke coverage.                                                                                                               | `package.json` · `server.js` · `scripts/hydrate-next-standalone-multiplayer-server.mjs` · `scripts/validate-multiplayer-packaged-socket.mjs`                                                                                        |
| C-7 | product | **SoT honest; wiring staged.** Co-op create no longer mints a dead local-only room code; the UI surfaces co-op hosting as unavailable until server-side match registration is wired. Live create/register remains staged in task 5.1.                                                                                                                                                                                                                                                                                | `src/pages/gameplay/campaigns/index.tsx:150` · `openspec/changes/reconcile-multiplayer-coop-reality/tasks.md`                                                                                                                       |
| C-8 | product | **SoT honest; wiring staged.** The default co-op proposal transport no longer leaves a guest action pending forever; it resolves as mechanically rejected while the live `CampaignSyncSession`/`CampaignGmArbiter` transport is still staged in task 5.2.                                                                                                                                                                                                                                                            | `src/components/campaign/coop/CampaignCoopRouteSurface.tsx:216` · `openspec/changes/reconcile-multiplayer-coop-reality/tasks.md`                                                                                                    |
...
- **E-2 (high, roadmap)** Bundled multiplayer transport is no longer the Wave-2 stub; the remaining roadmap risk is that co-op create / proposal / launch route wiring is still staged while the transport core is reachable (see C-5/C-6 and tasks 5.1-5.4).

### Cluster M-CAMP — Campaign economic loop (`product`/`parity`)

- **M-1 (high) — resolved 2026-06-21 by `close-campaign-economic-loop`** Salvage acceptance now materializes awarded units onto the root force and awarded parts into the shared `partsInventory`. Evidence: `campaignBayActions.test.ts`. `campaignBayActions.ts` · `mekhq ResolveScenarioTracker`.
- **M-2 (high) — resolved 2026-06-21 by `close-campaign-economic-loop`** Acquisition delivery now inserts delivered parts into the same `partsInventory` pool repair completion reads and consumes. Evidence: `acquisitionProcessor.test.ts`, `repairProgressProcessor.test.ts`. `acquisitionProcessor.ts` · `mekhq Quartermaster.addPart`.
- **M-3 (high) — resolved 2026-06-21 by `close-campaign-economic-loop`** Campaign prestige morale now filters `recentlyAppliedOutcomes` to the one-campaign-month window and the writer prunes old outcomes. Evidence: `moraleProcessor.test.ts`. `prestige/gatherMoraleSignals.ts` · `MHQMorale.java:422`.
- **M-4 (high) — resolved 2026-06-21 by `close-campaign-economic-loop`** Server and store persistence now round-trip `partsInventory`, `repairQueue`, `salvageAllocations`/reports, `pendingBattleOutcomes`, `processedBattleIds`, and `recentlyAppliedOutcomes`. Evidence: `serializeCampaign.test.ts`, `campaignPersistenceRoundTrip.test.ts`. `persistence/serializeCampaign.ts`.
- **M-5 (high) — resolved 2026-06-21 by `close-campaign-economic-loop`** Medicine skill is sourced from the roster/statblock/pilot model with fallback 7, and shorthanded penalties derive from assigned patient load vs capacity. Evidence: `standardMedical.test.ts`. `medical/standardMedical.ts` · `mekhq MedicalController.healPerson`.

### Cluster MP — Multiplayer/co-op truthfulness (`roadmap`/`product`)

- **MP-1 (high)** **SoT honest; wiring staged.** The active `reconcile-multiplayer-coop-reality` change re-gates networked/co-op requirements to an explicit not-yet-wired state, sweeps reachable archived `@spec` annotations to canonical specs, and keeps live transport/co-op launch work in staged task groups 3-5.
- **MP-2 (high->med)** **SoT honest; wiring staged.** Co-op mission launch is still not wired: `otherChoice` remains unsynced and launch still needs the staged task 5.3/5.4 path through `launchCoopMission`. The spec now names that as staged live-transport work instead of implying it is already delivered. `.../missions/[missionId]/launch.tsx:58`.
...
- [resolved 2026-06-21 by `close-campaign-economic-loop`] Salary calculation applies rank pay multiplier on top of experience/secondary-role salary; senior personnel now bill above base-rank peers. Evidence: `salaryService.test-helpers.ts`. `lib/finances/salaryService.ts` · `Person.java:4733`.
- [resolved 2026-06-21 by `close-campaign-economic-loop`] Kill XP now passes the actual after-action kill count into `awardKillXP`; a 5-kill ace earns more than a 0-kill survivor. Evidence: `postBattleProcessor.retryIdempotency.test.ts`. `postBattleProcessor.helpers.ts`.
- [resolved 2026-06-21 by `close-campaign-economic-loop`] Activity-log IDs now use a collision-free day discriminator so multiple daily logs are retained. Evidence: `campaignBayActions.test.ts`/focused store coverage. `useCampaignStore.dayActions.ts`.
- [resolved 2026-06-21 by `close-campaign-economic-loop`] `purchaseUnit` validates funds before mutation, debits the offer cost, adds the unit to the root force, and rejects insufficient funds without mutation. Evidence: `unitMarket.test.ts`. `lib/campaign/markets/unitMarket.ts` · `mekhq AbstractUnitMarket`.
...
- [resolved 2026-06-21] Match creation now has a per-host lobby cap plus expired-lobby TTL reap before `store.createMatch`; `/api/multiplayer/auth/token` is Zod-validated and KDF-rate-limited. `api/multiplayer/matches/index.ts:248`; `api/multiplayer/auth/token.ts:103`.
- [partial 2026-06-21] API-boundary validation has started on the multiplayer abuse surface: token and match-create bodies now use shared Zod schemas. Remaining API routes are outside this MP checkpoint. `api/multiplayer/matches/index.ts:84`.
- [resolved 2026-06-21] Lobby reconnects are bounded and stub-close / reconnect-limit failures render a terminal "multiplayer unavailable" panel. `multiplayer/lobby/[roomCode].tsx:11`.
