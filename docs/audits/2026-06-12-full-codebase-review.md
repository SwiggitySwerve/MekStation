...
- **loses an in-progress battle on refresh** (`loadSession` throws "Session not found" for any real game id),
...
1. **Electron desktop shell — renderer-reachable arbitrary file I/O.** `read-file`/`write-file`/`restore-backup` IPC handlers (`desktop/electron/main.ipc.ts:158/170/241`) pass a raw renderer-supplied `filePath` straight to `fs` with no validation, sandbox root, or allowlist. The renderer is a full Next.js app over HTTP with **no CSP** (`next.config.ts` has no `headers()`), **no `will-navigate` guard**, and `setWindowOpenHandler` blindly `shell.openExternal(url)`. `nodeIntegration:false`/`contextIsolation:true` are correctly set, so this is the residual gap — but in a signed, auto-updating, distributed binary it is a local-code-execution / exfiltration vector. **Highest-severity item the sweep did not otherwise surface.**
2. **Desktop is invisible to the verification gate.** `tsconfig.json` excludes `desktop/**`; CI only `tsc`-compiles + `electron-builder --dir` packages it; the desktop's own Jest suite is **never invoked** in any workflow. A whole platform target's correctness is gated only by "compiles + packages."
