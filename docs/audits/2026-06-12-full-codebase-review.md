...
| C-5 | product | **SoT honest; wiring staged.** WebSocket transport remains not-yet-wired: the `connection` handler still closes every socket with the Wave-2 stub path, and no networked match can run yet. The active `reconcile-multiplayer-coop-reality` spec re-gates this as a not-yet-wired contract and leaves live `ServerMatchHost` dispatch in staged tasks 3.1-3.3.                                                                                                                                                       | `server.js:242` · `openspec/changes/reconcile-multiplayer-coop-reality/tasks.md`                                                                                                                                                    |
| C-6 | product | **SoT honest; wiring staged.** Packaged Docker/Electron builds still run Next's `output:'standalone'` server with no WebSocket upgrade handler. The active spec now names the packaged-server upgrade-handler prerequisite and leaves the deploy-path decision in staged tasks 4.1-4.2.                                                                                                                                                                                                                              | `package.json:12` + `next.config.ts:89` · `openspec/changes/reconcile-multiplayer-coop-reality/tasks.md`                                                                                                                            |
...
| Change                                    | Covers                                                                                                                                                                                                                                                                             |
| ----------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `reconcile-multiplayer-coop-reality`      | In progress: C-5/C-6 remain live-transport/deploy-path wiring tasks; C-7/C-8 now fail honestly instead of dead-ending; MP-1 SoT is re-gated; MP-2 remains staged through launch-sync tasks; match-store cap, token KDF throttle, API Zod, and reconnect UX guardrails have landed. |
