...
export const BATTLEMECH_VALIDATION_REQUIREMENT_SUPPORT = {
  ...
  'physical-core-actions': helperOnly(
    ...
    'Physical action maps, runner behavior tests, event-sourced physical resolution, the MegaMek physical action class scope catalog, and the source-checked legality gate catalog cover punch, kick, push, charge, death from above, active TSM damage, underwater modifiers, successful push/charge/DFA hit displacement, charge/DFA miss displacement, runner automatic charge/DFA selection from movement state, shared same-board rejection, shared evading-attacker rejection, shared cargo-interaction attacker rejection, shared transported-passenger target rejection, shared swarming-target rejection, shared target-making-DFA rejection, shared airborne-target rejection, shared building-occupancy rejection, push attacker/target Mek unit-type rejection, push quad BattleMech rejection, push airborne-attacker rejection, push rear-flipped-arm rejection, push displacement-state/counter-push rejection, charge standing-Mek target rejection, charge non-Mek-to-infantry/ProtoMech target rejection, charge elevation-overlap rejection, charge/DFA target movement-complete/immobile rejection, charge/DFA displacement-state rejection, DFA infantry-family attacker rejection, DFA target-inside-building rejection, push both-arms-present rejection, push arm-fired helper/session/runner weapon-location rejection, and explicit scope splits for extra MegaMek physical classes',
    'BattleMech-applicable MegaMek classes such as brush-off, thrash, trip, grapple, break grapple, and jump-jet attacks remain unsupported; remaining stricter push/charge/DFA legality gates are cataloged but still unsupported',
    [
  ...
  'physical-self-risk': helperOnly(
    'physical-self-risk',
    'Runner and event-sourced physical behavior cover charge/DFA target damage, self-damage, successful push/charge/DFA displacement, charge/DFA miss displacement, source-backed immediate DFA miss fall damage/prone timing, DFA impossible-displacement destruction, source-backed successful-DFA attacker PSR +4, miss consequences, and PSR queueing',
    'Blocked charge displacement semantics, missed-DFA pilot-damage avoidance checks, and stale grid occupancy after displacement still need coverage',
    [
