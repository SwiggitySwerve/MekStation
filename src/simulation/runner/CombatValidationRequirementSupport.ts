...
export const BATTLEMECH_VALIDATION_REQUIREMENT_SUPPORT = {
  ...
  'special-weapon-families': helperOnly(
    ...
    'Several family-specific mechanics are helper-only until remaining iNarc ECM sensor effects, automatic C3 network assembly from hydrated equipment, AMS defender choice/arc rules, or Artemis exact-link/Nova-network/damage-lifecycle edges are wired',
    [
  ...
  'movement-enhancements': helperOnly(
    ...
    'Movement enhancement support catalogs MASC, supercharger, source-backed active TSM movement validation, source-backed standard MASC/Supercharger fixed failure target numbers from explicit prior-use state, and source-backed Partial Wing jump MP/heat validation against combat movement behavior',
    'MASC and supercharger remain helper-only until MovementType.Sprint, activation intents/wire payloads, alternate MASC option tables, IndustrialMek/support-unit supercharger roll adjustment, automatic prior-use turn-counter lifecycle, Edge rerolls, and failure critical-slot damage are wired; Partial Wing atmosphere and damaged critical-slot lifecycle refinements remain explicit gaps until combat state hydrates them',
  ...
  'physical-core-actions': helperOnly(
    ...
    'Physical action maps, runner behavior tests, event-sourced physical resolution, the MegaMek physical action class scope catalog, and the source-checked legality gate catalog cover punch, kick, push, charge, death from above, active TSM damage, underwater modifiers, successful push/charge/DFA hit displacement, source-backed two-level BattleMech displacement elevation caps, runner same-phase displacement grid occupancy refresh, blocked successful-charge displacement no-op/no-PSR semantics, charge/DFA miss displacement, runner automatic charge/DFA selection from movement state, shared same-board rejection, shared retreated/ejected targetability removal/rejection, shared evading-attacker rejection, shared cargo-interaction attacker rejection, shared transported-passenger target rejection, shared swarming-target rejection, shared target-making-DFA rejection, shared airborne-target rejection, shared building-occupancy rejection, helper-level invalid physical hex target rejection, source-backed gun-emplacement automatic success for punch/kick/DFA/melee targets, source-backed BattleMech charge gun-emplacement target-class rejection, push attacker/target Mek unit-type rejection, push quad BattleMech rejection, push airborne-attacker rejection, push rear-flipped-arm rejection, push displacement-state/counter-push rejection, push building/fuel-tank helper rejection, helper-level charge/DFA building and fuel-tank target rejection, charge standing-Mek target rejection, charge non-Mek-to-infantry/ProtoMech target rejection, charge elevation-overlap rejection, charge/DFA target movement-complete/immobile rejection, charge/DFA displacement-state rejection, helper-level DFA mechanical jump booster rejection, DFA infantry-family attacker rejection, helper-level DFA VTOL/WIGE elevation reach, DFA target-inside-building rejection, push both-arms-present rejection, push arm-fired helper/session/runner weapon-location rejection, and explicit scope splits for extra MegaMek physical classes',
    'BattleMech-applicable MegaMek classes such as brush-off, thrash, trip, grapple, break grapple, and jump-jet attacks remain unsupported; non-unit physical target command paths, non-unit building/fuel-tank physical target resolution, displacement domino-chain/friendly-avoidance/drop-ship-radius edges, VTOL/WIGE state hydration, mechanical jump booster step hydration, and remaining stricter push/charge/DFA legality gates are cataloged but not runtime-integrated',
    [
  ...
  'psr-trigger-catalog': helperOnly(
    'psr-trigger-catalog',
    'PSR trigger support catalogs damage, leg/actuator/gyro/engine, kicked, charged, DFA, pushed, shutdown, standing, terrain, skid, MASC, and supercharger triggers, including source-backed standard MASC/Supercharger fixed failure target numbers from explicit prior-use state',
    'Building-collapse remains helper-only; MASC and supercharger still lack alternate MASC option tables, IndustrialMek/support-unit supercharger roll adjustment, automatic prior-use turn-counter lifecycle, Edge rerolls, and failure critical-slot damage after runner movement queues explicit active-run triggers',
