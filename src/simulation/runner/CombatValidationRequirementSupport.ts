...
export const BATTLEMECH_VALIDATION_REQUIREMENT_SUPPORT = {
  ...
  'physical-core-actions': helperOnly(
    ...
    'Physical action maps, runner behavior tests, event-sourced physical resolution, the MegaMek physical action class scope catalog, and the source-checked legality gate catalog cover punch, kick, push, charge, death from above, active TSM damage, underwater modifiers, successful push/charge/DFA hit displacement, charge/DFA miss displacement, runner automatic charge/DFA selection from movement state, shared evading-attacker rejection, shared transported-passenger target rejection, shared swarming-target rejection, shared target-making-DFA rejection, push attacker/target Mek unit-type rejection, push quad BattleMech rejection, push rear-flipped-arm rejection, charge standing-Mek target rejection, charge non-Mek-to-infantry/ProtoMech target rejection, charge elevation-overlap rejection, charge target movement-complete/immobile rejection, DFA infantry-family attacker rejection, push both-arms-present rejection, push arm-fired helper/session rejection, and explicit scope splits for extra MegaMek physical classes',
