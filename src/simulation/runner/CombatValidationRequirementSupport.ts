...
export const BATTLEMECH_VALIDATION_REQUIREMENT_SUPPORT = {
  ...
  'physical-core-actions': helperOnly(
    ...
    'Physical action maps, runner behavior tests, event-sourced physical resolution, the MegaMek physical action class scope catalog, and the source-checked legality gate catalog cover punch, kick, push, charge, death from above, active TSM damage, underwater modifiers, source-backed target-evasion physical to-hit, successful push/charge/DFA hit displacement, source-backed two-level BattleMech displacement elevation caps, source-backed impassable-terrain and overgrown woods/jungle displacement rejection, runner same-phase displacement grid occupancy refresh, blocked successful-charge displacement no-op/no-PSR semantics, charge/DFA miss displacement, runner automatic charge/DFA selection from movement state, shared same-board rejection, shared retreated/ejected targetability removal/rejection, shared evading-attacker rejection, shared cargo-interaction attacker rejection, shared transported-passenger target rejection, shared swarming-target rejection, shared target-making-DFA rejection, shared airborne-target rejection, shared building-occupancy rejection, helper-level invalid physical hex target rejection, source-backed gun-emplacement automatic success for punch/kick/DFA/melee targets, source-backed selected-arm-missing punch rejection, source-backed missing-leg kick rejection, source-backed prone charge-attacker rejection, source-backed BattleMech charge gun-emplacement target-class rejection, push attacker/target Mek unit-type rejection, push quad BattleMech rejection, push airborne-attacker rejection, push rear-flipped-arm rejection, push displacement-state/counter-push rejection, push building/fuel-tank helper rejection, helper-level charge/DFA building and fuel-tank target rejection, charge standing-Mek target rejection, charge non-Mek-to-infantry/ProtoMech target rejection, charge elevation-overlap rejection, charge/DFA target movement-complete/immobile rejection, charge/DFA displacement-state rejection, DFA mechanical jump booster movement-step rejection, DFA infantry-family attacker rejection, DFA DropShip target rejection, DFA VTOL elevation reach with hydrated jump MP/elevation context in eligibility, event-sourced declaration/resolution, runner resolution, and automatic runner selection, DFA target-inside-building rejection, push both-arms-present rejection, push arm-fired helper/session/runner weapon-location rejection, and source-backed explicit scope splits for extra MegaMek physical classes',
    'BattleMech-applicable MegaMek classes such as brush-off, thrash, trip, grapple, break grapple, and jump-jet attacks remain unsupported; non-unit physical target command paths, non-unit building/fuel-tank physical target resolution, displacement domino-chain/friendly-avoidance/drop-ship-radius edges, runner WIGE motion-state hydration, and remaining stricter push/charge/DFA legality gates are cataloged but not runtime-integrated',
    [
