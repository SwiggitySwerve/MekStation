...
export const BATTLEMECH_VALIDATION_REQUIREMENT_SUPPORT = {
  ...
  'movement-enhancements': helperOnly(
    ...
    'MASC and supercharger remain helper-only until MovementType.Sprint, alternate MASC option tables, IndustrialMek/support-unit supercharger roll adjustment, Edge rerolls, separate first-step equipment-check timing, and non-BattleMech Supercharger motive-damage branches are wired; Partial Wing atmosphere and damaged critical-slot lifecycle refinements remain explicit gaps until combat state hydrates them',
  ...
  'psr-trigger-catalog': helperOnly(
    ...
    'Building-collapse remains helper-only; swamp bog-down is tracked as a terrain/stuck-state gap rather than a fall PSR; MASC and supercharger still lack alternate MASC option tables, IndustrialMek/support-unit supercharger roll adjustment, Edge rerolls, separate first-step equipment-check timing, and non-BattleMech Supercharger motive-damage branches after runner movement queues explicit active-run triggers',
