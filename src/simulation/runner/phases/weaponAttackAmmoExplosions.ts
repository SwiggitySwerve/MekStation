...
export interface IApplyCritAmmoExplosionsOptions {
  readonly currentState: IGameState;
  readonly events: IGameEvent[];
  readonly gameId: string;
  readonly unitId: string;
  readonly targetId: string;
  readonly damageResult: IResolveDamageResult;
  readonly d6Roller: () => number;
  readonly weaponsByUnit?: ReadonlyMap<string, readonly IWeapon[]>;
  readonly critUnitDestroyed: boolean;
  readonly critDestructionCause: DestructionCause;
}

export interface IApplyCritAmmoExplosionsResult {
  readonly currentState: IGameState;
  readonly critUnitDestroyed: boolean;
  readonly critDestructionCause: DestructionCause;
}

export function applyCritAmmoExplosions(
  options: IApplyCritAmmoExplosionsOptions,
): IApplyCritAmmoExplosionsResult {
  ...
  const criticalEvents = damageResult.criticalEvents ?? [];
    ...
    events.push(
      createGameEvent(
        ...
        GamePhase.WeaponAttack,
        {
          ...
          ...(explosion.binId !== undefined ? { binId: explosion.binId } : {}),
