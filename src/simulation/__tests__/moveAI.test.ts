...
import type { IHexCoordinate, IMovementCapability, IUnitPosition, IHex, IHexGrid } from '@/types/gameplay';

function createMockGrid(radius: number = 5, occupiedHexes: IHexCoordinate[] = []): IHexGrid {
...
describe('MoveAI', () => {
  describe('getValidMoves', () => {
    it('should return moves within MP range', () => {
      ...
      const moves = moveAI.getValidMoves(grid, position, MovementType.Walk, capability);
    ...
    it('should respect running MP when movement type is run', () => {
      ...
      const runMoves = moveAI.getValidMoves(grid, position, MovementType.Run, capability);
      const walkMoves = moveAI.getValidMoves(grid, position, MovementType.Walk, capability);
    ...
    it('should respect jump MP when movement type is jump', () => {
      ...
      const jumpMoves = moveAI.getValidMoves(grid, position, MovementType.Jump, capability);
      
      expect(jumpMoves.length).toBeGreaterThan(0);
      const maxDistance = Math.max(...jumpMoves.map((m: IMove) => 
        Math.max(
    ...
    it('should not include occupied hexes', () => {
      const moveAI = new MoveAI(DEFAULT_BEHAVIOR);
      const grid = createMockGrid(5, [{ q: 1, r: 0 }, { q: 0, r: 1 }]);
      const position = createUnitPosition(0, 0);
      const capability = createMovementCapability(2);
    ...
    it('should set correct movement type in returned moves', () => {
      ...
      walkMoves.forEach((m: IMove) => expect(m.movementType).toBe(MovementType.Walk));
      runMoves.forEach((m: IMove) => expect(m.movementType).toBe(MovementType.Run));
    });
  });
  ...
  describe('selectMove', () => {
    it('should return a move from the provided list', () => {
      ...
      const moves: IMove[] = [
        { destination: { q: 1, r: 0 }, facing: Facing.North, movementType: MovementType.Walk, mpCost: 1, heatGenerated: 1 },
        { destination: { q: 0, r: 1 }, facing: Facing.South, movementType: MovementType.Walk, mpCost: 1, heatGenerated: 1 },
        { destination: { q: -1, r: 0 }, facing: Facing.Southeast, movementType: MovementType.Walk, mpCost: 1, heatGenerated: 1 },
      ];
    ...
    it('should be deterministic with same seed', () => {
      ...
      const moves: IMove[] = [
        ...
        { destination: { q: 0, r: 1 }, facing: Facing.South, movementType: MovementType.Walk, mpCost: 1, heatGenerated: 1 },
        { destination: { q: -1, r: 0 }, facing: Facing.Southeast, movementType: MovementType.Walk, mpCost: 1, heatGenerated: 1 },
        { destination: { q: 0, r: -1 }, facing: Facing.Southwest, movementType: MovementType.Walk, mpCost: 1, heatGenerated: 1 },
        { destination: { q: 1, r: -1 }, facing: Facing.Northeast, movementType: MovementType.Walk, mpCost: 1, heatGenerated: 1 },
      ];
