      ...
      (set, get) => ({
        ...
        setTonnage: (tonnage) =>
          set((state) => {
            ...
            let updatedEquipment = filterOutEnhancementEquipment(
              state.equipment,
            );
        ...
        setInternalStructureType: (type) =>
          set((state) => {
            ...
            const structureEquipment =
              createInternalStructureEquipmentList(type);
        ...
        setArmorType: (type) =>
          set((state) => {
            ...
            const newPointsPerTon =
              getArmorDefinition(type)?.pointsPerTon ?? 16;
            const maxUsefulTonnage = ceilToHalfTon(
              maxTotalArmor / newPointsPerTon,
            );
            const cappedTonnage = Math.min(
              state.armorTonnage,
              maxUsefulTonnage,
            );
        ...
        addEquipment: (item: IEquipmentItem) => {
          ...
          if (item.variableEquipmentId) {
            ...
            if (
              ...
              const directFireTonnage =
                calculateDirectFireWeaponTonnage(weaponIds);
          ...
          set((state) => {
            ...
            const updatedEquipment =
              recalculateTargetingComputers(newEquipment);
            return {
              equipment: updatedEquipment,
              isModified: true,
              lastModifiedAt: Date.now(),
