...
export function mapHeatSinkType(
  source: string,
  techBase?: TechBase,
): HeatSinkType {
  const normalized = source.trim();
...
export function mapArmorType(
  ...
  if (
    lowerSource.includes('reflective') ||
    lowerSource.includes('laser-reflective')
  ) {
...
export function mapCockpitType(source: string): CockpitType {
  ...
  if (
    lowerSource.includes('superheavy') ||
    lowerSource.includes('super-heavy')
  ) {
