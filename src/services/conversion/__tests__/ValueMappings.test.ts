...
describe('ValueMappings', () => {
  ...
  describe('mapEngineType', () => {
    ...
    it('should use fuzzy matching with tech base for XL variants', () => {
      ...
      expect(mapEngineType('some xl engine', TechBase.CLAN)).toBe(
        EngineType.XL_CLAN,
      );
      expect(mapEngineType('some xl engine', TechBase.INNER_SPHERE)).toBe(
        EngineType.XL_IS,
      );
    });
  ...
  describe('mapStructureType', () => {
    it('should map standard structure', () => {
      ...
      expect(mapStructureType('Clan Standard')).toBe(
        InternalStructureType.STANDARD,
      );
    });
    ...
    it('should use fuzzy matching with tech base for Endo Steel variants', () => {
      ...
      expect(mapStructureType('some endo steel type', TechBase.CLAN)).toBe(
        InternalStructureType.ENDO_STEEL_CLAN,
      );
      expect(
        mapStructureType('some endo steel type', TechBase.INNER_SPHERE),
    ...
    it('should fuzzy match endo-composite structure', () => {
      expect(mapStructureType('some endo composite type')).toBe(
        InternalStructureType.ENDO_COMPOSITE,
      );
    });

    it('should fuzzy match reinforced structure', () => {
      expect(mapStructureType('some reinforced type')).toBe(
        InternalStructureType.REINFORCED,
      );
    });

    it('should fuzzy match composite structure', () => {
      expect(mapStructureType('some composite type')).toBe(
        InternalStructureType.COMPOSITE,
      );
    });

    it('should fuzzy match industrial structure', () => {
      expect(mapStructureType('some industrial type')).toBe(
        InternalStructureType.INDUSTRIAL,
      );
    });
  ...
  describe('mapHeatSinkType', () => {
    ...
    it('should use fuzzy matching with tech base for double variants', () => {
      // Direct mappings take precedence; fuzzy matching only for non-mapped strings
      expect(mapHeatSinkType('some double heat sink', TechBase.CLAN)).toBe(
        HeatSinkType.DOUBLE_CLAN,
      );
      expect(
        mapHeatSinkType('some double heat sink', TechBase.INNER_SPHERE),
    ...
    it('should fuzzy match compact heat sinks', () => {
      expect(mapHeatSinkType('some compact heat sink')).toBe(
  ...
  describe('mapArmorType', () => {
    ...
    it('should use fuzzy matching with tech base for Ferro-Fibrous variants', () => {
      // Direct mappings take precedence; fuzzy matching only for non-mapped strings
      expect(mapArmorType('some ferro fibrous armor', TechBase.CLAN)).toBe(
        ArmorTypeEnum.FERRO_FIBROUS_CLAN,
      );
      expect(
        mapArmorType('some ferro fibrous armor', TechBase.INNER_SPHERE),
    ...
    it('should fuzzy match reflective armor', () => {
      expect(mapArmorType('some reflective armor')).toBe(
        ArmorTypeEnum.REFLECTIVE,
      );
      expect(mapArmorType('laser-reflective type')).toBe(
        ArmorTypeEnum.REFLECTIVE,
      );
    });
    ...
    it('should fuzzy match heavy ferro armor', () => {
      expect(mapArmorType('some heavy ferro armor')).toBe(
        ArmorTypeEnum.HEAVY_FERRO,
      );
    });

    it('should fuzzy match light ferro armor', () => {
      expect(mapArmorType('some light ferro armor')).toBe(
        ArmorTypeEnum.LIGHT_FERRO,
      );
    });
