 ...
 */
export const MEGAMEKLAB_TYPE_MAP: Record<string, string> = {
  // ============ ENERGY WEAPONS ============
  // Standard Lasers
 ...
 */
export const NAME_NORMALIZATION_PATTERNS: Array<{
  pattern: RegExp;
  replacement: string;
}> = [
 ...
 */
export function stripQuantityPrefix(itemType: string): {
  count: number;
  cleanType: string;
} {
  ...
  if (rest && rest[0] === rest[0].toLowerCase()) {
    cleanType =
      prefix.toUpperCase() + rest.charAt(0).toUpperCase() + rest.slice(1);
  }
