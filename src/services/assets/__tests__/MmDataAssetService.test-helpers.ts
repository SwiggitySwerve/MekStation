...
function _createMockFetch(
  responses: Record<string, { ok: boolean; content?: string; status?: number }>,
) {
  return jest.fn().mockImplementation((url: string) => {
    ...
    for (const [pattern, response] of Object.entries(responses)) {
      if (
        ...
        pattern.includes(url.split('/').pop() || '')
      ) {
        if (response.ok) {
          return Promise.resolve({
            ok: true,
...
describe('getArmorPipPath', () => {
  it('should generate correct path for HEAD location', () => {
    const path = getMmDataAssetService().getArmorPipPath(MechLocation.HEAD, 9);
...
describe('preloadConfiguration', () => {
  ...
  it('should handle failed structure pip loads gracefully', async () => {
    ...
    await expect(
      getMmDataAssetService().preloadConfiguration(MechConfiguration.BIPED, 50),
...
describe('parseSVGToPaths', () => {
  it('should parse SVG content and return path elements', () => {
    const mockPath1 = { outerHTML: '<path d="M0 0 L10 10"/>' } as Pick<
      HTMLElement,
      'outerHTML'
    >;
    const mockPath2 = { outerHTML: '<path d="M5 5 L15 15"/>' } as Pick<
      HTMLElement,
      'outerHTML'
    >;
    ...
    mockFetch.mockImplementation((url: string) => {
      if (url === '/config/mm-data-assets.json') {
        return Promise.resolve({
          ok: true,
          json: () =>
            Promise.resolve({ version: 'v1.0.0', repository: 'test/repo' }),
...
describe('AssetLoadError', () => {
  ...
  it('should include all attempted sources in error', async () => {
    ...
    try {
      ...
      expect(assetError.attemptedSources.some((s) => s.includes('local'))).toBe(
        true,
      );
      expect(assetError.attemptedSources.some((s) => s.includes('cdn'))).toBe(
        true,
      );
      expect(
        assetError.attemptedSources.some((s) => s.includes('github-raw')),
      ).toBe(true);
    }
  });
...
describe('edge cases', () => {
  ...
  it('should handle all quad leg locations', () => {
    expect(
      getMmDataAssetService().getArmorPipPath(MechLocation.FRONT_RIGHT_LEG, 10),
    ...
    expect(
      getMmDataAssetService().getArmorPipPath(MechLocation.REAR_RIGHT_LEG, 10),
    ).toBe('/record-sheets/biped_pips/Armor_RRL_10_Humanoid.svg');
  });

  it('should handle zero armor count', () => {
    const path = getMmDataAssetService().getArmorPipPath(MechLocation.HEAD, 0);
