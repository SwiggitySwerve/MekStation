...
function getMissionStatusStyles(status: CampaignMissionStatus, isCurrent: boolean): {
  bg: string;
  border: string;
  text: string;
...
function MissionNode({
  ...
  const isClickable = onClick && mission.status !== CampaignMissionStatus.Locked;
  ...
  return (
    <div className="relative flex items-start">
      ...
      {/* Branch indicator */}
      {branchType && (
        <div className="absolute -left-4 top-3 text-xs font-medium">
          {branchType === 'victory' && (
...
{/* Node */}
      ...
      >
        {/* Status icon */}
        <div
          className={`w-10 h-10 rounded-full flex items-center justify-center ${styles.bg} ${styles.text}`}
        >
          {styles.icon}
        </div>
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            <h4 className={`font-medium truncate ${styles.text}`}>
              {mission.name}
            </h4>
            {mission.isFinal && (
              <span className="px-1.5 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider bg-violet-500/20 text-violet-400">
                Final
              </span>
            )}
          </div>
...
export function MissionTreeView({
  ...
  const buildTree = () => {
    ...
    const branches: Map<string, { mission: ICampaignMission; type: 'victory' | 'defeat' }[]> = new Map();
    ...
    for (const mission of sortedMissions) {
      const isDefeatBranch = sortedMissions.some((m) =>
        m.branches.some((b) => b.targetMissionId === mission.id && b.condition === 'defeat')
      );
    ...
    for (const mission of sortedMissions) {
      for (const branch of mission.branches) {
        const targetMission = sortedMissions.find((m) => m.id === branch.targetMissionId);
  ...
  if (missions.length === 0) {
    return (
      <div className="text-center py-8 text-text-theme-muted">
        ...
        <p className="text-sm mt-1">Add missions to see the campaign structure</p>
      </div>
    );
  }
        ...
        return (
          <div key={mission.id}>
            ...
            {missionBranches && missionBranches.length > 0 && (
              <div className="ml-8 mt-2 space-y-2 border-l-2 border-dashed border-border-theme-subtle pl-4">
                {missionBranches.map((branch) => (
                  <MissionNode
                    key={branch.mission.id}
                    mission={branch.mission}
                    isCurrent={branch.mission.id === currentMissionId}
                    onClick={onMissionClick ? () => onMissionClick(branch.mission) : undefined}
                    showConnector={false}
