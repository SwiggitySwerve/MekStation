...
describe('TurnoverSettingsPanel', () => {
  ...
  it('should render the settings panel', () => {
    render(<TurnoverSettingsPanel options={defaultOptions} onOptionsChange={onOptionsChange} />);
  ...
  it('should display enable toggle checked by default', () => {
    render(<TurnoverSettingsPanel options={defaultOptions} onOptionsChange={onOptionsChange} />);
    const toggle = screen.getByLabelText('Enable Turnover System') as HTMLInputElement;
  ...
  it('should display base target number input', () => {
    render(<TurnoverSettingsPanel options={defaultOptions} onOptionsChange={onOptionsChange} />);
    const input = screen.getByLabelText('Base Target Number') as HTMLInputElement;
  ...
  it('should display commander immunity toggle', () => {
    render(<TurnoverSettingsPanel options={defaultOptions} onOptionsChange={onOptionsChange} />);
    const toggle = screen.getByLabelText('Commander Immune') as HTMLInputElement;
    expect(toggle.checked).toBe(true);
  });
  ...
  it('should display modifier toggles', () => {
    ...
    expect(screen.getByLabelText('Use Mission Status Modifiers')).toBeInTheDocument();
  });

  it('should disable controls when turnover is disabled', () => {
    const disabledOptions: ICampaignOptions = { ...defaultOptions, useTurnover: false };
    render(<TurnoverSettingsPanel options={disabledOptions} onOptionsChange={onOptionsChange} />);

    const frequency = screen.getByLabelText('Check Frequency') as HTMLSelectElement;
    expect(frequency.disabled).toBe(true);

    const targetNumber = screen.getByLabelText('Base Target Number') as HTMLInputElement;
    expect(targetNumber.disabled).toBe(true);

    const commanderImmune = screen.getByLabelText('Commander Immune') as HTMLInputElement;
    expect(commanderImmune.disabled).toBe(true);
  });

  it('should display payout multiplier input', () => {
    render(<TurnoverSettingsPanel options={defaultOptions} onOptionsChange={onOptionsChange} />);
    const input = screen.getByLabelText(/Payout Multiplier/) as HTMLInputElement;
