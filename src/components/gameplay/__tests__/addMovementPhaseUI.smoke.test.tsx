 ...
 *   (Walk=+1, Run=+2, Sprint=+3, Evade=+4, Jump=max(3,jumpMP), Stationary=0)
...
describe('add-movement-phase-ui — smoke test', () => {
  describe('MovementHeatPreview', () => {
    ...
    it('renders +3 heat for TacOps Sprint', () => {
      render(<MovementHeatPreview movementType={MovementType.Sprint} />);
      const preview = screen.getByTestId('movement-heat-preview');
      expect(preview.dataset.heat).toBe('3');
      expect(preview.textContent).toContain('+3');
      expect(preview.textContent).toContain('Sprint');
    });
