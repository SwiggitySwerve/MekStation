...
import { type PlaybackSpeed, PLAYBACK_SPEEDS, formatSpeed } from '@/hooks/audit';
...
export function ReplaySpeedSelector({
  ...
  const handleKeyDown = useCallback((e: React.KeyboardEvent) => {
    if (e.key === 'Escape') {
      ...
      let newIndex: number;
      if (e.key === 'ArrowUp') {
        newIndex = currentIndex > 0 ? currentIndex - 1 : PLAYBACK_SPEEDS.length - 1;
      } else {
        newIndex = currentIndex < PLAYBACK_SPEEDS.length - 1 ? currentIndex + 1 : 0;
      }
  ...
  const handleSelectSpeed = useCallback((newSpeed: PlaybackSpeed) => {
    ...
    <div className={`relative ${className}`}>
      ...
      <button
        ...
        className={`
          ...
          bg-surface-raised/60 border border-border-theme-subtle
          hover:bg-surface-raised hover:border-border-theme
          transition-all duration-150 min-h-touch
          ${isOpen ? 'ring-2 ring-accent/30 border-border-theme' : ''}
        `}
      ...
      >
        <GaugeIcon />
        <span className={`font-mono font-semibold text-sm ${currentConfig.color}`}>
          {currentConfig.label}
        </span>
        <ChevronDownIcon />
      </button>
