...
const CHANGE_TYPE_STYLES: Record<DiffChangeType, string> = {
  ...
  removed: 'bg-red-500/20 text-red-300 border-red-500/30 line-through decoration-red-400/50',
  modified: 'bg-amber-500/20 text-amber-300 border-amber-500/30',
  unchanged: 'bg-surface-raised/30 text-text-theme-secondary border-border-theme-subtle/30',
};

const CHANGE_TYPE_INDICATORS: Record<DiffChangeType, { symbol: string; label: string }> = {
    ...
    >
      ...
      <span 
        className={`
          ...
          ${changeType === 'added' ? 'text-emerald-400' : ''}
          ${changeType === 'removed' ? 'text-red-400' : ''}
          ${changeType === 'modified' ? 'text-amber-400' : ''}
        `}
