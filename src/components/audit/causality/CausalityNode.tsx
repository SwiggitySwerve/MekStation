...
export function CausalityNode({
  ...
  const config = CATEGORY_CONFIG[node.event.category] || CATEGORY_CONFIG[EventCategory.Meta];
  ...
  return (
    <button
      ...
      className={`
        ...
        ${isFocusEvent ? 'ring-2 ring-accent/70 ring-offset-2 ring-offset-surface-deep' : ''}
        ${onClick ? 'cursor-pointer hover:scale-[1.02] hover:shadow-lg hover:shadow-black/20' : 'cursor-default'}
    ...
    >
      ...
      {/* Event Type */}
      <p className={`
        font-medium text-text-theme-primary truncate
      ...
      {!compact && (
        <p className="text-xs text-text-theme-muted mt-1">
      ...
      {compact && (
        <div className="
          ...
          px-2 py-1 bg-surface-raised text-xs text-text-theme-secondary
      ...
      <div className="absolute bottom-1 right-1.5 opacity-60">
        <span className="text-[9px] font-mono text-text-theme-muted">
          d{node.depth}
        </span>
      </div>
    </button>
