...
function generatePayloadSummary(payload: unknown): string {
  ...
  const meaningfulKeys = ['name', 'damage', 'target', 'source', 'amount', 'skill', 'result', 'location'];
  ...
  return (
    <Card
      ...
      className={`
        ...
        ${isSelected ? 'ring-2 ring-accent/50 bg-surface-base/70' : ''}
    ...
    >
      ...
      <div className="flex items-center justify-between gap-3 mb-2">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <span className={`
            ...
            bg-surface-raised/80 text-text-theme-secondary
            group-hover:text-text-theme-primary transition-colors
          `}>
            {config.icon}
          </span>
        ...
        {/* Timestamp */}
        <span className="text-xs text-text-theme-muted whitespace-nowrap flex-shrink-0">
      ...
      {/* Payload Summary */}
      {payloadSummary && (
        <p className="text-sm text-text-theme-secondary truncate mb-1">
          {payloadSummary}
        </p>
      )}
      ...
      {/* Causality Indicator */}
      {event.causedBy && (
        <div className="flex items-center gap-1.5 text-xs text-text-theme-muted mt-2 pt-2 border-t border-border-theme-subtle/30">
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-3.5 h-3.5">
            <path strokeLinecap="round" strokeLinejoin="round" d="M13.19 8.688a4.5 4.5 0 0 1 1.242 7.244l-4.5 4.5a4.5 4.5 0 0 1-6.364-6.364l1.757-1.757m13.35-.622 1.757-1.757a4.5 4.5 0 0 0-6.364-6.364l-4.5 4.5a4.5 4.5 0 0 0 1.242 7.244" />
          </svg>
          <span>{getCausalityLabel(event.causedBy)}</span>
          <code className="bg-surface-raised/50 px-1.5 py-0.5 rounded text-text-theme-secondary font-mono">
      ...
      <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity">
        <span className="text-[10px] font-mono text-text-theme-muted bg-surface-raised/50 px-1.5 py-0.5 rounded">
          #{event.sequence}
        </span>
      </div>
    </Card>
