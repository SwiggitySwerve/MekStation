...
describe('Layout Validation', () => {
  describe.each(ALL_LAYOUTS)('$name ($id)', (config) => {
    ...
    it('should have positive dimensions for all parts', () => {
      for (const [_loc, pos] of Object.entries(layout.positions) as Array<
        [MechLocation, ResolvedPosition]
      >) {
    ...
    it('should have front legs not overlapping with side torsos', () => {
      ...
      if (frontLeftLeg && leftTorso) {
        const overlap = boxesOverlap(
          {
            x: frontLeftLeg.x,
            y: frontLeftLeg.y,
            width: frontLeftLeg.width,
            height: frontLeftLeg.height,
          },
          {
            x: leftTorso.x,
            y: leftTorso.y,
            width: leftTorso.width,
            height: leftTorso.height,
          },
        );
        ...
        if (overlap) {
          ...
          console.error(
            `Front Left Leg overlaps Left Torso: ${overlapAmount.horizontal.toFixed(1)}px horizontal, ${overlapAmount.vertical.toFixed(1)}px vertical`,
          );
        }
      ...
      if (frontRightLeg && rightTorso) {
        const overlap = boxesOverlap(
          {
            x: frontRightLeg.x,
            y: frontRightLeg.y,
            width: frontRightLeg.width,
            height: frontRightLeg.height,
          },
          {
            x: rightTorso.x,
            y: rightTorso.y,
            width: rightTorso.width,
            height: rightTorso.height,
          },
        );
        ...
        if (overlap) {
          ...
          console.error(
            `Front Right Leg overlaps Right Torso: ${overlapAmount.horizontal.toFixed(1)}px horizontal, ${overlapAmount.vertical.toFixed(1)}px vertical`,
          );
        }
        ...
        const overlap = boxesOverlap(
          {
            x: rearLeftLeg.x,
            y: rearLeftLeg.y,
            width: rearLeftLeg.width,
            height: rearLeftLeg.height,
          },
          {
            x: leftTorso.x,
            y: leftTorso.y,
        ...
        const overlap = boxesOverlap(
          {
            x: rearRightLeg.x,
            y: rearRightLeg.y,
            width: rearRightLeg.width,
            height: rearRightLeg.height,
          },
          {
            x: rightTorso.x,
            y: rightTorso.y,
