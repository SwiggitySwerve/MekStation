...
function ExclamationCircleIcon({ className = 'w-5 h-5' }: { className?: string }) {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className={className}>
...
export function SyncStatusIndicator({
  ...
  return (
    <button
      onClick={onClick}
      className={`
        relative inline-flex items-center gap-2 px-2.5 py-1.5 
    ...
    >
      ...
      {/* Pending count badge */}
      {hasPending && (
        <span className="absolute -top-1 -right-1 flex items-center justify-center min-w-[18px] h-[18px] px-1 text-[10px] font-bold text-white bg-amber-500 rounded-full shadow-lg shadow-amber-500/30">
    ...
    >
      <div className="flex items-center gap-4">
        ...
        <div className="relative flex-shrink-0">
          <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-gray-600/50 to-gray-700/50 flex items-center justify-center">
            <UserIcon className="w-5 h-5 text-gray-400" />
          </div>
          {/* Connection dot */}
          <span
            className={`absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full border-2 border-gray-800 ${config.dotClass}`}
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            <span className="font-medium text-white truncate" title={peerId}>
              {displayName}
            </span>
            {failedCount > 0 && (
              ...
              <div className="flex items-center gap-1.5 text-gray-500">
                ...
                <span>
                  ...
                  {pendingSizeBytes !== undefined && pendingSizeBytes > 0 && (
                    <span className="text-gray-600"> ({formatBytes(pendingSizeBytes)})</span>
                  )}
                </span>
              </div>
        ...
        <div className="flex-shrink-0 flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
          {onFlush && pendingCount > 0 && (
            <Button
              ...
              className="!px-2"
              title={connectionState === 'connected' ? 'Flush pending messages' : 'Connect to flush'}
            >
              Flush
            </Button>
          )}
...
export function SyncStatusPanel({
  ...
  return (
    <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50">
      <div
        className={`bg-gray-800 border border-gray-700/50 rounded-2xl w-full max-w-xl mx-4 max-h-[85vh] flex flex-col shadow-2xl shadow-black/50 overflow-hidden ${className}`}
      >
          ...
          <div className="relative p-5 border-b border-gray-700/50">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <div className="w-11 h-11 rounded-xl bg-gradient-to-br from-cyan-500/30 to-blue-500/20 flex items-center justify-center shadow-lg shadow-cyan-500/10">
                  <SignalIcon className="w-5 h-5 text-cyan-400" />
                </div>
                <div>
                  <h2 className="font-bold text-white text-lg tracking-tight">Sync Status</h2>
                  <p className="text-sm text-gray-400">Message queue & peer connections</p>
                </div>
              </div>
              <button
                onClick={onClose}
        ...
        {isLoading && (
          <div className="flex items-center justify-center py-16">
            ...
            <span className="ml-3 text-gray-400 font-medium">Loading sync status...</span>
          </div>
        )}
        ...
        {error && !isLoading && (
          <div className="p-6">
            <div className="bg-red-900/30 border border-red-500/30 rounded-xl p-4 flex items-start gap-3">
              ...
              <div>
                <p className="text-red-300 font-medium">Failed to load sync status</p>
        ...
        {!isLoading && !error && (
          <>
            {/* Queue Stats */}
            <div className="grid grid-cols-4 gap-px bg-gray-700/30 flex-shrink-0">
              ...
              <div className="bg-gray-800/80 p-4 text-center">
                ...
                <div className="text-xs text-gray-500 uppercase tracking-wider mt-1">Expired</div>
              </div>
              <div className="bg-gray-800/80 p-4 text-center">
                <div className="text-2xl font-bold text-orange-400 tabular-nums">
                  {formatNumber(expiringSoon)}
                </div>
                <div className="text-xs text-gray-500 uppercase tracking-wider mt-1">Expiring</div>
              </div>
            </div>

            {/* Storage info */}
            {stats && stats.totalSizeBytes > 0 && (
              <div className="px-5 py-3 bg-gray-800/50 border-b border-gray-700/50 flex items-center justify-between text-sm">
            ...
            {(onFlushAll || onClearExpired) && (
              <div className="px-5 py-3 bg-gray-800/30 border-b border-gray-700/50 flex items-center gap-3">
                {onFlushAll && (
                  <Button
                    variant="secondary"
                    size="sm"
            ...
            <div className="flex-1 overflow-y-auto">
              {peerSummaries.length === 0 ? (
                <div className="p-8 text-center">
                  ...
                  <p className="text-gray-400 font-medium">No peers connected</p>
