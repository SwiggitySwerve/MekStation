...
export const TrendChart = memo<ITrendChartProps>(({
  ...
  const handleMouseMove = useCallback(
    (e: React.MouseEvent<SVGSVGElement>) => {
      ...
      const py = CHART_PADDING.top + plotHeight - ((point.value - minVal) / valRange) * plotHeight;
  ...
  return (
    ...
    >
      {onTimeRangeChange && (
        <div className="absolute top-4 right-4 z-10" data-testid="time-range-container">
          <select
            value={timeRange}
...
const ChartSVG: React.FC<ChartSVGProps> = ({
  ...
  const toY = (v: number) => plotTop + plotHeight - ((v - minVal) / valRange) * plotHeight;
    ...
    >
      ...
      {tooltip && (
        <>
          ...
          >
            <div
              className="bg-gray-900/90 text-white text-sm px-3 py-2 rounded-md pointer-events-none whitespace-nowrap"
              data-testid="tooltip"
            >
              <div className="font-medium">{formatDateLabel(tooltip.point.date)}</div>
