...
describe('Tab Switching Complete Test Suite', () => {
  ...
  describe('Sub-tab URL Synchronization', () => {
    ...
    it('should update lastSubTab when changing sub-tabs', () => {
      ...
      expect(useTabManagerStore.getState().getLastSubTab(tabId)).toBe(
        'structure',
      );
    ...
    it('should preserve lastSubTab for each unit independently when switching', () => {
      ...
      const unit1Id = createMockUnitWithData({
        name: 'Unit 1',
        lastSubTab: 'armor',
      });
      const unit2Id = createMockUnitWithData({
        name: 'Unit 2',
        lastSubTab: 'preview',
      });

      // Verify initial state
      expect(useTabManagerStore.getState().getLastSubTab(unit1Id)).toBe(
        'armor',
      );
      expect(useTabManagerStore.getState().getLastSubTab(unit2Id)).toBe(
        'preview',
      );
  ...
  describe('Sub-tab State Preservation', () => {
    ...
    it('should preserve lastSubTab through multiple rapid tab switches', () => {
      ...
      expect(useTabManagerStore.getState().getLastSubTab(tab3)).toBe(
        'criticals',
      );
    ...
    it('should not affect other units when updating one unit lastSubTab', () => {
      const unit1Id = createMockUnitWithData({
        name: 'Unit 1',
        lastSubTab: 'structure',
      });
      const unit2Id = createMockUnitWithData({
        name: 'Unit 2',
        lastSubTab: 'armor',
      });
      const unit3Id = createMockUnitWithData({
        name: 'Unit 3',
        lastSubTab: 'preview',
      });
      ...
      expect(useTabManagerStore.getState().getLastSubTab(unit3Id)).toBe(
        'preview',
      );
    });
  });
  ...
  describe('Data Integrity During Tab Switches', () => {
    describe('Unit Store Integrity', () => {
      ...
      it('should preserve tech base after switching', () => {
        ...
        expect(getUnitStore(isUnitId)?.getState().techBase).toBe(
          TechBase.INNER_SPHERE,
        );
        expect(getUnitStore(clanUnitId)?.getState().techBase).toBe(
          TechBase.CLAN,
        );
      });
    });
  ...
  describe('Edge Cases and Stress Tests', () => {
    ...
    it('should handle switching to same tab repeatedly', () => {
      const tabId = createMockUnitWithData({
        name: 'Test Unit',
        lastSubTab: 'armor',
      });
    ...
    it('should handle getLastSubTab for non-existent tab gracefully', () => {
      const result = useTabManagerStore
        .getState()
        .getLastSubTab('non-existent-id');
    ...
    it('should handle setLastSubTab for non-existent tab gracefully', () => {
      ...
      expect(
        useTabManagerStore.getState().getLastSubTab('non-existent-id'),
      ).toBeUndefined();
    });
  ...
  describe('Integration with Tab Manager Operations', () => {
    it('should preserve lastSubTab when reordering tabs', () => {
      const unit1 = createMockUnitWithData({
        name: 'First',
        lastSubTab: 'armor',
      });
      const unit2 = createMockUnitWithData({
        name: 'Second',
        lastSubTab: 'preview',
      });
      const unit3 = createMockUnitWithData({
        name: 'Third',
        lastSubTab: 'criticals',
      });
      ...
      expect(useTabManagerStore.getState().getLastSubTab(unit2)).toBe(
        'preview',
      );
      expect(useTabManagerStore.getState().getLastSubTab(unit3)).toBe(
        'criticals',
      );
    });

    it('should preserve lastSubTab when duplicating a tab', () => {
      const originalId = createMockUnitWithData({
        name: 'Original',
        lastSubTab: 'equipment',
      });
      ...
      expect(useTabManagerStore.getState().getLastSubTab(originalId)).toBe(
        'equipment',
      );
