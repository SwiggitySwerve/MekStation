...
jest.mock('@/services/equipment/EquipmentLoaderService', () => ({
  ...
  getEquipmentLoader: jest.fn(() => ({
    ...
    getAmmunitionById: jest.fn(
      (id: string) => mockAmmunition.find((a) => a.id === id) || null,
    ),
    getElectronicsById: jest.fn(
      (id: string) => mockElectronics.find((e) => e.id === id) || null,
    ),
    getMiscEquipmentById: jest.fn(
      (id: string) => mockMiscEquipment.find((e) => e.id === id) || null,
    ),
  })),
}));
