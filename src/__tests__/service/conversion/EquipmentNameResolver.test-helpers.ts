...
describe('EquipmentNameResolver', () => {
  ...
  describe('resolve() - Direct Type Mapping', () => {
    ...
    it('should resolve ER lasers (IS)', () => {
      expect(resolver.resolve('ISERSmallLaser', 'ER Small Laser').found).toBe(
        true,
      );
      expect(resolver.resolve('ISERMediumLaser', 'ER Medium Laser').found).toBe(
        true,
      );
      expect(resolver.resolve('ISERLargeLaser', 'ER Large Laser').found).toBe(
        true,
      );
    });
    ...
    it('should resolve ER lasers (Clan)', () => {
      expect(
        resolver.resolve('CLERSmallLaser', 'Clan ER Small Laser').found,
      ).toBe(true);
      expect(
        resolver.resolve('CLERMediumLaser', 'Clan ER Medium Laser').found,
      ).toBe(true);
      expect(
        resolver.resolve('CLERLargeLaser', 'Clan ER Large Laser').found,
      ).toBe(true);
    });

    it('should resolve pulse lasers (IS)', () => {
      expect(
        resolver.resolve('SmallPulseLaser', 'Small Pulse Laser').found,
      ).toBe(true);
      expect(
        resolver.resolve('MediumPulseLaser', 'Medium Pulse Laser').found,
      ).toBe(true);
      expect(
        resolver.resolve('LargePulseLaser', 'Large Pulse Laser').found,
      ).toBe(true);
    });

    it('should resolve pulse lasers (Clan)', () => {
      expect(
        resolver.resolve('CLSmallPulseLaser', 'Clan Small Pulse Laser').found,
      ).toBe(true);
      expect(
        resolver.resolve('CLMediumPulseLaser', 'Clan Medium Pulse Laser').found,
      ).toBe(true);
      expect(
        resolver.resolve('CLLargePulseLaser', 'Clan Large Pulse Laser').found,
      ).toBe(true);
    });
    ...
    it('should resolve ultra ACs (Clan)', () => {
      expect(resolver.resolve('CLUltraAC5', 'Clan Ultra AC/5').found).toBe(
        true,
      );
      expect(resolver.resolve('CLUltraAC10', 'Clan Ultra AC/10').found).toBe(
        true,
      );
    });
    ...
    it('should resolve gauss rifles', () => {
      ...
      expect(resolver.resolve('CLGaussRifle', 'Clan Gauss Rifle').found).toBe(
        true,
      );
    });
    ...
    it('should resolve streak SRMs', () => {
      ...
      expect(resolver.resolve('CLStreakSRM4', 'Clan Streak SRM 4').found).toBe(
        true,
      );
    });
    ...
    it('should resolve machine guns', () => {
      ...
      expect(
        resolver.resolve('ISLightMachineGun', 'Light Machine Gun').found,
      ).toBe(true);
      expect(
        resolver.resolve('ISHeavyMachineGun', 'Heavy Machine Gun').found,
      ).toBe(true);
    });
  });
  ...
  describe('resolve() - Equipment Categories', () => {
    it('should resolve AMS', () => {
      ...
      expect(resolver.resolve('CLAMS', 'Clan Anti-Missile System').found).toBe(
        true,
      );
    });
    ...
    it('should resolve C3 systems', () => {
      expect(resolver.resolve('C3MasterComputer', 'C3 Master').found).toBe(
        true,
      );
    ...
    it('should resolve ECM/BAP', () => {
      ...
      expect(
        resolver.resolve('BeagleActiveProbe', 'Beagle Active Probe').found,
      ).toBe(true);
    });

    it('should resolve targeting computers', () => {
      expect(
        resolver.resolve('ISTargetingComputer', 'Targeting Computer').found,
      ).toBe(true);
      expect(
        resolver.resolve('CLTargetingComputer', 'Clan Targeting Computer')
          .found,
      ).toBe(true);
    });
    ...
    it('should resolve heat sinks', () => {
      ...
      expect(
        resolver.resolve('ISDoubleHeatSink', 'Double Heat Sink').found,
      ).toBe(true);
      expect(
        resolver.resolve('CLDoubleHeatSink', 'Clan Double Heat Sink').found,
      ).toBe(true);
    });
