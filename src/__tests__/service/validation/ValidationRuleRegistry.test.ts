...
describe('ValidationRuleRegistry', () => {
  ...
  describe('register', () => {
    ...
    it('should throw for duplicate rule ID', () => {
      ...
      expect(() => registry.register(rule)).toThrow(
        "Rule with ID 'test-1' is already registered",
      );
    });
    ...
    it('should register multiple rules with different IDs', () => {
      ...
      registry.register(
        createTestRule('rule-3', ValidationCategory.CONSTRUCTION),
      );
  ...
  describe('getAllRules', () => {
    ...
    it('should sort rules by priority', () => {
      registry.register(
        createTestRule('rule-c', ValidationCategory.WEIGHT, 300),
      );
      registry.register(
        createTestRule('rule-a', ValidationCategory.WEIGHT, 100),
      );
      registry.register(
        createTestRule('rule-b', ValidationCategory.WEIGHT, 200),
      );
    ...
    it('should use default priority (100) when not specified', () => {
      registry.register(
        createTestRule('rule-a', ValidationCategory.WEIGHT, 50),
      );
      registry.register(createTestRule('rule-b', ValidationCategory.WEIGHT)); // Default 100
      registry.register(
        createTestRule('rule-c', ValidationCategory.WEIGHT, 150),
      );
  ...
  describe('getRulesByCategory', () => {
    beforeEach(() => {
      ...
      registry.register(
        createTestRule('construction-1', ValidationCategory.CONSTRUCTION),
      );
    });

    it('should filter by category', () => {
      const weightRules = registry.getRulesByCategory(
        ValidationCategory.WEIGHT,
      );
      expect(weightRules.length).toBe(2);
      expect(
        weightRules.every((r) => r.category === ValidationCategory.WEIGHT),
      ).toBe(true);
    });
    ...
    it('should also sort by priority', () => {
      registry.clear();
      registry.register(
        createTestRule('weight-b', ValidationCategory.WEIGHT, 200),
      );
      registry.register(
        createTestRule('weight-a', ValidationCategory.WEIGHT, 100),
      );
