...
export class MockVersionHistoryRepository {
  ...
  getVersionById = async (id: string): Promise<IVersionSnapshot | null> => {
...
export class MockOfflineQueueRepository {
  ...
  getById = async (id: string): Promise<IQueuedMessage | null> => {
    return this.messages.get(id) ?? null;
  };

  getPendingForPeer = async (
    peerId: string,
    limit = 50,
  ): Promise<IQueuedMessage[]> => {
  ...
  getAllPending = async (limit = 100): Promise<IQueuedMessage[]> => {
    return Array.from(this.messages.values())
  ...
  incrementAttempts = async (id: string): Promise<boolean> => {
    const message = this.messages.get(id);
    if (!message) return false;
    message.attempts++;
  ...
  markSending = async (id: string): Promise<boolean> => {
    const message = this.messages.get(id);
    if (!message) return false;
  ...
  markSent = async (id: string): Promise<boolean> => {
    const message = this.messages.get(id);
    if (!message) return false;
    message.status = 'sent';
    return true;
  ...
  markFailed = async (id: string): Promise<boolean> => {
    const message = this.messages.get(id);
    if (!message) return false;
  ...
  deleteSent = async (): Promise<number> => {
    let count = 0;
    const entries = Array.from(this.messages.entries());
    for (const [id, message] of entries) {
  ...
  deleteForPeer = async (peerId: string): Promise<number> => {
    let count = 0;
    const entries = Array.from(this.messages.entries());
    for (const [id, message] of entries) {
  ...
  deleteOlderThan = async (date: string): Promise<number> => {
    let count = 0;
    const entries = Array.from(this.messages.entries());
    for (const [id, message] of entries) {
  ...
  markExpired = async (): Promise<number> => {
    const now = new Date().toISOString();
    let count = 0;
  ...
  getPeerSummary = async (peerId: string): Promise<IPeerQueueSummary> => {
  ...
  getStats = async (): Promise<IQueueStats> => {
...
export class MockPermissionService {
  ...
  getGrantsForGrantee = async (granteeId: string): Promise<unknown[]> => {
...
export class MockP2PTransport {
  ...
  connect = async (peerId: string): Promise<boolean> => {
    this.connected.add(peerId);
    return true;
  };

  disconnect = async (peerId: string): Promise<void> => {
    this.connected.delete(peerId);
  };

  send = async (peerId: string, message: unknown): Promise<boolean> => {
