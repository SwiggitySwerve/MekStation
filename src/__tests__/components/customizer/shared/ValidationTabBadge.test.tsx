...
import { ValidationTabBadge, ValidationTabBadgeCompact } from '@/components/customizer/shared/ValidationTabBadge';
...
describe('ValidationTabBadge', () => {
  ...
  const warningCounts: TabValidationCounts = { errors: 0, warnings: 5, infos: 0 };
  ...
  describe('with zero counts', () => {
    ...
    it('should still return null when showZero is true but counts are zero', () => {
      const { container } = render(<ValidationTabBadge counts={zeroCounts} showZero={true} />);
      expect(container.firstChild).toBeNull();
    });
  });
  ...
  describe('with error counts', () => {
    ...
    it('should use singular form for 1 error', () => {
      render(<ValidationTabBadge counts={{ errors: 1, warnings: 0, infos: 0 }} />);
  ...
  describe('with large numbers', () => {
    it('should cap display at 99+', () => {
      render(<ValidationTabBadge counts={{ errors: 150, warnings: 0, infos: 0 }} />);
...
describe('ValidationTabBadgeCompact', () => {
  ...
  it('should apply custom className', () => {
    const { container } = render(
      <ValidationTabBadgeCompact errorCount={1} warningCount={0} className="my-class" />
    );
