 ...
 */
export function useTechBaseSync(
  ...
  const defaults = useMemo<ComponentDefaults>(
    () => ({
      ...
      structureType:
        filteredOptions.structures[0]?.type ?? InternalStructureType.STANDARD,
  ...
  const isEngineValid = useCallback(
    (engineType: EngineType): boolean => {
      return filteredOptions.engines.some((e) => e.type === engineType);
    },
    [filteredOptions.engines],
  );

  const isGyroValid = useCallback(
    (gyroType: GyroType): boolean => {
  ...
  const getValidatedSelections = useCallback(
    (current: IComponentSelections): IComponentSelections => {
      return {
        ...current,
        engineType: isEngineValid(current.engineType)
          ? current.engineType
          : defaults.engineType,
        gyroType: isGyroValid(current.gyroType)
          ? current.gyroType
          : defaults.gyroType,
        internalStructureType: isStructureValid(current.internalStructureType)
          ? current.internalStructureType
          : defaults.structureType,
        cockpitType: isCockpitValid(current.cockpitType)
          ? current.cockpitType
          : defaults.cockpitType,
        heatSinkType: isHeatSinkValid(current.heatSinkType)
          ? current.heatSinkType
          : defaults.heatSinkType,
        armorType: isArmorValid(current.armorType)
          ? current.armorType
          : defaults.armorType,
      };
    },
