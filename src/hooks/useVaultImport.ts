...
export function useVaultImport(): UseVaultImportState & UseVaultImportActions {
  ...
  const handleImportFile = useCallback(
    async <T>(
      ...
      if (!fileContent) {
        const err = { success: false, importedCount: 0, skippedCount: 0, replacedCount: 0, error: 'No file content' };
        setError(err.error);
        return err;
      }
      ...
      try {
        ...
        setError(message);
        setStep('preview');
        return { success: false, importedCount: 0, skippedCount: 0, replacedCount: 0, error: message };
      } finally {
        setImporting(false);
      }
    },
