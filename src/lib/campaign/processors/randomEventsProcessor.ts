...
import type { IDayProcessor, IDayProcessorResult, IDayEvent } from '../dayPipeline';
import type { ICampaign } from '@/types/campaign/Campaign';
import type { IRandomEvent } from '@/types/campaign/events/randomEventTypes';
import { processPrisonerEvents, countPrisoners } from '@/lib/campaign/events/prisonerEvents';
...
function randomEventToDay(evt: IRandomEvent): IDayEvent {
  return {
    ...
    severity: evt.severity === 'critical' ? 'critical' : evt.severity === 'major' ? 'warning' : 'info',
...
export const randomEventsProcessor: IDayProcessor = {
  ...
  process(campaign: ICampaign, date: Date): IDayProcessorResult {
    ...
    if (campaign.options.useLifeEvents !== false) {
      allRandomEvents.push(...processLifeEvents(campaign.personnel, dateStr, random));
    }
    ...
    if (campaign.options.usePrisonerEvents !== false) {
      ...
      allRandomEvents.push(...processPrisonerEvents(prisonerCount, dateStr, random));
    }
