...
describe('randomEventsProcessor', () => {
  ...
  it('should return no events when useRandomEvents is false', () => {
    ...
    const result = randomEventsProcessor.process(
      campaign,
      new Date('3025-01-01'),
    );
  ...
  it('should produce life events on celebration dates', () => {
    const campaign = createCampaign('Test', 'merc', {
    ...
    const lifeEvents = result.events.filter(
      (e) => e.type === 'random_event_life',
    );
  ...
  it('should not produce prisoner events when usePrisonerEvents is false', () => {
    const campaign = createCampaign('Test', 'merc', {
      useRandomEvents: true,
      usePrisonerEvents: false,
    });
    const result = randomEventsProcessor.process(
      campaign,
      new Date('3025-01-06T00:00:00Z'),
    );
    const prisonerEvents = result.events.filter(
      (e) => e.type === 'random_event_prisoner',
    );
  ...
  it('should produce Gray Monday events on correct dates when enabled', () => {
    const campaign = createCampaign('Test', 'merc', {
      useRandomEvents: true,
      simulateGrayMonday: true,
    });
    const result = randomEventsProcessor.process(
      campaign,
      new Date('3132-08-03T00:00:00Z'),
    );
    const historicalEvents = result.events.filter(
      (e) => e.type === 'random_event_historical',
    );
