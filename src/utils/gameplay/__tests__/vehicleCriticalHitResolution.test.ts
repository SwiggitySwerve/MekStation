...
describe('vehicleCriticalHitResolution', () => {
  ...
  describe('applyVehicleCritEffect', () => {
    ...
    it('rotor_damage adds a cumulative MP penalty and immobilizes at the cruise-MP threshold', () => {
      // MegaMek TWGameManager VTOL.CRIT_ROTOR_DAMAGE: each rotor-damage crit
      // adds 1 motive damage; the VTOL immobilizes once accumulated motive
      // damage reaches the original walk (cruise) MP.
      const state = createVehicleCombatState({
        unitId: 'vtol-1',
        motionType: GroundMotionType.VTOL,
        originalCruiseMP: 2,
        armor: { [VTOLLocation.ROTOR]: 2 } as Partial<
          Record<VehicleLocation | VTOLLocation, number>
        >,
        structure: { [VTOLLocation.ROTOR]: 1 } as Partial<
          Record<VehicleLocation | VTOLLocation, number>
        >,
      });
      const rotorCrit = vehicleCritFromRollForLocation([3, 3], {
        location: VTOLLocation.ROTOR,
        motionType: GroundMotionType.VTOL,
        engineType: EngineType.STANDARD,
      });
      expect(rotorCrit.kind).toBe('rotor_damage');

      const first = applyVehicleCritEffect(state, rotorCrit, {
        engineType: EngineType.STANDARD,
        hasAmmoInSlot: false,
      });
      expect(first.state.motive.penaltyMP).toBe(1);
      expect(first.state.motive.immobilized).toBe(false);

      const second = applyVehicleCritEffect(first.state, rotorCrit, {
        engineType: EngineType.STANDARD,
        hasAmmoInSlot: false,
      });
      expect(second.state.motive.penaltyMP).toBe(2);
      expect(second.state.motive.immobilized).toBe(true);
    });
