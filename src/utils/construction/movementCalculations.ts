...
export function getMovementModifiersFromEquipment(
...
export function calculateMaxRunMPWithModifiers(
  walkMP: number,
 ...
 */
export function getJumpJetDefinition(
  type: JumpJetType,
): JumpJetDefinition | undefined {
 ...
 */
export function calculateJumpJetSlots(
  jumpMP: number,
  jumpJetType: JumpJetType,
): number {
  const definition = getJumpJetDefinition(jumpJetType);
...
export function validateJumpConfiguration(
  ...
  if (jumpMP > maxJump) {
    errors.push(
      `Jump MP (${jumpMP}) exceeds maximum (${maxJump}) for ${jumpJetType}`,
    );
  }
