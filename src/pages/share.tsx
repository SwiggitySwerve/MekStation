  ...
  switch (link.scopeType) {
    ...
    case 'category':
      return link.scopeCategory
        ? link.scopeCategory.charAt(0).toUpperCase() + link.scopeCategory.slice(1)
        : 'Category';
    case 'folder':
      return 'Folder';
    case 'item':
...
export default function ManageShareLinksPage(): React.ReactElement {
  ...
  const [actionState, setActionState] = useState<ActionState>({ linkId: null, type: null });
  ...
  return (
    <>
      ...
      >
        ...
        {links.length === 0 ? (
          <EmptyState
            icon={<LinkIcon className="w-12 h-12 text-text-theme-muted mx-auto" />}
          ...
          <Card variant="dark" className="overflow-hidden">
                  ...
                  {links.map((link) => {
                    ...
                    return (
                      <tr
                        ...
                        className={`
                          transition-colors
                          ${!link.isActive ? 'opacity-60' : 'hover:bg-surface-raised/30'}
                        `}
                      >
                        {/* Label */}
                        <td className="px-4 py-3">
                          <div className="flex flex-col">
                            <span className="font-medium text-sm text-text-theme-primary">
                              {link.label || 'Untitled'}
                            </span>
                            <span className="text-xs text-text-theme-muted font-mono mt-0.5">
                            ...
                            {showDeleteConfirm ? (
                              ...
                              <>
                                ...
                                <Button
                                  ...
                                  className={copiedId === link.id ? 'text-emerald-400' : ''}
                                >
                                  {copiedId === link.id ? (
                                ...
                                <Button
                                  ...
                                  disabled={isProcessing}
                                  isLoading={isProcessing && actionState.type === 'toggle'}
                                  title={link.isActive ? 'Deactivate link' : 'Activate link'}
                                  className={link.isActive ? 'text-amber-400' : 'text-text-theme-muted'}
                                >
            ...
            {/* Summary Footer */}
            <div className="px-4 py-3 bg-surface-base/50 border-t border-border-theme-subtle/50">
