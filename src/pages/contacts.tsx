 ...
 */
function getDisplayName(contact: IContact): { primary: string; secondary: string | null } {
  if (contact.nickname) {
...
function ShieldExclamationIcon({ className = 'w-4 h-4' }: { className?: string }) {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className={className}>
...
function AddContactDialog({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      {/* Backdrop */}
      <div
        className="absolute inset-0 bg-surface-deep/90 backdrop-blur-sm"
        onClick={handleClose}
      />

      {/* Dialog */}
      <div className="relative w-full max-w-md bg-surface-base border border-border-theme rounded-2xl shadow-2xl shadow-black/50 overflow-hidden">
        {/* Header */}
        <div className="relative px-6 py-5 border-b border-border-theme-subtle bg-gradient-to-r from-cyan-950/30 to-transparent">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-semibold text-text-theme-primary">Add Contact</h2>
              <p className="text-sm text-text-theme-muted">Connect with another MekStation user</p>
            </div>
          </div>
          <button
            onClick={handleClose}
            className="absolute top-4 right-4 p-2 rounded-lg text-text-theme-muted hover:text-text-theme-primary hover:bg-surface-raised/50 transition-colors"
          >
            <XMarkIcon />
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          {/* Friend Code Input */}
          <div>
            <label className="block text-sm font-medium text-text-theme-secondary mb-2">
              Friend Code <span className="text-red-400">*</span>
            </label>
            <Input
              type="text"
              value={formData.friendCode}
              onChange={(e) => setFormData((prev) => ({ ...prev, friendCode: e.target.value }))}
              placeholder="Enter friend code (e.g., MK-XXXX-XXXX-XXXX)"
              accent="cyan"
              required
            />
            <p className="mt-1.5 text-xs text-text-theme-muted">
          ...
          <div>
            ...
            <textarea
              ...
              rows={3}
              className="w-full bg-surface-raised/50 border border-border-theme text-text-theme-primary placeholder-text-theme-secondary px-4 py-2 rounded-lg focus:outline-none focus:border-cyan-500 transition-colors resize-none"
            />
          </div>

          {/* Error Message */}
          {error && (
            <div className="p-3 rounded-lg bg-red-900/20 border border-red-600/30 text-red-400 text-sm">
              {error}
            </div>
          )}
...
function EditContactDialog({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-md bg-surface-base border border-border-theme rounded-2xl shadow-2xl shadow-black/50 overflow-hidden">
        {/* Header */}
        <div className="relative px-6 py-5 border-b border-border-theme-subtle bg-gradient-to-r from-violet-950/30 to-transparent">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-xl bg-violet-600/20 flex items-center justify-center">
              <PencilIcon className="w-5 h-5 text-violet-400" />
            </div>
            <div>
              <h2 className="text-lg font-semibold text-text-theme-primary">Edit Contact</h2>
              <p className="text-sm text-text-theme-muted">Update nickname and notes</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
          ...
          <div>
            ...
            <textarea
              ...
              placeholder="Add notes about this contact..."
              rows={3}
              className="w-full bg-surface-raised/50 border border-border-theme text-text-theme-primary placeholder-text-theme-secondary px-4 py-2 rounded-lg focus:outline-none focus:border-violet-500 transition-colors resize-none"
            />
          </div>
  ...
  return (
    <Card variant="dark" className="group relative overflow-hidden hover:border-border-theme transition-all duration-200">
      ...
      <div className="relative flex items-start gap-4">
        {/* Avatar */}
        <div className={`relative flex-shrink-0 w-14 h-14 rounded-xl ${avatarColor} flex items-center justify-center shadow-lg`}>
          ...
          <div
            className={`absolute -bottom-1 -right-1 w-4 h-4 rounded-full border-2 border-surface-base ${
              ...
              status === 'syncing' ? 'bg-cyan-500 animate-pulse' :
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-2">
            <div className="min-w-0">
              ...
              {displayName.secondary && (
                <p className="text-sm text-text-theme-muted truncate">{displayName.secondary}</p>
              )}
            </div>

            {/* Status badge */}
            <Badge variant={statusDisplay.variant} size="sm" className="flex-shrink-0">
              {status === 'syncing' && <SignalIcon className="w-3 h-3 mr-1 animate-pulse" />}
              {statusDisplay.label}
            </Badge>
          </div>
          ...
          <div className="mt-2 flex items-center gap-2">
            <span className="text-xs font-mono text-text-theme-muted bg-surface-raised/50 px-2 py-0.5 rounded">
          ...
          {/* Notes preview */}
          {contact.notes && (
            <p className="mt-2 text-sm text-text-theme-secondary line-clamp-1">
              {contact.notes}
            </p>
          )}
      ...
      <div className="mt-4 pt-4 border-t border-border-theme-subtle/50 flex items-center justify-end gap-2">
        {showDeleteConfirm ? (
          <>
            <span className="text-sm text-text-theme-muted mr-2">Delete this contact?</span>
            <Button
              variant="danger"
              size="sm"
          ...
          <>
            ...
            <Button
              ...
              className={contact.isTrusted ? 'text-emerald-400' : 'text-text-theme-muted'}
            >
              {contact.isTrusted ? (
...
export default function ContactsPage(): React.ReactElement {
  ...
  const [actionState, setActionState] = useState<ActionState>({ contactId: null, type: null });
  ...
  const handleAddContact = async (data: ContactFormData) => {
    ...
    try {
      ...
      setContactStatuses((prev) => ({ ...prev, [result.contact.id]: 'offline' }));
      setShowAddDialog(false);
    } catch (err) {
      setAddDialogError(err instanceof Error ? err.message : 'Failed to add contact');
    } finally {
      setIsAdding(false);
    }
  };
  ...
  const handleSaveEdit = async (id: string, nickname: string, notes: string) => {
    setIsSaving(true);
    ...
    try {
      const response = await fetch(`/api/vault/contacts/${id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ nickname: nickname || null, notes: notes || null }),
      });
      ...
      setContacts((prev) =>
        prev.map((c) =>
          c.id === id ? { ...c, nickname: nickname || null, notes: notes || null } : c
        )
      );
  ...
  const handleToggleTrust = async (contact: IContact) => {
    ...
    try {
      ...
      // Update local state
      setContacts((prev) =>
        prev.map((c) => (c.id === contact.id ? { ...c, isTrusted: !c.isTrusted } : c))
      );
    } catch (err) {
  ...
  const onlineCount = Object.values(contactStatuses).filter((s) => s === 'online').length;
  ...
  return (
    <>
      ...
      >
        ...
        {contacts.length === 0 ? (
          <EmptyState
            icon={<UsersIcon className="w-12 h-12 text-text-theme-muted mx-auto" />}
            ...
            <div className="mb-6 flex flex-wrap gap-4">
              <div className="flex items-center gap-2 px-4 py-2 bg-surface-base/30 rounded-lg border border-border-theme-subtle">
                ...
                <span className="text-sm text-text-theme-secondary">
                  <span className="font-medium text-text-theme-primary">{onlineCount}</span> online
                </span>
              </div>
              <div className="flex items-center gap-2 px-4 py-2 bg-surface-base/30 rounded-lg border border-border-theme-subtle">
                <ShieldCheckIcon className="w-4 h-4 text-emerald-400" />
                <span className="text-sm text-text-theme-secondary">
                  <span className="font-medium text-text-theme-primary">{trustedCount}</span> trusted
                </span>
              </div>
            </div>
