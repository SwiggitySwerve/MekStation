...
The system SHALL wire co-op campaign create, guest proposal transport, and mission launch onto the existing co-op core (`CampaignMatchHost`, `CampaignGmArbiter`, `CampaignSyncSession`). Co-op create SHALL register the match server-side; the route surface SHALL be mounted with a real proposal transport; and co-op mission launch SHALL synchronize both players' participation choices and route the launch through the composed both-forces encounter entry point rather than the single-player route.
...
- **GIVEN** `CampaignCoopRouteSurface` is mounted with a real `proposalTransport`
- **WHEN** a guest submits an `IGuestProposal`
- **THEN** the proposal SHALL be delivered to the host via the `CampaignSyncSession` / `CampaignGmArbiter` runtime bridge
- **AND** the proposal SHALL resolve to committed, vetoed, or mechanically-rejected through the real transport rather than the default unavailable transport's `session-closed` rejection.
