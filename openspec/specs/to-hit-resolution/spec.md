...
When a friendly teammate with line of sight to the target designates a called shot location, the to-hit resolution system SHALL allow the shooter to fire a called shot at that location with NO called shot to-hit penalty. The standard called shot penalty of +3 SHALL be waived entirely when a valid teammate spotter designates the location. This mechanic is OPTIONAL — implementations MAY leave the `teammateCalledShot` flag disabled to fall back to the standard +3 penalty.
