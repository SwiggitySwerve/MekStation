...
## Current customizer gate verification

These requirements describe the **desired** customizer PR gate: twelve cases in three files, `scripts/playwright/run-playwright.mjs`, `--project=chromium --retries=0 --trace=retain-on-failure`, job-local `npm run validate:assets:strict`, production build with `NEXT_PUBLIC_E2E_MODE=true` and `NEXT_PUBLIC_E2E_TEST=true`, standalone `MEKSTATION_E2E_SERVER_COMMAND='node .next/standalone/server.js'` on `HOSTNAME='127.0.0.1'`, and `lint-and-test` aggregator membership. They do not record that the gate is wired on `main`, that this candidate activated it, or that a final browser pack is in `main`.

CI modules from PR #1647 are in this candidate (`scripts/qc/openspec-ci-contracts.mjs` pins job `customizer-regressions`). Workflow job wiring and test prefixes are not merged: `.github/workflows/pr-checks.yml` has no `customizer-regressions` job, and the `lint-and-test` aggregator `needs` set omits it. This candidate's `e2e/customizer-record-sheet-rendering.spec.ts` has two cases; the six-case strengthened rendering pack is not in root or this tree.

Local twelve-case proof is retained in `.sisyphus/roadmap-completion-20260912/worktrees/browser` at commit `77d9a9480a3a47b1849f166db01278f93f3d8094` (3 catalog + 6 record-sheet + 3 edit-recovery). Publication of that commit is **blocked pending explicit user approval**. This document does not publish, move, or evolve that browser code. Dated parent `evidence/browser-local-verification.json` and `evidence/browser-commit.json` are planning inputs, not candidate closure. This spec does not claim new record-sheet assets were acquired or that remote CI passed.
