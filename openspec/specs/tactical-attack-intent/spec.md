# tactical-attack-intent Specification

## Purpose
Specify the intent-first weapon-attack flow on the tactical HUD: the player composes the volley as weapon-to-target assignments (plus the torso-twist intent) on a single authoritative surface, sees rules-derived to-hit, heat, and threshold consequences live, and commits the whole volley atomically with an explicit Fire (or declines with an explicit Hold Fire). Attack rules (`to-hit-resolution`, `secondary-target-tracking`, `weapon-resolution-system`, `heat-management-system`) are consumed verbatim - this capability owns the composition UX, not the math.
## Requirements
