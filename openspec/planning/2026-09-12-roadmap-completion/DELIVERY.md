...
5. Parent reviews the exact diff and test evidence. Use an independent reviewer for every PR; authority, privacy, migration, replay, idempotency, and concurrency changes require a reviewer with the relevant technical scope. Agent review is local engineering evidence; it does not manufacture GitHub approval.
...
7. Observe checks on the exact proposed head. Required checks must complete successfully; investigate failed/cancelled/stale results. Do not use administrative bypass, force-push, `--no-verify`, fabricated approval, or relaxed validators. Where a package requires a non-author approved GitHub review, wait for an eligible real review tied to that head. A new head requires fresh evaluation of review/check validity.
...
Allowed slice states: `planned`, `admitted`, `implementing`, `local-verified`, `pr-open`, `review-required`, `ci-running`, `merged`, `main-verified`, `archived`, `complete`, and `blocked`. Store blocking cause and next independent action without repeatedly retrying the same failed approach. The native goal's blocked state follows its separate three-turn rule.
