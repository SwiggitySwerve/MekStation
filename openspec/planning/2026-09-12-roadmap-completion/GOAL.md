...
7. **Tick** - only now check the `tasks.md` rows the unit holds, and record the `tick` receipt. The validator re-reads the live `tasks.md`, matches each row by source id and text rather than by line number, and fails if the row does not actually read `- [x]`.
...
- **Caps** are per unit and never exceed the owning node: at most 15 files and 500 non-generated changed lines, lower where the unit says so.
