...
const DEPRECATED_TERMS = [
  // Critical Slots
  {
    deprecated: /\bcrit slots\b/gi,
    canonical: 'critical slots',
    severity: 'error',
  },
  {
    deprecated: /\bcritical spaces\b/gi,
    canonical: 'critical slots',
    severity: 'error',
  },
  // Note: Skip "equipment slots" in rule descriptions or when comparing terms
  {
    deprecated: /\bequipment slots\b/gi,
    canonical: 'critical slots',
    severity: 'warning',
    skipInRules: true,
  },
  ...
  // Weight/Mass
  {
    deprecated: /\bcomponent mass\b/gi,
    canonical: 'component weight',
    severity: 'error',
  },
  {
    deprecated: /\bequipment mass\b/gi,
    canonical: 'equipment weight',
    severity: 'error',
  },
  {
    deprecated: /\bweapon mass\b/gi,
    canonical: 'weapon weight',
    severity: 'error',
  },

  // Tech Base
  {
    deprecated: /\btechnology base\b/gi,
    canonical: 'tech base',
    severity: 'warning',
  },
  ...
  {
    deprecated:
      /\bTournament\b(?!-legal|\/Advanced|\s+legality|\s+organization|\s+Validation)/g,
    canonical: 'Advanced',
    severity: 'error',
    context: 'rules level',
  },
  {
    deprecated: /\btech level\b/gi,
    canonical: 'rules level',
    severity: 'error',
  },
  {
    deprecated: /\bcomplexity level\b/gi,
    canonical: 'rules level',
    severity: 'warning',
  },
  ...
  {
    deprecated: /\b(mech|unit|location)\s+section\b/gi,
    canonical: 'location',
    severity: 'warning',
    context: 'mech body part',
  },
  { deprecated: /\bbody part\b/gi, canonical: 'location', severity: 'warning' },
  {
    deprecated: /\bcentre torso\b/gi,
    canonical: 'Center Torso',
    severity: 'error',
  },
  ...
  // Engine heat sinks
  {
    deprecated: /\binternal heat sinks\b/gi,
    canonical: 'engine-integrated heat sinks',
    severity: 'error',
  },
  // Note: Skip in rationale/explanation contexts where "additional" is descriptive
  {
    deprecated: /\badditional heat sinks\b/gi,
    canonical: 'external heat sinks',
    severity: 'warning',
    skipInRationale: true,
  },
  // Note: Skip in changelogs documenting what was changed
  {
    deprecated: /\bintegrated heat sink capacity\b/gi,
    canonical: 'engine integration capacity',
    severity: 'error',
    skipInChangelog: true,
  },
...
const PROPERTY_VIOLATIONS = [
  {
    pattern: /^\s*(?:readonly\s+)?tons:\s*number/gm,
    canonical: 'weight: number',
    severity: 'error',
  },
  {
    pattern: /^\s*(?:readonly\s+)?mass:\s*number/gm,
    canonical: 'weight: number',
    severity: 'error',
  },
  {
    pattern: /^\s*(?:readonly\s+)?slots:\s*number/gm,
    canonical: 'criticalSlots: number',
    severity: 'error',
    context: 'property definition (not "slots" alone in prose)',
  },
  {
    pattern: /^\s*(?:readonly\s+)?critSlots:\s*number/gm,
    canonical: 'criticalSlots: number',
    severity: 'error',
  },
  {
    pattern: /^\s*(?:readonly\s+)?introYear:/gm,
    canonical: 'introductionYear:',
    severity: 'error',
  },
  {
    pattern: /^\s*(?:readonly\s+)?faction:/gm,
    canonical: 'techBase:',
    severity: 'error',
  },
];
...
const CAPITALIZATION_ISSUES = [
  ...
  {
    pattern: /\bbattle mech\b/g,
    canonical: 'BattleMech or mech',
    severity: 'warning',
  },
  {
    pattern: /\binner sphere\b/g,
    canonical: 'Inner Sphere',
    severity: 'error',
    context: 'when referring to faction (not "inner sphere technology")',
  },
];
...
function checkFile(filePath) {
  ...
  lines.forEach((line, index) => {
    ...
    if (inCodeBlock) {
      PROPERTY_VIOLATIONS.forEach(
        ({ pattern, canonical, severity, context }) => {
          const matches = line.match(pattern);
          if (matches) {
            matches.forEach((match) => {
              // Skip camelCase variable names (e.g., const heatSink = ...)
              if (
                /^const\s+\w+/.test(line.trim()) ||
                /^let\s+\w+/.test(line.trim())
              ) {
                return;
              }
...
function formatViolation(violation, filePath) {
  const severityColor =
    violation.severity === 'error' ? colors.red : colors.yellow;
...
function validate() {
  console.log(
    `${colors.bold}${colors.blue}OpenSpec Terminology Validator${colors.reset}\n`,
  );
  console.log(`Scanning specs directory: ${SPECS_DIR}\n`);

  if (FIX_MODE) {
    console.log(
      `${colors.yellow}⚠️  FIX MODE: Automatic fixes are NOT implemented yet${colors.reset}\n`,
    );
  }
  ...
  if (allViolations.length === 0) {
    console.log(
      `${colors.green}${colors.bold}✓ No terminology violations found!${colors.reset}\n`,
    );
    console.log(
      `All ${stats.filesScanned} specifications are compliant with TERMINOLOGY_GLOSSARY.md\n`,
    );
    return 0;
  }

  console.log(
    `${colors.red}${colors.bold}Found ${allViolations.length} violation(s) in ${Object.keys(stats.byFile).length} file(s)${colors.reset}\n`,
  );
  ...
  Object.entries(byFile).forEach(([filePath, violations]) => {
    const relativePath = path.relative(path.join(__dirname, '..'), filePath);
    console.log(
      `${colors.bold}${relativePath}${colors.reset} (${violations.length} violation${violations.length !== 1 ? 's' : ''})\n`,
    );
  ...
  console.log(
    `  ${colors.red}Errors: ${stats.violations.error}${colors.reset}`,
  );
  console.log(
    `  ${colors.yellow}Warnings: ${stats.violations.warning}${colors.reset}`,
  );
  ...
  console.log(
    `  2. Update specs to use canonical terminology from TERMINOLOGY_GLOSSARY.md`,
  );
