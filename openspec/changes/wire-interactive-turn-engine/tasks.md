...
- [ ] 1.1 Confirm the engine path is ready: re-read `applyPhysicalAttack`
  ...
  not new rules.
- [ ] 1.2 Red-first C-2: write a failing test that drives the store's
...
- [ ] 1.3 Red-first C-3: write a failing driver test that puts the engine in
...
- [ ] 1.4 Red-first E-1: write a failing test that subscribes a throwing listener
...
- [ ] 2.1 Rewrite the store action `commitPhysicalAttack`
...
- [ ] 2.2 Update `PhysicalAttackPanel.handleConfirm`
...
- [ ] 2.3 Add the missing `physical-attack` dispatch case to the dock action
...
- [ ] 2.4 Update the existing `commitPhysicalAttack` unit tests
...
- [ ] 3.1 Add a PhysicalAttack branch to `runAITurnLogic`
...
- [ ] 3.2 Add the identical PhysicalAttack branch to `SpectatorView.runOneFullTurn`
...
- [ ] 3.3 Make 1.3 pass and add a full-turn driver test asserting each phase runs
  exactly once per tick and the loop reaches `isGameOver` on an elimination
  fixture (no livelock).

## 4. Combat-outcome bus hardening (E-1)

- [ ] 4.1 In `publishCombatOutcome` (`src/engine/combatOutcomeBus.ts:70`) replace
...
- [ ] 4.3 Confirm the engine guard at `src/engine/InteractiveSession.ts:783`
...
- [ ] 4.4 Re-run the multiplayer publish-idempotency integration test
...
- [ ] 5.1 Add a private `matchLogDiverged` flag (default healthy) to
...
- [ ] 5.2 Add a private `assertActiveForAction(method)` guard throwing the spec's
...
- [ ] 5.3 Add tests: a failed match-log append flips `hasMatchLogDiverged()`
...
- [ ] 6.1 Full verification: `npx tsc --noEmit --skipLibCheck`, `oxlint`,
...
- [ ] 6.2 Run `npx openspec validate wire-interactive-turn-engine --strict` and
  confirm it reports valid.
- [ ] 6.3 Update `docs/audits/2026-06-12-full-codebase-review.md` Wave 1 row for
