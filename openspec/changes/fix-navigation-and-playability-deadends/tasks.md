...
- [x] 1.1 Confirm the `/gameplay` 404: verify no `src/pages/gameplay/index.tsx`
  ...
  resolves to 404.
- [x] 1.2 Confirm the `listMatchLogs` reader signature and `MatchLogSummary`
...
- [x] 1.3 Confirm `GET /api/campaigns` returns `ICampaignSummary[]` via
...
- [x] 5.1 Relabel the existing auto path in `QuickGamePlay.tsx` / the Quick Game
  setup surface as "Auto-Resolve" per design D4 so it is honestly named.
- [x] 5.2 Add a low-friction interactive "Skirmish" on-ramp that launches an
...
- [x] 5.3 Add a test asserting the auto path is labeled Auto-Resolve and that
...
- [x] 6.1 Wire the campaign list page to `GET /api/campaigns`
  (`listCampaignSummaries`) per design D5 so multiple campaigns are listed.
- [x] 6.2 Add a switcher action to `useCampaignStore` that loads a chosen
...
- [x] 6.3 Add a test asserting the list shows >1 campaign after two creations
...
- [x] 7.1 Convert `CampaignDashboardPage` (`CampaignDashboardPage.tsx:54`) to
...
- [x] 7.2 Add a test asserting the dashboard re-renders when the campaign store
  updates; confirm task 1.4's red test now passes.

## 8. Onboarding entry point + glossary (medium)

- [x] 8.1 Add a first-run onboarding/tutorial entry to `navigationCards` in
  `src/pages/index.tsx:23` per design D7.
- [x] 8.2 Add an inline glossary surface defining BV, gunnery, piloting, heat,
  and PSR.
- [x] 8.3 Add a test asserting the onboarding entry is reachable from home and
  the glossary surface defines the core jargon terms.

## 9. Multiplayer entry setup path (medium)

- [x] 9.1 Add a vault-setup link/route and a path-forward affordance to the
  vault-password gate in `src/pages/multiplayer/index.tsx:216` per design D8,
  without changing the auth model or transport.
- [x] 9.2 Add a test asserting the multiplayer entry page surfaces a setup path
  and a path-forward affordance when no vault token exists.

## 10. Verification and documentation

- [x] 10.1 Full verification: `npx tsc --noEmit --skipLibCheck`, lint, and the
  affected Jest/route suites green (sections 2–9).
- [x] 10.2 Run `npx openspec validate fix-navigation-and-playability-deadends --strict`.
- [x] 10.3 Update the 2026-06-12 review tracker: mark U-1/U-2/U-3/U-4 and the
