...
- [x] 1.3 Layout is responsive: action panel collapses on widths < 1024px
      into a drawer
...
- [x] 2.2 Clicking an empty hex clears selection
...
- [x] 4.1 With no unit selected, panel shows placeholder
      `"Select a unit to view its status"`
- [x] 4.2 With a unit selected, panel header shows designation, chassis,
      tonnage, controlling side badge
- [x] 4.3 Panel scrolls independently of the map

## 5. Action Panel — Armor Diagram

- [x] 5.1 Reuse existing `ArmorPip` to render per-location armor and
      internal structure
...
- [x] 7.1 List every weapon on the unit with name, location, short/med/long
      range, damage, heat
- [x] 7.2 Reuse existing `AmmoCounter` inline for ammo-consuming weapons
- [x] 7.3 Destroyed or jammed weapons render disabled with status badge

## 8. Action Panel — SPA List

- [x] 8.1 List the pilot's Special Pilot Abilities by name
- [x] 8.2 Each SPA shows a short description tooltip on hover
- [x] 8.3 Empty list renders `"No SPAs"` placeholder
...
- [x] 9.3 Unconscious pilots render a red banner over the wound track

## 10. Phase Tracker

- [x] 10.1 Reuse `PhaseBanner` to show current phase name
- [x] 10.2 Banner shows turn number (`Turn N`) beside phase name
- [x] 10.3 Banner shows active side with the same side color used on tokens
...
- [x] 11.2 Newest event appears at the top; list auto-scrolls to top on
      append
- [x] 11.3 Each entry shows phase, actor (unit designation when
...
- [x] 12.1 Clicking two different tokens in sequence swaps the action
      panel content
- [x] 12.2 Emitting a phase change from the session advances the phase
      tracker
- [x] 12.3 Emitting a damage event adds an event log entry
- [x] 12.4 Resizing the viewport below 1024px collapses the action panel
      into a drawer
