...
A retroactive campaign correction SHALL declare every affected state and artifact family before commit. The authority SHALL build a replacement branch from a trusted prior head, regenerate declared projections and artifacts, and activate only after deterministic validation.
...
- **THEN** activation SHALL record its invalidation or replacement explicitly
- **AND** later use of the stale artifact SHALL be rejected
