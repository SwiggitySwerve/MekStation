...
- [x] 1.1 Trace the persistence loop end-to-end and record findings in the PR
...
- [x] 1.2 Write a red-first reproduction test: launch an interactive session over
...
- [x] 1.3 Confirm whether `fromMatchLog` returns a data `IGameSession` vs a live
...
- [x] 2.1 Add (or compose from existing factories) a single client-facing
...
- [x] 2.2 Make the helper classify failures: throw a distinct corrupt-log error
...
- [x] 2.3 Unit-test `recoverInteractiveSession`: (a) happy path returns a drivable
...
- [x] 3.1 In `loadSessionLogic` (`src/stores/useGameplayStore.session.ts`),
...
- [x] 3.2 Replace the blanket `throw new Error('Session not found')` with the
...
- [x] 3.3 Thread the recover dependency through `loadSession` in
...
- [x] 4.1 In `usePreBattleLaunch` (`src/components/gameplay/pages/preBattle/usePreBattleLaunch.ts`,
...
- [x] 4.2 Add a launch test asserting that immediately after the interactive
...
- [x] 5.1 Register a `beforeunload` guard (in `useGameplayStore.ts` wiring or a
  ...
  pattern (design D5).
- [x] 5.2 Test the guard: active interactive session → handler registered and
...
- [x] 6.1 Create `src/stores/__tests__/useGameplayStore.recover.test.ts` (or a
...
- [x] 6.2 Add the not-found and corrupt-log cases to the same suite: unknown id →
  precise not-found error in `state.error`; corrupt log → corrupt error with no
  demo fallback.

## 7. Verification and documentation

- [x] 7.1 Full verification: `npx tsc --noEmit --skipLibCheck`, lint, affected Jest
...
- [x] 7.2 Run `npx openspec validate persist-and-recover-interactive-battles --strict`
  and confirm it prints valid.
- [x] 7.3 Update the C-9 row in `docs/audits/2026-06-12-full-codebase-review.md`
