...
- [ ] 1.3 Add Zod runtime schemas for envelope metadata and entity-link roles, rejecting missing durable IDs, invalid versions, non-root branches, and caller-assigned final revisions or observation positions.
...
- [ ] 2.1 Define one adapter conformance suite for atomic expected-head append, contiguous per-branch revisions, canonical predecessor/event digests, observation-cursor safety with permitted numeric gaps, command idempotency/collision, entity history, rollback, and restart.
