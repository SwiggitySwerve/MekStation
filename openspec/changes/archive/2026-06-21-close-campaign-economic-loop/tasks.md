...
- [x] 1.1 Read the repair-queue ticket model end-to-end (`repairQueueBuilderProcessor.ts`,
...
- [x] 1.2 Inventory every reader of `recentlyAppliedOutcomes` (grep `recentlyAppliedOutcomes`
...
- [x] 1.3 Read `serializeCampaign`'s shared field-map constant and `deserializeCampaignBody`
...
- [x] 1.4 Write red-first probes proving each leak is observable today:
...
- [x] 2.1 Add a `partsInventory` field to `ICampaign` (the warehouse pool) and a typed
  accessor/mutator helper; add it to the campaign-creation default as empty.
- [x] 2.2 Register `partsInventory` in the shared serialize field-map so it round-trips
  (joins task 5's work) and a pre-existing-field-less save deserializes to empty.

## 3. Repair ticket completion (C-4)

- [x] 3.1 Add a `repairProgressProcessor` day-processor that consumes available
  tech-hours, promotes `queued → in-progress`, decrements remaining hours, and on a
  ticket reaching zero transitions it `completed`.
- [x] 3.2 On ticket completion, restore the ticket's recorded armor/structure points on
...
- [x] 3.3 Have the parts-matching step read required parts from `partsInventory`
  (consuming them on completion); a ticket whose part is unavailable waits rather than
  completing.
- [x] 3.4 Register the processor in the day pipeline after the builder; delete or demote
  `useRepairStore.advanceRepairs` per task 1.1's finding so there is one completion
  authority.
- [x] 3.5 Update/add reachable repair tests: a multi-day pipeline run drives a ticket to
  `completed` and restores the unit; task 1.4(a) now passes.

## 4. Salvage and acquisition materialization (M-1, M-2)

- [x] 4.1 In `setSalvageItemStatus`, when a candidate is accepted, materialize an awarded
  *unit* onto the roster/force and an awarded *part* into `partsInventory`; a declined
  candidate materializes nothing.
- [x] 4.2 In `acquisitionProcessor.processDeliveries`, on a delivered request increment
  `partsInventory` by the delivered part(s) — the same pool salvage feeds and repair
  reads.
- [x] 4.3 Tests: an accepted salvage unit appears on the roster and an accepted salvage
...
- [x] 5.1 Add `repairQueue`, `salvageAllocations` (+ reports), `pendingBattleOutcomes`,
...
- [x] 5.2 Ensure `processedBattleIds` (Set/array dedup guard) serializes as an array and
  deserializes back to its runtime shape.
- [x] 5.3 Tests: a fully-populated round-trip preserves all four fields (1.4(e) passes);
  the field-map drift type-test still guards them.

## 6. Campaign morale sliding window (M-3)

- [x] 6.1 In `gatherMoraleSignals`, filter `recentlyAppliedOutcomes` to entries within one
  campaign-month of `currentDate` before counting victories/defeats (MHQMorale parity).
- [x] 6.2 Prune `recentlyAppliedOutcomes` at the writer so it cannot grow unbounded
  (filter-only fallback if task 1.2 finds another consumer needs the full history).
- [x] 6.3 Tests: outcomes spread across several months score within a bounded window and
  do not ratchet to Elite (1.4(d) passes).

## 7. Doctor medicine skill (M-5)

- [x] 7.1 Add a `medicine` skill to the doctor roster-entry/pilot model with a typed
  default.
- [x] 7.2 Source `getMedicineSkillValue` from the model (fallback to 7 only when unset);
  implement `getShorthandedModifier` from the doctor's patient-load vs capacity.
- [x] 7.3 Tests: a green doctor and an elite doctor produce materially different heal
...
- [x] 8.1 Add the rank pay multiplier (and secondary-role base) to `salaryService`
  per-person salary (`Person.java:4733`); leave `XP_SALARY_MULTIPLIER` unchanged.
- [x] 8.2 Pass the actual kill count into `awardKillXP` at
  `postBattleProcessor.helpers.ts:114` instead of the literal `1`.
- [x] 8.3 Replace `dayNumber ?? 0` activity-log id derivation
...
- [x] 8.4 Implement `unitMarket.purchaseUnit` to validate funds, debit the cost, and add
...
- [x] 8.5 Tests for each: senior personnel billed with rank multiplier; a 5-kill ace earns
...
- [x] 9.1 Full verification: `npx tsc --noEmit --skipLibCheck`, `oxlint`, `oxfmt --check`,
...
- [x] 9.2 Run `npx openspec validate close-campaign-economic-loop --strict` and confirm it
  prints valid.
- [x] 9.3 Update `docs/audits/2026-06-12-full-codebase-review.md` M-CAMP rows (C-4, M-1,
