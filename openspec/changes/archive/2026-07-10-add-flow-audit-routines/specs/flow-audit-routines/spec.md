# flow-audit-routines Specification (Delta)
...
- Codifies the user-facing half of the 2026-07-09 council decision (`openspec/council-decisions/2026-07-09-ui-audit-testability-architecture.md`): interactive route-mounted audits for iteration, headless invariants as CI authority.
- Flows reach checkpoints by driving the real UI (route-mounted, per the seam-first doctrine). Scenario-pack accelerated entry (council wave W4) may later slot in as a flow prelude without changing this contract.
