...
if (
  progression.winRate !== null &&
  (progression.winRate < 0 || progression.winRate > 1)
) {
  // invalid - emit error
}
```
...
```typescript
if (
  config.atRiskThresholds.woundedPercent < 0 ||
  config.atRiskThresholds.woundedPercent > 1
) {
  // invalid - emit error
}
if (
  config.atRiskThresholds.lowBVPercent < 0 ||
  config.atRiskThresholds.lowBVPercent > 1
) {
  // invalid - emit error
}
```
