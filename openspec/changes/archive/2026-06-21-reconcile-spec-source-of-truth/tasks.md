...
- [x] 1.1 Re-confirm the ACAR drift: read `src/lib/combat/acar.ts` `calculateVictoryProbability`
...
- [x] 1.2 Re-confirm the placeholder count: `grep -rl "TBD - created by archiving" openspec/specs/`
...
- [x] 1.3 Re-confirm the duplicate-owner drift: `critical-hit-resolution/spec.md:121` says
...
- [x] 1.4 Write a red-first guard probe: a fixture spec.md containing `TBD - created by archiving`
...
- [x] 2.1 Apply the `combat-resolution` spec delta: tighten the `ACAR System` requirement to name
...
- [x] 2.2 Confirm no ACAR production code change is needed (code already linear); add/confirm a
...
- [x] 3.1 Author `openspec/scripts/spec-purpose-lint.ts` (+ a `validate-spec-purpose.js` runner if
...
- [x] 3.2 Add the known-placeholder allowlist (the 125 from task 1.2) and the known duplicate-owner
...
- [x] 3.3 Add a `package.json` script (`spec:purpose:validate` / `:strict`) and wire it into the
      spec-validation gate so a NEW placeholder Purpose fails CI immediately while allowlisted
      legacy ones are tracked.
- [x] 3.4 Confirm task 1.4's red probe now passes (guard exits non-zero on a fresh placeholder,
      zero on a real Purpose).

## 4. One-spec-per-capability rule (spec delta)

- [x] 4.1 Apply the `validation-rules-master` spec delta adding the `Spec Source-of-Truth
      Integrity` requirement (no-placeholder-Purpose guard + one-spec-per-capability /
      authoritative-owner rule + pointer mechanic).
- [x] 4.2 Extend the lint check to flag a code home claimed by >1 non-pointer capability, seeded
      from the duplicate-owner allowlist, so the allowlist must shrink to empty over time.
- [x] 4.3 Record the active-change ownership/order matrix for shared capabilities before
...
- [x] 5.1 Backfill the 125 placeholder Purposes in one mechanical sweep — write each capability's
...
- [x] 5.2 Resolve the `critical-hit-resolution` ↔ `critical-hit-system` duplicate: pick the
...
- [x] 5.3 Reconcile the `validation-rules-master` (stale 2025-11-28) /
...
- [x] 6.1 Run the guard against the full tree: NEW placeholders fail, allowlisted legacy ones are
      reported as tracked debt; ACAR unit test green; `npx tsc --noEmit --skipLibCheck` + lint on
      the new script.
- [x] 6.2 Run `npx openspec validate reconcile-spec-source-of-truth --strict` and confirm it prints
      valid.
- [x] 6.3 Update `docs/audits/2026-06-12-full-codebase-review.md` SoT-cluster rows: ACAR scenario
