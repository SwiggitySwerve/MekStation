...
#### Scenario: Altitude-control MP is spent before later movement projection

- **GIVEN** a selected movement-phase VTOL or WiGE unit has replayed a
  represented altitude-control action with altitude-control step count and MP
  cost metadata
- **WHEN** the map projects a later movement destination for that same unit
- **THEN** the destination MP cost SHALL include the pending altitude-control MP
- **AND** the destination legality SHALL be evaluated against the remaining MP
  after pending altitude-control steps
- **AND** the projected movement hex and any same-hex movement options SHALL
  expose altitude-control step count and MP cost metadata
- **AND** committed movement validation SHALL accept or reject the destination
  using the same altitude-control-adjusted cost, path, heat, invalid reason, and
  details as the rendered projection
- **AND** the committed movement event SHALL expose represented
  altitude-control metadata and clear the pending altitude-control reserve from
  replayed unit state.
