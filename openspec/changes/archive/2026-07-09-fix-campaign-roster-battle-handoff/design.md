...
> **Archive-time correction (2026-07-08):** as designed below the seed was "encounter id", but forces are created before the encounter id exists (createAssignedForce precedes createConfiguredEncounter). Implemented seed is `${campaign.id}:${missionId}` per orchestrator ruling — same logical encounter, same OpFor, no creation reordering. The in-code comment in materializeCampaignMissionEncounter.ts documents the contract.
