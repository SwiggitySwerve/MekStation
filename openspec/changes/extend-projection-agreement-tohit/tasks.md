...
- [x] 1.1 Trace the semi-guided context flow end-to-end and document (in the PR
...
- [x] 1.2 Write a red-first to-hit agreement probe in
...
- [x] 1.3 Document that the existing anchor
...
- [x] 2.1 In `deriveToHitProjection` (`combatProjection.toHit.ts`), build a
...
- [x] 2.2 Pass that context as the seventh argument to BOTH the
  `calculateToHitWithC3` branch (`combatProjection.toHit.ts:225`) and the
  `calculateToHit` branch (`:242`).
- [x] 2.3 Verify a moving TAG-designated semi-guided target now projects with the
...
- [x] 3.1 In `CombatPlanningPanel.tsx`, replace the hand-built `attackerState`
...
- [x] 3.2 Replace the hand-built `targetState` memo (`:233-244`, which hardcodes
...
- [x] 3.3 Extend `buildToHitForecast` (`toHit/forecast.ts:103`) to accept and
...
- [x] 3.4 Add a `CombatPlanningPanel` test pinning a displayed forecast TN (for a
...
- [x] 4.1 Retrofit the existing represented agreement cases in
...
- [x] 4.2 Promote task 1.2's red probe into the permanent suite; confirm it now
  PASSES after task 2. Add an ECM-protected variant where the semi-guided
  cancellation does NOT apply, and a non-TAG variant.
- [x] 4.3 Add an explicit comment/assertion documenting that the suite's anchor is
...
- [x] 5.2 Update `docs/audits/2026-06-12-full-codebase-review.md` finding B-1 row:
