...
This OpenSpec change SHALL ship through ten reviewed PRs. **CAMP-PROOF (≤5 files/360 lines)** adds `camp01-authority-receipt.mjs`, its validator, tests, package wiring, and opt-in JSON-reporter support in `scripts/playwright/run-playwright.mjs`. Its literal versioned `WAVE_CONTRACTS` table uses strict `camp01-authority-receipt/v1`; the writer itself executes the exact platform-shell string passed as one `--command="<literal command>"` argument, refuses caller-supplied `command-result.json`, makes each nested Playwright wrapper invocation write a unique temporary JSON report, and atomically normalizes all invocation results into one command result with HEAD/wave/argv/exit code/per-invocation command id/observed and failed counts/exact `file::describe::test` ids and statuses. Bounded test recorders write `wave-result.json`; H's writer also generates `proof02-repairs.json` only from validated `--repair="<test-id>|<cause-fingerprint>|<owner-login>|<openspec-change>|<merged-pr>|<merge-sha>|<approving-review-id>|<reviewer-login>"` arguments. In `exact-main` mode the validator SHALL fetch `origin/main`, require `HEAD === receipt.sha === refs/remotes/origin/main`, require every repair SHA to be its ancestor, verify through the GitHub API that each PR belongs to this repository, is merged at that SHA, and has the declared author plus an approval by the declared non-author reviewer on the merged head, and use the repair SHA's Git tree to require the named active/archived OpenSpec change; distinct cause fingerprints require pairwise-distinct PR-author logins. `proof02-repairs.json` records the resolved main SHA and immutable PR/review ids. The writer hashes exactly each row's `artifacts`, applies each literal `===true`, `===0`, or `>=1` predicate, and rejects caller-authored command/repair substitutes, extra/missing keys, unrecognized test ids, branch-head exact-main claims, foreign/unmerged/wrong-SHA PRs, author mismatch, self/wrong-head approval, absent repair-SHA OpenSpec trees, post-SHA change paths, same-owner distinct causes, or SHA/path/digest drift. CAMP-PROOF has `wave=camp-proof`, receipt `camp-proof-<sha>/authority-receipt.json`, `assertions=[unknownFieldsRejected===true,missingFieldsRejected===true,headShaMatched===true,pathShaMatched===true,inputDigestsMatched===true,exactMainRegenerated===true]`, `artifacts=[command-result.json,wave-result.json]`. Every later row runs `write --wave=<id> --receipt=<path> --sha=<sha> --input=<non-command-artifact> --command="<literal command>"` then `validate --wave=<id> --receipt=<path> --expected-sha=<sha> --mode=reviewed-head`, and repeats both with merged exact main and `mode=exact-main`. CAMP-PROOF through CAMP-01D are prerequisites; CAMP-01E–H deliver the original outcome.

1. **CAMP-00 — loopback (≤4/180):** `validate:multiplayer:packaged-socket`; receipt `camp00-loopback-<sha>/authority-receipt.json`; `assertions=[boundAddressIsLoopback===true,expectedAddressMatched===true,unspecifiedAddressRejected===true]`; `artifacts=[command-result.json,listener-result.json]`.
...
5. **CAMP-01D — launch enforcement (≤12/450):** exact fast-forward/dashboard/`launchCoopMission` Jest command plus readiness/co-op gates; receipt `camp01d-launch-<sha>/authority-receipt.json`; `assertions=[catalogReady===true,canonicalLaunchSucceeded===true,encounterLookupCount===0,reuseResultCount===0,createEncounterCount===0,launchEncounterCount===0]`; `artifacts=[command-result.json,wave-result.json]`.
