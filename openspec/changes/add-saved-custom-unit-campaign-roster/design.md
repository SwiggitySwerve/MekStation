...
3. **CAMP-01B — authoritative co-op snapshot (≤14/480):** `npm run verify:qc:coop-campaign-journey` plus `src/types/campaign/__tests__/CampaignSync.test.ts`, `src/pages-modules/gameplay/campaigns/__tests__/CampaignCoopEntryPanel.test.tsx`, `src/lib/multiplayer/server/__tests__/CampaignHostRegistry.test.ts`, and `src/lib/campaign/sync/__tests__/sharedCampaignState.integration.test.ts` writes `.sisyphus/evidence/playtest/camp01b-snapshot-<sha>/authority-receipt.json` with campaign/match/revision, membership/source equality, and hydration booleans.
4. **CAMP-01C — participation authorization (≤12/450):** the co-op gate plus `src/types/multiplayer/__tests__/Protocol.test.ts`, `src/lib/multiplayer/server/__tests__/bindCampaignSyncConnection.test.ts`, and `src/lib/campaign/coop/__tests__/coopRuntimeSession.test.ts` writes `.sisyphus/evidence/playtest/camp01c-participation-<sha>/authority-receipt.json` with server-derived identity/role and full-force/forged/foreign/stale rejection booleans.
5. **CAMP-01D — launch-path enforcement (≤12/450):** `npm run qc:command:readiness-stable:quick`, `verify:qc:coop-campaign-journey`, `src/lib/campaign/fastForward/__tests__/fastForwardCombatRunner.test.ts`, and `src/components/gameplay/pages/campaigns/dashboard/__tests__/CampaignDashboardPage.reactivity.test.tsx` writes `.sisyphus/evidence/playtest/camp01d-launch-<sha>/authority-receipt.json` with catalog state, canonical success, and blocked lookup/reuse/create/launch counts.
6. **CAMP-01E — saved-design picker and roster identity (≤10/450):** `npm.cmd test -- --watchAll=false --runTestsByPath` over planned `src/components/gameplay/pages/campaigns/create/__tests__/savedCustomUnitCampaignAdapter.test.ts` plus existing `src/components/gameplay/pages/campaigns/create/__tests__/CreateCampaignPage.RosterStep.test.tsx` and `src/components/gameplay/pages/campaigns/create/__tests__/CreateCampaignPage.rosterPersistence.test.ts` writes `.sisyphus/evidence/playtest/camp01e-picker-<sha>/authority-receipt.json` with saved id, fresh instance id, ref/source equality, root-force membership, programmatic-name, and 390px booleans.
7. **CAMP-01F — durable creation commit (≤8/400):** `npm.cmd test -- --watchAll=false --runTestsByPath` over planned `src/components/gameplay/pages/campaigns/create/__tests__/CreateCampaignPage.submitPersistence.test.tsx` and existing `src/stores/campaign/__tests__/useCampaignPersistenceStore.test.ts` writes `.sisyphus/evidence/playtest/camp01f-persistence-<sha>/authority-receipt.json` with method/status, campaign/ref/source equality, suppressed-failure, same-id retry, and no-conflict-overwrite booleans.
