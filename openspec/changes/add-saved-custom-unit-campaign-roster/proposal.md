...
Before this capability shipped, the campaign creation roster exposed only four representative stock BattleMechs. A player could customize and save a canonical unit, prove that the custom-unit API assigned a durable `custom-*` identity, and still could not select that saved design for a campaign. That historical gap blocked the customizer-to-campaign authority journey and forced campaign creation to discard the design the player just saved.
...
- Keep saved custom units visible and recoverable in mixed-roster mission readiness. The authoritative admission guard SHALL admit supported server-saved biped BattleMechs through the exact custom combat catalog and SHALL return a per-unit blocker for local-only, unsupported, malformed, deleted, or otherwise absent custom references; canonical-only fast-forward remains canonical-only.
- Add a browser trust anchor that proves the same custom id through save, campaign creation, server-backed campaign/force persistence, cold reload, and Mech Bay, then proves both supported custom admission and canonical admission from the mixed roster into playable combat with authoritative session/post-battle persistence after navigation and cold reload.
...
- `mission-contracts`: Preserve saved-custom roster identity at mission readiness while admitting exact supported custom references through the authoritative custom catalog and refusing unresolved or unsupported references; fast-forward remains canonical-only.
...
- Expanding custom combat beyond server-saved, supported biped BattleMechs, or changing the canonical-only fast-forward contract.
...
- No new campaign construction-payload schema, combat engine, or dependency is intended. This parent consumes the separately specified `enable-saved-custom-unit-combat` capability for supported server-saved biped BattleMechs; co-op participation still narrows to verified minimal force choice, and CAMP-01H proves the existing canonical and supported-custom launch routes without broadening fast-forward admission.
