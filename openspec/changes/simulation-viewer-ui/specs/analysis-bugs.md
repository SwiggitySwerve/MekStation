...
function statesEqual(state1: IGameState, state2: IGameState): boolean {
  ...
  for (const unitId in state1.units) {
    ...
    // Check position
    if (unit1.position.q !== unit2.position.q || 
        unit1.position.r !== unit2.position.r) {
      return false;
    }
    
    // Check armor/structure/heat
    if (!deepEqual(unit1.armor, unit2.armor) ||
...
function validateThresholds(thresholds: IAnomalyThresholds): string[] {
  ...
  if (thresholds.heatSuicideThreshold < 0 || thresholds.heatSuicideThreshold > 100) {
...
// Calculate preview
const wouldDetect = currentAnomalies.filter(a => 
  a.type === "heat-suicide" && a.actualValue! > newThreshold
).length; // 2 (35 and 32)

const wouldSuppress = currentAnomalies.filter(a => 
  a.type === "heat-suicide" && a.actualValue! <= newThreshold
).length; // 1 (28)
```
