...
- [ ] 1.1 Trace the canonical `normalizeEquipmentId` in
...
- [ ] 1.2 Inventory importers of each local normalizer
...
- [ ] 1.3 Write a red-first equipment-key agreement probe: feed a fixed corpus
  ...
  fix).
- [ ] 1.4 Write a red-first picker probe: assert the live
...
- [ ] 2.1 Confirm the public signature of the canonical
...
- [ ] 2.2 Route `src/services/conversion/MTFParser.mapping.ts:75` through the
  canonical normalizer (delegate or adapter); replace the local body with a
  re-export shim or hard-delete per task 1.2.
- [ ] 2.3 Route `src/services/units/unitLoaderService/equipmentResolution.ts:132`
...
- [ ] 2.4 Route `src/simulation/runner/UnitHydration.ts:710` through the canonical
...
- [ ] 2.5 Flip the task 1.3 probe to assert all routed call sites now resolve the
  corpus to the *same* canonical key.

## 3. Hex math + picker consolidation

- [ ] 3.1 Delete `_pixelToHex` and `_roundHex` from
...
- [ ] 3.2 Designate `src/utils/gameplay/hexMath.ts` `hexDistance`
  ...
  the shim boundary).
- [ ] 3.3 Keep `hexToPixel`/`pixelToHex` (layout math) in one home
...
- [ ] 3.4 Flip the task 1.4 picker probe to assert the live `pixelToHex` now
...
- [ ] 4.1 Remove the `export * from '@/utils/equipment/weapons/utilities'` runtime
...
- [ ] 4.2 Add an ownership header comment to `src/stores/useForceStore.ts`
...
- [ ] 5.2 Run `npx openspec validate consolidate-equipment-and-hex-duplication
  --strict` and confirm it prints valid.
- [ ] 5.3 Update `docs/audits/2026-06-12-full-codebase-review.md` H-ARCH cluster:
