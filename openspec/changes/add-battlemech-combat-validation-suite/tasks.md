...
- [x] 2.32.11 Add source-backed BattleMech displacement elevation-cap checks for push/charge/DFA displacement helpers, event-sourced resolution, and runner resolution while leaving domino step-out and runtime grounded DropShip source-context hydration explicit.
- [x] 2.32.12 Refresh runner physical-phase grid occupancy after displacement payloads so later same-phase displacement legality sees newly occupied hexes, while leaving domino step-out and runtime grounded DropShip source-context hydration explicit.
...
- [x] 2.32.19 Add source-backed occupied-hex domino positional displacement cascades for push/charge/DFA/charge-miss through helper, event-sourced, and runner resolution while leaving domino step-out fallout and runtime grounded DropShip source-context hydration explicit.
- [x] 2.32.20 Add source-backed DFA-miss friendly occupied displacement avoidance through helper, event-sourced, and runner resolution while leaving domino step-out fallout and runtime grounded DropShip source-context hydration explicit.
...
- [x] 2.32.22 Add source-backed helper coverage for grounded DropShip radius-two `Compute.getValidDisplacement` search order while leaving runtime DropShip footprint/source occupancy hydration explicit.
