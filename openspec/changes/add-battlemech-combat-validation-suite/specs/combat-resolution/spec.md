...
- **AND** Low Arms SHALL stay helper-only until local behavior matches the pinned MegaMek or MekHQ authority
- **AND** the physical-restriction resolver row SHALL remain helper-only while any assigned quirk has source-backed branches that are not represented locally
