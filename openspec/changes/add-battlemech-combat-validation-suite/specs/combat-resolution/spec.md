...
- **AND** request-spot command commits SHALL preserve the active unit id and selected target unit id in their structured payload before any future SpotAction-style lifecycle or wire dispatch consumes the action
...
- **AND** MekStation-only water ground-disallow, terrain LOS side-path, atmosphere, dust, and mines boundaries SHALL use MekStation deviation source references where the behavior or absence is local
