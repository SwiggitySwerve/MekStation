...
- [ ] 1.2 S2: with S1's mirror admitting matches at mode `shadow` / `enabled`, prove `readEffectiveHead` and the live branch admission consult reachable against a REAL journal-backed head for a live match. Per History B's repair (PR #1674) exactly ONE `STALE_BRANCH` arm is reachable on the live path — a host still serving an identity (root / baseline, or a prior `servedBranchId`) the store has since superseded by activation (`!onLivePath`) — answered with the active head and a resync action. A persisted head naming a NON-EFFECTIVE (superseded) branch is a separate shape, unreachable by construction on that path (activation supersedes the prior branch and installs the replacement head in one transaction), and is refused `MATCH_QUARANTINED` (`history-integrity-blocked`) as persisted corruption, naming no head — not `STALE_BRANCH`. Prove the four illegal command shapes the umbrella's 14.2-a receipt names end to end through a journal head installed by S1's mirror, not a hand-constructed fixture head.
