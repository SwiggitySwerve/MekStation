...
- [ ] 7. Add entity-scoped authority schema and atomic store contracts
  What to do / Must NOT do: add additive tables/records for generalized authority-event envelopes, owning streams, indexed stable entity-instance references, receipts, batches, outbox, sessions, participants, viewer cursors, branches, supersession, checkpoints, outcome receipts, private audit references, and append-once private rejection audits keyed by session/command identity; extend durable/in-memory adapters with expected-head append plus entity-history and point-in-time resolution; preserve current snapshots and reads. Store each fact once in one owning stream, never one copy per referenced entity. Do not add an event-store service, broker, workflow engine, CRDT, Git object database, or new runtime dependency in this slice.
  ...
  Acceptance criteria (agent-executable): repeated migrations/backfill are idempotent; constraints reject duplicate authority identities and broken predecessor lineage; one multi-entity command stores one event record with indexed actor/target/session references; a customized unit instance resolves across prior sessions at an explicit branch/head with matching full-replay digest; legacy snapshots remain readable; and durable/in-memory contract suites agree.
