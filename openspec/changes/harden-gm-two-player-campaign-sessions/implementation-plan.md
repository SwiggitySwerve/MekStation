...
- Major-merge definition and gate: every todo marked `Commit: Y` is a major merge. Todo 5 is the only pre-harness implementation merge: update to exact main, rerun its exact native-store QA, and archive the receipt. Todo 26 lands next and establishes `fixture-smoke`. After Todo 26, every major merge updates to exact main, reruns the just-merged task's exact QA, sets `$mergeSha = (git rev-parse --short=12 HEAD).Trim()`, runs the applicable staged three-context subset, and archives both receipts before the next dependent PR: `fixture-smoke` before Todo 15, `membership-smoke` for Todo 15, and the evolving `smoke` subset after Todo 15. Task 34's all-gates runner is the final cutover gate; it is not retroactively required for earlier merges.
...
- Wave 4 — authority, delivery, campaign, replay, and fault foundations: todos 8, 11, 12, 14, 21, 28.
...
- 3 | 26 | 16, 32, 34 | 2, 4 |
- 4 | 26 | 32, 34 | 2, 3 |
- 5 | none | 6, 7, 10, 26, 27, 34 | none |
- 6 | 5 | 7, 10, 11, 28, 34 | 26, 27 |
- 7 | 5, 6, 26 | 8, 12, 14, 19, 21, 28, 34 | 11 |
- 8 | 7 | 9, 13, 17, 19, 34 | 12, 14, 21 |
- 9 | 8 | 13, 19, 20, 29, 34 | 15, 17 |
- 10 | 5, 6, 26 | 11, 12, 29, 34 | 27 |
- 11 | 6, 10 | 13, 18, 29, 34 | 7, 28 |
- 12 | 7, 10 | 13, 15, 17, 29, 34 | 8, 14, 21 |
- 13 | 8, 9, 11, 12 | 29, 33, 34 | 15, 17 |
- 14 | 7 | 15, 19, 23, 32, 34 | 8, 12, 21 |
- 15 | 12, 14, 26 | 16, 17, 25, 29, 30, 32, 34 | 9, 13 |
...
- 26 | 5 | 2, 3, 4, 7, 10, 15, 27, 28, 29, 30, 31, 32, 33, 34 | 6 |
...
- [x] 2. Preserve SUPERHEAVY gyro through unit round-trip
  What to do / Must NOT do: add a failing 101+ weight customized-unit serialize/load round-trip, repair only the enum/mapping seam that downgrades `GyroType.SUPERHEAVY`, and retain all other serialization output, including equipment slot placement.
  Parallelization: Wave 3 | Blocked by: 26 | Blocks: 16, 32, 34.
  References (executor has NO interview context - be exhaustive): `src/utils/serialization/CustomUnitSerializer.ts:139-216`; `src/services/units/unitLoaderService/componentMappers.ts:82-92`; `src/services/units/unitLoaderService/equipmentMapping.ts`; `src/__tests__/unit/utils/serialization/CustomUnitSerializer.test.ts`; `src/__tests__/service/units/EquipmentMapping.test.ts`; OpenSpec `campaign-management` requirement `Customized Units Adopt Canonically`.
  ...
  QA scenarios (name the exact tool + invocation): `npm.cmd test -- --watchAll=false --runInBand --runTestsByPath src/__tests__/service/units/EquipmentMapping.test.ts src/__tests__/service/units/UnitLoaderService.test.ts src/__tests__/service/units/UnitLoaderServiceMappers.test.ts src/__tests__/unit/utils/serialization/UnitSerializer.test.ts src/__tests__/unit/utils/serialization/CustomUnitSerializer.test.ts`; SUPERHEAVY round-trip and unresolved-equipment metadata preservation pass, while unknown-enum fallback remains unchanged; Evidence `.omo/evidence/.../task-2-gm-two-player-live-campaign-sandbox/`.
  Commit: Y | `fix(serialization): preserve superheavy gyro type`
  Verification: PR #1071 squash-merged as `fcebf449c6744b4158428c681e4b2160b4668904`; gyro/schema exact-main regression passed. The follow-up loader mapping fix preserves equipment slot positions, rear mounting, and linked ammo through the 105-ton customized-unit round-trip. Receipts: `.omo/evidence/gm-two-player-live-campaign-sandbox/task-2-gm-two-player-live-campaign-sandbox/gyro-receipt.md` and `.omo/evidence/gm-two-player-live-campaign-sandbox/task-2-gm-two-player-live-campaign-sandbox/slot-roundtrip-receipt.md`.

- [x] 3. Close CAMP-03 zero-BV mission materialization
  ...
  Verification: exact-main reproduction on `6dc7b48829c1376a4f039231c7a0306eb1da97c6` passed the live wizard-to-pre-battle seam with positive BV and authoritative player-unit IDs. The original zero-BV root cause was already repaired by PR #1072 (`44db64e493d8979b4d9442eb8585baac691a9708`), which restored the tracked canonical MegaMek BV source. Adding the missing cold-reload trust anchor exposed a launch race: local state navigated before the debounced authoritative campaign save, so an immediate reload rehydrated encounter and force rows but returned `404` for the campaign. Both briefing launch and dashboard-generated missions now await the shared local-plus-authoritative checkpoint before routing and surface save failure in place. The strengthened Node 22 isolated Playwright proof (`task-3-persisted-mission-current-r8`) passes after reload with positive BV, authoritative unit/pilot identity, and a persisted campaign mission whose scenario IDs contain the launched encounter; the server log records campaign `PUT` before navigation and campaign `GET` after reload.

- [x] 4. Persist Quick Game bootstrap before navigation
  ...
  Parallelization: Wave 3 | Blocked by: 26 | Blocks: 32, 34.
  ...
  Commit: Y | Phase A `fix(quickgame): persist recoverable bootstrap events`; Phase B `fix(quickgame): preserve spectator recovery mode`
  Delivery split: the no-waiver review-size gate requires two sequential PRs. Phase A owns bootstrap persistence, typed rejection/no-adoption, match-key routing, and store-level cold recovery. Phase B owns spectator-route intent through cold reload plus the two-mode browser screenshot proof. Do not mark Todo 4 complete until both PRs merge and pass exact-main audits.
  Verification: Phase A PR #1081 merged as `068cb76f64638cff18c5ad49a94bcde6dc74f1e8` and passed its exact-main audit. Phase B pre-merge verification passed 7 focused suites (39 tests), direct nonincremental TypeScript, changed-file lint/format/no-excuse gates, production build, strict OpenSpec/QC, fixture smoke 1/1, and isolated Chromium interactive plus spectator cold recovery 2/2 with unique screenshots. The game-session page was decomposed below the 400-line lint limit while preserving the 500-changed-line/15-file PR gate. The final exact-main Phase B audit remains required before the next todo begins. Receipt: `.omo/evidence/gm-two-player-live-campaign-sandbox/task-4-gm-two-player-live-campaign-sandbox/quickgame-bootstrap-receipt.md`.
...
- [x] 5. Clear the native SQLite durability preflight
  ...
  Commit: Y | `test(qc): add native sqlite ABI preflight`

- [ ] 6. Capture red authority, crash, and migration baselines
  What to do / Must NOT do: in a disposable isolated worktree, add the future contract tests for atomic batches, receipt collisions, commit-before-publish, crash-before/after commit, idempotent backfill, ambiguous ownership, rollback compatibility, stable intent retry identity, separate authority/viewer sequences, replay/live overlap, sequence collision, and bidirectional heartbeat; record that each fails for its intended missing behavior, then discard only that temporary worktree. Do not commit or merge red tests and do not alter the canonical implementation worktree.
  Parallelization: Wave 2 | Blocked by: 5 | Blocks: 7, 10, 11, 28, 34.
  References (executor has NO interview context - be exhaustive): OpenSpec `tasks.md` 1.2-1.3; `event-store` requirements `Atomic Command Event Batches`, `Command Receipts Persist for the Authority Lifetime`; `campaign-persistence` requirement `Campaign Journal Migration Is Additive and Idempotent`; `multiplayer-sync` intent/viewer-sequence/heartbeat requirements; `src/lib/multiplayer/server/IMatchStore.ts`; `ServerMatchHostIntent.ts:145-193`; `CampaignHostRegistry.ts:170-233`; `src/types/multiplayer/Protocol.ts`; `src/lib/multiplayer/client.ts`.
  Acceptance criteria (agent-executable): the two disposable RED suites fail only the named assertions for partial append, false publication, duplicate receipt, migration ambiguity, rollback compatibility, retry identity, private authority sequence exposure, replay/live overlap, sequence collision, and one-way heartbeat; the disposable worktree is removed after copying a durable red-to-green requirement manifest and console output to evidence; `git diff --exit-code` is clean in the canonical worktree.
  QA scenarios (name the exact tool + invocation): from the disposable worktree run `npm.cmd test -- --watchAll=false --runInBand --runTestsByPath src/lib/multiplayer/server/__tests__/authority-contract.red.test.ts src/lib/multiplayer/__tests__/protocol-client-contract.red.test.ts`; expected result is nonzero with every named missing-contract assertion and no setup/module error; copy the requirement-to-failure manifest and output to Evidence `.omo/evidence/.../task-6-gm-two-player-live-campaign-sandbox/`, then remove the disposable test files with the worktree.
  Commit: N | evidence-only red baseline; Tasks 7-11 and 19 recreate the applicable tests beside their implementations and merge only green slices.

- [ ] 7. Add entity-scoped authority schema and atomic store contracts
  What to do / Must NOT do: add additive tables/records for generalized authority-event envelopes, owning streams, stream-local revisions and predecessor digests, database-wide commit positions, command indexes, indexed stable entity-instance references, receipts, batches, outbox, sessions, participants, viewer cursors, branches, supersession, checkpoints, outcome receipts, private audit references, and append-once private rejection audits keyed by session/command identity; extend durable/in-memory adapters with expected-stream-revision append plus entity-history and point-in-time resolution by stream revision, commit position, or event identity; add a pure deterministic event decoder/upcaster registry that preserves original rows and rejects unknown types/versions; preserve current snapshots and reads. Store each fact once in one owning stream, never one copy per referenced entity. Do not use commit position as an expected global write head, silently ignore an unknown event/version, or add an event-store service, broker, workflow engine, CRDT, Git object database, or new runtime dependency in this slice.
  Parallelization: Wave 3 | Blocked by: 5, 6, 26 | Blocks: 8, 12, 14, 19, 21, 28, 34.
  ...
  Acceptance criteria (agent-executable): repeated migrations/backfill are idempotent; constraints reject duplicate stream revisions, duplicate commit positions, invalid command indexes, and broken stream-scoped predecessor lineage; unrelated streams append without comparing a global expected head; one multi-entity command stores one event record with indexed actor/target/session references; a customized unit instance resolves across prior sessions at an explicit branch/stream revision or commit position with matching full-replay digest; a supported old event version replays through a pure upcaster without rewriting its row; an unknown version or missing pinned replay input quarantines only that session with no partial state/effect; legacy snapshots remain readable; and durable/in-memory contract suites agree.
  QA scenarios (name the exact tool + invocation): `npm.cmd test -- --watchAll=false --runInBand --runTestsByPath src/lib/multiplayer/server/__tests__/AuthoritySchemaMigration.test.ts src/lib/multiplayer/server/__tests__/AuthorityEventReplay.test.ts src/lib/multiplayer/server/__tests__/DurableMatchStore.test.ts src/lib/multiplayer/server/__tests__/InMemoryMatchStore.test.ts src/lib/campaign/persistence/__tests__/campaignMigration.test.ts` (the authority migration and event replay files are created by this task); repeated migration/legacy-read cases and one supported schema upcast pass, while ambiguous ownership, uniqueness collision, unknown version, missing pinned input, and replay-side-effect fixtures fail closed; Evidence `.omo/evidence/.../task-7-gm-two-player-live-campaign-sandbox/`.
...
- [ ] 10. Add stable intent identity and terminal receipts
  ...
  Parallelization: Wave 3 | Blocked by: 5, 6, 26 | Blocks: 11, 12, 29, 34.
...
- [ ] 11. Add gapless viewer delivery and exact-once client application
  ...
  Parallelization: Wave 4 | Blocked by: 6, 10 | Blocks: 13, 18, 29, 34.
...
- [ ] 14. Move shared campaign authority to a durable journal
  ...
  QA scenarios (name the exact tool + invocation): `npm.cmd test -- --watchAll=false --runInBand --runTestsByPath src/lib/multiplayer/server/__tests__/CampaignJournalMigration.test.ts src/lib/multiplayer/server/__tests__/CampaignHostRegistry.test.ts src/lib/multiplayer/server/__tests__/CampaignMatchHost.test.ts src/lib/campaign/persistence/__tests__/campaignMigration.test.ts` (the journal migration file is created by this task); restart recovery passes, while ambiguous migration, stale same-field conflict, and corrupt-journal fixtures block only their campaign; Evidence `.omo/evidence/.../task-14-gm-two-player-live-campaign-sandbox/`.
...
- [ ] 15. Add non-playing GM plus two durable player slots
  ...
  Parallelization: Wave 5 | Blocked by: 12, 14, 26 | Blocks: 16, 17, 25, 29, 30, 32, 34.
  ...
  Acceptance criteria (agent-executable): restart preserves three roles and ownership, third player rejects, cross-force command rejects with zero append, readiness invalidates by revision, GM loss pauses and players never inherit authority; the staged `membership-smoke` proves these roles through the isolated three-context fixture.
  QA scenarios (name the exact tool + invocation): `npm.cmd test -- --watchAll=false --runInBand --runTestsByPath src/lib/multiplayer/server/__tests__/CampaignSyncSession.test.ts src/lib/multiplayer/server/__tests__/CampaignGmArbiter.test.ts src/lib/multiplayer/server/__tests__/CampaignMatchHost.test.ts`, then `npm.cmd run verify:qc:gm-two-player-campaign -- --group=membership-smoke --run-id=task-15-membership-smoke`; GM/P1/P2 join-ready-resume survives restart, while third-player, cross-owner, and GM-loss fixtures reject or pause with zero unauthorized append; Evidence `.omo/evidence/.../task-15-gm-two-player-live-campaign-sandbox/`.
...
- [ ] 18. Enforce replay snapshot timeline and export parity
  What to do / Must NOT do: reuse the same projector for replay/resync/snapshot/cold recovery/timeline/export; add per-viewer digests; authorize private GM audit lookup separately; append rejection audits once by session/command identity without gameplay or player-surface mutation; negative-scan raw and rendered artifacts.
  ...
  Acceptance criteria (agent-executable): same viewer/event has equal authorized fields/digest on all surfaces; player cannot fetch private audit reference; rejected-command retry retains exactly one authorized audit row and no gameplay/player-visible fact; forbidden-term and inferable-gap scans are empty.
  QA scenarios (name the exact tool + invocation): `npm.cmd test -- --watchAll=false --runInBand --runTestsByPath src/lib/multiplayer/server/__tests__/ViewerProjectionParity.test.ts src/lib/multiplayer/server/__tests__/rejectionAudit.test.ts src/lib/multiplayer/server/__tests__/replayStream.test.ts src/engine/__tests__/InteractiveSession.recovery.test.ts`, then `npm.cmd run verify:qc:replay-recovery` (the parity and rejection-audit files are created by this task); live/replay/snapshot/timeline/export digests match for each viewer, duplicate rejection identity keeps one private audit row, the full replay-recovery gate passes, and player-private-audit lookup plus forbidden-term/gap scans reject; Evidence `.omo/evidence/.../task-18-gm-two-player-live-campaign-sandbox/`.
...
- [ ] 22. Activate append-only combat rewind branches
  ...
  QA scenarios (name the exact tool + invocation): `npm.cmd test -- --watchAll=false --runInBand --runTestsByPath src/engine/__tests__/InteractiveSession.rewind.test.ts src/engine/__tests__/InteractiveSession.recovery.test.ts src/lib/multiplayer/server/__tests__/ServerMatchHost.fogOfWarIntegration.test.ts src/lib/multiplayer/server/__tests__/reconnectionFlow.test.ts`, then `npm.cmd run verify:qc:gm:campaign-ledger` (the rewind file is created by this task); pre-receipt rewind and the full GM campaign-ledger gate pass, while player commit, rebuilding/stale command, and post-receipt rewind reject; Evidence `.omo/evidence/.../task-22-gm-two-player-live-campaign-sandbox/`.
...
- [x] 26. Build isolated three-context browser fixture
  ...
  Acceptance criteria (agent-executable): contexts have distinct auth/storage and identities, the fixture exposes deterministic server/session/database handles without ambient-process ownership, and the new package script accepts only the registered groups `fixture-smoke,membership-smoke,evidence-smoke,fault-smoke,smoke,authority,visibility,combat,campaign,failure,performance,all,traceability,quality,manual-setup,scope`; capability-dependent groups initially return typed `NOT_IMPLEMENTED` until their owning todo wires them.
  QA scenarios (name the exact tool + invocation): `npm.cmd run verify:qc:gm-two-player-campaign -- --group=fixture-smoke --run-id=task-26-fixture-smoke`; three isolated contexts reach the harness-owned safe baseline and preserve distinct storage/identities, while cross-context storage contamination, foreign process/database cleanup, or unknown group exits nonzero; durable role admission is intentionally not asserted until Todo 15; Evidence `.omo/evidence/.../task-26-gm-two-player-live-campaign-sandbox/`.
...
- [ ] 27. Add evidence bundle and truly read-only durable probe
  ...
  Parallelization: Wave 3 | Blocked by: 5, 26 | Blocks: 29, 30, 31, 32, 33, 34.
...
- [ ] 28. Add production-rejected scoped fault seams
  ...
  Parallelization: Wave 4 | Blocked by: 6, 7, 26 | Blocks: 29, 30, 31, 32, 33, 34.
...
- [ ] 30. Implement strict visibility/finalization E2E-19 through E2E-30
  ...
  Acceptance criteria (agent-executable): the exact visibility command below passes all 12; negative searches find zero GM-private/opponent-hidden/stream-revision/commit-position/private-ID/inferable-gap data in player artifacts.
...
- [ ] 33. Implement strict performance/UX/evidence E2E-71 through E2E-80
  ...
  Acceptance criteria (agent-executable): the exact performance command below records p95<=250 ms, p99<=750 ms, catch-up<=2 s, the numeric queue/chunk/RSS limits above, healthy GM/P1 p95<=250 ms and p99<=750 ms while P2 is slowed, peak RSS for every process, all nine accessible states, complete evidence, scoped cleanup on the recorded runner class, and a green long-campaign validator.
  QA scenarios (name the exact tool + invocation): `$runId = "task-33-$([DateTimeOffset]::UtcNow.ToUnixTimeMilliseconds())"; npm.cmd run verify:qc:gm-two-player-campaign -- --group=performance --run-id=$runId --warmups=20 --samples=200; if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }; npm.cmd run verify:qc:campaign-long`; budgets and the long-campaign gate pass, while injected slow-client, threshold, and foreign-cleanup fixtures fail with retained evidence; Evidence `test-results/gm-two-player/$runId/`.
