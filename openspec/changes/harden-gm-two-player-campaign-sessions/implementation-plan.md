...
- [ ] 4. Persist Quick Game bootstrap before navigation
  ...
  Verification: Phase A focused tests prove exact `GameCreated`,`GameStarted` writes, match metadata, cold reset/load recovery for interactive and spectator sessions, typed storage failure with no adoption, match-ID navigation, and no stale-session navigation. Phase B and the final Todo 4 receipt remain pending. Receipt: `.omo/evidence/gm-two-player-live-campaign-sandbox/task-4-gm-two-player-live-campaign-sandbox/quickgame-bootstrap-receipt.md`.
