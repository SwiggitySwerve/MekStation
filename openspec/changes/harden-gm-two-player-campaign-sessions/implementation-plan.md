...
- [ ] 7. Add entity-scoped authority schema and atomic store contracts
  What to do / Must NOT do: add additive tables/records for generalized authority-event envelopes, owning streams, stream-local revisions and predecessor digests, database-wide commit positions, command indexes, indexed stable entity-instance references, receipts, batches, outbox, sessions, participants, viewer cursors, branches, supersession, checkpoints, outcome receipts, private audit references, and append-once private rejection audits keyed by session/command identity; extend durable/in-memory adapters with expected-stream-revision append plus entity-history and point-in-time resolution by stream revision, commit position, or event identity; preserve current snapshots and reads. Store each fact once in one owning stream, never one copy per referenced entity. Do not use commit position as an expected global write head, and do not add an event-store service, broker, workflow engine, CRDT, Git object database, or new runtime dependency in this slice.
