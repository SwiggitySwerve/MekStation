...
4. Completed sidecars: `refine-combat-turn-rail` is archived after its canonical specs were synchronized; `persist-interactive-combat-events` is a merged product outcome whose separate OpenSpec archive closeout remains pending. That remaining closeout is workflow hygiene rather than an authority dependency and should finish before the next unrelated runtime branch.
...
- [ ] 7. Add authority schema and atomic store contracts
  What to do / Must NOT do: add additive tables/records for receipts, batches, outbox, sessions, participants, viewer cursors, branches, supersession, checkpoints, outcome receipts, private audit references, and append-once private rejection audits keyed by session/command identity; extend durable/in-memory adapters; preserve current snapshots and reads.
