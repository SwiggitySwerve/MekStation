...
- [ ] 2.4 Add migration/entity-history/idempotency/rollback contract tests, including one-record multi-entity references, customized-unit instance lineage across sessions, explicit point-in-time replay proof, one supported schema upcast without stored-row mutation, unknown-version and missing-pinned-input quarantine with no partial effects, repeated backfill, ambiguous ownership fail-closed behavior, and a schema reader compatible with preserved materialized snapshots; run `npm.cmd test -- --runInBand src/lib/multiplayer/server/__tests__/AuthorityEventReplay.test.ts src/lib/multiplayer/server/__tests__/DurableMatchStore.test.ts src/lib/campaign/persistence/__tests__/campaignMigration.test.ts`.
