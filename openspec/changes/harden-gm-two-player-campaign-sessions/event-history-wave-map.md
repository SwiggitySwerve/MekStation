...
7. Every leaf implementation PR MUST stay under 1000 non-generated changed lines and 15 changed files and MUST own one user-visible or contract-visible behavior seam. If a named task group cannot fit, update and re-review its OpenSpec to split the PR before implementation; there is no size-cap exception. (Line cap raised 500 -> 1000 on 2026-08-28 by program-owner decision to fit CLI-worker throughput; the one-seam rule, the 15-file cap, and mandatory adversarial mutation review are unchanged. Receipts dated before 2026-08-28 were measured against the 500 cap then in force and are not rewritten.)
