...
- `coop-campaign-sync`: Add one non-playing GM plus two tactical player memberships, per-player force ownership and readiness, process-restart recovery, explicit GM-loss pause, and scenario progression convergence gates. Also MODIFIES `Co-op campaign command authority projection` to name which host screen carries which GM control: approve and veto per pending proposal on the co-op command screen; preview, manual takeover and GM correction on the GM ledger route (roadmap U91, owner decision PK-u90-controls, 2026-09-25).
...
- `gm-combat-interventions`: Add GM-only rewind commit authority with non-mutating player rewind requests, the retryable `PROJECTION_REBUILDING` refusal during rebuild, and the coordinated post-receipt correction boundary. WITHDRAWN 2026-09-04 (owner ruling on the 2026-09-03 inventory-and-sync council): finalized correction commits, replacement-branch creation and activation, stale-branch rejection, and per-viewer fog rebuild moved verbatim to the successor change `adopt-combat-journal-cutover-and-gm-rewind` because the combat journal cutover they depend on is not funded here (boxes 14.1, 14.2, 14.4, 14.5 are gated on `journal-cutover`; rows E2E-40 to 44 and E2E-76 run as strict expected failures until it lands).
...
- **Related planning:** the execution order, small-PR boundaries, proof gates, and rollback points are maintained in `implementation-plan.md`; `event-history-wave-map.md` assigns the overlapping journal/replay/privacy/combat/campaign/effect/branch work to focused leaf changes while this package remains the program-level acceptance contract.
