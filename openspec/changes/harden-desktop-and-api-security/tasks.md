...
- [x] 1.1 Write a red-first desktop IPC probe: a Jest test that invokes the
...
- [x] 1.2 Enumerate every production caller of `electronAPI.readFile`/`writeFile`/
...
- [x] 1.3 Run `npx tsc --noEmit` against the desktop sources (after a trial removal of
      ...
      surface risk).
- [x] 1.4 Write a red-first API probe: a test that fires N rapid
...
- [x] 2.1 Add a `resolveWithinSandbox(filePath, allowedRoots)` helper (in
...
- [x] 2.2 Wire `resolveWithinSandbox` into the `read-file` (`main.ipc.ts:158`),
...
- [x] 2.3 Build the allowed-root set from the Electron `userData` data dir
      (`main.window.ts:163` pattern) and the `SettingsService` backup directory; pass
      it into `setupIpcHandlers` via `IMainIpcContext`.
- [x] 2.4 Add unit tests covering: in-root read/write succeeds; out-of-root read/write
...
- [x] 3.1 Define a single exported CSP policy constant (e.g.
...
- [x] 3.2 Pin the same policy in the desktop main process via
...
- [x] 3.3 Add a `will-navigate` handler to `mainWindow.webContents` in `main.window.ts`
...
- [x] 3.4 Replace the unconditional `shell.openExternal(url)` in
      ...
      parse failure.
- [x] 3.5 Shake out CSP violations against the running renderer (report-only), tighten
      `script-src`/`style-src` to the minimum the real app needs, then flip the header
      to enforcing `Content-Security-Policy`.
- [x] 3.6 Add tests/assertions: window factory registers a `will-navigate` handler and
      an `onHeadersReceived` CSP pin; `openExternal` is called for `https:` and NOT for
      `file:`/`javascript:`.

## 4. Desktop verification coverage in CI

- [x] 4.1 Bring desktop sources into a typechecked surface (remove `desktop/**/*` from
...
- [x] 4.2 Add a `desktop-tests` job to `.github/workflows/pr-checks.yml` that installs
      and runs the desktop Jest suite (`desktop/` working dir).
- [x] 4.3 Add the new desktop typecheck + `desktop-tests` jobs to the `lint-and-test`
...
- [x] 5.1 Create `src/lib/api/` boundary module: `parseBody(schema, req, res)` (Zod
...
- [x] 5.2 Author Zod schemas for the abuse-surface route bodies and replace the
...
- [x] 5.3 Apply `rateLimit` in front of the KDF/creation work in
      ...
      over limit.
- [x] 5.4 Apply `applySecurityHeaders` on the responses of the three hardened routes.
- [x] 5.5 Add an API boundary integration suite: malformed body → 400 before any
...
- [x] 6.1 Full verification: `npx tsc --noEmit` (now including desktop), lint, the new
...
- [x] 6.2 Run `npx openspec validate harden-desktop-and-api-security --strict` and
      confirm it reports valid.
- [x] 6.3 Update `docs/audits/2026-06-12-full-codebase-review.md` cluster S / Gaps
