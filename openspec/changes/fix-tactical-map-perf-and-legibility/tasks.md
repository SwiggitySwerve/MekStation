...
- [ ] 1.1 Grep every reader of `ITacticalMapHexProjection.isHovered` /
...
- [ ] 1.2 Write a red-first render-perf probe: mount `HexMapDisplay` with a
...
- [ ] 1.3 Write a red-first firing-arc probe: render `FiringArcOverlay` for a
...
- [ ] 2.1 Remove `hoveredHex` (and `selectedHex`) from
...
- [ ] 2.2 Remove `hoveredHex`/`selectedHex` from the
  ...
  mouse-move.
- [ ] 2.3 Recompute selection-derived presentation (intent/status that used
  ...
  for unchanged cells.
- [ ] 2.4 Confirm projection-object identity for an unchanged hex is referentially
  equal before and after a pure hover change.

## 3. Bound firing-arc text labels (T-3)

- [ ] 3.1 In `FiringArcOverlayComponent` (`FiringArcOverlay.tsx:276-315`) replace
...
- [ ] 3.2 Zoom-gate label visibility: below a configured zoom threshold, suppress
...
- [ ] 3.3 Confirm `classifyFiringArcHexes` output and the arc fill/shape area
...
- [ ] 4.1 Confirm this change does NOT modify `HexCell.labels.tsx`; note in the PR
...
- [ ] 5.1 Promote the 1.2 probe to a passing render-scope assertion: a pure hover
  change re-renders only the entered + exited cells (not the full board).
- [ ] 5.2 Add a projection-identity test: the `tacticalMapProjectionLookup` map
...
- [ ] 5.3 Promote the 1.3 probe to a passing assertion: arc text-label node count
...
- [ ] 6.1 Full verification: `npx tsc --noEmit --skipLibCheck`, lint, the affected
  ...
  --strict`.
- [ ] 6.2 Update `docs/audits/2026-06-12-full-codebase-review.md` (or the
