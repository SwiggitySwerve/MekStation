...
> Progress + sequencing note (2026-08-20): the CAMP-01F/G/H gate is satisfied (all landed 2026-08-12) and implementation opened. Re-verified baseline: the proposal's 2026-08-07 deep-link evidence is stale — cold deep links now resolve from the server on dashboard and subpages, and unknown ids 404 honestly (live cold-context probe, this date). Landed slices: task 1.5's client half (list-fetch failure surfaces an error state with retry instead of the false empty state) and task 1.3's journal-free half (storage-rehydrated campaigns are head-validated once per session via the new `rehydratedCampaignId` mark + route-loader refetch-replace; dirty state exempt; in-session creations unmarked). **Discovered overlap, unreconciled:** the journal-bound remainder of Group 1 (genesis events, command→append pipeline, stream-revision cache keys) overlaps `adopt-campaign-event-journal-authority` — the campaign-journal implementation slice of `harden-gm-two-player-campaign-sessions`, which carries its own dependency gates and the clause "overlapping umbrella deltas SHALL be reconciled through the program wave map"; neither change references the other. Journal-bound tasks here SHALL NOT be implemented until the program owner reconciles the two (supersede, sequence, or merge); non-overlapping tasks may proceed.
