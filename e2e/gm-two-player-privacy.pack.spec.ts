 ...
 * These rows require the ladder's journal fixture arm. They do not
 *   establish production cutover, fog support or a non-playing GM seat;
 *   an armed refusal is a product blocker, never an expected failure.
 * WHAT EACH ROW PROVES TODAY, measured under the arm on 2026-09-22:
 *   E2E-19 passes end to end. The preview answers 200 and the GM-only
 *   readback, the payload-free player read, the granted/denied access
 *   audit pair and the negative search of every player view all hold,
 *   so the draft-privacy letter is proven on this channel.
 *   E2E-25 reaches the approve door and is refused there. The reason
 *   does travel (the commit request carries it) and the correction
 *   lease does admit the named head and digest, so the row proves the
 *   transport and the lease binding; it does NOT yet prove the
 *   redaction letter, because the commit never commits.
 * WHAT E2E-25 IS BLOCKED ON, and why it is not an expected failure:
 *   the commit refuses `candidate-verification-failed` - the candidate
 *   branch anchors to the JOURNAL's event at the base revision, while
 *   `materializeBranchPath` re-derives the parent segment through
 *   `matchStoreBranchSegmentReader`, which mints its own ids and
 *   digests from the match store's parallel line. Under the arm the
 *   two lines carry different event ids for the same revision, so the
 *   anchor check cannot pass. That is the journal cutover's work, not
 *   this pack's: the row stays a strict failure until a journal-backed
 *   segment reader serves this path. Weakening it, or marking it an
 *   expected failure, would hide a real refusal.
 *
