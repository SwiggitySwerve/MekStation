 ...
 * Admission measurement (2026-09-21): both previews return STALE_BRANCH
 *   with the arm enabled; approve returns correction-lease-held because
 *   expectedDigest is empty. The E2E-19/25 assertions after those doors
 *   remain unproven until the lobby can name an admissible rewind head.
 *
  ...
  test('E2E-25 approved correction keeps its reason private @E2E-25', async ({
    ...
    try {
      ...
      await fixture.hostPage
        ...
        .fill(reason);
      const preview = await previewCorrection(fixture);
      // Soft only so a refused preview also measures the real approve door.
      expect.soft(preview.status(), await preview.text()).toBe(200);
