...
const minorDisc = report.allResults.filter(
  (r: any) => r.rootCause === 'minor-discrepancy',
);
...
console.log(
  `  Under-calculated: ${underCalc.length} (avg ${(underCalc.reduce((s: number, r: any) => s + r.percentDiff, 0) / underCalc.length).toFixed(2)}%)`,
);
console.log(
  `  Over-calculated: ${overCalc.length} (avg ${(overCalc.reduce((s: number, r: any) => s + r.percentDiff, 0) / overCalc.length).toFixed(2)}%)`,
);

// Analyze by tech base
const byTech: Record<
  string,
  { count: number; avgPct: number; under: number; over: number }
> = {};
...
for (const [tech, data] of Object.entries(byTech)) {
  console.log(
    `  ${tech}: ${data.count} units, avg ${(data.avgPct / data.count).toFixed(2)}%, under=${data.under}, over=${data.over}`,
  );
}
...
for (const [range, data] of Object.entries(byTonnage)) {
  console.log(
    `  ${range}: ${data.count} units, avg ${(data.avgPct / data.count).toFixed(2)}%`,
  );
}
...
const sortedUnder = underCalc
  .sort((a: any, b: any) => a.percentDiff - b.percentDiff)
  .slice(0, 15);
for (const r of sortedUnder) {
  const bd = r.breakdown;
  console.log(
    `  ${(r.chassis + ' ' + r.model).padEnd(35)} idx=${r.indexBV} calc=${r.calculatedBV} diff=${r.difference} (${r.percentDiff.toFixed(1)}%) def=${bd?.defensiveBV?.toFixed(0)} off=${bd?.offensiveBV?.toFixed(0)} wep=${bd?.weaponBV?.toFixed(0)} ammo=${bd?.ammoBV?.toFixed(0)} spd=${bd?.speedFactor?.toFixed(2)} defEq=${bd?.defensiveEquipBV?.toFixed(0)}`,
  );
}

console.log('\nSample over-calculated minor discrepancies (sorted by % diff):');
const sortedOver = overCalc
