...
const underCalc = report.allResults.filter(
  (r: any) => r.percentDiff < -1.0 && r.percentDiff > -5.0,
);
    ...
    features.push({
      ...
      hasTC: allSlots.some(
        (s) =>
          ...
          s.includes('targetingcomputer') ||
          s === 'istargetingcomputer' ||
          s === 'cltargetingcomputer',
      ),
      hasTSM: allSlots.some(
        (s) => s.includes('triple strength myomer') || s.includes('tsm'),
      ),
      hasMASC: allSlots.some(
        (s) => s.includes('masc') && !s.includes('supercharger'),
      ),
      hasSupercharger: allSlots.some((s) => s.includes('supercharger')),
      hasArtemis: allSlots.some(
        (s) =>
          s.includes('artemis') &&
          !s.includes('ammo') &&
          !s.includes('capable'),
...
const featureNames: (keyof UnitFeatures)[] = [
  'hasTC',
  'hasTSM',
  'hasMASC',
  'hasSupercharger',
...
for (const feat of featureNames) {
  ...
  if (withFeat.length > 0) {
    const avgWith =
      withFeat.reduce((s, f) => s + f.pctDiff, 0) / withFeat.length;
    const avgWithout =
      withoutFeat.length > 0
        ? withoutFeat.reduce((s, f) => s + f.pctDiff, 0) / withoutFeat.length
        : 0;
    console.log(
      `  ${String(feat).padEnd(25)} present=${String(withFeat.length).padStart(4)} avgPct=${avgWith.toFixed(2).padStart(7)}% | absent=${String(withoutFeat.length).padStart(4)} avgPct=${avgWithout.toFixed(2).padStart(7)}%`,
    );
  }
}
...
const tcUnits = features
  .filter((f) => f.hasTC)
  .sort((a, b) => a.pctDiff - b.pctDiff);
for (const f of tcUnits.slice(0, 10)) {
  console.log(
    `  ${f.id.padEnd(45)} gap=${f.diff} (${f.pctDiff.toFixed(2)}%) ammoBV=${f.ammoBV}`,
  );
}
...
for (const tb of ['INNER_SPHERE', 'CLAN', 'MIXED']) {
  ...
  if (tbUnits.length > 0) {
    ...
    console.log(
      `  ${tb.padEnd(15)} n=${String(tbUnits.length).padStart(4)} avgPct=${avg.toFixed(2).padStart(7)}%`,
    );
  }
}
