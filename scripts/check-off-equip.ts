...
const underCalc = report.allResults.filter(
  (r: any) => r.percentDiff < -1.0 && r.percentDiff > -5.0 && r.breakdown,
);

console.log(`Undercalculated minor-disc: ${underCalc.length}\n`);

// Collect all equipment across gap units
const equipCounts: Record<
  string,
  { count: number; totalGap: number; units: string[] }
> = {};
...
for (const r of underCalc) {
  ...
  try {
    ...
    for (const eq of u.equipment) {
      ...
      if (!equipCounts[id])
        equipCounts[id] = { count: 0, totalGap: 0, units: [] };
      equipCounts[id].count++;
      equipCounts[id].totalGap += gap;
      if (equipCounts[id].units.length < 3)
        equipCounts[id].units.push(r.unitId);
    }
      ...
      for (const [loc, slots] of Object.entries(u.criticalSlots)) {
        ...
        for (const slot of slots) {
          ...
          if (
            ...
            slo.includes('sensors') ||
            slo.includes('cockpit') ||
            slo.includes('shoulder') ||
            slo.includes('upper arm') ||
            slo.includes('lower arm') ||
            slo.includes('hand') ||
            slo.includes('hip') ||
            slo.includes('upper leg') ||
            slo.includes('lower leg') ||
            slo.includes('foot') ||
            slo.includes('engine') ||
            slo === 'is double heat sink' ||
            slo === 'cl double heat sink' ||
            slo === 'clan double heat sink' ||
            slo.includes('laser heat sink') ||
            slo.includes('armor') ||
            slo.includes('ammo') ||
            slo.includes('case') ||
          ...
          const hasMatch = Array.from(equipIds).some(
            (eid) =>
              eid.includes(critNorm) ||
              critNorm.includes(eid.replace(/-/g, '')) ||
              slo.includes(eid.replace(/-/g, ' ')),
          );
          if (!hasMatch) {
            const key = 'CRIT:' + slo;
            if (!equipCounts[key])
              equipCounts[key] = { count: 0, totalGap: 0, units: [] };
            equipCounts[key].count++;
            equipCounts[key].totalGap += gap;
            if (equipCounts[key].units.length < 3)
              equipCounts[key].units.push(r.unitId);
          }
        }
      }
    }
...
const sorted = Object.entries(equipCounts).sort(
...
for (const r of underCalc) {
  ...
  try {
    ...
    const rawGap =
      (r.indexBV - b.defensiveBV) / b.speedFactor -
      b.offensiveBV / b.speedFactor;
    console.log(
      `${r.unitId.padEnd(45)} gap=${String(r.difference).padStart(5)} rawGap=${rawGap.toFixed(1).padStart(6)} wBV=${b.weaponBV} ton=${u.tonnage} equip=${u.equipment.map((e: any) => e.id).join(', ')}`,
    );
  } catch (_error) {
    // Ignore expected failure in one-off tooling.
    void _error;
