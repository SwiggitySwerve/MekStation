...
for (const u of sorted.slice(0, 20)) {
  ...
  console.log(
    `${u.unitId} (${unit.techBase}, ${unit.tonnage}t) ref=${u.indexBV} calc=${u.calculatedBV} diff=+${u.difference} (+${u.percentDiff.toFixed(1)}%)`,
  );
  console.log(
    `  cockpit=${unit.cockpit || 'STANDARD'} mod=${b?.cockpitModifier} gyro=${unit.gyro?.type || 'STANDARD'}`,
  );
  console.log(
    `  defBV=${b?.defensiveBV?.toFixed(0)} offBV=${b?.offensiveBV?.toFixed(0)} SF=${b?.speedFactor}`,
  );
  console.log(
    `  weapBV=${b?.weaponBV?.toFixed(0)} rawWeapBV=${b?.rawWeaponBV?.toFixed(0)} halvedBV=${b?.halvedWeaponBV?.toFixed(0)}`,
  );
  console.log(
    `  ammoBV=${b?.ammoBV} weightBonus=${b?.weightBonus?.toFixed(0)} physBV=${b?.physicalWeaponBV?.toFixed(0)} offEquip=${b?.offEquipBV}`,
  );
  console.log(
    `  HE=${b?.heatEfficiency} heatDiss=${b?.heatDissipation} moveHeat=${b?.moveHeat}`,
  );
  console.log(
    `  armorBV=${b?.armorBV?.toFixed(0)} structBV=${b?.structureBV?.toFixed(0)} gyroBV=${b?.gyroBV?.toFixed(0)}`,
  );
  console.log(
    `  defFactor=${b?.defensiveFactor} maxTMM=${b?.maxTMM} explPenalty=${b?.explosivePenalty}`,
  );
  ...
  console.log(
    `  Expected offBV: ${offExpected.toFixed(0)}, actual: ${b?.offensiveBV?.toFixed(0)}, excess: ${offOver.toFixed(0)}`,
  );
  ...
  const hasTSM = (unit.equipment || []).some((eq: any) =>
    eq.id.toLowerCase().includes('tsm'),
  );
  const hasMASC = (unit.equipment || []).some((eq: any) =>
    eq.id.toLowerCase().includes('masc'),
  );
  const hasSC = (unit.equipment || []).some((eq: any) =>
  ...
  // Movement
  console.log(
    `  Move: walk=${unit.movement?.walk} run=${unit.movement?.run} jump=${unit.movement?.jump || 0} runMP=${b?.runMP} jumpMP=${b?.jumpMP}`,
  );
