  ...
  try {
    ...
    if (weaponSlots.size > 0 && res.breakdown.weaponBV === 0) {
      ...
      for (const w of weaponSlots)
        unresolvedWeaponCounts.set(w, (unresolvedWeaponCounts.get(w) || 0) + 1);
    }
  } catch (_error) {
    // Ignore expected failure in one-off tooling.
    void _error;
...
for (const res of under) {
  ...
  try {
    ...
    const hasSpecial = specialWeapons.some((sw) =>
      allSlots.some((s) => s.includes(sw)),
    );
    if (hasSpecial) {
      hasSpecialCount++;
...
console.log(
  `Without special weapons: ${noSpecialCount} (avgGap=${(noSpecialGapSum / noSpecialCount).toFixed(1)})`,
);
...
for (const [w, v] of Object.entries(weaponPresence).sort(
  (a, b) => b[1].count - a[1].count,
)) {
  console.log(
    `  ${w}: ${v.count} units (avgGap=${(v.totalGap / v.count).toFixed(1)})`,
  );
}
...
for (const res of defGapUnits) {
  ...
  try {
    ...
    if (specialWeapons.some((sw) => allSlots.some((s) => s.includes(sw))))
      defGapWithSpecial++;
