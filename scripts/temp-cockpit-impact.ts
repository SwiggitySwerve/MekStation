...
interface Result {
  ...
  percentDiff: number;
  status: string;
  breakdown?: { defensiveBV: number; offensiveBV: number };
}
...
for (const r of report.allResults) {
  ...
  if (Math.abs(impliedMod - 0.95) < 0.01) {
    ...
    if (cockpit === 'STANDARD') {
      ...
      const newAbsPct = Math.abs(
        ((withoutSmall - r.indexBV) / r.indexBV) * 100,
      );
      ...
      if (oldAbsPct < newAbsPct) {
        ...
        if (hurt <= 10) {
          console.log(
            `HURT: ${r.chassis} ${r.model} idx=${r.indexBV} withSmall=${withSmall} withoutSmall=${withoutSmall} oldPct=${oldAbsPct.toFixed(1)}% newPct=${newAbsPct.toFixed(1)}%`,
          );
        }
      } else {
        neutral++;
