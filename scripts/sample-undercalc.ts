...
for (const band of [
  ...
  const pool = underCalc.filter(
    (r: any) => r.percentDiff >= band[0] && r.percentDiff < band[1],
  );
...
for (const r of samples) {
  ...
  console.log(
    `  Index BV: ${r.indexBV}  Calc BV: ${r.calculatedBV}  Gap: ${r.difference} (${r.percentDiff.toFixed(2)}%)`,
  );
  console.log(`  Defensive BV: ${b.defensiveBV?.toFixed(2)}`);
  console.log(
    `    armorBV: ${b.armorBV?.toFixed(2)}, structBV: ${b.structureBV?.toFixed(2)}, gyroBV: ${b.gyroBV?.toFixed(2)}`,
  );
  console.log(
    `    defEquipBV: ${b.defensiveEquipBV?.toFixed(2)}, explosivePen: ${b.explosivePenalty?.toFixed(2)}`,
  );
  ...
  console.log(
    `    physicalBV: ${b.physicalWeaponBV?.toFixed(2) || 'N/A'}, weightBonus: ${b.weightBonus?.toFixed(2) || 'N/A'}`,
  );
  ...
  console.log(
    `  Base (def+off): ${currentBase.toFixed(2)}, need: ${neededBase.toFixed(2)}, delta: ${(neededBase - currentBase).toFixed(2)}`,
  );
  ...
  console.log(
    `  Unique equipment: ${uniqueSlots.filter((s) => !s.includes('Actuator') && !s.includes('Engine') && !s.includes('Gyro') && !s.includes('Life Support') && !s.includes('Sensors') && !s.includes('Cockpit') && !s.includes('Endo') && !s.includes('Ferro')).join(', ')}`,
  );
}
