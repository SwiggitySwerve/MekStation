...
const units = r.allResults.filter(
  (u) => Math.abs(u.percentDiff) > 0.1 && u.breakdown,
);
...
for (const [f, v] of Object.entries(byFactor).sort(
  ...
  console.log(
    '  factor=' +
      ...
      v.under +
      ' bias=' +
      (v.over > v.under ? '+OVER' : v.under > v.over ? '-UNDER' : 'EVEN'),
  );
}
...
for (const [f, v] of Object.entries(bySF).sort(
  ([a], [b]) => Number(a) - Number(b),
)) {
...
for (const u of highExpl.slice(0, 10)) {
  ...
  console.log(
    '    ' +
      ...
      b.explosivePenalty +
      ' base=' +
      (b.armorBV + b.structureBV + b.gyroBV) +
      ' diff=' +
