...
console.log(
  `=== ${overcalcWithAmmo.length} Overcalculated Units with Ammo but 0 Penalty ===\n`,
);
...
for (const entry of overcalcWithAmmo.slice(0, 12)) {
  ...
  console.log(
    `--- ${entry.unitId} (${u.tonnage}t, ${techBase}, engine: ${engineType}) ---`,
  );
  console.log(
    `  Overcalc: +${entry.difference} (+${entry.percentDiff.toFixed(1)}%)`,
  );

  // Collect per-location info
  const locInfo: Record<
    string,
    { ammo: string[]; case1: boolean; case2: boolean; weapons: string[] }
  > = {};
  ...
  if (u.criticalSlots) {
    for (const [loc, slots] of Object.entries(u.criticalSlots)) {
      if (!Array.isArray(slots)) continue;
      if (!locInfo[loc])
        locInfo[loc] = { ammo: [], case1: false, case2: false, weapons: [] };
      ...
      for (const slot of slots) {
        ...
        if (lo.includes('ammo') && !lo.includes('ammo feed')) {
          const nonExplosive =
            ...
            lo.includes('nail');
          if (!nonExplosive)
            locInfo[loc].ammo.push(slot.replace(/\s*\(omnipod\)/gi, '').trim());
        }
        if (
          lo.includes('case ii') ||
          lo.includes('caseii') ||
        ...
        if (lo.includes('gauss') && !lo.includes('ammo')) {
          locInfo[loc].weapons.push(
            slot.replace(/\s*\(omnipod\)/gi, '').trim() + ' [gauss=explosive]',
          );
        }
      }
    }
  ...
  if (techBase === 'CLAN') {
    if (locInfo['LEFT_TORSO'] && !locInfo['LEFT_TORSO'].case2)
      locInfo['LEFT_TORSO'].case1 = true;
    if (locInfo['RIGHT_TORSO'] && !locInfo['RIGHT_TORSO'].case2)
      locInfo['RIGHT_TORSO'].case1 = true;
  }
  ...
  for (const [loc, info] of Object.entries(locInfo)) {
    ...
    const isProtected =
      info.case2 ||
      (info.case1 &&
        (loc === 'LEFT_TORSO' || loc === 'RIGHT_TORSO') &&
        !['XL', 'XXL'].includes(engineType.replace(/[^A-Z]/g, ''))) ||
      (info.case1 && (loc === 'LEFT_ARM' || loc === 'RIGHT_ARM'));

    const engineSideSlots =
      engineType.includes('XL') &&
      !engineType.includes('CLAN') &&
      !engineType.includes('LIGHT')
        ? 3
        : engineType.includes('XXL')
          ? techBase === 'CLAN'
            ? 4
            : 6
          : techBase === 'CLAN' && engineType.includes('XL')
            ? 2
            : 0;

    const reason = info.case2
      ? 'CASE II'
      : info.case1 && (loc === 'LEFT_TORSO' || loc === 'RIGHT_TORSO')
        ? `CASE + engine ${engineSideSlots} side crits (${engineSideSlots >= 3 ? 'NOT protected' : 'protected'})`
        : info.case1
          ? 'CASE in arm'
          : 'NO CASE';

    console.log(
      `  ${loc}: ${info.ammo.length} ammo${info.weapons.length > 0 ? ' + ' + info.weapons.length + ' explosive wpn' : ''} | CASE: ${info.case1 ? 'YES' : 'no'} | CASE II: ${info.case2 ? 'YES' : 'no'} | ${reason}`,
    );

    // Show ammo detail for unprotected locations
    if (!isProtected && info.ammo.length > 0) {
      console.log(
        `    *** UNPROTECTED ammo should have ${info.ammo.length * 15} BV penalty! ***`,
      );
