...
for (const [loc, slots] of Object.entries(unit.criticalSlots || {})) {
  ...
  for (const s of slots) {
    ...
    const isAMS =
      ...
      (lo.includes('ams') && !lo.includes('ammo'));
    const isECM =
      (lo.includes('ecm') ||
    ...
    const isAPod =
      (lo.includes('a-pod') || lo === 'isapod' || lo === 'clapod') &&
      !lo.includes('ammo');
    ...
    if (
      ...
      defItems.push(clean);
      console.log(
        `  ${loc}: ${clean} → resolved=${res.resolved} bv=${res.battleValue}${isAPod ? '' : isAPodNew ? ' (needs antipersonnel pattern)' : ''}`,
      );
    }
    prev = clean;
  }
...
for (const [loc, slots] of Object.entries(unit.criticalSlots || {})) {
  ...
  for (const s of slots) {
    ...
    if (
      lo.includes('ammo') &&
      (lo.includes('ams') || lo.includes('anti-missile') || lo.includes('apds'))
    ) {
      console.log(`  ${loc}: ${s}`);
    }
  }
