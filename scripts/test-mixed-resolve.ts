...
function resolveWeaponForUnit(
  ...
  if (
    ...
    if (!normalizedIS2.startsWith('clan-'))
      clanCandidates.push('clan-' + normalizedIS2);
    if (!lo.startsWith('clan-') && lo !== normalizedIS2)
      clanCandidates.push('clan-' + lo);
    console.log(
      `  MIXED fallback for ${id}: candidates=${JSON.stringify(clanCandidates)}`,
    );
