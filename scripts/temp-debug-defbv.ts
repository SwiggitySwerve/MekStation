...
for (const ton of [
  ...
  if (!t) {
    console.log(`${ton}t: MISSING`);
    continue;
  }
  const total =
    t.head + t.centerTorso + t.sideTorso * 2 + t.arm * 2 + t.leg * 2;
  console.log(
    `${ton}t: HD=${t.head} CT=${t.centerTorso} ST=${t.sideTorso} ARM=${t.arm} LEG=${t.leg} → total=${total}`,
  );
}
...
// Find ALB-5U
const target = index.units.find(
  (u: any) =>
    u.model === 'ALB-5U' || (u.chassis === 'Albatross' && u.model === 'ALB-5U'),
);
if (!target) {
  // Try broader search
  ...
  console.log(
    '\nAlbatross variants:',
    albs.map((u: any) => `${u.chassis} ${u.model} (BV=${u.bv})`),
  );
  process.exit(1);
}
...
for (const [loc, val] of Object.entries(unit.armor.allocation)) {
  if (typeof val === 'number') {
    totalArmor += val;
    console.log(`  Armor ${loc}: ${val}`);
  } else if (val && typeof val === 'object') {
    const v = val as { front: number; rear: number };
    totalArmor += v.front + v.rear;
    console.log(
      `  Armor ${loc}: front=${v.front} rear=${v.rear} total=${v.front + v.rear}`,
    );
  }
}
...
if (st) {
  const totalStruct =
    st.head + st.centerTorso + st.sideTorso * 2 + st.arm * 2 + st.leg * 2;
  console.log(
    `Total structure: ${totalStruct} (HD=${st.head} CT=${st.centerTorso} ST=${st.sideTorso}×2 ARM=${st.arm}×2 LEG=${st.leg}×2)`,
  );
  ...
  console.log(
    `Structure BV: ${totalStruct} × 1.5 × 1.0 (standard) × ${engineMult} (Light) = ${structBV}`,
  );
  ...
  console.log(
    `Base defensive: ${armorBV} + ${structBV} + ${gyroBV} = ${baseDef}`,
  );
  ...
  console.log(
    `Run MP: ${runMP}, Jump MP: ${jumpMP}, Best MP: ${bestMP}, TMM: ${tmm}, Def factor: ${defFactor}`,
  );
  console.log(
    `Expected defensive BV (no equip/penalties): ${baseDef} × ${defFactor} = ${baseDef * defFactor}`,
  );
}
...
for (const ton of Object.keys(megamekTable)
  .map(Number)
  .sort((a, b) => a - b)) {
  const mm = megamekTable[ton];
  const ours = STRUCTURE_POINTS_TABLE[ton];
  if (!ours) {
    console.log(`${ton}t: MISSING from our table`);
    totalMismatches++;
    continue;
  }
  ...
  const match =
    mm.head === ours.head &&
    mm.ct === ours.centerTorso &&
    mm.st === ours.sideTorso &&
    mm.arm === ours.arm &&
    mm.leg === ours.leg;
  if (!match) {
    console.log(`${ton}t: MISMATCH!`);
    console.log(
      `  Ours:    HD=${ours.head} CT=${ours.centerTorso} ST=${ours.sideTorso} ARM=${ours.arm} LEG=${ours.leg} → ${ourTotal}`,
    );
    console.log(
      `  MegaMek: HD=${mm.head} CT=${mm.ct} ST=${mm.st} ARM=${mm.arm} LEG=${mm.leg} → ${mmTotal}`,
    );
