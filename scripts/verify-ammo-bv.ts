...
for (const size of [5, 10, 15, 20]) {
  ...
  const status =
    ourBV === expected
      ? 'OK'
      : `MISMATCH (ours=${ourBV}, expected=${expected})`;
  console.log(
    `  LRM-${size}: catalog=${ourBV ?? 'MISSING'} expected=${expected} ${status}`,
  );
}
...
for (const size of [2, 4, 6]) {
  ...
  console.log(
    `  SRM-${size}: catalog=${ourBV ?? 'MISSING'} expected=${expected} ${status}`,
  );
}

console.log('\nGeneric (flat) entries vs size-specific:');
console.log(
  `  lrm-ammo (generic): BV=${ourValues['lrm-ammo']} -- used when size can't be determined`,
);
console.log(
  `  srm-ammo (generic): BV=${ourValues['srm-ammo']} -- used when size can't be determined`,
);
console.log(
  `  streak-srm-ammo (generic): BV=${ourValues['streak-srm-ammo']} -- used when size can't be determined`,
);
console.log(
  `  mrm-ammo (generic): BV=${ourValues['mrm-ammo']} -- used when size can't be determined`,
);
...
for (const key of [
  ...
  console.log(
    `  ${key}: ${found ? `FOUND in catalog (BV=${found.battleValue})` : 'NOT FOUND'}`,
  );
}
...
const perSizeIds = ammoCatalog.items.filter((i: any) =>
  /^ammo-(lrm|srm)-\d+$/.test(i.id),
);
...
console.log(
  'The per-size "ammo-lrm-20" entry gets keyed by item.id = "ammo-lrm-20" and canon "ammolrm20"',
);
console.log('');
console.log(
  'QUESTION: When a crit slot says "IS Ammo LRM-20", which key does it resolve to?',
);
console.log(
  'Rule: /^(?:is\\s*)?ammo\\s+lrm-(\\d+)$/ => ammo-lrm-${m[1]} => "ammo-lrm-20"',
);
console.log('Catalog HAS "ammo-lrm-20" with BV=23');
console.log('');
console.log(
  'So the per-size resolution SHOULD work. Let me check if the BV values are correct...',
);
console.log('');
...
for (const uid of lrmUnits) {
  ...
  console.log(
    `  Index BV: ${result.indexBV}  Calc: ${result.calculatedBV}  Gap: ${result.difference} (${result.percentDiff?.toFixed(2)}%)`,
  );
...
if (archer) {
  console.log(
    `  Index BV: ${archer.indexBV}  Calc: ${archer.calculatedBV}  Gap: ${archer.difference}`,
  );
  console.log(
    `  AmmoBV: ${archer.breakdown?.ammoBV}  WeaponBV: ${archer.breakdown?.weaponBV}`,
  );
  ...
  console.log(
    `  Current ammo: 6 tons LRM-20 (BV=23) + 2 tons Streak SRM-4 (BV=10) = ${6 * 23 + 2 * 10}`,
  );
  console.log(
    `  If LRM-20=27: 6*27 + 2*10 = ${6 * 27 + 2 * 10}, diff = ${6 * 27 + 2 * 10 - (6 * 23 + 2 * 10)}`,
  );
  console.log(
    `  With SF=${sf}: extra = ${((6 * 27 + 2 * 10 - (6 * 23 + 2 * 10)) * sf).toFixed(1)} => new gap would be ${archer.difference + Math.round((6 * 27 + 2 * 10 - (6 * 23 + 2 * 10)) * sf)}`,
  );
}
