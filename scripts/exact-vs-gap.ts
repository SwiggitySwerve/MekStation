...
const gapUnits = report.allResults.filter(
...
console.log(
  `  Exact: ${exactDHS}/${exactUnits.length} DHS (${((exactDHS / exactUnits.length) * 100).toFixed(1)}%)`,
);
console.log(
  `  Gap:   ${gapDHS}/${gapUnits.length} DHS (${((gapDHS / gapUnits.length) * 100).toFixed(1)}%)\n`,
);
...
for (const r of exactUnits) {
  ...
  try {
    ...
    console.log(
      `${r.unitId}: idx=${r.indexBV} calc=${r.calculatedBV} wBV=${b.weaponBV} sf=${b.speedFactor}`,
    );
    console.log(
      `  ${u.tonnage}t ${u.techBase} walk=${u.movement.walk} jump=${u.movement.jump || 0} engine=${u.engine.type} HS=${u.heatSinks.count}x${u.heatSinks.type}`,
    );
...
for (const r of gapUnits) {
  ...
  try {
    ...
    console.log(
      `  wBV=${b.weaponBV} sf=${b.speedFactor} rawOff=${ourRaw.toFixed(1)} needed=${neededRaw.toFixed(1)} rawGap=${(neededRaw - ourRaw).toFixed(1)}`,
    );
    console.log(`  Equipment: ${u.equipment.map((e: any) => e.id).join(', ')}`);
    ...
    if (engType.includes('XXL') && jumpMP > 0)
      jumpHeat = Math.max(6, jumpMP * 2);
