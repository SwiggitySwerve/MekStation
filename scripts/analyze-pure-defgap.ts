...
const specialWeapons = [
  ...
  'plasma rifle',
  'nova cews',
  'improved medium heavy',
  'improved small heavy',
  'improved large heavy',
  'streak lrm',
  'atm',
  'rotary',
  'heavy gauss',
...
for (const res of r.allResults) {
  ...
  if (Math.abs(rawOff - expectedRawOff) / Math.max(1, expectedRawOff) >= 0.02)
    continue;
...
console.log(
  `Pure defensive-gap units (no special weapons, rawOff OK): ${pureDefGap.length}`,
);
console.log(
  `Average gap: ${(pureDefGap.reduce((s: number, u: any) => s + u.gap, 0) / pureDefGap.length).toFixed(1)}`,
);
console.log(
  `Average abs diff: ${(pureDefGap.reduce((s: number, u: any) => s + Math.abs(u.difference), 0) / pureDefGap.length).toFixed(1)}`,
);

// Check if gap correlates with explosive penalty
const withExpl = pureDefGap.filter(
  (u: any) => u.breakdown.explosivePenalty > 0,
);
const noExpl = pureDefGap.filter(
  (u: any) => u.breakdown.explosivePenalty === 0,
);
console.log(
...
console.log(
  `Without explosive penalty: ${noExpl.length} (avgGap=${(noExpl.reduce((s: number, u: any) => s + u.gap, 0) / noExpl.length).toFixed(1)})`,
);
...
console.log(
  `  Gap ≈ penalty (within 5): ${penaltyFixCount} of ${withExpl.length}`,
);

// For units WITHOUT explosive penalty, what is the gap?
// Check: does the gap correlate with defEquip?
const noExplNoEquip = noExpl.filter(
  (u: any) => u.breakdown.defensiveEquipBV === 0,
);
const noExplHasEquip = noExpl.filter(
  (u: any) => u.breakdown.defensiveEquipBV > 0,
);
console.log(
...
console.log(
  `No penalty, has defEquip: ${noExplHasEquip.length} (avgGap=${noExplHasEquip.length ? (noExplHasEquip.reduce((s: number, u: any) => s + u.gap, 0) / noExplHasEquip.length).toFixed(1) : 'N/A'})`,
);
...
for (const u of purePure.slice(0, 20)) {
  const d = u.unitData;
  console.log(
    `${u.unitId}: ${d.tonnage}t ${d.techBase} ${d.engine.type} gap=${u.gap.toFixed(1)} diff=${u.difference} (${u.percentDiff.toFixed(1)}%)`,
  );
  console.log(
    `  armorBV=${u.breakdown.armorBV.toFixed(1)} structBV=${u.breakdown.structureBV.toFixed(1)} gyroBV=${u.breakdown.gyroBV.toFixed(1)} defFactor=${u.breakdown.defensiveFactor.toFixed(2)} walk=${d.movement.walk} jump=${d.movement.jump || 0}`,
  );
}
...
for (const [tb, units] of Object.entries(byTech)) {
  console.log(
    `  ${tb}: ${units.length} (avgGap=${(units.reduce((s: number, u: any) => s + u.gap, 0) / units.length).toFixed(1)})`,
  );
}
