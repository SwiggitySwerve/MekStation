...
for (const u of outside1) {
  ...
  if (unresolved.length > 0) {
    unresolvedCount++;
    console.log(
      `  ${u.chassis} ${u.model}: ${u.percentDiff.toFixed(2)}% - ${unresolved.join('; ')}`,
    );
  }
}
...
const barely = outside1.filter(
  (u) => Math.abs(u.percentDiff) > 1 && Math.abs(u.percentDiff) <= 1.5,
);
console.log(
  `Count: ${barely.length} (${barely.filter((u) => u.percentDiff < 0).length} under, ${barely.filter((u) => u.percentDiff > 0).length} over)`,
);
...
for (const u of outside1.filter((x) => x.percentDiff > 1)) {
  ...
  for (const eq of data.equipment) {
    ...
    if (
      id.includes('ammo') ||
      id.includes('heat-sink') ||
      id.includes('jump-jet')
    )
      continue;
...
const topWeapons = Object.entries(weaponFreq)
  .sort((a, b) => b[1] - a[1])
  .slice(0, 15);
...
console.log(
  `Total: ${ccUnits.length}, avg: ${(ccUnits.reduce((s, u) => s + u.percentDiff, 0) / ccUnits.length).toFixed(2)}%`,
);
