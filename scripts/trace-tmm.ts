...
const index = JSON.parse(
  fs.readFileSync(path.join(basePath, 'index.json'), 'utf8'),
);
...
for (const id of unitIds) {
  ...
  if (!unitData) {
    console.log(`${id}: unit data not found`);
    continue;
  }
  ...
  const hasMASC = equipment.some(
    (e: any) =>
      e.id.toLowerCase().includes('masc') &&
      !e.id.toLowerCase().includes('ammo'),
  );
  const hasSC = equipment.some((e: any) =>
    e.id.toLowerCase().includes('supercharger'),
  );
  const hasTSM = equipment.some(
    (e: any) =>
      e.id.toLowerCase().includes('tsm') ||
      e.id.toLowerCase().includes('triple-strength'),
  );
  ...
  console.log(
    `  Walk: ${walk}, BVWalk: ${bvWalk}, Run: ${runMP}, Jump: ${jump}`,
  );
  ...
  console.log(
    `  Index BV: ${result.indexBV}, Calc BV: ${result.calculatedBV}, Diff: ${result.percentDiff?.toFixed(2)}%`,
  );
  console.log(
    `  Breakdown - DefBV: ${result.breakdown?.defensiveBV?.toFixed(2)}, SF: ${result.breakdown?.speedFactor}`,
  );
  if (hasMASC || hasSC || hasTSM) {
    console.log(
      `  Special equipment: MASC=${hasMASC}, SC=${hasSC}, TSM=${hasTSM}`,
    );
  }
}
...
const within1 = data.allResults.filter(
  (r: any) => r.status === 'within1' && r.status !== 'exact',
);
const within5undercalc = data.allResults.filter(
  (r: any) => r.status === 'within5' && r.percentDiff < -1,
);
const within5overcalc = data.allResults.filter(
  (r: any) => r.status === 'within5' && r.percentDiff > 1,
);
...
for (const r of within5undercalc) {
  ...
  const hasMASC2 = eq.some(
    (e: any) =>
      e.id.toLowerCase().includes('masc') &&
      !e.id.toLowerCase().includes('ammo'),
  );
  const hasSC2 = eq.some((e: any) =>
    e.id.toLowerCase().includes('supercharger'),
  );
  const hasTSM2 = eq.some(
    (e: any) =>
      e.id.toLowerCase().includes('tsm') ||
      e.id.toLowerCase().includes('triple-strength'),
  );
...
for (const [tmm, data2] of Object.entries(tmmCounts).sort(
  (a, b) => Number(a[0]) - Number(b[0]),
)) {
  console.log(
    `  TMM ${tmm}: ${data2.count} units, avg diff: ${(data2.totalPctDiff / data2.count).toFixed(2)}%`,
  );
}
