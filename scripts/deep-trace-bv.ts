...
const {
  ...
  calculateTMM,
} = require('../src/utils/construction/battleValueCalculations');
const {
  resolveEquipmentBV,
...
const {
  ...
  getCockpitModifier,
} = require('../src/types/validation/BattleValue');
...
const d = JSON.parse(
  fs.readFileSync(path.join('public/data/units/battlemechs', ie.path), 'utf8'),
);

console.log('=== COUGAR T MANUAL BV TRACE ===');
console.log(
  `Tonnage: ${d.tonnage}, Walk: ${d.movement.walk}, Jump: ${d.movement.jump || 0}`,
);
console.log(
  `Engine: ${d.engine.type}, HS: ${d.heatSinks.type} × ${d.heatSinks.count}`,
);
...
} = require('../src/types/construction/InternalStructureType');
...
for (const [loc, slots] of Object.entries(d.criticalSlots || {})) {
  for (const s of slots as string[]) {
    ...
    allSlots.push(s);
    if (
      s.toLowerCase().includes('heatsink') ||
      s.toLowerCase().includes('heat sink')
    )
      hsCount++;
  }
}
...
const dhsSlots = allSlots.filter(
  (s) =>
    s.toLowerCase().includes('doubleheatsink') ||
    s.toLowerCase().includes('double heat sink'),
);
...
console.log(
  `DHS crit slots: ${dhsSlots.length}, external DHS: ${externalDHS}, integral: ${integralHS}, total: ${totalHS}`,
);
...
for (const eq of d.equipment) {
  ...
  console.log(
    `  ${eq.id}: IS_BV=${result.battleValue} Clan_BV=${clanResult.battleValue}`,
  );
}
...
const critProbe = allSlots.some(
  (s) =>
    s.toLowerCase().includes('probe') || s.toLowerCase().includes('active'),
);
console.log(`\nCrit TC: ${critTC}, ECM: ${critECM}, Probe: ${critProbe}`);

// Manual BV calculation
const armorMult = getArmorBVMultiplier(
  d.armor.type.toLowerCase().replace(/_/g, '-').replace('clan', '').trim() ||
    'ferro-fibrous-clan',
);
...
for (const name of [
  ...
  console.log(
    `  getArmorBVMultiplier('${name}'): ${getArmorBVMultiplier(name)}`,
  );
}
...
console.log(`\nValidation result:`);
console.log(
  `  calc=${res.calculatedBV}, ref=${res.indexBV}, diff=${res.difference}`,
);
console.log(
  `  def=${res.breakdown.defensiveBV}, off=${res.breakdown.offensiveBV}`,
);
console.log(
  `  armorBV=${res.breakdown.armorBV}, structBV=${res.breakdown.structureBV}, gyroBV=${res.breakdown.gyroBV}`,
);
console.log(
  `  defEquipBV=${res.breakdown.defensiveEquipBV}, pen=${res.breakdown.explosivePenalty}`,
);
console.log(`  defFactor=${res.breakdown.defensiveFactor}`);
console.log(
  `  weaponBV=${res.breakdown.weaponBV}, ammoBV=${res.breakdown.ammoBV}, sf=${res.breakdown.speedFactor}`,
);
...
const baseDef =
  res.breakdown.armorBV +
  res.breakdown.structureBV +
  res.breakdown.gyroBV +
  res.breakdown.defensiveEquipBV -
  res.breakdown.explosivePenalty;
const neededDefFactor = (defBV + totalGap) / baseDef;
console.log(
  `  Needed defFactor for def gap: ${neededDefFactor.toFixed(4)} (current: ${res.breakdown.defensiveFactor})`,
);
// What speedFactor would close the gap?
const rawOff = offBV / res.breakdown.speedFactor;
const neededSF = (offBV + totalGap) / rawOff;
console.log(
  `  Needed speedFactor for off gap: ${neededSF.toFixed(4)} (current: ${res.breakdown.speedFactor})`,
);
