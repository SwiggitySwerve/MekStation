...
try {
  ...
  for (const item of d.items || []) {
    ...
    if (!ammoLookup.has(canon))
      ammoLookup.set(canon, {
        bv: item.battleValue,
        src: 'catalog-canon:' + item.id,
      });
  }
} catch (e) {
...
for (const [id, bv, wt] of hc) {
  ...
  console.log(
    `  "${id}" (canon: "${canon}"): bv=${bv}, existed=${exists}, canonExisted=${canonExists}`,
  );
}
...
for (const slot of testSlots) {
  ...
  if (m) {
    ...
    for (const id of ids) {
      ...
      if (found) {
        console.log(`  lu.get("${id}") => bv=${found.bv} (${found.src})`);
        break;
      }
  ...
  console.log(
    `  canonKey fallback: "${canonKey}" => ${ce ? `bv=${ce.bv} (${ce.src})` : 'NOT FOUND'}`,
  );
}
