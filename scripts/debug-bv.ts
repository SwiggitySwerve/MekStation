...
function resolveAmmoBV(slotName: string): { bv: number; weaponType: string } {
  ...
  const cleaned = lower
    .replace(/\(omnipod\)/gi, '')
    .replace(/\(armored\)/gi, '')
    .replace(/^(is|clan|cl)\s+/, '')
    .replace(/\s*-\s*half$/i, '')
    .replace(/artemis[- ]?(?:iv|v)?[- ]?capable/gi, '')
    .replace(/narc[- ]?capable/gi, '')
    .replace(/\(clan\)/gi, '')
    .trim();

  if (ammoByName!.has(cleaned)) {
    const e = ammoByName!.get(cleaned)!;
    return { bv: e.battleValue, weaponType: e.compatibleWeaponIds[0] || '' };
  }

  let ammoId = '';
  let weaponType = '';

  if (lower.includes('lrm')) {
    const m = lower.match(/lrm[- ]?(\d+)/);
    if (m) {
      ammoId = `ammo-lrm-${m[1]}`;
      weaponType =
        lower.includes('clan') || lower.includes('cl ')
          ? `clan-lrm-${m[1]}`
          : `lrm-${m[1]}`;
    }
  } else if (lower.includes('srm') && lower.includes('streak')) {
    const m = lower.match(/srm[- ]?(\d+)/);
    if (m) {
      ammoId = `ammo-streak-srm-${m[1]}`;
      weaponType =
        lower.includes('clan') || lower.includes('cl ')
          ? `clan-streak-srm-${m[1]}`
          : `streak-srm-${m[1]}`;
  ...
  } else if (lower.includes('mml')) {
    const m = lower.match(/mml[- ]?(\d+)/);
    if (m) {
      ammoId = lower.includes('srm')
        ? `ammo-mml-${m[1]}-srm`
        : `ammo-mml-${m[1]}-lrm`;
      weaponType = `mml-${m[1]}`;
    }
  } else if (lower.includes('atm')) {
    const m = lower.match(/atm[- ]?(\d+)/);
    if (m) {
      ammoId = `ammo-atm-${m[1]}`;
      weaponType = `atm-${m[1]}`;
    }
  ...
  if (ammoId && ammoByName!.has(ammoId)) {
    const e = ammoByName!.get(ammoId)!;
    return {
      bv: e.battleValue,
      weaponType: weaponType || e.compatibleWeaponIds[0] || '',
    };
  }
  ...
  if (weaponType) {
    ...
    for (const entry of ammoCatalog!) {
      if (
        entry.compatibleWeaponIds.includes(normalizedWT) ||
        entry.compatibleWeaponIds.includes(weaponType)
      ) {
