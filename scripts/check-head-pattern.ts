...
for (const entry of index.units) {
  ...
  try {
    ...
    const lsCount = slots.filter(
      (s: any) => s && s.includes('Life Support'),
    ).length;
...
const patternStats: Record<
  string,
  { count: number; overcalc5: number; within1: number; avgPct: number }
> = {};
for (const u of units) {
  if (!patternStats[u.pattern])
    patternStats[u.pattern] = { count: 0, overcalc5: 0, within1: 0, avgPct: 0 };
...
for (const [pat, stats] of Object.entries(patternStats)
  ...
  console.log(
    `  [${String(stats.count).padStart(4)}x] ${pat.padEnd(35)} avgPct=${avg.padStart(7)}% overcalc5=${stats.overcalc5} within1=${stats.within1}`,
  );
}
...
const standardCockpitLike = units.filter(
  (u) => !u.sensorAt4 || u.lsCount === 2,
);

console.log(`\n=== HEURISTIC: Sensors in slot 4 + LS=1 ===`);
console.log(`Small-cockpit-like (SEN@4 + LS=1): ${smallCockpitLike.length}`);
console.log(
  `  Overcalc 3-7%: ${smallCockpitLike.filter((u) => u.pctDiff > 3 && u.pctDiff < 7).length}`,
);
console.log(
  `  Within 1%: ${smallCockpitLike.filter((u) => Math.abs(u.pctDiff) <= 1).length}`,
);
console.log(
  `  Would break if 0.95x applied: ${smallCockpitLike.filter((u) => Math.abs(u.pctDiff) <= 1).length}`,
);

console.log(`\nStandard-cockpit-like: ${standardCockpitLike.length}`);
console.log(
  `  Overcalc 3-7%: ${standardCockpitLike.filter((u) => u.pctDiff > 3 && u.pctDiff < 7).length}`,
);
console.log(
  `  Within 1%: ${standardCockpitLike.filter((u) => Math.abs(u.pctDiff) <= 1).length}`,
);
...
for (const u of ls1_noSen4.slice(0, 10)) {
  console.log(
    `  ${u.id.padEnd(40)} pct=${u.pctDiff.toFixed(2)}% pattern=${u.pattern}`,
  );
}
