...
for (const uid of targets) {
  ...
  if (r) {
    console.log(
      `${uid}: calc=${r.calculatedBV} ref=${r.indexBV} diff=${r.difference} pct=${r.percentDiff?.toFixed(1)}%`,
    );
    if (r.breakdown) {
      console.log(
        `  weaponBV=${r.breakdown.weaponBV} ammoBV=${r.breakdown.ammoBV} sf=${r.breakdown.speedFactor} he=${r.breakdown.heatEfficiency}`,
      );
    }
  } else {
...
console.log(
  `         ${within5.length}/${valid.length} within 5% (${((within5.length / valid.length) * 100).toFixed(1)}%)`,
);
