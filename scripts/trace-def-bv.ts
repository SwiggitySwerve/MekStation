...
for (const unitId of traceIds) {
  ...
  if (!r) {
    console.log(`${unitId}: NOT IN REPORT`);
    continue;
  }
  ...
  if (rawEngineType === 'XL' && data.techBase === 'CLAN')
    engineType = 'XL_CLAN';
  else if (rawEngineType === 'XL') engineType = 'XL_IS';
  else if (rawEngineType === 'XXL' && data.techBase === 'CLAN')
    engineType = 'XXL_CLAN';
  ...
  console.log(
    `  indexBV=${r.indexBV} calcBV=${r.calculatedBV} diff=${r.difference} (${r.percentDiff?.toFixed(1)}%)`,
  );
  console.log(`  --- Armor ---`);
  console.log(
    `  totalArmor=${totalArmor} armorType=${armorType} armorMult=${armorMult}`,
  );
  console.log(`  armorBV = ${totalArmor} * 2.5 * ${armorMult} = ${armorBV}`);
  console.log(`  --- Structure ---`);
  console.log(
    `  totalStructure=${totalStructure} structType=${structureType} structMult=${structMult}`,
  );
  ...
  console.log(
    `  defEquipBV=${b.defensiveEquipBV} explPenalty=${b.explosivePenalty}`,
  );
  console.log(`  --- BaseDef ---`);
  console.log(
    `  baseDef = ${armorBV} + ${structBV} + ${gyroBV} + ${b.defensiveEquipBV} - ${b.explosivePenalty} = ${baseDef + b.defensiveEquipBV - b.explosivePenalty}`,
  );
  ...
  console.log(`  --- Result ---`);
  console.log(
    `  ourDefBV = baseDef * defFactor = ${(baseDef + b.defensiveEquipBV - b.explosivePenalty).toFixed(2)} * ${defFactor} = ${((baseDef + b.defensiveEquipBV - b.explosivePenalty) * defFactor).toFixed(2)}`,
  );
  console.log(`  reportedDefBV = ${reportedDefBV.toFixed(2)}`);
  console.log(
    `  expectedDefBV = indexBV - offBV = ${r.indexBV} - ${b.offensiveBV.toFixed(2)} = ${expectedDefBV.toFixed(2)}`,
  );
  console.log(`  --- Gap Analysis ---`);
  console.log(`  defBV gap = ${(expectedDefBV - reportedDefBV).toFixed(2)}`);
  console.log(`  baseDef gap (pre-factor) = ${baseDefGap.toFixed(2)}`);
  console.log(
    `  baseDef gap / tonnage = ${(baseDefGap / data.tonnage).toFixed(4)}`,
  );
  console.log(
    `  baseDef gap / totalArmor = ${(baseDefGap / totalArmor).toFixed(4)}`,
  );
  console.log(
    `  baseDef gap / totalStructure = ${(baseDefGap / totalStructure).toFixed(4)}`,
  );
  ...
  for (const altTMM of [maxTMM - 1, maxTMM + 1, maxTMM + 2]) {
    ...
    const altDefBV =
      (baseDef + b.defensiveEquipBV - b.explosivePenalty) * altFactor;
    const altGap = r.indexBV - b.offensiveBV - altDefBV;
    if (Math.abs(altGap) < Math.abs(expectedDefBV - reportedDefBV)) {
      console.log(
        `  ** Alt TMM=${altTMM} factor=${altFactor}: defBV=${altDefBV.toFixed(2)} gap=${altGap.toFixed(2)} (CLOSER)`,
      );
    }
  }
