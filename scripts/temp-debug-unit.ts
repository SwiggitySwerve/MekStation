...
console.log(
  `Engine: ${ud.engine.type}/${ud.engine.rating}, Walk: ${ud.movement.walk}, Jump: ${ud.movement.jump}`,
);
...
for (const eq of ud.equipment || []) {
  ...
  console.log(
    `  ${eq.id.padEnd(35)} @ ${eq.location.padEnd(15)} → norm=${norm.padEnd(25)} BV=${r.battleValue} heat=${r.heat} resolved=${r.resolved}`,
  );
}
...
for (const [loc, slots] of Object.entries(ud.criticalSlots || {})) {
  for (const slot of slots as (string | null)[]) {
    if (!slot) continue;
    if (
      /endo|ferro|engine|gyro|cockpit|life support|sensors|shoulder|upper arm|lower arm|hand|hip|upper leg|lower leg|foot|actuator/i.test(
        slot,
      )
    )
      continue;
