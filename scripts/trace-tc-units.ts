...
for (const u of valid) {
  ...
  if (!unit) continue;
  const hasTC =
    (unit.equipment || []).some((eq: any) => {
      const id = eq.id.toLowerCase();
      return (
        id.includes('targeting-computer') || id === 'istc' || id === 'cltc'
      );
    }) ||
    (unit.criticalSlots &&
...
for (const u of sorted.slice(0, 15)) {
  ...
  console.log(
    `  offBV=${b?.offensiveBV?.toFixed(0)} defBV=${b?.defensiveBV?.toFixed(0)} SF=${b?.speedFactor} cockpit=${b?.cockpitModifier}`,
  );
  console.log(
    `  weapBV=${b?.weaponBV?.toFixed(0)} ammoBV=${b?.ammoBV} physBV=${b?.physicalWeaponBV?.toFixed(0)} offEquip=${b?.offEquipBV}`,
  );
  ...
  for (const eq of unit.equipment || []) {
    ...
    console.log(
      `    ${eq.id.padEnd(40)} @${eq.location.padEnd(12)} bv=${res.battleValue} heat=${res.heat} type=${res.type}`,
    );
  }
    ...
    for (const [loc, slots] of Object.entries(unit.criticalSlots)) {
      ...
      for (const s of slots) {
        if (
          s &&
          typeof s === 'string' &&
          (s as string).toLowerCase().includes('targeting computer')
        ) {
  ...
  console.log(
    `  Needed offBV: ${neededOff.toFixed(0)}, gap: ${offGap.toFixed(0)}`,
  );
  console.log('');
}
...
const sortedOver = [...tcOver].sort(
  (a: any, b: any) => b.difference - a.difference,
);
for (const u of sortedOver.slice(0, 10)) {
  const b = u.breakdown;
  console.log(
    `  ${u.unitId.padEnd(40)} diff=+${u.difference} (+${u.percentDiff.toFixed(1)}%) hasTC=${b?.hasTargetingComputer}`,
  );
}
