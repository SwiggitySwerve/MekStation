...
for (const [id, item] of ammoMap) {
  if (id.includes('streak') && id.includes('srm')) {
    console.log(
      `  ${id}: BV=${item.battleValue} (${item.name}) [${item.file}]`,
    );
  }
}
...
for (const u of outside1) {
  ...
  if (b.ammoBV === 0 || b.ammoBV === undefined) {
    ...
    if (fs.existsSync(unitFile)) {
      ...
      const hasAmmo = (unitData.equipment || []).some(
        (e) => e.id?.includes('ammo') || e.name?.toLowerCase().includes('ammo'),
      );
      if (hasAmmo) {
        missingAmmo++;
        console.log(
          `  ${u.chassis} ${u.model} (${u.id}): ammoBV=0 but has ammo equipment! diff=${u.difference > 0 ? '+' : ''}${u.difference}`,
        );
      }
    }
  }
...
for (const u of outside1
  .filter((u) => u.difference < 0)
  .sort((a, b) => a.difference - b.difference)
  .slice(0, 20)) {
  const b = u.breakdown || {};
  console.log(
    `  ${u.chassis} ${u.model}: diff=${u.difference} ammoBV=${b.ammoBV || 0} weapBV=${b.weaponBV || 0} off=${b.offensiveBV?.toFixed(1)} def=${b.defensiveBV?.toFixed(1)}`,
  );
}
