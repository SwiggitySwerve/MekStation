...
for (const uid of cephalusIds) {
  ...
  for (const eq of unit.equipment || []) {
    ...
    console.log(
      `  ${eq.id} @ ${eq.location}: bv=${res.battleValue} heat=${res.heat} resolved=${res.resolved} norm=${norm}`,
    );
  }
...
if (koshi) {
  console.log(
    `${koshi.tonnage}t ${koshi.techBase} engine=${JSON.stringify(koshi.engine)} movement=${JSON.stringify(koshi.movement)}`,
  );
  for (const eq of koshi.equipment || []) {
    const res = resolveEquipmentBV(eq.id);
    console.log(
      `  ${eq.id} @ ${eq.location}: bv=${res.battleValue} heat=${res.heat} resolved=${res.resolved}`,
    );
  }
}
...
if (barghest) {
  console.log(
    `${barghest.tonnage}t ${barghest.techBase} engine=${JSON.stringify(barghest.engine)} movement=${JSON.stringify(barghest.movement)}`,
  );
  ...
  for (const [loc, slots] of Object.entries(barghest.criticalSlots || {})) {
    ...
    const ammo = (slots as string[]).filter(
      (s) => s && s.toLowerCase().includes('ammo'),
    );
...
if (osteon) {
  console.log(
    `${osteon.tonnage}t ${osteon.techBase} engine=${JSON.stringify(osteon.engine)} movement=${JSON.stringify(osteon.movement)}`,
  );
