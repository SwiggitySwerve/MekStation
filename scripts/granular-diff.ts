...
const all = report.allResults.filter(
  (r: any) => r.status !== 'error' && r.breakdown,
);
...
const exact = all.filter(
  (r: any) => r.status === 'exact' || r.status === 'within1',
);

console.log(
  `Total: ${all.length} | Exact/Within1: ${exact.length} | Wrong: ${wrong.length} (${underc.length} under, ${overc.length} over)`,
);
...
// Print header
const header =
  'Field'.padEnd(22) + groups.map((g) => g.name.padStart(12)).join('');
console.log(header);
console.log('-'.repeat(header.length));
for (const f of fields) {
  const row =
    f.padEnd(22) + groups.map((g) => avgField(g.arr, f).padStart(12)).join('');
  console.log(row);
}
...
for (const g of groups) {
  const withHalved = g.arr.filter(
    (r: any) => (r.breakdown?.halvedWeaponCount ?? 0) > 0,
  );
  const avgHalvedPct =
    withHalved.length > 0
      ? (
          (withHalved.reduce(
            (s: number, r: any) =>
              s +
              r.breakdown.halvedWeaponBV / Math.max(1, r.breakdown.rawWeaponBV),
            0,
          ) /
            withHalved.length) *
          100
      ...
      : '0.0';
  console.log(
    `  ${g.name}: ${withHalved.length}/${g.arr.length} units have halved weapons, avg halved%=${avgHalvedPct}`,
  );
}
...
for (const tb of techBases) {
  ...
  if (tbUnder.length > 0) {
    const avgGap =
      tbUnder.reduce((s: number, r: any) => s + Math.abs(r.difference), 0) /
      tbUnder.length;
    ...
    console.log(
      `    avg weaponBV: ${avgWeaponBV} (raw: ${avgRawWeaponBV}, halved: ${avgHalvedBV})`,
    );
...
const heBuckets: Record<
  string,
  { exact: number; under: number; over: number }
> = {};
for (const r of all) {
...
for (const [bucket, stats] of Object.entries(heBuckets).sort()) {
  ...
  console.log(
    `  HE ${bucket.padEnd(6)}: ${stats.exact} exact, ${stats.under} under, ${stats.over} over (${wrongPct}% wrong)`,
  );
}
...
for (const [bucket, stats] of Object.entries(dfBuckets).sort(
  ...
  const wrongPct = (((stats.under + stats.over) / total) * 100).toFixed(1);
  console.log(
    `  defF=${bucket}: ${stats.exact} exact, ${stats.under} under, ${stats.over} over (${wrongPct}% wrong of ${total})`,
  );
}
...
for (const r of underc
  ...
  console.log(
    `  DEF: armor=${b.armorBV} struct=${b.structureBV} gyro=${b.gyroBV} equip=${b.defEquipBV} amsAmmo=${b.amsAmmoBV} armored=${b.armoredComponentBV} harjel=${b.harjelBonus} penalty=-${b.explosivePenalty}`,
  );
  console.log(
    `       factor=${b.defensiveFactor} (TMM=${b.maxTMM}) → defensiveBV=${b.defensiveBV.toFixed(1)}`,
  );
  console.log(
    `  OFF: weapons=${b.weaponBV.toFixed(1)} (raw=${b.rawWeaponBV.toFixed(1)}, halved=${b.halvedWeaponBV.toFixed(1)}, ${b.halvedWeaponCount}/${b.weaponCount} halved)`,
  );
  console.log(
    `       ammo=${b.ammoBV} weight=${b.weightBonus} phys=${b.physicalWeaponBV} equip=${b.offEquipBV}`,
  );
  console.log(
    `       heat: eff=${b.heatEfficiency} (diss=${b.heatDissipation} move=${b.moveHeat})`,
  );
  console.log(
    `       speed=${b.speedFactor} → offensiveBV=${b.offensiveBV.toFixed(1)}`,
  );
  console.log(
    `  MOD: cockpit=${b.cockpitModifier} (${b.cockpitType}) move: w=${b.walkMP} r=${b.runMP} j=${b.jumpMP}`,
  );
  console.log(
    `  REF: ${r.indexBV} CALC: ${r.calculatedBV} NEED: +${Math.abs(r.difference)}`,
  );
}

// === SECTION 8: Top 10 overcalculated ===
console.log('\n=== TOP 10 OVERCALCULATED - GRANULAR ===');
for (const r of overc
  ...
  console.log(
    `  DEF: armor=${b.armorBV} struct=${b.structureBV} gyro=${b.gyroBV} equip=${b.defEquipBV} penalty=-${b.explosivePenalty}`,
  );
  console.log(
    `       factor=${b.defensiveFactor} (TMM=${b.maxTMM}) → defensiveBV=${b.defensiveBV.toFixed(1)}`,
  );
  ...
  console.log(
    `  REF: ${r.indexBV} CALC: ${r.calculatedBV} EXCESS: ${r.difference}`,
  );
}
