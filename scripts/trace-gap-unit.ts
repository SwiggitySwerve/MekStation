...
for (const r of gaps.slice(0, 5)) {
  ...
  console.log(`\n--- ${r.unitId} ---`);
  console.log(
    `  Tonnage: ${u.tonnage}  TechBase: ${u.techBase}  Engine: ${u.engine.type} ${u.engine.rating}`,
  );
  console.log(
    `  Walk: ${u.movement.walk}  Jump: ${u.movement.jump || 0}  MASC: ${u.movement.masc || false}`,
  );
  console.log(`  HS: ${u.heatSinks.count}x ${u.heatSinks.type}`);
  console.log(
    `  Cockpit: ${u.cockpit || 'STANDARD'}  Gyro: ${u.gyro?.type || 'STANDARD'}`,
  );
  console.log(
    `  Equipment: ${u.equipment.map((e: any) => e.id + '@' + e.location).join(', ')}`,
  );
  ...
  console.log(
    `  Report sf=${b.speedFactor} — ${sf === b.speedFactor ? 'MATCH' : 'MISMATCH!'}`,
  );
  ...
  console.log(
    `  OffBV: ours=${b.offensiveBV.toFixed(1)} expected=${expectedOff.toFixed(1)} gap=${(expectedOff - b.offensiveBV).toFixed(1)}`,
  );
  console.log(
    `  RawOff: ours=${ourRaw.toFixed(1)} expected=${expectedRaw.toFixed(1)} gap=${(expectedRaw - ourRaw).toFixed(1)}`,
  );
  console.log(
    `  weightBonus=${u.tonnage} weaponBV=${b.weaponBV} sum=${b.weaponBV + u.tonnage}`,
  );
  console.log(
    `  ourRaw should = weaponBV + weightBonus + physicalBV + offEquipBV = ${b.weaponBV} + ${u.tonnage} + 0 + 0 = ${b.weaponBV + u.tonnage}`,
  );
  console.log(
    `  ourRaw actual = ${ourRaw.toFixed(1)} — delta from expected = ${(ourRaw - (b.weaponBV + u.tonnage)).toFixed(1)}`,
  );
}
