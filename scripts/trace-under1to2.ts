...
const lowBVEquip: Record<
  string,
  { count: number; resolvedBV: number; units: string[] }
> = {};
...
for (const u of under1to2) {
  ...
  for (const eq of unit.equipment || []) {
    ...
    if (
      ...
      if (!unresolvedEquip[key].units.includes(u.unitId))
        unresolvedEquip[key].units.push(u.unitId);
    }
  }
}

console.log('UNRESOLVED EQUIPMENT (BV=0):');
for (const [id, info] of Object.entries(unresolvedEquip).sort(
  (a, b) => b[1].count - a[1].count,
)) {
  console.log(
    `  ${id.padEnd(45)} count=${info.count} units=${info.units.length}`,
  );
...
for (const u of under1to2) {
  ...
  const equipIds = new Set(
    (unit.equipment || []).map((eq: any) => normalizeEquipmentId(eq.id)),
  );
  ...
  for (const [loc, slots] of Object.entries(unit.criticalSlots)) {
    ...
    for (const s of slots) {
      ...
      if (res.battleValue > 0) {
        const key = clean;
        if (!critOnlyWeapons[key])
          critOnlyWeapons[key] = { count: 0, bv: res.battleValue };
...
for (const [name, info] of Object.entries(critOnlyWeapons)
...
for (const [name, count] of Object.entries(unresolvedAmmo)
  .sort((a, b) => b[1] - a[1])
  .slice(0, 20)) {
...
for (const u of sorted.slice(0, 15)) {
  ...
  console.log(
    `  ${u.unitId.padEnd(40)} diff=${u.difference} (${u.percentDiff.toFixed(1)}%) baseGap=${gap.toFixed(0)} tech=${unit.techBase}`,
  );
