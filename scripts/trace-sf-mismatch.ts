...
console.log(
  `Speed factor mismatches: ${sfMismatchCount} out of ${report.allResults.length}\n`,
);

for (const m of mismatches) {
  console.log(`${m.id}`);
  console.log(
    `  ${m.tonnage}t ${m.techBase} ${m.engine} walk=${m.walk} jump=${m.jump} masc=${m.masc}`,
  );
  console.log(
    `  Computed: runMP=${m.computed.runMP} mp=${m.computed.mp} sf=${m.computed.sf}`,
  );
  ...
  for (let testMP = 0; testMP <= 30; testMP++) {
    ...
    if (Math.abs(testSF - m.reported.sf) < 0.005) {
      console.log(
        `  Reported sf implies mp=${testMP} → runMP=${testMP - Math.round(m.jump / 2)} (diff from computed: ${testMP - m.computed.mp})`,
      );
      break;
    }
  }
