...
for (const id of ids) {
  ...
  console.log(`\n=== ${id} ===`);
  console.log(
    `  calc=${u.calculatedBV} ref=${u.referenceBV} diff=${u.difference} (${u.percentDiff?.toFixed(2)}%)`,
  );
  if (u.breakdown) {
    const b = u.breakdown;
    console.log(
    ...
    console.log(
      `  defEquipBV=${b.defensiveEquipBV} explosivePen=${b.explosivePenalty}`,
    );
    console.log(
      `  defensiveFactor=${b.defensiveFactor} cockpitMod=${b.cockpitModifier}`,
    );
    console.log(
      `  defBV = (${b.armorBV}+${b.structureBV}+${b.gyroBV}+${b.defensiveEquipBV}-${b.explosivePenalty}) * ${b.defensiveFactor} * ${b.cockpitModifier}`,
    );
    const rawDef =
      (b.armorBV +
        b.structureBV +
    ...
    console.log(
      `  offEquipBV=${b.offensiveEquipBV} weightBonus=${b.weightBonus}`,
    );
    console.log(`  offensiveBV=${b.offensiveBV}`);
    const rawOff =
      b.weaponBV + b.ammoBV + (b.weightBonus || 0) + (b.offensiveEquipBV || 0);
    console.log(
      `  rawOff=${rawOff} * speedFactor=${b.speedFactor} = ${(rawOff * b.speedFactor).toFixed(1)}`,
    );
    console.log(
      `  total = defBV + offBV = ${rawDef.toFixed(1)} + ${b.offensiveBV} = ${(rawDef + b.offensiveBV).toFixed(1)}`,
    );
  }
...
for (const u of penaltyMissing
  .sort((a: any, b: any) => b.difference - a.difference)
  .slice(0, 15)) {
  console.log(
    `  ${u.unitId}: +${u.difference} (+${u.percentDiff.toFixed(1)}%) pen=${u.breakdown.explosivePenalty} defF=${u.breakdown.defensiveFactor}`,
  );
}
