...
function findJsonFiles(dir: string, files: string[] = []): string[] {
  ...
  for (const entry of entries) {
    ...
    if (entry.isDirectory()) {
      ...
      !entry.name.includes('UnitVerifier')
    ) {
      files.push(fullPath);
    }
  }
...
async function main(): Promise<void> {
  ...
  const sorted = Array.from(equipmentMap.values()).sort(
    (a, b) => b.count - a.count,
  );

  console.log(
    `Processed ${totalFiles} files with ${totalEquipment} total equipment items\n`,
  );
  ...
  for (const item of sorted) {
    ...
    if (
      type.includes('laser') ||
      (type.includes('er ') && type.includes('laser'))
    ) {
  ...
  for (const [category, items] of Object.entries(byCategory)) {
    ...
    for (const item of items.slice(0, 50)) {
      // Limit output
      console.log(
        `  '${item.itemType}': '???', // ${item.techBase}, ${item.itemName} (x${item.count})`,
      );
    }
    if (items.length > 50) {
  ...
  for (const item of sorted.slice(0, 100)) {
    ...
    console.log(
      `  '${item.itemType}': '${suggestedId}', // ${item.itemName} (x${item.count})`,
    );
  }
