...
// Units in 1-2% band
const nearMissUnder = valid.filter(
  (x: any) => x.percentDiff >= -2 && x.percentDiff < -1,
);
const nearMissOver = valid.filter(
...
for (const u of nearMissUnder.sort(
    ...
    for (const [, slots] of Object.entries(unit.criticalSlots)) {
      if (Array.isArray(slots))
        for (const s of slots as string[])
          if (s) allCrits.push(s.toLowerCase());
    }
  }

  if (allCrits.some((c) => c.includes('ams') || c.includes('antimissile')))
    underPatterns.hasAMS++;
  if (allCrits.some((c) => c.includes('c3'))) underPatterns.hasC3++;
  if (allCrits.some((c) => c.includes('artemis'))) underPatterns.hasArtemis++;
  if (
    allCrits.some(
      (c) =>
        c.includes('targeting computer') ||
        c.includes('targetingcomputer') ||
        c.includes('istc'),
    )
  )
    underPatterns.hasTC++;
  if (allCrits.some((c) => c.includes('tsm') || c.includes('triple strength')))
    underPatterns.hasTSM++;
  if (
    allCrits.some(
      (c) =>
        c.includes('ecm') ||
    ...
    underPatterns.hasECM++;
  if (allCrits.some((c) => c.includes('narc'))) underPatterns.hasNarc++;
  if (
    allCrits.some(
      (c) =>
        c.includes('probe') || c.includes('beagle') || c.includes('bloodhound'),
    )
  )
    underPatterns.hasProbe++;
  ...
  if (nearMissUnder.indexOf(u) < 20) {
    ...
    if (
      allCrits.some(
        (c) =>
          c.includes('ecm') || c.includes('guardian') || c.includes('angel'),
      )
    )
      tags.push('ecm');
    ...
    if (
      allCrits.some(
        (c) =>
          c.includes('probe') ||
          c.includes('beagle') ||
          c.includes('bloodhound'),
      )
    )
      tags.push('probe');

    console.log(
      `  ${u.unitId.padEnd(42)} ${u.percentDiff.toFixed(2)}% diff=${u.difference} [${tags.join(',')}]`,
    );
  }
}
...
console.log(
  `  AMS=${underPatterns.hasAMS} C3=${underPatterns.hasC3} Artemis=${underPatterns.hasArtemis} TC=${underPatterns.hasTC} TSM=${underPatterns.hasTSM} ECM=${underPatterns.hasECM} Narc=${underPatterns.hasNarc} Probe=${underPatterns.hasProbe}`,
);
...
for (const u of nearMissOver.sort(
...
console.log(
  `  AMS=${overPatterns.hasAMS} C3=${overPatterns.hasC3} Artemis=${overPatterns.hasArtemis} TC=${overPatterns.hasTC} TSM=${overPatterns.hasTSM} ECM=${overPatterns.hasECM} Narc=${overPatterns.hasNarc} Probe=${overPatterns.hasProbe}`,
);
...
for (const u of allNearMiss) {
  ...
  if (unresolved.length > 0) {
    unresolvedCount++;
    console.log(
      `  ${u.unitId.padEnd(42)} ${u.percentDiff.toFixed(2)}% unresolved: ${unresolved.join(', ')}`,
    );
  }
}
console.log(
  `  Total with unresolved: ${unresolvedCount}/${allNearMiss.length}`,
);
