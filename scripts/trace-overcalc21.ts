...
for (const d of exactFive) {
  ...
  if (lifeSupCount === 2) {
    ...
    console.log(
      `  Fluff has 'small cockpit': ${JSON.stringify(unit.fluff || {})
        .toLowerCase()
        .includes('small cockpit')}`,
    );

    // Check for other cockpit types
    const hasTorsoMounted = critsLo.some(
      (s) => s.includes('torso-mounted') || s.includes('torsocockpit'),
    );
...
for (const iu of indexData.units) {
  try {
    ...
    if (
      lifeSupCount === 1 &&
      !cockpit.includes('SMALL') &&
      !cockpit.includes('TORSO') &&
      !cockpit.includes('INTERFACE') &&
      !cockpit.includes('DRONE')
    ) {
      totalMislabeled++;
    }
...
console.log(
  'Mislabeled small cockpits (1 Life Support, cockpit=STANDARD):',
  totalMislabeled,
);
console.log('Correctly labeled SMALL cockpits:', totalCorrectSmall);
console.log(
  'Standard cockpits with 2 Life Support (correct):',
  totalStdWith2LS,
);
