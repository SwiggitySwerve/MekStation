...
for (const id of clanWeapons) {
  ...
  const diff =
    res.battleValue !== isRes.battleValue ? ` (IS=${isRes.battleValue})` : '';
  console.log(
    `  ${id.padEnd(30)} -> BV=${String(res.battleValue).padStart(4)} heat=${res.heat} resolved=${res.resolved}${diff}`,
  );
}
...
for (const id of clanDirect) {
  ...
  console.log(
    `  ${id.padEnd(30)} -> BV=${String(res.battleValue).padStart(4)} heat=${res.heat} resolved=${res.resolved}`,
  );
}
