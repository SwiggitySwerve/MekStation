...
for (const [eng, s] of Object.entries(engineStats).sort(
  (a, b) => b[1].under + b[1].over - (a[1].under + a[1].over),
)) {
  const total = s.exact + s.under + s.over;
  const errPct = (((s.under + s.over) / total) * 100).toFixed(1);
  console.log(
    `  ${eng.padEnd(18)} ${s.exact} exact, ${s.under} under, ${s.over} over (${errPct}% wrong of ${total})`,
  );
}
...
for (const [hs, s] of Object.entries(hsStats).sort(
  (a, b) => b[1].under + b[1].over - (a[1].under + a[1].over),
)) {
  const total = s.exact + s.under + s.over;
  const errPct = (((s.under + s.over) / total) * 100).toFixed(1);
  console.log(
    `  ${hs.padEnd(18)} ${s.exact} exact, ${s.under} under, ${s.over} over (${errPct}% wrong of ${total})`,
  );
}

// ANALYSIS 3: Tonnage distribution
console.log('\n=== TONNAGE vs ERROR ===');
const tonBuckets: Record<
  string,
  {
    exact: number;
    under: number;
    ...
    t <= 35
      ...
      : t <= 55
        ...
        : t <= 75
          ...
          : 'Assault(80+)';
  if (!tonBuckets[b])
    tonBuckets[b] = { exact: 0, under: 0, over: 0, underGap: 0, overGap: 0 };
  if (Math.abs(u.percentDiff) <= 1) tonBuckets[b].exact++;
  else if (u.percentDiff < 0) {
    tonBuckets[b].under++;
    tonBuckets[b].underGap += Math.abs(u.difference);
  } else {
    tonBuckets[b].over++;
    tonBuckets[b].overGap += u.difference;
  }
}
for (const [b, s] of Object.entries(tonBuckets).sort()) {
  const total = s.exact + s.under + s.over;
  console.log(
    `  ${b.padEnd(18)} ${s.exact} exact, ${s.under} under(avg ${(s.underGap / Math.max(1, s.under)).toFixed(0)}BV), ${s.over} over(avg ${(s.overGap / Math.max(1, s.over)).toFixed(0)}BV) (${(((s.under + s.over) / total) * 100).toFixed(1)}% wrong)`,
  );
}
...
for (const [cp, s] of Object.entries(cpStats).sort(
  (a, b) => b[1].under + b[1].over - (a[1].under + a[1].over),
)) {
  const total = s.exact + s.under + s.over;
  console.log(
    `  ${cp.padEnd(18)} ${s.exact} exact, ${s.under} under, ${s.over} over (${(((s.under + s.over) / total) * 100).toFixed(1)}% wrong of ${total})`,
  );
}
...
for (const [b, s] of Object.entries(epBuckets).sort()) {
  const total = s.exact + s.under + s.over;
  console.log(
    `  penalty=${b.padEnd(6)} ${s.exact} exact, ${s.under} under, ${s.over} over (${(((s.under + s.over) / total) * 100).toFixed(1)}% wrong of ${total})`,
  );
}
...
for (const u of under.slice(0, 50)) {
  ...
  if (effHS && effHS !== unitHS) {
    ...
    if (hsCountMismatch <= 5) {
      console.log(
        `  ${u.unitId}: unit.hs=${unitHS} eff=${effHS} type=${unit.heatSinks?.type} diss=${b.heatDissipation}`,
      );
    }
  }
}
