...
for (const k of checkKeys) {
  ...
  console.log(
    `  ${k.padEnd(40)} → ${v ? `BV=${v.bv}, wt=${v.weaponType}` : 'NOT FOUND'}`,
  );
}
...
for (const name of testNames) {
  ...
  console.log(
    `  lu.get(norm + '-ammo') → ${JSON.stringify(ammoLookup.get(norm + '-ammo'))}`,
  );
  ...
  const tests: [string, RegExp, (m: RegExpMatchArray) => string[]][] = [
    [
      'line853-lbx-ammo',
      /^(?:is\s*)?lb\s*(\d+)-x\s*(?:ac\s*)?ammo$/,
      (m) => [`is-lb-${m[1]}-x-ammo`, `lb-${m[1]}-x-ammo`],
    ],
    [
      'line854-lbx-cluster',
      /^(?:is\s*)?lb\s*(\d+)-x\s*(?:ac\s*)?cluster\s*ammo$/,
      (m) => [`is-lb-${m[1]}-x-cluster-ammo`, `lb-${m[1]}-x-cluster-ammo`],
    ],
    [
      'line921-clan-uac',
      /^cl(?:an)?\s*ultra\s*ac[/-](\d+)\s*ammo$/,
      (m) => [`clan-uac-${m[1]}-ammo`, `uac-${m[1]}-ammo`],
    ],
    [
      'line944-cl-chemlas',
      /^cl(?:an)?\s*mediumchemlaser\s*ammo$/,
      (_) => [`clan-medium-chemical-laser-ammo`],
    ],
    [
      'line937-cl-hmg',
      /^cl(?:an)?\s*heavy\s*machine\s*gun\s*ammo$/,
      (_) => [`heavy-mg-ammo`, `heavy-machine-gun-ammo-full`],
    ],
  ...
  for (const [label, re, idsFn] of tests) {
    ...
    if (m) {
      ...
      for (const id of ids) {
        ...
        console.log(
          `    lu.get('${id}') → ${found ? `BV=${found.bv}, wt=${found.weaponType}` : 'NOT FOUND'}`,
        );
      }
    }
  }
  ...
  console.log(
    `  lu.get(canonKey) → ${JSON.stringify(ammoLookup.get(canonKey))}`,
  );
}
