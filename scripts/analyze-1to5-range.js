...
for (const u of units) {
  const t = u.breakdown?.techBase || 'unknown';
  tech[t] = (tech[t] || 0) + 1;
}
...
const avgOver = over.length
  ? over.reduce((s, u) => s + u.percentDiff, 0) / over.length
  : 0;
const avgUnder = under.length
  ? under.reduce((s, u) => s + u.percentDiff, 0) / under.length
  : 0;
console.log(
  'Avg overcalc:',
  avgOver.toFixed(2) + '%',
...
console.log(
  '  Overcalc with halved:',
  halvedUnits.filter((u) => u.percentDiff > 0).length,
);
console.log(
  '  Undercalc with halved:',
  halvedUnits.filter((u) => u.percentDiff < 0).length,
);
...
const cockpitMods = {};
for (const u of units) {
  const c = u.breakdown?.cockpitModifier || 1.0;
  const k = c.toFixed(2);
  cockpitMods[k] = (cockpitMods[k] || 0) + 1;
}
...
for (const u of units) {
  ...
  if (f) {
    const k = f.toFixed(1);
    defFactors[k] = (defFactors[k] || 0) + 1;
  }
}
console.log('Defensive factors:', JSON.stringify(defFactors));

// Look for units with ammoBV issues
const zeroAmmo = units.filter(
  (u) => u.breakdown?.ammoBV === 0 && u.breakdown?.weaponBV > 0,
);
