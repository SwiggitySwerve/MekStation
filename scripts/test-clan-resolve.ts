...
for (const id of testIds) {
  ...
  console.log(
    `  ${id.padEnd(40)} → norm="${norm}" bv=${res.battleValue} heat=${res.heat} ${res.resolved ? 'OK' : 'FAIL'}`,
  );
}
...
for (const wid of clanWeapons) {
  ...
  if (wid !== normalizedIS && !wid.startsWith('clan-'))
    candidates.push('clan-' + wid);
  ...
  console.log(
    `  ${wid.padEnd(35)} IS: bv=${isResult.battleValue} ${isResult.resolved ? 'OK' : 'FAIL'}`,
  );
  console.log(`    candidates: [${candidates.join(', ')}]`);
  if (clanResult) {
    console.log(
      `    → CLAN: bv=${clanResult.battleValue} via "${clanResult.id}" (delta=${clanResult.battleValue - isResult.battleValue})`,
    );
  } else {
