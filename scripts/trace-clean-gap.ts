...
const cleanUnderCalc = results.filter(
  (r: Result) =>
    r.calculatedBV !== null &&
    r.difference !== null &&
    r.difference < -5 &&
...
interface UnitData {
  ...
  armor: { type: string; allocation: any };
...
console.log(
  'Name                           | Ton | Gap | %Diff | DefBV  | OffBV  | SF   | Engine    | Armor     | Structure',
);
console.log(
  '-------------------------------|-----|-----|-------|--------|--------|------|-----------|-----------|-----------',
);
...
for (const r of cleanUnderCalc) {
  ...
  try {
    ...
    structCounts[unit.structure.type] =
      (structCounts[unit.structure.type] || 0) + 1;
  } catch (_error) {
    // Ignore expected failure in one-off tooling.
    void _error;
  }
...
if (stdCount > 0) {
  ...
  for (const [ton, gaps] of Object.entries(stdGapByTon).sort(
