...
console.log(
  'Undercalculated:',
  outside1.filter((u) => u.difference < 0).length,
);
...
for (const u of outside1) {
  ...
  const crits = Object.values(data.criticalSlots || {})
    .flat()
    .filter(Boolean)
    .map((s) => s.toLowerCase().replace(/\s*\(omnipod\)/g, ''));
  ...
  // Structure
  if (crits.some((s) => s.includes('endo steel') || s.includes('endosteel')))
    tags.push('ENDO');
  if (
    crits.some(
      (s) => s.includes('endo-composite') || s.includes('endocomposite'),
    )
  )
    tags.push('ENDO-COMP');
  ...
  // Armor
  if (
    data.armor?.type === 'FERRO_FIBROUS' ||
    crits.some(
      (s) =>
        s.includes('ferro-fibrous') &&
        !s.includes('light') &&
        !s.includes('heavy'),
    )
  )
  ...
  if (crits.some((s) => s.includes('stealth') && s.includes('armor')))
    tags.push('STEALTH');
  if (crits.some((s) => s.includes('reactive') && s.includes('armor')))
    tags.push('REACTIVE');
  if (crits.some((s) => s.includes('reflective') && s.includes('armor')))
    tags.push('REFLECTIVE');
  if (crits.some((s) => s.includes('hardened') && s.includes('armor')))
    tags.push('HARDENED');
  ...
  if (crits.some((s) => s.includes('artemis v') || s.includes('artemisv')))
    tags.push('ART5');
  ...
  if (crits.some((s) => s.includes('capacitor') || s.includes('ppc capacitor')))
    tags.push('PPC_CAP');
  if (crits.some((s) => s.includes('streak'))) tags.push('STREAK');
  if (crits.some((s) => s.includes('lrt') || s.includes('srt')))
    tags.push('TORPEDO');
  if (
    crits.some((s) => s.includes('ultra ac') || s.includes('ultra autocannon'))
  )
    tags.push('UAC');
  if (
    crits.some(
      (s) => s.includes('rotary ac') || s.includes('rotary autocannon'),
    )
  )
    tags.push('RAC');
  ...
  if (
    crits.some(
      (s) =>
        s.includes('light ppc') ||
        s.includes('er ppc') ||
        s.includes('snub-nose') ||
        s.includes('heavy ppc'),
    )
  )
    tags.push('PPC_VARIANT');
  if (crits.some((s) => s.includes('mml'))) tags.push('MML');
  if (crits.some((s) => s.includes('narc') || s.includes('inarc')))
    tags.push('NARC');
  ...
  if (
    crits.some(
      (s) =>
        s.includes('bap') ||
        s.includes('bloodhound') ||
        s.includes('active probe'),
    )
  )
    tags.push('PROBE');
  ...
  for (const tag of tags) {
    if (!features[tag])
      features[tag] = { over: 0, under: 0, totalAbsDiff: 0, units: [] };
    features[tag].totalAbsDiff += Math.abs(u.difference);
    if (u.difference > 0) features[tag].over++;
    else features[tag].under++;
    features[tag].units.push(
      `${u.chassis} ${u.model} (${u.difference > 0 ? '+' : ''}${u.percentDiff.toFixed(1)}%)`,
    );
  }
}

// Sort by total impact
const sorted = Object.entries(features).sort(
  (a, b) => b[1].totalAbsDiff - a[1].totalAbsDiff,
);
...
for (const [tag, data] of sorted) {
  ...
  const bias =
    data.over > data.under
      ? 'OVER'
      : data.under > data.over
        ? 'UNDER'
        : 'MIXED';
  console.log(
    `${tag}: ${total} units (${data.over} over, ${data.under} under) totalGap=${data.totalAbsDiff} bias=${bias}`,
  );
}

// Show units with specific interesting features
console.log('\n=== PROTOTYPE UNITS ===');
if (features.PROTOTYPE)
  features.PROTOTYPE.units.forEach((u) => console.log('  ' + u));

console.log('\n=== TORPEDO UNITS ===');
if (features.TORPEDO)
  features.TORPEDO.units.forEach((u) => console.log('  ' + u));
...
console.log('\n=== PPC_CAP UNITS ===');
if (features.PPC_CAP)
  features.PPC_CAP.units.forEach((u) => console.log('  ' + u));

console.log('\n=== RADICAL_HS UNITS ===');
if (features.RADICAL_HS)
  features.RADICAL_HS.units.forEach((u) => console.log('  ' + u));

console.log('\n=== PARTIAL_WING UNITS ===');
if (features.PARTIAL_WING)
  features.PARTIAL_WING.units.forEach((u) => console.log('  ' + u));

console.log('\n=== COOLANT_POD UNITS ===');
if (features.COOLANT_POD)
  features.COOLANT_POD.units.forEach((u) => console.log('  ' + u));
...
console.log('\n=== APOLLO UNITS ===');
if (features.APOLLO)
  features.APOLLO.units.forEach((u) => console.log('  ' + u));
...
for (const [issue, count] of Object.entries(unresolvedCount).sort(
  (a, b) => b[1] - a[1],
)) {
