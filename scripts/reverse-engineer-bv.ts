...
for (const u of under) {
  ...
  const weaponTypes: Record<
    string,
    { count: number; bv: number; ids: string[] }
  > = {};
  for (const eq of unit.equipment) {
    ...
    const key = lo;
    if (!weaponTypes[key])
      weaponTypes[key] = { count: 0, bv: res.battleValue, ids: [] };
  ...
  if (types.length === 1) {
      ...
      for (const [, slots] of Object.entries(unit.criticalSlots)) {
        ...
        for (const s of slots) {
          if (
            s &&
            typeof s === 'string' &&
            (s.toLowerCase().startsWith('cl') ||
              s.toLowerCase().startsWith('clan'))
          ) {
    ...
    if (Math.abs(neededPerWeapon - info.bv) > 2) {
      weaponGaps.push({
        ...
        weaponId: key,
        currentBV: info.bv,
        neededBV: Math.round(neededPerWeapon),
...
const sorted = Object.entries(byWeapon).sort(
...
for (const [weaponId, gaps] of sorted.slice(0, 25)) {
  ...
  console.log(
    `  ${weaponId.padEnd(35)} ${gaps.length} units: current=${avgCurrent.toFixed(0)} needed=${avgNeeded.toFixed(0)} ratio=${ratio.toFixed(3)} tech=${techBases} clan=${clanCount}/${gaps.length}`,
  );
  // Show individual samples
  for (const g of gaps.slice(0, 3)) {
    console.log(
      `    ${g.unitId}: ${g.currentBV} → ${g.neededBV} (${g.techBase}${g.isClan ? ' [CLAN]' : ''})`,
    );
  }
}
...
for (const u of under) {
  ...
  for (const eq of unit.equipment) {
    const lo = eq.id.toLowerCase().replace(/^\d+-/, '');
    if (lo.includes('ammo') || lo.includes('heat') || lo.includes('case'))
      continue;
    ...
    if (
      ...
      if (clanISExamples.length < 15) {
        clanISExamples.push(
          `${u.unitId}: ${eq.id} IS_bv=${isRes.battleValue} Clan_bv=${clanRes.battleValue} diff=${clanRes.battleValue - isRes.battleValue}`,
        );
      }
    }
  }
...
console.log(
  `Clan/Mixed weapons with IS < Clan BV: ${clanResolvedAsIS} instances`,
);
