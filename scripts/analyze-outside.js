...
for (const u of over.slice(0, 25)) {
  ...
  if (data) {
    ...
    if (b.cockpitType && b.cockpitType !== 'standard')
      flags.push('cockpit:' + b.cockpitType);
    if (crits.some((s) => s.toLowerCase().includes('supercharger')))
      flags.push('SC');
    if (crits.some((s) => s.toLowerCase().includes('targeting computer')))
      flags.push('TC');
    if (crits.some((s) => s.toLowerCase().includes('prototype')))
      flags.push('PROTO');
    if (
      crits.some(
        (s) =>
          s.toLowerCase().includes('re-engineered') ||
          s.toLowerCase().includes('reengineered'),
      )
    )
      flags.push('RE-ENG');
    if (crits.some((s) => s.toLowerCase().includes('radical')))
      flags.push('RADICAL');
    if (crits.some((s) => s.toLowerCase().includes('stealth')))
      flags.push('STEALTH');
    if (crits.some((s) => s.toLowerCase().includes('null'))) flags.push('NSS');
    if (crits.some((s) => s.toLowerCase().includes('void'))) flags.push('VSS');
    if (crits.some((s) => s.toLowerCase().includes('chameleon')))
      flags.push('CHAMELEON');
    if (
      crits.some(
        (s) =>
          s.toLowerCase().includes('composite') &&
          !s.toLowerCase().includes('structure'),
      )
    )
      flags.push('COMP');
    if (data.structure && data.structure.type === 'COMPOSITE')
      flags.push('COMP-STRUCT');
    if (data.gyro && data.gyro.type === 'NONE') flags.push('NO-GYRO');
    if (data.gyro && data.gyro.type === 'SUPERHEAVY') flags.push('SH-GYRO');
  }
  console.log(
    `  ${u.percentDiff > 0 ? '+' : ''}${u.percentDiff.toFixed(2)}%  ${u.chassis} ${u.model} (gap=${u.difference > 0 ? '+' : ''}${u.difference}) [${flags.join(', ')}]`,
  );
}
...
for (const u of under.slice(0, 25)) {
  ...
  if (data) {
    ...
    if (crits.some((s) => s.toLowerCase().includes('targeting computer')))
      flags.push('TC');
    if (crits.some((s) => s.toLowerCase().includes('artemis')))
      flags.push('ARTEMIS');
    if (crits.some((s) => s.toLowerCase().includes('apollo')))
      flags.push('APOLLO');
    if (crits.some((s) => s.toLowerCase().includes('ppc capacitor')))
      flags.push('PPC-CAP');
    if (crits.some((s) => s.toLowerCase().includes('stealth')))
      flags.push('STEALTH');
    if (data.armor && data.armor.type && data.armor.type !== 'STANDARD')
      flags.push('armor:' + data.armor.type);
  }
  console.log(
    `  ${u.percentDiff.toFixed(2)}%  ${u.chassis} ${u.model} (gap=${u.difference}) [${flags.join(', ')}]`,
  );
}
...
for (const u of barely.sort(
  (a, b) => Math.abs(a.percentDiff) - Math.abs(b.percentDiff),
)) {
  const data = unitMap.get(u.unitId);
  const flags = [];
  if (data) {
  ...
  console.log(
    `  ${u.percentDiff > 0 ? '+' : ''}${u.percentDiff.toFixed(2)}%  ${u.chassis} ${u.model} (${u.difference > 0 ? '+' : ''}${u.difference}) [${flags.join(',')}]`,
  );
}
