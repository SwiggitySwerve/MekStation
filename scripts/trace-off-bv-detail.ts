...
for (const targetId of targetIds) {
  ...
  console.log(
    `  Index BV: ${u.indexBV}, Calculated: ${u.calculatedBV}, Gap: ${u.difference} (${u.percentDiff.toFixed(1)}%)`,
  );
  console.log(
    `  DefBV: ${bd.defensiveBV.toFixed(2)}, OffBV: ${bd.offensiveBV.toFixed(2)}`,
  );
  console.log(
    `  WeaponBV: ${bd.weaponBV.toFixed(2)}, AmmoBV: ${bd.ammoBV.toFixed(2)}`,
  );
  console.log(`  SpeedFactor: ${bd.speedFactor.toFixed(4)}`);
  console.log(`  CockpitMod: ${bd.cockpitModifier || 1.0}`);
  console.log(
    `  DefEquipBV: ${bd.defensiveEquipBV}, ExplPenalty: ${bd.explosivePenalty}`,
  );
  ...
  console.log(
    `  Needed baseOff (before speed factor): ${neededBaseOff.toFixed(2)}`,
  );
  console.log(`  Our baseOff (before speed factor): ${ourBaseOff.toFixed(2)}`);
  console.log(
    `  Base offensive gap: ${(neededBaseOff - ourBaseOff).toFixed(2)}`,
  );
  console.log(
    `  This gap * speedFactor = ${((neededBaseOff - ourBaseOff) * bd.speedFactor).toFixed(2)}`,
  );
  console.log('');
}
...
const underUnits = report.allResults.filter(
  (u: any) => u.percentDiff < -1 && u.percentDiff > -5,
);

let weaponCountData: Array<{
  id: string;
  wpnCount: number;
  gapPct: number;
  gap: number;
...
const wpnQ2 = weaponCountData.slice(
  Math.floor(weaponCountData.length / 4),
  Math.floor(weaponCountData.length / 2),
);
const wpnQ3 = weaponCountData.slice(
  Math.floor(weaponCountData.length / 2),
  Math.floor((weaponCountData.length * 3) / 4),
);
const wpnQ4 = weaponCountData.slice(
  Math.floor((weaponCountData.length * 3) / 4),
);

function avgField(
  arr: typeof weaponCountData,
  field: 'wpnCount' | 'gapPct' | 'gap',
) {
...
console.log('---------|--------------|---------|----------');
console.log(
  `Q1 (few) | ${avgField(wpnQ1, 'wpnCount').toFixed(1).padStart(12)} | ${avgField(wpnQ1, 'gapPct').toFixed(2)}% | ${avgField(wpnQ1, 'gap').toFixed(1)}`,
);
console.log(
  `Q2       | ${avgField(wpnQ2, 'wpnCount').toFixed(1).padStart(12)} | ${avgField(wpnQ2, 'gapPct').toFixed(2)}% | ${avgField(wpnQ2, 'gap').toFixed(1)}`,
);
console.log(
  `Q3       | ${avgField(wpnQ3, 'wpnCount').toFixed(1).padStart(12)} | ${avgField(wpnQ3, 'gapPct').toFixed(2)}% | ${avgField(wpnQ3, 'gap').toFixed(1)}`,
);
console.log(
  `Q4 (many)| ${avgField(wpnQ4, 'wpnCount').toFixed(1).padStart(12)} | ${avgField(wpnQ4, 'gapPct').toFixed(2)}% | ${avgField(wpnQ4, 'gap').toFixed(1)}`,
);

// Also check: gap vs tech base
console.log('\n=== GAP vs TECH BASE ===\n');
const techBaseGroups = new Map<
  string,
  { count: number; totalGapPct: number }
>();
for (const u of underUnits) {
  const unitPath = unitPathMap.get(u.unitId);
  if (!unitPath || !fs.existsSync(unitPath)) continue;
...
for (const [tb, data] of techBaseGroups) {
  console.log(
    `${tb}: ${data.count} units, avg gap: ${(data.totalGapPct / data.count).toFixed(2)}%`,
  );
}
