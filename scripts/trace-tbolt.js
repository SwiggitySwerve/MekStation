...
for (const [id, data] of unitMap) {
  ...
  const tbWeapons = data.equipment.filter((e) =>
    e.id.toLowerCase().includes('thunderbolt'),
  );
  ...
  console.log(
    `${flag} ${u.chassis} ${u.model}: ${u.percentDiff > 0 ? '+' : ''}${u.percentDiff.toFixed(2)}% (diff=${u.difference}) TB=[${tbWeapons.map((e) => e.id).join(', ')}]`,
  );
  if (flag) {
    console.log(
      `  weaponBV=${b.weaponBV} raw=${b.rawWeaponBV} halved=${b.halvedWeaponBV}`,
    );
    console.log(
...
console.log(
  'If TB-20 heat was 0 (original catalog value), these units would have even HIGHER BV (more overcalculated)',
);
console.log(
  'So the heat=8 override is CORRECT direction but may not be enough to explain the overcalculation',
);
