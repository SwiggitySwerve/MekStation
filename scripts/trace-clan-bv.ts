...
function resolveWeaponForUnit(
  ...
  console.log(
    `    lo="${lo}", isResult: resolved=${isResult.resolved} BV=${isResult.battleValue}`,
  );
  ...
  console.log(
    `    enterClanBranch=${enterClan} (techBase==='CLAN': ${techBase === 'CLAN'}, isClanEquip: ${!!isClanEquip})`,
  );
  ...
  if (enterClan) {
    ...
    console.log(
      `    normalizedIS="${normalizedIS}", candidates=${JSON.stringify(candidates)}`,
    );
    ...
    for (const cid of candidates) {
      ...
      if (cr.resolved && cr.battleValue > 0) {
        ...
        console.log(
          `    => NOT returning: cr.bv(${cr.battleValue}) <= isResult.bv(${isResult.battleValue})`,
        );
      }
    }
  }
