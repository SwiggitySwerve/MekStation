...
console.log(
  `Within 1%: ${tmCockpit.filter((u) => Math.abs(u.percentDiff) <= 1).length}`,
);
console.log(`Over 1%: ${tmCockpit.filter((u) => u.percentDiff > 1).length}`);

const avgDiff =
  tmCockpit.reduce((s, u) => s + u.percentDiff, 0) / tmCockpit.length;
...
for (const u of tmCockpit) {
  ...
  console.log(
    `  ${u.chassis} ${u.model}: ${u.percentDiff.toFixed(2)}% ref=${u.indexBV} calc=${u.calculatedBV} gap=${u.difference} sf=${b.speedFactor} df=${b.defensiveFactor}${isQuad ? ' [QUAD]' : ''}${hasTC ? ' [TC]' : ''}${hasECM ? ' [ECM]' : ''} tech=${b.techBase}`,
  );
}
...
console.log(
  `If we multiply all calcs by ${avgRatio.toFixed(4)}, how many come within 1%?`,
);
let fixedCount = 0;
for (const u of tmCockpit) {
...
console.log(
  `Would bring ${fixed095}/${tmCockpit.length} within 1% (dividing by 0.95)`,
);
...
for (const u of tmCockpit.filter((x) => Math.abs(x.percentDiff) > 2)) {
  ...
  console.log(
    `  ${u.chassis} ${u.model}: defBV=${defBV.toFixed(0)} offBV=${offBV.toFixed(0)} total=${(defBV + offBV).toFixed(0)} ref=${u.indexBV} gap=${u.difference}`,
  );
  console.log(
    `    defPortion=${((defBV / (defBV + offBV)) * 100).toFixed(1)}% offPortion=${((offBV / (defBV + offBV)) * 100).toFixed(1)}%`,
  );
}
