...
for (const iu of targetUnits) {
  ...
  if (
    unit.configuration === 'LAM' ||
    unit.configuration === 'Tripod' ||
    unit.configuration === 'QuadVee'
  )
    continue;
  ...
  const armorMult = getArmorBVMultiplier(
    armorType === 'stealth' ? 'standard' : armorType,
  );
...
console.log(
  'Tonnage | ArmorBV avg | StructBV avg | GyroBV | PreFactor avg | EngineType sample',
);
console.log(
  '--------|-------------|--------------|--------|---------------|------------------',
);
for (const [ton, group] of Array.from(byTonnage.entries()).sort(
  (a, b) => a[0] - b[0],
)) {
  ...
  console.log(
    `${String(ton).padStart(4)}t   | ${avgArmor.toFixed(1).padStart(11)} | ${avgStruct.toFixed(1).padStart(12)} | ${group[0].gyroBV.toFixed(1).padStart(6)} | ${avgPre.toFixed(1).padStart(13)} | ${engTypes}`,
  );
}
...
if (ton100.length > 0) {
  ...
  console.log(
    `  Average pre-factor defensive BV for 100t: ${avgPre100.toFixed(1)}`,
  );
  console.log(
    `  60 BV gap is ${((60 / avgPre100) * 100).toFixed(1)}% of pre-factor`,
  );
  console.log(
    `  With defensive factor ~1.2, the pre-factor gap would be ~${(60 / 1.2).toFixed(1)} BV`,
  );
  console.log(
    `  ${((50 / avgPre100) * 100).toFixed(1)}% of pre-factor => could be a missing component`,
  );
}
...
for (const iu of indexData.units) {
  ...
  console.log(
    `  Engine: ${unit.engine.type} (${engineType}), engineMult: ${engineMult}`,
  );
  console.log(`  Armor: ${unit.armor.type}, total points: ${totalArmor}`);
  console.log(
    `  Structure: ${unit.structure.type}, total points: ${totalStructure}`,
  );
  console.log(`  Gyro: ${unit.gyro.type}`);
  console.log(
    `  ArmorBV = ${totalArmor} × 2.5 × ${armorMult} = ${armorBV.toFixed(1)}`,
  );
  console.log(
    `  StructBV = ${totalStructure} × 1.5 × ${structMult} × ${engineMult} = ${structureBV.toFixed(1)}`,
  );
  console.log(
    `  GyroBV = ${unit.tonnage} × ${gyroMult} = ${gyroBV.toFixed(1)}`,
  );
  console.log(
    `  Pre-factor total = ${(armorBV + structureBV + gyroBV).toFixed(1)}`,
  );
    ...
    for (const [loc, slots] of Object.entries(unit.criticalSlots)) {
      ...
      for (const slot of slots) {
        ...
        if (
          ...
          defItems.push(
            `${loc}: ${clean} (Spikes - MISSING from our defEquip!)`,
          );
        } else if (lo.includes('minesweep')) {
          defItems.push(
            `${loc}: ${clean} (Minesweeper - MISSING from our defEquip!)`,
          );
        }
      }
    }
