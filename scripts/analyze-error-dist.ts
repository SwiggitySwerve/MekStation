...
const valid = r.allResults.filter(
...
console.log(
  'Off >1%: ' +
    off.length +
    ' (' +
    off.filter((x: any) => x.percentDiff < 0).length +
    ' under, ' +
    off.filter((x: any) => x.percentDiff > 0).length +
    ' over)',
);
console.log(
  '1-2%: ' + off.filter((x: any) => Math.abs(x.percentDiff) <= 2).length,
);
console.log(
  '2-3%: ' +
    off.filter(
      (x: any) => Math.abs(x.percentDiff) > 2 && Math.abs(x.percentDiff) <= 3,
    ).length,
);
console.log(
  '3-5%: ' +
    off.filter(
      (x: any) => Math.abs(x.percentDiff) > 3 && Math.abs(x.percentDiff) <= 5,
    ).length,
);
console.log(
  '5-10%: ' +
    off.filter(
      (x: any) => Math.abs(x.percentDiff) > 5 && Math.abs(x.percentDiff) <= 10,
    ).length,
);
console.log(
  '>10%: ' + off.filter((x: any) => Math.abs(x.percentDiff) > 10).length,
);
...
console.log(
  '\n1-2% avg gap: ' +
    (
      g12.reduce((s: number, x: any) => s + Math.abs(x.difference), 0) /
      g12.length
    ).toFixed(1) +
    ' BV',
);
...
console.log(
  '  1-2% under: ' +
    ...
    ' (avg ' +
    (
      g12u.reduce((s: number, x: any) => s + Math.abs(x.difference), 0) /
      g12u.length
    ).toFixed(1) +
    ' BV)',
);
console.log(
  '  1-2% over: ' +
    ...
    ' (avg ' +
    (
      g12o.reduce((s: number, x: any) => s + Math.abs(x.difference), 0) /
      g12o.length
    ).toFixed(1) +
    ' BV)',
);
...
for (const f of fields) {
  console.log(
    f.padEnd(22) +
      avg(exact, f).toFixed(1).padStart(10) +
      avg(g12u, f).toFixed(1).padStart(10) +
      avg(g12o, f).toFixed(1).padStart(10),
  );
}
...
for (const u of g12u) {
  ...
  if (unres.length > 0) {
    withUnresolved++;
    unresolvedCount += unres.length;
  }
}
console.log(
  '\n1-2% under with unresolved weapons: ' +
    withUnresolved +
    '/' +
    g12u.length +
    ' (' +
...
for (const u of g12u.slice(0, 10)) {
  const b = u.breakdown;
  console.log(
    `${u.unitId} gap=${u.difference} (${u.percentDiff.toFixed(1)}%) wBV=${b?.weaponBV?.toFixed(0)} rawW=${b?.rawWeaponBV?.toFixed(0)} halvd=${b?.halvedWeaponBV?.toFixed(0)} ammo=${b?.ammoBV} he=${b?.heatEfficiency} sf=${b?.speedFactor}`,
  );
