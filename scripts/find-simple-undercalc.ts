...
for (const r of under.slice(0, 30)) {
  ...
  try {
    ...
    const eqHasTC = ud.equipment.some((e: any) =>
      e.id.toLowerCase().includes('targeting'),
    );

    console.log(
      `${(r.chassis + ' ' + r.model).padEnd(35)} ${numWeapons}wpn diff=${r.difference} (${r.percentDiff.toFixed(1)}%) weapBV=${bd.weaponBV.toFixed(0)} off=${bd.offensiveBV.toFixed(0)} def=${bd.defBV?.toFixed(0) || bd.defensiveBV.toFixed(0)} sf=${bd.speedFactor} missingBase=${missingBase.toFixed(1)} ton=${ud.tonnage} critTC=${hasTC} eqTC=${eqHasTC}`,
    );
  } catch (_error) {
