...
for (const entry of index.units) {
  ...
  try {
    ...
    const lsCount = head.filter(
      (s: any) => s && s.includes('Life Support'),
    ).length;
    const headEquip = head.filter(
      (s: any) =>
        s && !['Life Support', 'Sensors', 'Cockpit'].includes(s) && s !== null,
    );
    units.push({
      id: entry.id,
      lsCount,
...
const ls1_overcalc_other = ls1.filter(
  (u) => u.pctDiff > 1.0 && (u.pctDiff <= 3.0 || u.pctDiff >= 7.0),
);
const ls1_undercalc = ls1.filter((u) => u.pctDiff < -1.0);
console.log(`  Within 1%: ${ls1_within1.length}`);
console.log(
  `  Overcalc 3-7% (likely small cockpit): ${ls1_overcalc_5pct.length}`,
);
...
console.log(
  `Would break: ${wouldBreak.length} out of ${ls1_within1.length} currently-good LS=1 units`,
);
for (const u of wouldBreak.slice(0, 10)) {
  ...
  console.log(
    `  ${u.id.padEnd(45)} ${r.percentDiff.toFixed(2)}% → ${newPct.toFixed(2)}% headEquip=[${u.headEquip.join(', ')}]`,
  );
}
...
for (const [e, count] of Object.entries(equipCounts)
  .sort((a, b) => b[1] - a[1])
  .slice(0, 20)) {
  console.log(`  ${e}: ${count}`);
}
