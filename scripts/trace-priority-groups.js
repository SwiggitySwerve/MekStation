...
for (const u of outliers) {
  ...
  clanXLNone.push({
    ...
    heatEff: b.heatEfficiency,
    heatDiss: b.heatDissipation,
...
for (const u of clanXLNone) {
  ...
  console.log(
    `  ${u.name.padEnd(35)} diff=${String(u.diff).padStart(5)} (${u.pct.toFixed(1)}%)  def=${String(Math.round(u.defBV)).padStart(5)} (${defPct}%) off=${String(Math.round(u.offBV)).padStart(5)} sf=${u.speedFactor.toFixed(2)} df=${u.defFactor.toFixed(2)} hEff=${u.heatEff} wBV=${Math.round(u.weaponBV)} aBV=${Math.round(u.ammoBV)}`,
  );
}
...
for (const u of lightNone) {
  console.log(
    `  ${u.name.padEnd(35)} diff=${String(u.diff).padStart(5)} (${u.pct.toFixed(1)}%)  def=${String(Math.round(u.defBV)).padStart(5)} off=${String(Math.round(u.offBV)).padStart(5)} sf=${u.speedFactor.toFixed(2)} df=${u.defFactor.toFixed(2)} hEff=${u.heatEff} struct=${Math.round(u.structBV)} armor=${Math.round(u.armorBV)}`,
  );
}
...
for (const u of fusionTC) {
  console.log(
    `  ${u.name.padEnd(35)} diff=${String(u.diff).padStart(5)} (${u.pct.toFixed(1)}%)  def=${String(Math.round(u.defBV)).padStart(5)} off=${String(Math.round(u.offBV)).padStart(5)} wBV=${String(Math.round(u.weaponBV)).padStart(5)} sf=${u.speedFactor.toFixed(2)} df=${u.defFactor.toFixed(2)} hEff=${u.heatEff}`,
  );
}
...
for (const u of noneUnits.slice(0, 20)) {
  const dir = u.diff > 0 ? '+' : '';
  console.log(
    `  ${u.name.padEnd(35)} ${dir}${String(u.diff).padStart(4)} (${u.pct.toFixed(2)}%) ${(u.engine + '/' + u.tech).padEnd(20)} ${u.tonnage}t  def=${Math.round(u.defBV)} off=${Math.round(u.offBV)} hEff=${u.heatEff} wBV=${Math.round(u.weaponBV)} aBV=${Math.round(u.ammoBV)}`,
  );
}
...
for (const u of outliers) {
  ...
  bapUnits.push({
    ...
    defBV: b.defensiveBV,
    offBV: b.offensiveBV,
    defEquipBV: b.defEquipBV || 0,
    weaponBV: b.weaponBV,
    speedFactor: b.speedFactor,
    defFactor: b.defensiveFactor,
    engine: unit.engine?.type,
    tech: unit.techBase,
    otherSystems: otherFlags.length,
  });
}
bapUnits.sort((a, b) => a.diff - b.diff);
for (const u of bapUnits) {
  console.log(
    `  ${u.name.padEnd(35)} diff=${String(u.diff).padStart(5)} (${u.pct.toFixed(1)}%)  defEquip=${Math.round(u.defEquipBV)} def=${Math.round(u.defBV)} off=${Math.round(u.offBV)} ${u.engine}/${u.tech}`,
  );
}
