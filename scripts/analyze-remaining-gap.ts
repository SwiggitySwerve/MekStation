...
const minorDisc = report.allResults.filter(
  (r: any) => Math.abs(r.percentDiff) > 1.0 && Math.abs(r.percentDiff) <= 5.0,
);
...
console.log(
  `  Undercalculated: ${underCalc.length} (avg ${(underCalc.reduce((s: number, r: any) => s + r.percentDiff, 0) / underCalc.length).toFixed(2)}%)`,
);
console.log(
  `  Overcalculated: ${overCalc.length} (avg ${(overCalc.reduce((s: number, r: any) => s + r.percentDiff, 0) / overCalc.length).toFixed(2)}%)`,
);
...
for (const b of bands) {
  const count = minorDisc.filter(
    (r: any) => r.percentDiff >= b.min && r.percentDiff < b.max,
  ).length;
...
for (const [tb, data] of Object.entries(byTechBase)) {
  console.log(
    `  ${tb}: ${data.count} units, avg ${(data.totalGap / data.count).toFixed(2)}%`,
  );
}
...
for (const [bin, data] of Object.entries(ammoBins)) {
  if (data.count > 0)
    console.log(
      `  ${bin.padEnd(18)} n=${String(data.count).padStart(4)} avgPct=${(data.totalPct / data.count).toFixed(2).padStart(7)}%`,
    );
}
...
for (const [bin, data] of Object.entries(tonBins).sort()) {
  console.log(
    `  ${bin.padEnd(20)} n=${String(data.count).padStart(4)} avgPct=${(data.totalPct / data.count).toFixed(2).padStart(7)}%`,
  );
}
...
console.log('\n=== SAMPLE UNITS (-2% to -3%) ===');
const sample = underCalc
  .filter((r: any) => r.percentDiff < -2 && r.percentDiff > -3)
  .slice(0, 10);
for (const r of sample) {
  console.log(
    `${r.unitId.padEnd(45)} idx=${r.indexBV} calc=${r.calculatedBV} gap=${r.difference} (${r.percentDiff.toFixed(2)}%) ammoBV=${r.breakdown?.ammoBV} wBV=${r.breakdown?.weaponBV} defBV=${r.breakdown?.defensiveBV.toFixed(0)} sf=${r.breakdown?.speedFactor}`,
  );
}
