...
for (const [b, c] of Object.entries(buckets).sort(
...
console.log(
  `  -1% to -3%: ${minus1to3} (could move to within-1% with small fix)`,
);
...
console.log(`\n  Total: ${pcts.length}`);
console.log(
  `  Would need to fix ${pcts.length - within1 - pcts.filter((p) => Math.abs(p) <= 1).length} to reach 95% within-1%`,
);
const target95 = Math.ceil(pcts.length * 0.95);
console.log(
  `  Need ${target95} within 1% for 95% target (have ${within1}, need ${target95 - within1} more)`,
);
const target99 = Math.ceil(pcts.length * 0.99);
const within5 = pcts.filter((p) => Math.abs(p) <= 5).length;
console.log(
  `  Need ${target99} within 5% for 99% target (have ${within5}, need ${target99 - within5} more)`,
);
