...
for (const u of underc) {
  ...
  console.log(`\n=== ${u.unitId} ===`);
  console.log(
    `  techBase: ${unit.techBase}, gap: ${u.difference} (${u.percentDiff?.toFixed(1)}%)`,
  );
  ...
  for (const eq of unit.equipment) {
    ...
    const couldBeClan =
      lo.includes('clan-') || lo.includes('cl-') || isCLPrefix;
    ...
    if (unit.techBase === 'MIXED' || unit.techBase === 'CLAN') {
      ...
      console.log(
        `    critSlot: "${critName}" | CL-prefix: ${isCLPrefix} | equipId hasClan: ${lo.includes('clan-')} | techBase: ${unit.techBase}`,
      );
    }
  }
  ...
  if (unit.criticalSlots) {
    ...
    for (const [loc, slots] of Object.entries(unit.criticalSlots)) {
      ...
      for (const slot of slots) {
        if (
          slot &&
          typeof slot === 'string' &&
          /^CL/i.test(slot.replace(/\s*\(omnipod\)/gi, '').trim())
        ) {
    ...
    if (clSlots.length > 0) {
      ...
      if (clSlots.length > 10)
        console.log(`    ... and ${clSlots.length - 10} more`);
    }
  }
}
