...
function loadUnit(id) {
  ...
  try {
    return JSON.parse(
      fs.readFileSync(
        path.join(__dirname, '../public/data/units/battlemechs', ie.path),
        'utf8',
      ),
    );
  } catch {
...
const valid = r.allResults.filter(
  (x) => x.status !== 'error' && x.percentDiff !== null && x.breakdown,
);
...
for (const u of outliers) {
  ...
  if (crits.includes('streak')) {
    streakCount++;
    if (isUnder) streakUnder++;
    else streakOver++;
  }
  if (crits.includes('ultra') || crits.includes('uac')) {
    ultraCount++;
    if (isUnder) ultraUnder++;
    else ultraOver++;
  }
  if (crits.includes('rotary') || crits.includes('rac')) {
    rotaryCount++;
    if (isUnder) rotaryUnder++;
    else rotaryOver++;
  }
  if (crits.includes(' (os)') || crits.includes('-os')) {
    osCount++;
    if (isUnder) osUnder++;
    else osOver++;
  }
...
console.log(
  ...
  outliers.length,
  '(under:',
  outliers.filter((x) => x.percentDiff < 0).length,
  'over:',
  outliers.filter((x) => x.percentDiff > 0).length + ')',
);
console.log(
...
const streakSamples = outliers
  .filter((x) => {
    const unit = loadUnit(x.unitId);
    return (
      unit &&
      JSON.stringify(unit.criticalSlots || {})
        .toLowerCase()
        .includes('streak')
    );
  })
...
for (const u of streakSamples) {
  ...
  for (const w of b.weapons) {
    ...
    const isUltra =
      (w.id || '').toLowerCase().includes('ultra') ||
      (w.id || '').toLowerCase().includes('uac');
    const isRotary =
      (w.id || '').toLowerCase().includes('rotary') ||
      (w.id || '').toLowerCase().includes('rac');
    if (isStreak || isUltra || isRotary) {
      console.log(
        '  ' +
          (w.id || '?').padEnd(30) +
          ' bv=' +
          (w.bv || 0).toString().padStart(4) +
          ' heat=' +
          (w.heat || 0).toString().padStart(2) +
          ' [' +
          (isStreak ? 'STREAK' : '') +
          (isUltra ? 'ULTRA' : '') +
          (isRotary ? 'ROTARY' : '') +
    ...
    if (id.includes('ultra') || id.includes('uac')) {
      console.log(
        '  ' +
          ...
          ' heat=' +
          (w.heat || 0).toString().padStart(2) +
          ' [ULTRA - MegaMek should use heat*2]',
      );
    }
  }
}
    ...
    if (id.includes('rotary') || id.includes('rac')) {
      console.log(
        '  ' +
          ...
          ' heat=' +
          (w.heat || 0).toString().padStart(2) +
          ' [ROTARY - MegaMek should use heat*6]',
      );
    }
  }
}
