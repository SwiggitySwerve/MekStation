...
for (const iu of indexData.units) {
  ...
  let critHSCount = 0;
  const allSlots = Object.values(ud.criticalSlots)
    .flat()
  ...
  for (const slot of allSlots) {
    ...
    if (
      ...
      lo === 'isdoubleheatsink' ||
      lo === 'cldoubleheatsink' ||
      lo === 'isdouble heat sink' ||
      lo === 'cldouble heat sink' ||
  ...
  const equipHS = ud.equipment.filter((eq) => {
    ...
    return (
      lo.includes('heat-sink') ||
      lo.includes('heatsink') ||
      lo.includes('heat sink')
    );
  }).length;

  const isOmni =
    ud.equipment.some((eq) => eq.id.toLowerCase().includes('omnipod')) ||
...
console.log(
  `Mismatches: ${mismatchCount} (${((mismatchCount / totalUnits) * 100).toFixed(1)}%)`,
);
...
for (const [tb, items] of Object.entries(byTech)) {
  ...
  const underCalc = items.filter(
    (i) => i.percentBVDiff !== null && i.percentBVDiff < -1,
  );
  console.log(
    `  ${tb}: ${items.length} mismatches (avg diff: ${avgDiff.toFixed(1)} HS), ${underCalc.length} under-calculated`,
  );
}
...
const candidates = mismatchDetails.filter(
  (m) => m.diff > 0 && m.percentBVDiff !== null && m.percentBVDiff < -1,
);
console.log(
  `  ${candidates.length} units where crit HS > declared AND BV under-calculated by >1%`,
);
if (candidates.length > 0) {
  const avgBVDiff =
    candidates.reduce((s, c) => s + c.percentBVDiff!, 0) / candidates.length;
  console.log(`  Average BV under-calculation: ${avgBVDiff.toFixed(2)}%`);
  console.log(
    `  Average extra HS from crits: ${(candidates.reduce((s, c) => s + c.diff, 0) / candidates.length).toFixed(1)}`,
  );
}
...
const sorted = candidates
  .sort((a, b) => a.percentBVDiff! - b.percentBVDiff!)
  .slice(0, 20);
for (const m of sorted) {
  console.log(
    `  ${m.name.padEnd(40)} ${m.techBase.padEnd(6)} declared=${m.declared} crit=${m.critCounted} diff=${m.diff > 0 ? '+' : ''}${m.diff} BV%=${m.percentBVDiff!.toFixed(1)}% DHS=${m.isDHS} omni=${m.isOmni}`,
  );
}

// Also check: units where equipment list HS count differs
console.log(`\nTop 20 mismatches (crit < declared, over-calculated):`);
const overCalc = mismatchDetails
  .filter((m) => m.diff < 0 && m.percentBVDiff !== null && m.percentBVDiff > 1)
  .sort((a, b) => b.percentBVDiff! - a.percentBVDiff!)
  .slice(0, 20);
