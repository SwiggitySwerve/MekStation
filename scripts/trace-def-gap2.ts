...
for (const id of targets) {
  ...
  if (!res || !res.breakdown) {
    console.log(`${id}: NOT FOUND or no breakdown`);
    continue;
  }
  ...
  console.log(
    `${d.tonnage}t ${d.techBase} ${d.engine.type} ER=${d.engine.rating}`,
  );
  console.log(
    `armor=${d.armor.type} struct=${d.structure.type} cockpit=${d.cockpit || 'STANDARD'} gyro=${d.gyro?.type || 'STANDARD'}`,
  );
  console.log(
    `walk=${d.movement.walk} jump=${d.movement.jump || 0} HS=${d.heatSinks.type}(${d.heatSinks.count})`,
  );
  console.log(
    `  defBV=${b.defensiveBV.toFixed(1)} offBV=${b.offensiveBV.toFixed(1)}`,
  );
  console.log(
    `  armorBV=${(b.armorBV ?? 0).toFixed(1)} structBV=${(b.structureBV ?? 0).toFixed(1)} gyroBV=${(b.gyroBV ?? 0).toFixed(1)} defFactor=${(b.defensiveFactor ?? 0).toFixed(2)}`,
  );
  console.log(`  defEquip=${b.defensiveEquipBV} explPen=${b.explosivePenalty}`);
  console.log(
    `  weapBV=${b.weaponBV} ammoBV=${b.ammoBV} speedF=${b.speedFactor}`,
  );
  ...
  console.log(
    `  baseDef=${baseDef.toFixed(1)} baseDef*defFactor=${expectedDefBV.toFixed(1)} (reported defBV=${b.defensiveBV.toFixed(1)})`,
  );
  ...
  console.log(
    `  neededDefBV=${neededDefBV.toFixed(1)} neededBaseDef=${neededBaseDef.toFixed(1)} baseDefGap=${(neededBaseDef - baseDef).toFixed(1)}`,
  );
