...
for (const iu of index.units) {
  ...
  try {
    ...
    const gyroCount = ctSlots.filter(
      (s: any) =>
        s &&
        typeof s === 'string' &&
    ...
    let method = 'none';
    if (gyroCount === 2 && declaredGyro === 'STANDARD') {
      detectedGyro = 'COMPACT';
      method = 'crit-count';
    } else if (gyroCount === 6 && declaredGyro === 'STANDARD') {
      detectedGyro = 'XL';
      method = 'crit-count';
    }
    ...
    if (detectedGyro === declaredGyro) {
      ...
      if (
        ...
        allFluff.includes('compact gyro') &&
        declaredGyro !== 'COMPACT'
      ) {
        detectedGyro = 'COMPACT';
        method = 'fluff';
      }
...
const improved = undercalcGyro.filter(
  (r) => Math.abs(r.correctedGap) < Math.abs(r.currentGap),
);
const fixed = undercalcGyro.filter(
  (r) => Math.abs(r.correctedGap) <= r.mulBV * 0.01,
);
...
for (const [method, units] of Object.entries(byMethod)) {
  const imp = units.filter(
    (r) => Math.abs(r.correctedGap) < Math.abs(r.currentGap),
  ).length;
  const fix = units.filter(
    (r) => Math.abs(r.correctedGap) <= r.mulBV * 0.01,
  ).length;
  console.log(
    `  ${method}: ${units.length} units, ${imp} improved, ${fix} fixed`,
  );
}
...
for (const [type, units] of Object.entries(byType)) {
  ...
  console.log(
    `  ${type}: ${units.length} units, ${imp} improved, ${fix} fixed`,
  );
}
...
for (const r of undercalcGyro.slice(0, 50)) {
  const status =
    Math.abs(r.correctedGap) <= r.mulBV * 0.01
      ? 'FIXED'
      : Math.abs(r.correctedGap) < Math.abs(r.currentGap)
        ? 'BETTER'
        : 'WORSE';
  console.log(
    `  ${status.padEnd(6)} ${r.unitId.padEnd(40).slice(0, 40)} ${r.detectedGyro.padEnd(12)} gap=${String(r.currentGap).padStart(5)} → ${String(r.correctedGap).padStart(5)}  (method=${r.detectionMethod})`,
  );
}

// ALL results (not just undercalculated)
console.log(`\n=== All gyro mismatches (${allResults.length} total) ===`);
const allImproved = allResults.filter(
  (r) => Math.abs(r.correctedGap) < Math.abs(r.currentGap),
);
const allFixed = allResults.filter(
  (r) => Math.abs(r.correctedGap) <= r.mulBV * 0.01,
);
const allWorse = allResults.filter(
  (r) => Math.abs(r.correctedGap) > Math.abs(r.currentGap),
);
console.log(
  `Improved: ${allImproved.length}, Fixed to 1%: ${allFixed.length}, Made worse: ${allWorse.length}`,
);
...
const remainingUndercalc = report.allResults.filter((r: any) => {
  ...
  if (gyroFix && Math.abs(gyroFix.correctedGap) <= r.indexBV * 0.01)
    return false;
  return true;
});
console.log(
