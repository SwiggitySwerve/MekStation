...
console.log(
  `Checking ${undercalcUnits.length} undercalculating within5 units...\n`,
);
...
for (const result of undercalcUnits) {
  ...
  for (const eq of equipment) {
    ...
    if (!resolved.resolved || resolved.battleValue === 0) {
      ...
      if (!unresolvedCounts[key])
        unresolvedCounts[key] = { count: 0, units: [] };
...
console.log(
  `Total unresolved: ${totalUnresolved} (${((totalUnresolved / totalWeapons) * 100).toFixed(1)}%)\n`,
);

// Sort by frequency
const sorted = Object.entries(unresolvedCounts).sort(
...
console.log(
  `Total unresolved: ${allTotalUnresolved} (${((allTotalUnresolved / allTotalWeapons) * 100).toFixed(1)}%)\n`,
);

const allSorted = Object.entries(allUnresolved).sort(
  (a, b) => b[1].count - a[1].count,
);
