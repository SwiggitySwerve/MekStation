...
const interesting = over
  .filter((res: any) => {
    const entry = index.units.find((e: any) => e.id === res.unitId);
    if (!entry) return false;
    const siblings = byChassis[entry.chassis];
    if (!siblings || siblings.length < 3) return false;
    const correctCount = siblings.filter(
      (s: any) => Math.abs(s.percentDiff) <= 1,
    ).length;
...
console.log(
  `Found ${interesting.length} overcalculated units with mostly-correct siblings\n`,
);
...
for (const res of interesting.slice(0, 15)) {
  ...
  const siblings = byChassis[entry.chassis];
  const correctSibling = siblings?.find(
    (s: any) => Math.abs(s.percentDiff) <= 1 && s.unitId !== res.unitId,
  );

  console.log(`${'='.repeat(70)}`);
  console.log(
    `${res.unitId}: ${d.tonnage}t ${d.techBase} diff=+${res.difference} (${res.percentDiff.toFixed(1)}%)`,
  );
  console.log(`  idx=${res.indexBV} calc=${res.calculatedBV}`);
  console.log(
    `  offBV=${b.offensiveBV?.toFixed(1)} (wep=${b.weaponBV?.toFixed(1)} ammo=${b.ammoBV?.toFixed(1)} speed=${b.speedFactor?.toFixed(3)})`,
  );
  console.log(
    `  defBV=${(res.calculatedBV - b.offensiveBV).toFixed(1)} (armor=${b.armorBV?.toFixed(1)} struct=${b.structureBV?.toFixed(1)} gyro=${b.gyroBV?.toFixed(1)} defEq=${b.defensiveEquipBV?.toFixed(1)} pen=${b.explosivePenalty} defF=${b.defensiveFactor?.toFixed(2)})`,
  );
  console.log(
    `  heatDiss=${b.heatDissipation} heatEff=${b.heatEfficiency} moveHeat=${b.moveHeat}`,
  );
  ...
  for (const [loc, slots] of Object.entries(d.criticalSlots || {})) {
    ...
    for (const s of slots) {
      ...
      if (
        ...
        lo.includes('endo') ||
        lo.includes('ferro') ||
        lo.includes('reactive') ||
        lo.includes('reflective') ||
        lo.includes('stealth') ||
        lo.includes('hardened') ||
        lo.includes('light ferro') ||
        lo.includes('heavy ferro') ||
        lo.includes('structure') ||
        lo.includes('armor')
      )
        continue;
      // Skip ammo
  ...
  const physicals = weapons.filter((w) => {
    ...
    return (
      ...
      lo.includes('mace') ||
      lo.includes('talons') ||
      lo.includes('retractable blade') ||
      lo.includes('lance') ||
      lo.includes('vibroblade') ||
      lo.includes('chain whip') ||
      lo.includes('battle claw') ||
      lo.includes('backhoe')
    );
  });
  if (physicals.length > 0)
    console.log(`  PHYSICAL WEAPONS: ${physicals.join(', ')}`);
  ...
  if (correctSibling) {
    ...
    if (cb) {
      console.log(
        `  --- Correct sibling: ${cs.unitId} diff=${cs.difference} (${cs.percentDiff?.toFixed(1)}%)`,
      );
      console.log(
        `      offBV=${cb.offensiveBV?.toFixed(1)} (wep=${cb.weaponBV?.toFixed(1)} ammo=${cb.ammoBV?.toFixed(1)} speed=${cb.speedFactor?.toFixed(3)})`,
      );
      console.log(
        `      defBV=${(cs.calculatedBV - cb.offensiveBV).toFixed(1)} defF=${cb.defensiveFactor?.toFixed(2)}`,
      );
    }
  }
  console.log();
