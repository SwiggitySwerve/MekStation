...
console.log(
  `Units where 0.95x brings within 1% (from >3% overcalc): ${candidates.length}`,
);
...
for (const c of candidates) {
  ...
  const lsCount = c.headPattern
    .split('|')
    .filter((s) => s.trim().includes('Life Support')).length;
...
for (const c of candidates.slice(0, 20)) {
  const lsCount = c.headPattern
    ...
    .filter((s) => s.trim().includes('Life Support')).length;
  console.log(
    `  ${c.id.padEnd(45)} idx=${c.indexBV} calc=${c.calcBV} adj=${c.adjustedBV} ${c.origPct.toFixed(2)}%→${c.newPct.toFixed(2)}% LS=${lsCount}`,
  );
}

// Impact on accuracy: if we fix these, how many units would be within 1%?
const currentWithin1 = report.allResults.filter(
  (r: any) => Math.abs(r.percentDiff) <= 1.0,
).length;
const newWithin1 =
  currentWithin1 + candidates.filter((c) => Math.abs(c.newPct) <= 1.0).length;
console.log(
  `\nCurrent within 1%: ${currentWithin1} (${((currentWithin1 / report.allResults.length) * 100).toFixed(1)}%)`,
);
console.log(
  `After fix within 1%: ${newWithin1} (${((newWithin1 / report.allResults.length) * 100).toFixed(1)}%)`,
);
