...
for (const entry of idx.units) {
  ...
  try {
    ...
    const armorKeys = unit.armor?.allocation
      ? Object.keys(unit.armor.allocation)
      : [];
    const hasQuadLocs = armorKeys.some((k) =>
      quadLocIndicators.includes(k.toUpperCase()),
    );
    const critKeys = unit.criticalSlots ? Object.keys(unit.criticalSlots) : [];
    const hasQuadCrits = critKeys.some((k) =>
      quadLocIndicators.includes(k.toUpperCase()),
    );
...
console.log(
  `=== Misconfigured Quads (data=quad, config!=quad): ${misconfigs.length} ===`,
);
for (const m of misconfigs) {
  console.log(
    `  ${m.id.padEnd(40)} config="${m.config}" tonnage=${m.tonnage} diff=${m.diff} (${m.pct}%) ${m.status}`,
  );
}
