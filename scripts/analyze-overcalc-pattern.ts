...
for (const r of overcalc5pct) {
  ...
  try {
    ...
    const pattern = Array.isArray(head)
      ? head.map((s: any) => s || 'empty').join(' | ')
      : 'no-head';
...
for (const [ton, count] of Object.entries(tonnageDist).sort(
...
for (const [pat, count] of Object.entries(headCritPatterns)
  .sort((a, b) => b[1] - a[1])
  .slice(0, 10)) {
...
const near_1_05263 = ratios.filter(
  (r: any) => Math.abs(r.ratio - 1 / 0.95) < 0.005,
); // small cockpit miss
...
for (const r of near_1_05263.slice(0, 15)) {
  console.log(
    `  ${r.id.padEnd(45)} ratio=${r.ratio.toFixed(5)} pct=${r.pct.toFixed(2)}%`,
  );
}
