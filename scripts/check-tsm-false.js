...
for (const u of overcalc) {
  ...
  const hasTSMCrit =
    crits.includes('tsm') ||
    crits.includes('triple strength') ||
    crits.includes('triplestrength');

  // Also check if unit has partial wing (sometimes confused?)
  const hasPartialWing =
    crits.includes('partial wing') || crits.includes('partialwing');
  const hasMASC = crits.includes('masc');

  const flag = hasTSMCrit ? '' : ' [FALSE TSM?]';
  if (!hasTSMCrit) {
    console.log(
      `${u.chassis} ${u.model}: +${u.percentDiff.toFixed(2)}% wt=${wt} tonnage=${tonnage} ratio=${(wt / tonnage).toFixed(2)}${flag}`,
    );
    ...
    console.log(
      `  Without TSM: est=${estimatedFix} newDiff=${newPct.toFixed(2)}%`,
    );
    console.log('');
  }
}
...
console.log(
  `Within 1%: ${tsmUnits.filter((u) => Math.abs(u.percentDiff) <= 1).length}`,
);
console.log(`Over 1%: ${tsmUnits.filter((u) => u.percentDiff > 1).length}`);
console.log(`Under 1%: ${tsmUnits.filter((u) => u.percentDiff < -1).length}`);
console.log(
  `Avg diff: ${(tsmUnits.reduce((s, u) => s + u.percentDiff, 0) / tsmUnits.length).toFixed(3)}%`,
);
