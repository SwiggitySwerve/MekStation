...
for (const r of report.allResults) {
  ...
  try {
    ...
    if (
      u.engine.type !== 'FUSION' &&
      u.engine.type !== 'CLAN_XL' &&
      u.engine.type !== 'XL' &&
      u.engine.type !== 'LIGHT'
    )
      continue;
...
console.log(
  `Clean units (no ammo, no defEquip, no explosive): ${cleanUnits.length}`,
);
...
console.log(
  'Our delta = rawOff - (weaponBV + tonnage) -- should be ~0 if no physical/offEquip',
);
console.log(
  'MegaMek delta = neededRaw - (weaponBV + tonnage) -- what MegaMek adds extra\n',
);
...
console.log(
  'Unit'.padEnd(45) + ' Ton  TB      Engine    wBV   ourΔ   mmΔ    gap  pct%',
);
...
const avgMmDelta =
  cleanUnits.reduce((s, u) => s + u.megamekDelta, 0) / cleanUnits.length;
const avgOurDelta =
  cleanUnits.reduce((s, u) => s + u.ourDelta, 0) / cleanUnits.length;
...
for (const [ton, deltas] of Object.entries(byTon).sort(
  (a, b) => Number(a[0]) - Number(b[0]),
)) {
  const avg = deltas.reduce((s, d) => s + d, 0) / deltas.length;
  if (deltas.length >= 2) {
    console.log(
      `  ${ton}t: n=${String(deltas.length).padStart(3)} avgΔ=${avg.toFixed(1).padStart(6)}`,
    );
  }
}
