...
for (const u of out) {
  ...
  if (
    crits.includes('targeting computer') ||
    crits.includes('targetingcomputer')
  )
    addTag('TC');
  if (crits.includes('radical')) addTag('RADICAL_HS');
  if (crits.includes('ppc capacitor') || crits.includes('ppccapacitor'))
    addTag('PPC_CAP');
  if (crits.includes('one-shot') || crits.includes('oneshot'))
    addTag('ONE_SHOT');
  ...
  if (
    crits.includes('prototype') ||
    crits.includes('re-engineered') ||
    crits.includes('reengineered')
  )
    addTag('PROTOTYPE_EQUIP');
  if (
    crits.includes('case ii') ||
    crits.includes('caseii') ||
    crits.includes('clcase')
  )
    addTag('CASE_II');
  if (crits.includes('supercharger') || crits.includes('super charger'))
    addTag('SUPERCHARGER');
  if (crits.includes('partial wing') || crits.includes('partialwing'))
    addTag('PARTIAL_WING');
  if (data.gyro?.type === 'NONE') addTag('NONE_GYRO');
  if (crits.includes('improved jump jet') || crits.includes('improvedjumpjet'))
    addTag('IMPROVED_JJ');
  if (crits.includes('heavy ppc') || crits.includes('heavyppc'))
    addTag('HEAVY_PPC');
  if (crits.includes('bombast')) addTag('BOMBAST_LASER');
  if (crits.includes('vsp') || crits.includes('variable speed'))
    addTag('VSP_LASER');
...
for (const [tag, data] of Object.entries(tags).sort(
  (a, b) =>
    b[1].over.length +
    b[1].under.length -
    (a[1].over.length + a[1].under.length),
)) {
  const avgOver = data.over.length
    ? (data.over.reduce((s, u) => s + u.pct, 0) / data.over.length).toFixed(2)
    : '-';
  const avgUnder = data.under.length
    ? (data.under.reduce((s, u) => s + u.pct, 0) / data.under.length).toFixed(2)
    : '-';
  console.log(
    tag.padEnd(20),
    String(data.over.length).padEnd(6),
    String(data.under.length).padEnd(6),
    String(data.over.length + data.under.length).padEnd(6),
    avgOver.padStart(8),
    avgUnder.padStart(10),
  );
}
...
for (const [tag, data] of Object.entries(tags)) {
  ...
  if (overPct > 70 || overPct < 30) {
    ...
    for (const u of all) {
      console.log(
        `  ${u.name}: ${u.pct > 0 ? '+' : ''}${u.pct.toFixed(2)}% gap=${u.gap}`,
      );
    }
  }
}
