...
console.log(
  `Exact: ${exact.length}, Within 1%: ${within1.length}, Within 5%: ${within5.length}`,
);
...
console.log(
  `  Overcalculated: ${b1to5Over.length} (avg +${(b1to5Over.reduce((s: number, r: any) => s + getPctDiff(r), 0) / b1to5Over.length).toFixed(2)}%)`,
);
console.log(
  `  Undercalculated: ${b1to5Under.length} (avg ${(b1to5Under.reduce((s: number, r: any) => s + getPctDiff(r), 0) / b1to5Under.length).toFixed(2)}%)`,
);

// Breakdown analysis for >5% discrepancies
console.log(`\n=== OVER 5% DISCREPANCIES (${over5.length} units) ===`);
const over5Sorted = over5.sort(
  (a: any, b: any) => Math.abs(getPctDiff(b)) - Math.abs(getPctDiff(a)),
);
for (const r of over5Sorted.slice(0, 25)) {
  ...
  console.log(
    `  ${getName(r).padEnd(35)} ref=${getRef(r)} calc=${getCalc(r)} ${direction} ${getPctDiff(r).toFixed(1)}%`,
  );
...
const over5ByTech: Record<
  string,
  { count: number; over: number; under: number }
> = {};
...
const sortedUnresolved = Object.entries(unresolvedWeapons).sort(
  (a, b) => b[1] - a[1],
);
