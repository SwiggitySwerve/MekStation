...
const under = outliers
  .filter((x) => x.difference < 0)
  .sort((a, b) => a.percentDiff - b.percentDiff);

console.log(
  `Outliers: ${outliers.length} (over=${over.length}, under=${under.length})`,
);
...
function analyzeFeatures(units, label) {
  ...
  for (const u of units) {
    ...
    const hasMASC = crits.some(
      (s) => s && typeof s === 'string' && /masc/i.test(s),
    );
    const hasSC = crits.some(
      (s) => s && typeof s === 'string' && /supercharger/i.test(s),
    );
    const hasTC = crits.some(
      (s) => s && typeof s === 'string' && /targeting/i.test(s),
    );
    const hasC3 = crits.some(
      (s) => s && typeof s === 'string' && /c3/i.test(s),
    );
    const hasTSM = crits.some(
      (s) => s && typeof s === 'string' && /triple.*strength|tsm/i.test(s),
    );
    const hasStealth = crits.some(
      (s) => s && typeof s === 'string' && /stealth/i.test(s),
    );
    const hasECM = crits.some(
      (s) => s && typeof s === 'string' && /ecm/i.test(s),
    );
  ...
  for (const [f, data] of Object.entries(features).sort(
    (a, b) => b[1].count - a[1].count,
  )) {
    console.log(
      `  ${f.padEnd(25)} n=${String(data.count).padStart(3)} avgGap=${(data.totalGap / data.count).toFixed(1).padStart(7)}`,
    );
  }
}
...
for (const u of over.slice(0, 15)) {
  ...
  const flags =
    [hasMASC && 'MASC', hasSC && 'SC', hasTC && 'TC']
      .filter(Boolean)
      .join(',') || '-';
  console.log(
    `  ${(u.chassis + ' ' + u.model).padEnd(40)} diff=+${String(u.difference).padStart(4)} (${u.percentDiff.toFixed(2)}%) ${data.tech} ${flags}`,
  );
}
...
for (const u of under.slice(-15)) {
  ...
  const flags =
    [hasMASC && 'MASC', hasSC && 'SC', hasTC && 'TC', hasC3 && 'C3']
      .filter(Boolean)
      .join(',') || '-';
  console.log(
    `  ${(u.chassis + ' ' + u.model).padEnd(40)} diff=${String(u.difference).padStart(5)} (${u.percentDiff.toFixed(2)}%) ${data.tech} ${flags}`,
  );
}
