...
console.log(
  `  Undercalculated: ${under.length}, Overcalculated: ${over.length}\n`,
);
...
for (const u of under
  ...
  console.log(
    `  ${u.unitId.padEnd(42)} ${u.percentDiff.toFixed(1)}% diff=${u.difference} weap=${b.weaponBV?.toFixed(0)} ammo=${b.ammoBV} SF=${b.speedFactor} HE=${b.heatEfficiency} [${tags.join(',')}]`,
  );
...
for (const [chassis, units] of [...byChassis.entries()]
  .filter(([, v]) => v.length >= 2)
  .sort((a, b) => b[1].length - a[1].length)) {
  const avg =
    units.reduce((s: number, u: any) => s + u.percentDiff, 0) / units.length;
  console.log(
    `  ${chassis.padEnd(30)} ${units.length} units, avg ${avg.toFixed(1)}%`,
  );
}
...
console.log(`\n=== BAND SUMMARY ===`);
console.log(
  `Under: 1-2%: ${underBands['1-2%']}, 2-5%: ${underBands['2-5%']}, 5%+: ${underBands['5%+']}`,
);
console.log(
  `Over:  1-2%: ${overBands['1-2%']}, 2-5%: ${overBands['2-5%']}, 5%+: ${overBands['5%+']}`,
);
