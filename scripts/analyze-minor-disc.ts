...
const minorDisc = report.allResults.filter(
  (x: any) => x.rootCause === 'minor-discrepancy',
);
...
for (const u of minorDisc) {
  ...
  try {
    ...
    const b = u.breakdown;
    const rawGap =
      (u.indexBV - b.defensiveBV) / b.speedFactor -
    ...
    for (const [loc, slots] of Object.entries(u.criticalSlots)) {
      ...
      for (const s of slots as string[]) {
        ...
        if (
          ...
          sl.includes('mace') ||
          sl.includes('talon') ||
          sl.includes('retractable blade') ||
          sl.includes('lance') ||
          sl.includes('spike') ||
          sl.includes('vibroblade')
        ) {
          hasPhys = true;
...
for (const [k, v] of Object.entries(physTypes).sort(
...
for (const units of Object.values(buckets)) {
  for (const u of units as any[]) {
    ...
    if (hasTC) {
      tcCount++;
      tcGapSum += u.difference;
    } else {
      noTcCount++;
      noTcGapSum += u.difference;
    }
  }
}
console.log('\n=== Targeting Computer impact ===');
console.log(
  '  With TC: n=' + tcCount + ' avgDiff=' + (tcGapSum / tcCount).toFixed(1),
);
console.log(
  '  No TC: n=' + noTcCount + ' avgDiff=' + (noTcGapSum / noTcCount).toFixed(1),
);
