...
const avgAbsDiff =
  noIssue.reduce((s: number, u: any) => s + Math.abs(u.difference), 0) /
  noIssue.length;
...
for (const u of noIssueOver.slice(0, 15)) {
  const b = u.breakdown;
  console.log(
    `  ${u.unitId.padEnd(40)} ref=${u.indexBV} calc=${u.calculatedBV} diff=${u.difference} (${u.percentDiff.toFixed(1)}%) DF=${b?.defensiveFactor} SF=${b?.speedFactor} HE=${b?.heatEfficiency}`,
  );
}
