...
for (const [id, data] of unitMap) {
  ...
  if (expected !== data.movement.walk && expected > 0) {
    ...
    if (u && Math.abs(u.percentDiff) > 1) {
      console.log(
        `  ${data.chassis} ${data.model}: walk=${data.movement.walk} expected=${expected} (rating=${data.engine.rating} ton=${data.tonnage}) [${u.percentDiff.toFixed(2)}%]`,
      );
      walkIssues++;
    }
  }
...
for (const [id, data] of unitMap) {
  ...
  const jjCount = allCrits.filter((s) => {
    ...
    return (
      lo === 'jump jet' ||
      lo === 'improved jump jet' ||
      lo.includes('jumpjet') ||
      lo.includes('prototypejumpjet') ||
      lo.includes('prototype jump jet')
    );
  }).length;
  // Improved JJ take 2 slots each
  const hasImproved = allCrits.some(
    (s) =>
      s.toLowerCase().includes('improved jump jet') ||
      s.toLowerCase().includes('improvedjumpjet'),
  );
  ...
  const pwBonus =
    hasPartialWing && effectiveJJ > 0 ? (data.tonnage <= 55 ? 2 : 1) : 0;
  ...
  if (effectiveJJ > 0 && data.movement.jump !== effectiveJJ) {
    ...
    if (data.movement.jump === effectiveJJ + pwBonus && pwBonus > 0) {
      ...
      if (u && u.percentDiff > 1) {
        console.log(
          `  PW DOUBLE-COUNT: ${data.chassis} ${data.model}: jump=${data.movement.jump} JJ=${effectiveJJ} pwBonus=${pwBonus} [${u.percentDiff.toFixed(2)}%]`,
        );
        jumpIssues++;
      }
      ...
      if (u && Math.abs(u.percentDiff) > 0.5) {
        console.log(
          `  MISMATCH: ${data.chassis} ${data.model}: jump=${data.movement.jump} JJ=${effectiveJJ} [${u ? u.percentDiff.toFixed(2) + '%' : 'N/A'}]`,
        );
        jumpIssues++;
      }
    }
...
for (const [id, data] of unitMap) {
  ...
  if (hasAmmoWeapon && !hasAmmo) {
    ...
    if (u && Math.abs(u.percentDiff) > 1) {
      console.log(
        `  ${data.chassis} ${data.model}: has ammo weapon but no ammo in crits [${u.percentDiff.toFixed(2)}%]`,
      );
      ammoIssues++;
    }
  }
...
for (const [id, data] of unitMap) {
  ...
  const critHS = allCrits.filter((s) => {
    ...
    return (
      (lo.includes('double heat sink') ||
        lo.includes('doubleheatsin') ||
        lo.includes('isdoubleheatsin') ||
        lo.includes('cldoubleheatsin')) &&
      !lo.includes('ammo')
    );
  }).length;
  ...
  if (isDHS && Math.abs(data.heatSinks.count - expectedTotal) > 2) {
    console.log(
      `  ${data.chassis} ${data.model}: declared=${data.heatSinks.count} expected~${expectedTotal} (engine=${engineHS} crits=${critHSCount}) [${u.percentDiff.toFixed(2)}%]`,
    );
    hsIssues++;
  }
}
...
for (const [at, counts] of Object.entries(armorTypes).sort(
