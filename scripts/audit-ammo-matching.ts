...
// ---------------------------------------------------------------------------
let ammoLookupCache: Map<string, { bv: number; weaponType: string }> | null =
  null;
...
function buildAmmoLookup(): Map<string, { bv: number; weaponType: string }> {
  ...
  try {
    const d = JSON.parse(
      fs.readFileSync(
        path.resolve(
          process.cwd(),
          'public/data/equipment/official/ammunition.json',
        ),
        'utf-8',
      ),
    );
    for (const item of d.items || []) {
      ...
      if (!ammoLookupCache.has(canon))
        ammoLookupCache.set(canon, { bv: item.battleValue, weaponType: wt });
    }
  } catch {
    /* ignore */
  }
  ...
  const hc: Array<[string, number, string]> = [
    ...
    ['clan-improved-lrm-5-ammo', 6, 'lrm-5'],
    ['clan-improved-lrm-10-ammo', 11, 'lrm-10'],
    ['clan-improved-lrm-15-ammo', 17, 'lrm-15'],
    ['clan-improved-lrm-20-ammo', 23, 'lrm-20'],
    ...
    ['clanimprovedlrm5ammo', 6, 'lrm-5'],
    ['clanimprovedlrm10ammo', 11, 'lrm-10'],
    ['clanimprovedlrm15ammo', 17, 'lrm-15'],
    ['clanimprovedlrm20ammo', 23, 'lrm-20'],
  ...
  for (const [id, bv, wt] of hc) {
    if (!ammoLookupCache.has(id))
      ammoLookupCache.set(id, { bv, weaponType: wt });
    const canon = id.replace(/[^a-z0-9]/g, '');
    if (!ammoLookupCache.has(canon))
      ammoLookupCache.set(canon, { bv, weaponType: wt });
  }
...
function resolveAmmoByPattern(
  ...
  const rules: Rule[] = [
    ...
    { re: /^(?:mek\s*)?taser\s*ammo$/, ids: (_) => [`mg-ammo`] },
    {
      ...
      ids: (m) => [`streak-srm-${m[1]}-ammo`, `streak-srm-ammo`],
    },
    {
    ...
    { re: /^cl(?:an)?\s*lrm(\d+)\s*ammo$/, ids: (m) => [`ammo-lrm-${m[1]}`] },
    {
      ...
      ids: (m) => [`ammo-srm-${m[1]}`, `clan-ammo-srm-${m[1]}`],
    },
    {
    ...
    },
    {
      ...
      ids: (m) => [`clan-streak-srm-${m[1]}`, `streak-srm-ammo`],
    },
    {
...
function main() {
  ...
  const minorUnder = (report.allResults as ValidationResult[]).filter(
    (u) => u.percentDiff < -0.01 && u.percentDiff >= -5.0,
  );
  ...
  for (const result of minorUnder) {
    ...
    for (const [_loc, slots] of Object.entries(unitData.criticalSlots)) {
      ...
      for (const slot of slots) {
        ...
        if (pr && pr.bv > 0) {
          ammoEntries.push({
            slotName: clean,
            resolvedWeaponType: pr.weaponType,
            bv: pr.bv,
            resolved: true,
          });
        } else {
          ...
          if (ar.resolved && ar.battleValue > 0) {
            ammoEntries.push({
              slotName: clean,
              resolvedWeaponType: normalizeWeaponKey(ar.weaponType),
              bv: ar.battleValue,
              resolved: true,
            });
          } else if (pr) {
    ...
    for (const ammo of ammoEntries) {
      if (!ammo.resolved || ammo.bv === 0) {
        ammoResults.push({
          slotName: ammo.slotName,
          resolvedWeaponType: ammo.resolvedWeaponType,
          bv: ammo.bv,
          matched: false,
        });
        continue;
      ...
      const directMatch = findMatchingWeaponBV(
        ammo.resolvedWeaponType,
        weaponBVByType,
      );
  ...
  console.log(
    `Units with at least one ammo-weapon mismatch: ${unitsWithMismatch}`,
  );
  console.log(
    `Total orphaned ammo BV (lost to mismatches): ${totalOrphanedBV}`,
  );
  console.log(
    `Average orphaned BV per mismatched unit: ${unitsWithMismatch > 0 ? (totalOrphanedBV / unitsWithMismatch).toFixed(1) : 0}`,
  );
  console.log();
  ...
  for (const detail of mismatchDetails.slice(0, 30)) {
    ...
    for (const a of orphaned) {
      console.log(
        `  ORPHANED: "${a.slotName}" -> weaponType="${a.resolvedWeaponType}" bv=${a.bv}`,
      );
    }
  }
  ...
  const patternCounts: Record<
    string,
    { count: number; totalBV: number; examples: string[] }
  > = {};
    ...
    for (const a of detail.ammoEntries) {
      if (!a.matched) {
        ...
        if (!patternCounts[key])
          patternCounts[key] = { count: 0, totalBV: 0, examples: [] };
  ...
  const sorted = Object.entries(patternCounts).sort(
    (a, b) => b[1].totalBV - a[1].totalBV,
  );
  for (const [wtype, data] of sorted) {
    console.log(
      `  "${wtype}": ${data.count} ammo entries, ${data.totalBV} total BV lost`,
    );
  ...
  console.log(
    `Total BV gap across ${minorUnder.length} undercalculated units: ${totalGap.toFixed(0)}`,
  );
  console.log(`Total orphaned ammo BV: ${totalOrphanedBV}`);
  console.log(
    `Ammo matching explains ${totalGap > 0 ? ((totalOrphanedBV / totalGap) * 100).toFixed(1) : 0}% of the total gap`,
  );
  console.log(
    `(Note: orphanedBV is raw ammo BV before speedFactor multiplier)`,
  );

  // Estimate with average speed factor
  const avgSpeedFactor =
    minorUnder.reduce((s, u) => s + (u.breakdown?.speedFactor ?? 1), 0) /
    minorUnder.length;
  console.log(`Average speed factor: ${avgSpeedFactor.toFixed(3)}`);
  console.log(
    `Estimated total BV impact (orphanedBV * avgSpeedFactor): ${(totalOrphanedBV * avgSpeedFactor).toFixed(0)}`,
  );
  console.log(
    `Adjusted ammo matching explains ${totalGap > 0 ? (((totalOrphanedBV * avgSpeedFactor) / totalGap) * 100).toFixed(1) : 0}% of the total gap`,
  );
}
