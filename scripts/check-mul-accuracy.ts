...
console.log(
  `  Exact: ${mulExact} (${((mulExact / mulTotal) * 100).toFixed(1)}%)`,
);
console.log(
  `  Within 1%: ${mulWithin1} (${((mulWithin1 / mulTotal) * 100).toFixed(1)}%)`,
);
console.log(
  `  Within 5%: ${mulWithin5} (${((mulWithin5 / mulTotal) * 100).toFixed(1)}%)`,
);

console.log(`\n=== NON-MUL BV (${nonMulTotal} units) ===`);
console.log(
  `  Exact: ${nonMulExact} (${((nonMulExact / nonMulTotal) * 100).toFixed(1)}%)`,
);
console.log(
  `  Within 1%: ${nonMulWithin1} (${((nonMulWithin1 / nonMulTotal) * 100).toFixed(1)}%)`,
);
console.log(
  `  Within 5%: ${nonMulWithin5} (${((nonMulWithin5 / nonMulTotal) * 100).toFixed(1)}%)`,
);

// Also check: 1-2% band by MUL status
const nearMiss = valid.filter(
...
for (const band of [
  ...
  {
    name: '0-1%',
    fn: (x: any) => Math.abs(x.percentDiff) > 0 && Math.abs(x.percentDiff) <= 1,
  },
  {
    name: '1-2%',
    fn: (x: any) => Math.abs(x.percentDiff) > 1 && Math.abs(x.percentDiff) <= 2,
  },
  {
    name: '2-5%',
    fn: (x: any) => Math.abs(x.percentDiff) > 2 && Math.abs(x.percentDiff) <= 5,
  },
  ...
  console.log(
    `  ${band.name.padEnd(10)} total=${String(items.length).padStart(5)} MUL=${String(mul).padStart(5)} NonMUL=${String(nonMul).padStart(5)}`,
  );
}
