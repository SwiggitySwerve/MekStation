...
const fivePctOver = report.allResults.filter(
  (r: any) => r.percentDiff >= 4.5 && r.percentDiff <= 5.5,
);
...
for (const r of fivePctOver) {
  ...
  armorTypes[ud.armor?.type || 'unknown'] =
    (armorTypes[ud.armor?.type || 'unknown'] || 0) + 1;
  engineTypes[ud.engine?.type || 'unknown'] =
    (engineTypes[ud.engine?.type || 'unknown'] || 0) + 1;
}
...
for (const r of fivePctOver.slice(0, 15)) {
  ...
  console.log(
    `  ${(r.chassis + ' ' + r.model).padEnd(35)} idx=${r.indexBV} calc=${r.calculatedBV} diff=${r.difference} (${r.percentDiff.toFixed(1)}%) cockpit=${ud.cockpit} config=${ud.configuration}`,
  );
}

const exactRatio = fivePctOver.map((r: any) => r.calculatedBV / r.indexBV);
const avgRatio =
  exactRatio.reduce((s: number, r: number) => s + r, 0) / exactRatio.length;
...
if (cockpitModCheck.length > 0) {
  ...
  console.log(
    `  If cockpit mod 0.95: ${Math.round((r.breakdown.defensiveBV + r.breakdown.offensiveBV) * 0.95)}`,
  );
  console.log(
    `  If cockpit mod 1.00: ${Math.round(r.breakdown.defensiveBV + r.breakdown.offensiveBV)}`,
  );
  console.log(
    `  Def: ${r.breakdown.defensiveBV}, Off: ${r.breakdown.offensiveBV}`,
  );
}
