...
console.log(
  `=== BOUNDARY OUTLIERS (1-5%): ${boundary.length} total (${under.length} under, ${over.length} over) ===`,
);

// Show those closest to 1% threshold (smallest absolute pctDiff)
const closest = boundary
  .slice()
...
closest.slice(0, 30).forEach((x) => {
  ...
  console.log(
    `  ${x.unitId.padEnd(45)} ${sign}${x.percentDiff.toFixed(2)}%  diff=${sign}${x.difference}  exp=${x.indexBV}  calc=${x.calculatedBV}`,
  );
});
...
for (const b of closest.slice(0, 30)) {
  ...
  for (const [loc, slots] of Object.entries(unit.criticalSlots)) {
    for (const s of slots) {
      ...
      else if (lo.includes('lrm') || lo.includes('srm') || lo.includes('mrm'))
        key = 'missile-weapon';
      else if (lo.includes('laser')) key = 'laser';
      else if (lo.includes('gauss')) key = 'gauss';
      else if (lo.includes('autocannon') || lo.includes(' ac/'))
        key = 'autocannon';
      if (!seen.has(key)) {
        seen.add(key);
...
const sorted = Object.entries(equipSeen).sort(
...
for (const [k, v] of sorted.slice(0, 20)) {
  console.log(
    `  ${k.padEnd(23)} ${String(v.under).padStart(5)} ${String(v.over).padStart(5)} ${String(v.under + v.over).padStart(6)}`,
  );
}
