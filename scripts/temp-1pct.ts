...
const nearMiss = report.allResults.filter(
  (r: any) => r.percentDiff >= -2.5 && r.percentDiff < -1.0,
);
...
for (const r of nearMiss) {
  ...
  const hasTargComp =
    ud.equipment?.some((e: any) =>
      e.id.toLowerCase().includes('targeting-computer'),
    ) ||
    (ud.criticalSlots &&
  ...
  armorType[ud.armor?.type || 'unknown'] =
    (armorType[ud.armor?.type || 'unknown'] || 0) + 1;
}
...
for (const r of nearMiss.slice(0, 20)) {
  ...
  const weapCount = (ud.equipment || []).filter(
    (e: any) =>
      !e.id.toLowerCase().includes('ammo') &&
      !e.id.toLowerCase().includes('targeting') &&
      !e.id.toLowerCase().includes('tsm'),
  ).length;
  console.log(
    `  ${(r.chassis + ' ' + r.model).padEnd(35)} ${r.percentDiff.toFixed(1)}% tech=${ud.techBase.padEnd(5)} ${ud.tonnage}t walk=${ud.movement.walk} jmp=${ud.movement.jump} hs=${ud.heatSinks.count}${ud.heatSinks.type.includes('DOUBLE') ? 'D' : 'S'} weps=${weapCount} def=${bd?.defensiveBV?.toFixed(0)} off=${bd?.offensiveBV?.toFixed(0)}`,
  );
}
