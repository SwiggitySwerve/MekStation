...
for (const r of report.allResults) {
  ...
  try {
      ...
      for (const [loc, slots] of Object.entries(data.criticalSlots)) {
        ...
        for (const s of slots as any[]) {
          if (
            s &&
            typeof s === 'string' &&
            s.toLowerCase().includes('improved jump jet')
          ) {
            hasIJJ = true;
            break;
          }
    ...
    const hasAmmo = data.equipment.some((e: any) =>
      e.id.toLowerCase().includes('ammo'),
    );
...
console.log(
  `Total: ${analyses.length} (exact: ${exact.length}, undercalc: ${undercalc.length}, overcalc: ${overcalc.length})`,
);
...
function analyzeFeature(name: string, testFn: (a: UnitAnalysis) => boolean) {
  ...
  const exactWithout = withoutFeature.filter(
    (a) => Math.abs(a.diff) <= 1,
  ).length;

  const pctWith =
    withFeature.length > 0
      ? ((exactWith / withFeature.length) * 100).toFixed(1)
      : 'N/A';
  const pctWithout =
    withoutFeature.length > 0
      ? ((exactWithout / withoutFeature.length) * 100).toFixed(1)
      : 'N/A';

  const avgDefGapWith =
    withFeature.length > 0
      ? (
          withFeature.reduce((s, a) => s + a.defGap, 0) / withFeature.length
        ).toFixed(1)
      : 'N/A';
  const avgDefGapWithout =
    withoutFeature.length > 0
      ? (
          withoutFeature.reduce((s, a) => s + a.defGap, 0) /
...
analyzeFeature(
  'Non-standard engine (XL/LIGHT/XXL/COMPACT)',
  (a) => !['FUSION', 'STANDARD'].includes(a.engineType.toUpperCase()),
);
analyzeFeature('Endo Steel', (a) =>
  a.structureType.toUpperCase().includes('ENDO'),
);
analyzeFeature(
  'Non-standard structure',
  (a) => !a.structureType.toUpperCase().includes('STANDARD'),
);
analyzeFeature(
  'Non-standard armor',
  (a) => !a.armorType.toUpperCase().includes('STANDARD'),
);
...
console.log(
  'Count where |defGap| > |offGap|: ' +
    undercalc.filter((a) => Math.abs(a.defGap) > Math.abs(a.offGap)).length,
);
console.log(
  'Count where |offGap| > |defGap|: ' +
    undercalc.filter((a) => Math.abs(a.offGap) > Math.abs(a.defGap)).length,
);
console.log(
  'Avg defGap: ' +
    (undercalc.reduce((s, a) => s + a.defGap, 0) / undercalc.length).toFixed(1),
);
console.log(
  'Avg offGap: ' +
    (undercalc.reduce((s, a) => s + a.offGap, 0) / undercalc.length).toFixed(1),
);
...
for (const eng of ['FUSION', 'LIGHT', 'XL', 'XXL', 'COMPACT']) {
  ...
  const avgGap = (units.reduce((s, a) => s + a.diff, 0) / units.length).toFixed(
    1,
  );
  const avgDefGap = (
    units.reduce((s, a) => s + a.defGap, 0) / units.length
  ).toFixed(1);
  console.log(
    `  ${eng}: n=${units.length} avgDiff=${avgGap} avgDefGap=${avgDefGap}`,
  );
}
...
const exactFeatures = {
  fusionEngine: exact.filter((a) => a.engineType.toUpperCase() === 'FUSION')
    .length,
  xlEngine: exact.filter((a) => a.engineType.toUpperCase() === 'XL').length,
  lightEngine: exact.filter((a) => a.engineType.toUpperCase() === 'LIGHT')
    .length,
  standardStruct: exact.filter((a) =>
    a.structureType.toUpperCase().includes('STANDARD'),
  ).length,
  endoSteel: exact.filter((a) => a.structureType.toUpperCase().includes('ENDO'))
    .length,
  standardArmor: exact.filter((a) =>
    a.armorType.toUpperCase().includes('STANDARD'),
  ).length,
...
for (const [k, v] of Object.entries(exactFeatures)) {
  console.log(
    `  ${k}: ${v} / ${exact.length} (${((v / exact.length) * 100).toFixed(1)}%)`,
  );
}
