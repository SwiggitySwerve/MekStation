...
for (let i = 0; i < buckets.length - 1; i++) {
  const count = pcts.filter(
    (p) => p >= buckets[i] && p < buckets[i + 1],
  ).length;
  if (count > 0)
    console.log(`  ${buckets[i]}% to ${buckets[i + 1]}%: ${count} units`);
}

console.log(`\n--- Top 20 largest offensive BV shortfalls ---`);
for (const d of diffs.slice(0, 20)) {
  console.log(
    `${d.name.padEnd(35)} def=${d.defBV.toFixed(0)} off=${d.offBV.toFixed(0)} refBV=${d.refBV} impliedOff=${d.impliedOff.toFixed(0)} offDiff=${d.offDiff.toFixed(0)} (${d.offDiffPct.toFixed(1)}%)`,
  );
}

console.log(`\n--- 20 units with SMALLEST offensive diff (closest to 0) ---`);
const sorted2 = [...diffs].sort(
  (a, b) => Math.abs(a.offDiffPct) - Math.abs(b.offDiffPct),
);
