...
function writeFixtureSet(tempDir: string, hasReleaseGap: boolean) {
  ...
  writeJson(packageJsonPath, {
    scripts: {
      ...
      'verify:rules':
        'npm run validate:combat:gaps -- --format=summary --expect-total=0 && npm run validate:combat:gaps -- --level=out-of-scope --format=summary --expect-total=149 && npx openspec validate --all --strict',
