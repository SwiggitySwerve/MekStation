...
describe('combat catalog rules parity QC validator', () => {
  ...
  it('validates the current combat catalog parity manifest', () => {
    ...
    expect(result.stdout).toContain('expectedOutOfScope=149');
  ...
  it('emits automation-friendly JSON for catalog/rules parity', () => {
    ...
    expect(manifest.expectedOutOfScopeSummary.total).toBe(149);
  ...
  it('rejects stale out-of-scope count expectations', () => {
    ...
    surface!.commands = surface!.commands.map((command) =>
      command.replace(' --expect-total=149', ''),
    );
    ...
    expect(result.status).toBe(1);
    expect(result.stdout).toContain('--expect-total=149');
  });
  ...
  it('rejects non-BattleMech scope matrices that drift back to partial', () => {
    const registry = readJson<{
      surfaces: Array<{
        surfaceId: string;
        coverageStatus: string;
    ...
    expect(result.status).toBe(1);
    expect(result.stdout).toContain(
      'non-battlemech-combat-scope-matrix must keep coverageStatus in ready-with-scope.',
    );
  });
