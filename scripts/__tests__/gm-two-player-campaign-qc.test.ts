...
describe('GM and two-player campaign QC runner', () => {
  it('registers the approved catalog and deterministic implemented plans', () => {
    ...
    // E2E-79 (umbrella 22.3). Predicted red of this pin before the
    // catalog grew the group: Object.keys(REGISTERED_GROUPS) lacked
    // `cleanup-ownership`, so the first expect(toEqual(groups)) printed
    // it after `performance`. Absence from SPEC_BY_GROUP also left the
    // group in the NOT_IMPLEMENTED loop. The plan pin plus implemented
    // membership keep a silent skip from looking like a pass.
    const cleanupOwnershipPlan = core.buildRunPlan({
      group: 'cleanup-ownership',
      runId: 'task-22-cleanup-ownership',
      repoRoot,
    });
    expect(cleanupOwnershipPlan.args).toEqual([
      path.join(repoRoot, 'scripts/playwright/run-playwright.mjs'),
      'test',
      '--project=chromium',
    ...
    expect(cleanupOwnershipPlan.environment.MEKSTATION_E2E_SERVER_COMMAND).toBe(
      'node server.js',
    );

    // `combat-pack1` is the E2E-31/32/34 subset (umbrella 21.3). The
    // bare `combat` name stays reserved, same as privacy-pack versus
    // visibility. Predicted red of this pin before the catalog grew
    // the group: Object.keys(REGISTERED_GROUPS) lacked `combat-pack1`
    // before `combat`. Absence from SPEC_BY_GROUP also left the group
    // in the NOT_IMPLEMENTED loop.
    const combatPack1Plan = core.buildRunPlan({
      group: 'combat-pack1',
      runId: 'task-21-combat-pack1',
      repoRoot,
    });
    expect(combatPack1Plan.args).toEqual([
      path.join(repoRoot, 'scripts/playwright/run-playwright.mjs'),
      'test',
      '--project=chromium',
    ...
    expect(combatPack1Plan.environment.MEKSTATION_E2E_SERVER_COMMAND).toBe(
      'node server.js',
    );
