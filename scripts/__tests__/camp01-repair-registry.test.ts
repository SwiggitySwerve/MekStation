...
describe('CAMP-01 durable repair registry', () => {
  ...
  it.each([
    ['K07', 'event-bytes', 'registration event canonicality drift'],
    ['K08', 'declaration-bytes', 'repair declaration canonicality drift'],
    ['K09', 'source-bytes', 'repair source canonicality drift'],
    ['K10', 'digest-substitution', 'registration payload digest drift'],
    ['K11', 'duplicate', 'duplicate repair registration'],
    ['K12', 'reordered', 'registration order drift'],
    ['K13', 'forged-link', 'registration chain link drift'],
    ['K14', 'frozen', 'frozen wave registration rejected'],
    ['K18', 'stale-name', 'registration event digest drift'],
  ])('[%s %s guard] rejects registry chain forgery', (id, mutation, guard) => {
  ...
  it('[K17 duplicate registration fail-closed guard] rejects before publishing and leaves the registry readable', () => {
    expect(invoke({ action: 'forgery', mutation: 'duplicate-live' })).toEqual({
      ok: true,
      value: {
        rejected:
