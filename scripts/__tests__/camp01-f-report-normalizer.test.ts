...
describe('CAMP-01F persistence authority reporter', () => {
  ...
  // prettier-ignore
  it.each([['missing','authority observation missing'],['fabricated-title','authority observation missing'],['failed','authority observation drift'],['path-drop','caller attachment drop'],['path-and-body','caller attachment drop'],['extra-attachment','persistence attachment drift'],['duplicate-attachment','duplicate persistence attachment'],['content-type','persistence attachment malformed'],['malformed-body','persistence attachment malformed'],['empty-body','persistence attachment malformed'],['extra-key','persistence facts drift'],['missing-key','persistence facts drift'],['identity','persistence identity drift'],['method','persistence authority drift'],['source','persistence authority drift'],['boolean','persistence authority drift'],['collision','normalized report publication failed']])('fails closed for Playwright mutation %s', (mutation, message) => {
