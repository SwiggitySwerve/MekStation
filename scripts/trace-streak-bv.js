...
for (const u of r.allResults) {
  ...
  if (!data) continue;
  const hasStreak = (data.equipment || []).some((e) =>
    e.id.toLowerCase().includes('streak'),
  );
...
for (const { u, data } of outsideStreak.sort(
  (a, b) => b.u.percentDiff - a.u.percentDiff,
)) {
  const dir = u.difference > 0 ? '+' : '';
  const streakEquip = (data.equipment || []).filter((e) =>
    e.id.toLowerCase().includes('streak'),
  );
  const b = u.breakdown || {};
  ...
  const streakCrits = [
    ...new Set(
      crits
        .filter((c) => c.toLowerCase().includes('streak'))
        .map((c) => c.replace(/\s*\(omnipod\)/gi, '')),
    ),
  ];

  console.log(
    `${dir}${u.percentDiff.toFixed(1)}% ${u.chassis} ${u.model} (diff=${dir}${u.difference}) tech=${data.techBase}`,
  );
  console.log(
    `  weapons: ${(data.equipment || []).map((e) => e.id).join(', ')}`,
  );
  console.log(`  streak equip: ${streakEquip.map((e) => e.id).join(', ')}`);
  console.log(`  streak crits: ${streakCrits.join(', ')}`);
  console.log(
    `  weapBV=${b.weaponBV} raw=${b.rawWeaponBV} halved=${b.halvedWeaponBV}`,
  );
  console.log(
    `  ammoBV=${b.ammoBV} speedF=${b.speedFactor} heatEff=${b.heatEfficiency}`,
  );
  console.log();
}
...
for (const { u, data } of outsideStreak) {
  ...
  for (const c of new Set(streakCrits)) {
    ...
    console.log(
      `  ${u.chassis} ${u.model}: "${c.replace(/\s*\(omnipod\)/gi, '')}" → ${isClan ? 'CLAN' : isIS ? 'IS' : 'UNKNOWN'} (techBase=${data.techBase})`,
    );
  }
}
