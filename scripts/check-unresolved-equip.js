...
for (const f of bvAnalysis.findJsonFiles('public/data/equipment/official')) {
  try {
...
// Load name mappings
const nameMappings = bvAnalysis.readJson(
...
for (const id of checkList) {
  ...
  const mapped = Object.entries(nameMappings).find(
    ([k]) => k.toLowerCase() === id,
  )?.[1];
  ...
  if (direct) {
    console.log(
      `  ${id}: BV=${direct.battleValue} heat=${direct.heat ?? '-'} (direct catalog)`,
    );
  } else if (mappedEntry) {
    console.log(
      `  ${id}: BV=${mappedEntry.battleValue} heat=${mappedEntry.heat ?? '-'} (via ${mapped})`,
    );
  } else {
    // Try partial matching
    const partials = [...equipMap.keys()].filter(
      (k) => k.includes(id) || id.includes(k),
    );
    if (partials.length > 0) {
      console.log(
        `  ${id}: NOT FOUND - partial matches: ${partials.slice(0, 3).join(', ')}`,
      );
    } else {
