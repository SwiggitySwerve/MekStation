...
console.log(
  '  AMS, ECM/Guardian/Angel/Watchdog/NovaCEWS, BAP/Beagle/Bloodhound,',
);
    ...
    for (const [, slots] of Object.entries(unit.criticalSlots)) {
      ...
      for (const slot of slots) {
        ...
        if (lo.includes('viral') && lo.includes('jammer')) {
          ...
          if (
            !missingEquipCounts['viral-jammer'].units.includes(valUnit.unitId)
          )
        ...
        if (lo.includes('bridge') && lo.includes('layer')) {
          ...
          if (
            !missingEquipCounts['bridge-layer'].units.includes(valUnit.unitId)
          )
        ...
        if (
          lo.includes('screen') &&
          lo.includes('launcher') &&
          !lo.includes('ammo')
        ) {
          missingEquipCounts['screen-launcher'].count++;
          if (
            !missingEquipCounts['screen-launcher'].units.includes(
              valUnit.unitId,
            )
          )
...
for (const [name, data] of Object.entries(missingEquipCounts)) {
  console.log(
    `${name.padEnd(17)} | ${String(data.units.length).padStart(5)} | ${String(data.count).padStart(5)}`,
  );
...
console.log(
  'MegaMek: Cockpit modifier (small=0.95, interface=1.3, etc) is applied to',
);
console.log(
  'the FINAL BV (defensive + offensive), NOT within the defensive calculation.',
);
console.log(
  'Our code: Same — getCockpitModifier() applied in calculateTotalBV() line 980.',
);
...
console.log(
  'MegaMek: NO heat sink BV in processDefensiveEquipment/processDefensiveValue.',
);
console.log('Heat sinks only affect offensive BV via heat efficiency.');
console.log(
  'Our code: Same — heat sinks only used for heatDissipation in offensive calc.',
);
...
console.log(
  'MegaMek: HarJel II/III counted as defensive equipment (F_HARJEL_II/III).',
);
console.log(
  'Our code: HarJel handled via armor bonus in validate-bv.ts:1167-1186. ✓',
);
...
console.log(
  `Simple undercalculated units (no def equip, no explosive penalty): ${simpleUnder.length}\n`,
);
...
for (const valUnit of simpleUnder.slice(0, 8)) {
  ...
  try {
    ...
    const totalArmor = Object.values(unit.armor?.allocation || {}).reduce(
      (sum: number, v: any) => {
    ...
    const totalStruct =
      structureByTonnage[unit.tonnage] || Math.round(unit.tonnage * 1.71);
    ...
    console.log(
      `  Index: ${valUnit.indexBV}, Calc: ${valUnit.calculatedBV}, Diff: ${valUnit.difference} (${valUnit.percentDiff.toFixed(1)}%)`,
    );
    console.log(`  Def BV: ${defBV.toFixed(1)}, Off BV: ${offBV.toFixed(1)}`);
    console.log(
      `  Armor: ${totalArmor}pts, Structure: ${totalStruct}pts, Tonnage: ${unit.tonnage}`,
    );
    console.log(
      `  Engine: ${unit.engine?.type}, Gyro: ${unit.gyro?.type}, Armor type: ${unit.armor?.type}`,
    );
    console.log(
      `  Raw est: armor=${armorBV.toFixed(0)} + struct=${structBV.toFixed(0)} + gyro=${gyroBV.toFixed(0)} = ${rawComponents.toFixed(0)}`,
    );
    console.log(`  Implied def factor: ${estimatedDefFactor.toFixed(3)}`);
    console.log(
      `  Walk: ${unit.movement?.walk}, Jump: ${unit.movement?.jump || 0}`,
    );
    console.log('');
  } catch (e) {
    /* skip */
...
const undercalcUnits = report.allResults.filter(
  (u: any) => u.percentDiff < -1.5 && u.percentDiff > -5,
);
console.log(
  `Undercalculated units in -1.5% to -5% range: ${undercalcUnits.length}`,
);
