...
for (const uid of testUnits) {
  ...
  console.log(`\n${'='.repeat(70)}`);
  console.log(
    `${uid} (${unit.techBase}, ${unit.tonnage}t) ref=${result.indexBV} calc=${result.calculatedBV} gap=${result.difference}`,
  );

  const b = result.breakdown;
  console.log(
    `  DefBV: ${b.defensiveBV?.toFixed(0)}  OffBV: ${b.offensiveBV?.toFixed(0)}`,
  );
  console.log(
    `  WeaponBV: ${b.weaponBV?.toFixed(0)}  AmmoBV: ${b.ammoBV}  WeightBonus: ${b.weightBonus?.toFixed(0)}`,
  );
  console.log(
    `  SF: ${b.speedFactor}  CockpitMod: ${b.cockpitModifier}  HE: ${b.heatEfficiency}`,
  );
  ...
  for (const eq of unit.equipment) {
    ...
    const normKey = normalizeEquipmentId(
      eq.id.toLowerCase().replace(/^\d+-/, ''),
    );
    console.log(
      `    ${eq.id.padEnd(30)} normKey=${normKey.padEnd(20)} bv=${res.battleValue} heat=${res.heat} ${res.resolved ? '' : 'UNRESOLVED'}`,
    );
  }
  ...
  for (const eq of unit.equipment) {
    ...
    for (let i = 0; i < qty; i++) {
      ...
      const normId = normalizeEquipmentId(
        eq.id.toLowerCase().replace(/^\d+-/, ''),
      );
  ...
  console.log(
    `  Components: weapon=${b.weaponBV?.toFixed(0)} ammo=${b.ammoBV} weight=${b.weightBonus?.toFixed(0)} phys=${b.physicalWeaponBV?.toFixed(0) ?? 0} eq=${b.offEquipBV ?? 0}`,
  );
}
...
try {
  ...
  const iatmAmmo = ammoCat.items.filter((item: any) =>
    item.id.toLowerCase().includes('iatm'),
  );
  for (const item of iatmAmmo) {
    console.log(
      `  ${item.id}: bv=${item.battleValue} compatibleWeapons=${JSON.stringify(item.compatibleWeaponIds)}`,
    );
  }
  if (iatmAmmo.length === 0) console.log('  NO iATM ammo entries in catalog');
} catch (e) {
  console.log('  Error reading catalog');
}
...
try {
  ...
  const atmAmmo = ammoCat.items.filter((item: any) =>
    item.id.toLowerCase().includes('atm'),
  );
  for (const item of atmAmmo) {
    console.log(
      `  ${item.id}: bv=${item.battleValue} compatible=${JSON.stringify(item.compatibleWeaponIds || [])}`,
    );
  }
} catch (e) {
  console.log('  Error reading catalog');
