...
const by10t: Record<
  string,
  { count: number; totalGap: number; avgPct: number }
> = {};
for (const g of gaps) {
...
for (const [bucket, data] of Object.entries(by10t).sort((a, b) =>
  a[0].localeCompare(b[0]),
)) {
  console.log(
    `  ${bucket}: ${data.count} units, avg gap ${(data.totalGap / data.count).toFixed(1)} BV`,
  );
}
...
console.log(
  `  Median: ${pctOfOff[Math.floor(pctOfOff.length / 2)].toFixed(2)}%`,
);
console.log(
  `  Mean: ${(pctOfOff.reduce((s, v) => s + v, 0) / pctOfOff.length).toFixed(2)}%`,
);
console.log(
  `  P25: ${pctOfOff[Math.floor(pctOfOff.length * 0.25)].toFixed(2)}%`,
);
console.log(
  `  P75: ${pctOfOff[Math.floor(pctOfOff.length * 0.75)].toFixed(2)}%`,
);

// Check if gap/SF gives a consistent value (suggesting missing offensive component)
console.log('\nGap divided by speed factor:');
const gapPerSF: number[] = gaps
  .filter((g) => g.sf > 0)
  .map((g) => g.gap / g.sf);
gapPerSF.sort((a, b) => a - b);
console.log(
  `  Median: ${gapPerSF[Math.floor(gapPerSF.length / 2)].toFixed(1)} BV`,
);
console.log(
  `  Mean: ${(gapPerSF.reduce((s, v) => s + v, 0) / gapPerSF.length).toFixed(1)} BV`,
);
console.log(
  `  P25: ${gapPerSF[Math.floor(gapPerSF.length * 0.25)].toFixed(1)} BV`,
);
console.log(
  `  P75: ${gapPerSF[Math.floor(gapPerSF.length * 0.75)].toFixed(1)} BV`,
);
...
console.log(
  `  Median: ${pctOfDef[Math.floor(pctOfDef.length / 2)].toFixed(2)}%`,
);
console.log(
  `  Mean: ${(pctOfDef.reduce((s, v) => s + v, 0) / pctOfDef.length).toFixed(2)}%`,
);

// Check gap vs tonnage directly
console.log('\nGap vs tonnage (is gap proportional to tonnage?):');
const gapPerTon: number[] = gaps
  .filter((g) => g.tonnage > 0)
  .map((g) => g.gap / g.tonnage);
gapPerTon.sort((a, b) => a - b);
console.log(
  `  Median gap/ton: ${gapPerTon[Math.floor(gapPerTon.length / 2)].toFixed(2)}`,
);
console.log(
  `  Mean gap/ton: ${(gapPerTon.reduce((s, v) => s + v, 0) / gapPerTon.length).toFixed(2)}`,
);

// Check gap vs tonnage*SF (weight bonus * speed factor)
console.log('\nGap vs tonnage*SF:');
const gapVsWeightSF: number[] = gaps
  .filter((g) => g.tonnage > 0 && g.sf > 0)
  .map((g) => g.gap / (g.tonnage * g.sf));
gapVsWeightSF.sort((a, b) => a - b);
console.log(
  `  Median gap/(ton*SF): ${gapVsWeightSF[Math.floor(gapVsWeightSF.length / 2)].toFixed(3)}`,
);
console.log(
  `  Mean gap/(ton*SF): ${(gapVsWeightSF.reduce((s, v) => s + v, 0) / gapVsWeightSF.length).toFixed(3)}`,
);
...
console.log(
  `  Gap ~= tonnage*SF (within 15%): ${fullTonMatch} / ${gaps.length}`,
);
...
for (const r of under.slice(0, 200)) {
  ...
  let ud: any;
  try {
    ud = JSON.parse(fs.readFileSync(path.join(basePath, iu.path), 'utf-8'));
  } catch {
    continue;
  }
  const allCrits = Object.values(ud.criticalSlots || {})
    .flat()
...
console.log(
  `  Has lower arm: ${hasLowerArmActuator}, Missing lower arm: ${missingLowerArm}`,
);
