...
if (fs.existsSync(cachePath)) {
  ...
  for (const u of idx.units) {
    ...
    if (entry) {
      ...
      if (
        entry.mulBV > 0 &&
        (entry.matchType === 'exact' || entry.matchType === 'fuzzy')
      ) {
...
const within1 = valid.filter(
  (x: any) => Math.abs(x.percentDiff) <= 1 && x.status !== 'exact',
);
...
function refSource(units: any[]): { mul: number; idx: number; mulPct: string } {
  ...
  return {
    mul,
    idx: idxOnly,
    mulPct: ((mul / units.length) * 100).toFixed(0) + '%',
  };
}

console.log('=== REFERENCE BV SOURCE BY CATEGORY ===\n');
console.log(
  `Exact match (${exact.length}): ${JSON.stringify(refSource(exact))}`,
);
console.log(
  `Within 1% (${within1.length}): ${JSON.stringify(refSource(within1))}`,
);
console.log(
  `Over 1-2% (${over1to2.length}): ${JSON.stringify(refSource(over1to2))}`,
);
console.log(
  `Over 2-5% (${over2to5.length}): ${JSON.stringify(refSource(over2to5))}`,
);
console.log(`Over 5%+ (${over5.length}): ${JSON.stringify(refSource(over5))}`);
console.log(
  `Under 1-2% (${under1to2.length}): ${JSON.stringify(refSource(under1to2))}`,
);
console.log(
  `Under 2-5% (${under2to5.length}): ${JSON.stringify(refSource(under2to5))}`,
);
console.log(
  `Under 5%+ (${under5.length}): ${JSON.stringify(refSource(under5))}`,
);
...
for (const u of over5.sort((a: any, b: any) => b.percentDiff - a.percentDiff)) {
  ...
  console.log(
    `  ${u.unitId.padEnd(42)} ref=${u.indexBV} calc=${u.calculatedBV} +${u.percentDiff.toFixed(1)}% ` +
...
for (const u of [...over5, ...over2to5]) {
  ...
  if (mulBV && iu && mulBV !== iu.bv) {
    console.log(
      `  ${u.unitId.padEnd(42)} indexBV=${iu.bv} mulBV=${mulBV} refUsed=${u.indexBV} calc=${u.calculatedBV}`,
    );
  }
}
