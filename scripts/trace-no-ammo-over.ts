...
for (const res of targets.slice(0, 15)) {
  ...
  console.log(
    `  armorBV=${b.armorBV?.toFixed(1)} structBV=${b.structureBV?.toFixed(1)} gyroBV=${b.gyroBV?.toFixed(1)} defEquip=${b.defensiveEquipBV?.toFixed(1)} pen=${b.explosivePenalty}`,
  );
  console.log(
    `  defFactor=${b.defensiveFactor?.toFixed(2)} offBV=${b.offensiveBV?.toFixed(1)} (wep=${b.weaponBV?.toFixed(1)} speed=${b.speedFactor?.toFixed(3)})`,
  );
  console.log(
    `  armor=${d.armor?.type} struct=${d.structure?.type} cockpit=${d.cockpit || 'STANDARD'}`,
  );
  ...
  if (defItems.length > 0)
    console.log(`  defEquip crits: ${defItems.join(', ')}`);
  if (d.equipment?.length) {
    const eqNames = d.equipment
      .map((e: any) => `${e.id}@${e.location}`)
      .join(', ');
