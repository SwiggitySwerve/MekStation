...
const cleanUnderCalc = results.filter((r: Result) => {
  ...
  if (r.breakdown.defensiveEquipBV !== 0 || r.breakdown.explosivePenalty !== 0)
    return false;
  ...
  if (!iu) return false;
  try {
    const unit: UnitData = JSON.parse(
      fs.readFileSync(
        path.resolve('public/data/units/battlemechs', iu.path),
        'utf-8',
      ),
    );
    ...
    return (
      (engUp === 'FUSION' || engUp === 'STANDARD') &&
      !armUp.includes('FERRO') &&
      !armUp.includes('REACTIVE') &&
      !armUp.includes('REFLECTIVE') &&
      !armUp.includes('HARDENED') &&
      !armUp.includes('STEALTH') &&
      !armUp.includes('LAMELLOR') &&
...
for (const r of cleanUnderCalc) {
  ...
  console.log(
    `  Index BV: ${r.indexBV}  |  Our BV: ${ourTotal}  |  Gap: ${gap}`,
  );
  console.log(
    `  Walk ${walkMP}, Run ${runMP}, Jump ${jumpMP}  |  TMM: ${tmm}  |  DefFactor: ${defFactor}`,
  );
  console.log(`  Armor: ${totalArmor} pts -> ${armorBV.toFixed(1)} BV`);
  console.log(
    `  Structure: ${totalStructure} pts -> ${structureBV.toFixed(1)} BV`,
  );
  console.log(`  Gyro: ${gyroBV.toFixed(1)} BV`);
  console.log(
    `  Pre-factor: ${preFactorDef.toFixed(1)} -> with defFactor ${defFactor}: ${expectedDefBV.toFixed(1)}`,
  );
  console.log(
    `  Our defensive BV: ${ourDefBV.toFixed(1)}  (delta from expected: ${(ourDefBV - expectedDefBV).toFixed(1)})`,
  );
  console.log(
    `  Our offensive BV: ${ourOffBV.toFixed(1)}  |  SF: ${r.breakdown!.speedFactor}`,
  );
  console.log(
    `  IF gap is all defensive: implied defBV = ${impliedDefBV.toFixed(1)}, defGap = ${defGap.toFixed(1)}`,
  );
  console.log(
    `  IF gap is all offensive: implied offBV = ${impliedOffBV.toFixed(1)}, offGap = ${offGap.toFixed(1)}`,
  );
