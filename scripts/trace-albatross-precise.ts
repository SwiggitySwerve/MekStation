...
const totalStruct =
  structPts.head +
  structPts.centerTorso +
  structPts.sideTorso * 2 +
  structPts.arm * 2 +
  structPts.leg * 2;
const structMult = STRUCTURE_BV_MULTIPLIERS['standard'] || 1.0;
const structBV = totalStruct * 1.5 * structMult;
console.log(
  `Structure: total=${totalStruct} points × 1.5 × ${structMult} = ${structBV}`,
);
...
const gyroType = (unit.gyro?.type || 'STANDARD')
  ...
  .replace(/[_\s]+/g, '-');
const gyroMult =
  GYRO_BV_MULTIPLIERS[gyroType] || GYRO_BV_MULTIPLIERS['standard'] || 0.5;
const gyroBV = unit.tonnage * gyroMult;
console.log(`Gyro: ${unit.tonnage}t × ${gyroMult} (${gyroType}) = ${gyroBV}`);

// Engine type
const engineType = unit.engine.type.toLowerCase().includes('xl')
  ? 'xl'
  : unit.engine.type.toLowerCase().includes('xxl')
    ? 'xxl'
    : unit.engine.type.toLowerCase().includes('light')
      ? 'light'
      : unit.engine.type.toLowerCase().includes('compact')
        ? 'compact'
        : 'standard';
const engineMult =
  ENGINE_BV_MULTIPLIERS[engineType] || ENGINE_BV_MULTIPLIERS['standard'] || 1.0;
...
for (const [loc, slots] of Object.entries(crits)) {
  ...
  for (const s of slots) {
    ...
    if (lo.includes('ammo') && !lo.includes('gauss') && !lo.includes('plasma'))
      explosiveSlots++;
  }
}
...
console.log(`\nDefensive BV:`);
console.log(
  `  armorBV (${armorBV}) + structBV (${structBV}) + gyroBV (${gyroBV}) + defEquipBV (${defEquipBV}) - explosive (${explosivePenalty})`,
);
console.log(`  = ${preDef}`);
console.log(
  `  × defensiveFactor (${defensiveFactor}) × engineMult (${engineMult})`,
);
console.log(`  = ${totalDefBV}`);
console.log(
  `  Report says: ${report.allResults.find((r: any) => r.unitId === 'albatross-alb-5w')?.breakdown?.defensiveBV}`,
);

// Check what report's offensive breakdown looks like
const result = report.allResults.find(
  (r: any) => r.unitId === 'albatross-alb-5w',
);
if (result) {
  ...
  console.log(
    `  totalBV = def + off = ${off.defensiveBV + off.offensiveBV} → cockpit mod → ${result.calculatedBV}`,
  );
  ...
  console.log(
    `  Need defensiveBV = ${neededTotal} - ${off.offensiveBV.toFixed(2)} = ${neededDef.toFixed(2)}`,
  );
...
for (const [loc, val] of Object.entries(armorAlloc)) {
  if (typeof val === 'number') console.log(`  ${loc}: ${val}`);
    ...
    console.log(
      `  ${loc}: front=${(val as any).front} rear=${(val as any).rear}`,
    );
}
