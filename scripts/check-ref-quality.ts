...
console.log(
  `Exact MUL match: ${mulExact} (outliers: ${mulExactOutlier}, ${((mulExactOutlier / mulExact) * 100).toFixed(1)}%)`,
);
console.log(
  `Fuzzy MUL match: ${mulFuzzy} (outliers: ${mulFuzzyOutlier}, ${((mulFuzzyOutlier / mulFuzzy) * 100).toFixed(1)}%)`,
);
console.log(
  `No MUL match:    ${mulNone} (outliers: ${mulNoneOutlier}, ${mulNone > 0 ? ((mulNoneOutlier / mulNone) * 100).toFixed(1) : 0}%)`,
);
  ...
  const mulStatus =
    entry?.matchType === 'exact'
      ...
      : entry?.matchType === 'fuzzy'
        ...
        : 'NO_MUL';
  console.log(
    `  ${u.unitId.padEnd(45)} ref=${u.indexBV} calc=${u.calculatedBV} diff=${u.percentDiff.toFixed(1)}% ${mulStatus}`,
  );
}
...
for (const id of named) {
  ...
  console.log(
    `  ${id.padEnd(45)} MUL=${entry?.matchType || 'none'} mulBV=${entry?.mulBV || 'n/a'} mulName=${entry?.mulName || 'n/a'} ref=${r?.indexBV} calc=${r?.calculatedBV}`,
  );
}
