...
const offensiveEquipPatterns = [
  ...
  {
    pattern: /\bbap\b|active\s*probe|beagle/i,
    name: 'Active Probe/BAP',
    bv: 0,
  },
...
for (const b of boundary) {
  if (
    b.breakdown &&
    (b.breakdown.offEquipBV > 0 || b.breakdown.offensiveEquipmentBV > 0)
  ) {
    withOffEquip++;
    console.log(
      `  ${b.unitId}: offEquipBV=${b.breakdown.offEquipBV || b.breakdown.offensiveEquipmentBV}`,
    );
  }
}
  ...
  for (const [loc, slots] of Object.entries(unit.criticalSlots)) {
    for (const s of slots) {
      ...
      if (
        ...
        lo.includes('cockpit') ||
        lo === 'shoulder' ||
        lo === 'upper arm actuator' ||
        lo === 'lower arm actuator' ||
        lo === 'hand actuator' ||
        lo === 'hip' ||
        lo === 'upper leg actuator' ||
        lo === 'lower leg actuator' ||
        lo === 'foot actuator' ||
      ...
      if (
        ...
        continue;
      if (!equipFreq[lo])
        equipFreq[lo] = { count: 0, under: 0, over: 0, units: [] };
...
const sorted = Object.entries(equipFreq).sort(
  (a, b) => b[1].count - a[1].count,
);
console.log('Equipment'.padEnd(40) + 'Count  Under  Over');
for (const [eq, data] of sorted.slice(0, 40)) {
  console.log(
    `  ${eq.padEnd(38)} ${String(data.count).padStart(5)} ${String(data.under).padStart(6)} ${String(data.over).padStart(5)}`,
  );
}
...
for (const b of boundary) {
  ...
  const hasC3 = allSlots.some(
    (s) => s.includes('c3') && !s.includes('c3 ammo'),
  );
  if (hasC3) {
    const c3items = allSlots.filter(
      (s) => s.includes('c3') && !s.includes('c3 ammo'),
    );
    const sign = b.difference > 0 ? '+' : '';
    console.log(
      `  ${b.unitId.padEnd(40)} ${sign}${b.percentDiff.toFixed(2)}%  C3: ${[...new Set(c3items)].join(', ')}`,
    );
  }
}
