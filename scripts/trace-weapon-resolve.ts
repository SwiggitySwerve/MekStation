...
const underc = report.allResults
  .filter(
    (r: any) =>
      r.status !== 'error' && r.percentDiff !== null && r.percentDiff < -1,
  )
  .sort((a: any, b: any) => a.percentDiff - b.percentDiff);

let totalMissing = 0;
let missingWeapons: Record<string, number> = {};
let wrongResolve: Record<
  string,
  { isBV: number; clanBV: number; count: number }
> = {};
...
console.log(
  '=== WEAPONS WITH IS/CLAN BV DIFFERENCE (on Clan/Mixed undercalculated units) ===',
);
const sorted = Object.entries(wrongResolve).sort(
  (a, b) =>
    (b[1].clanBV - b[1].isBV) * b[1].count -
    (a[1].clanBV - a[1].isBV) * a[1].count,
);
let totalImpact = 0;
for (const [wep, info] of sorted) {
  ...
  totalImpact += impact;
  console.log(
    `  ${wep}: IS=${info.isBV} Clan=${info.clanBV} delta=${info.clanBV - info.isBV} × ${info.count} = ${impact} total impact`,
  );
}
...
const sortedMissing = Object.entries(missingWeapons).sort(
  (a, b) => b[1] - a[1],
);
...
for (const [wep, count] of sortedMissing) {
  ...
  if (weaponBV[clanId] !== undefined) {
    console.log(
      `  ${wep} → clan-${wep} BV=${weaponBV[clanId]}, ${count} occurrences`,
    );
  }
}
...
for (const u of underc.slice(0, 20)) {
  ...
  for (const eq of unit.equipment) {
    ...
    if (bestBV > 0) {
      ...
      if (clanBV > isBV) {
        weaponDetails.push(
          `${lo}: IS=${isBV} Clan=${clanBV} → should be ${clanBV}`,
        );
      }
    }
  }
