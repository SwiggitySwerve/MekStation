...
for (const id of ids) {
  ...
  console.log(
    `${id} → norm:${norm} → IS: bv=${isRes.battleValue} heat=${isRes.heat} (${isRes.resolved ? 'OK' : 'MISS'}) → Clan(norm): bv=${clanRes.battleValue} (${clanRes.resolved ? 'OK' : 'MISS'}) → Clan(raw): bv=${clanRes2.battleValue} (${clanRes2.resolved ? 'OK' : 'MISS'})`,
  );
}
