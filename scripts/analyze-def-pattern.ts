...
console.log(
  `Analyzing ${defGapUnits.length} defensive-gap undercalculated units\n`,
);
...
for (const res of defGapUnits) {
  ...
  try {
    ...
    analyses.push({
      ...
      hasXL:
        d.engine.type.toUpperCase().includes('XL') &&
...
console.log(
  `Average gap/tonnage: ${(analyses.reduce((s, a) => s + a.baseDefGap / a.tonnage, 0) / analyses.length).toFixed(2)}`,
);
...
console.log(
  `Avg gap/structBV: ${(analyses.reduce((s, a) => s + a.baseDefGap / Math.max(1, a.structBV), 0) / analyses.length).toFixed(3)}`,
);
...
console.log('\n=== By Armor Type ===');
for (const [at, v] of Object.entries(byArmor).sort(
  (a, b) => b[1].count - a[1].count,
)) {
  console.log(
    `  ${at}: n=${v.count} avgGap=${(v.totalGap / v.count).toFixed(1)}`,
  );
}
...
for (const a of analyses) {
  if (!byEngine[a.engineType])
    byEngine[a.engineType] = { count: 0, totalGap: 0 };
  byEngine[a.engineType].count++;
  byEngine[a.engineType].totalGap += a.baseDefGap;
}
console.log('\n=== By Engine Type ===');
for (const [et, v] of Object.entries(byEngine).sort(
  (a, b) => b[1].count - a[1].count,
)) {
  console.log(
    `  ${et}: n=${v.count} avgGap=${(v.totalGap / v.count).toFixed(1)}`,
  );
}
...
console.log('\n=== By Tech Base ===');
for (const [tb, v] of Object.entries(byTech).sort(
  (a, b) => b[1].count - a[1].count,
)) {
  console.log(
    `  ${tb}: n=${v.count} avgGap=${(v.totalGap / v.count).toFixed(1)}`,
  );
}
...
console.log(`\n=== DefEquip presence ===`);
console.log(
  `  No defEquip: n=${noDefEquip.length} avgGap=${noDefEquip.length ? (noDefEquip.reduce((s, a) => s + a.baseDefGap, 0) / noDefEquip.length).toFixed(1) : 'N/A'}`,
);
console.log(
  `  Has defEquip: n=${hasDefEquip.length} avgGap=${hasDefEquip.length ? (hasDefEquip.reduce((s, a) => s + a.baseDefGap, 0) / hasDefEquip.length).toFixed(1) : 'N/A'}`,
);
...
console.log(`\n=== Explosive penalty ===`);
console.log(
  `  No penalty: n=${noExpl.length} avgGap=${noExpl.length ? (noExpl.reduce((s, a) => s + a.baseDefGap, 0) / noExpl.length).toFixed(1) : 'N/A'}`,
);
console.log(
  `  Has penalty: n=${hasExpl.length} avgGap=${hasExpl.length ? (hasExpl.reduce((s, a) => s + a.baseDefGap, 0) / hasExpl.length).toFixed(1) : 'N/A'} avgPenalty=${hasExpl.length ? (hasExpl.reduce((s, a) => s + a.explPen, 0) / hasExpl.length).toFixed(1) : 'N/A'}`,
);

// Show the 15 units with the largest gap RELATIVE to baseDef
console.log(`\n=== Top 15 units by gap/baseDef ratio ===`);
const byRatio = analyses
  .filter((a) => a.baseDefGap > 0)
  .sort((a, b) => {
...
for (const a of byRatio.slice(0, 15)) {
  ...
  console.log(
    `  ${a.unitId}: gap=${a.baseDefGap.toFixed(1)} baseDef=${baseDef.toFixed(1)} ratio=${ratio.toFixed(3)} (${a.techBase} ${a.engineType} armor=${a.armorType})`,
  );
}
