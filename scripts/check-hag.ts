...
for (const u of valid) {
  ...
  try {
        ...
        (slots: any) =>
          ...
          slots.some(
            (s: any) =>
              ...
              !s.toLowerCase().includes('hagel'),
          ),
      );
    if (!hasHAG) continue;
    const hagEquip = (unit.equipment || []).filter((eq: any) =>
      eq.id.toLowerCase().includes('hag'),
    );
    ...
    for (const eq of hagEquip) {
      ...
      console.log(
        `  equip: ${eq.id} → norm=${norm} bv=${res.battleValue} resolved=${res.resolved}`,
      );
    }
    ...
    for (const [loc, slots] of Object.entries(unit.criticalSlots || {})) {
      ...
      for (const s of slots as string[]) {
        if (
          ...
          console.log(
            `  crit: [${loc}] ${s} → bv=${res.battleValue} resolved=${res.resolved}`,
          );
