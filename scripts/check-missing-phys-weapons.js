      ...
      if (pattern.test(lo)) {
        ...
        if (!missingPhysWeapons[name].find((x) => x.id === u.unitId)) {
          missingPhysWeapons[name].push({
            ...
            pct: u.percentDiff,
            status: u.status,
            physBV: u.breakdown.physicalWeaponBV,
...
for (const [name, units] of Object.entries(missingPhysWeapons)) {
  ...
  const outsideThreshold = units.filter(
    (u) => u.status !== 'exact' && u.status !== 'within1',
  );
  ...
  for (const u of units) {
    ...
    console.log(
      `  ${u.id.padEnd(45)} diff=${String(u.diff).padStart(5)} (${(u.pct || 0).toFixed(2)}%) phys=${u.physBV} ton=${u.tonnage}${marker}`,
    );
  }
}
