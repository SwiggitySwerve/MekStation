    ...
    for (const cid of candidates) {
      ...
      if (cr.resolved && cr.battleValue > 0) {
        ...
        if (isResult.battleValue === cr.battleValue)
          return { ...cr, source: `clan(=is):${cid}` };
      }
    }
  }

  if (isResult.resolved && isResult.battleValue > 0)
    return { ...isResult, source: 'is' };
...
for (const eq of unit.equipment) {
  ...
  if (lo.includes('ammo')) {
    console.log(`  SKIP ammo: ${eq.id}`);
    continue;
  }
  if (lo.includes('heatsink') || lo.includes('heat-sink')) {
    console.log(`  SKIP HS: ${eq.id}`);
    continue;
  }
  ...
  console.log(
    `  ${eq.id} @ ${eq.location}: clan=${clanDetected} → bv=${res.battleValue} heat=${res.heat} df=${isDirectFire} source=${res.source}`,
  );
}
