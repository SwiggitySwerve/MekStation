...
function isDefEquip(id: string): boolean {
  ...
  return (
    ...
    lo.includes('electronic') ||
    lo.includes('ewequipment') ||
    lo.includes('bap') ||
...
const unresolvedDefEquip = new Map<
  string,
  { count: number; unitCount: number; units: string[] }
>();
...
const sorted = [...unresolvedDefEquip.entries()].sort(
  (a, b) => b[1].unitCount - a[1].unitCount,
);
for (const [name, info] of sorted) {
  console.log(
    `  ${name.padEnd(50)} slots=${String(info.count).padStart(4)} units=${String(info.unitCount).padStart(4)} ex: ${info.units.slice(0, 3).join(', ')}`,
  );
}
...
const sortedWeap = [...unresolvedWeapons.entries()].sort(
  (a, b) => b[1].count - a[1].count,
);
for (const [name, info] of sortedWeap) {
  console.log(
    `  ${name.padEnd(50)} count=${String(info.count).padStart(4)} ex: ${info.units.slice(0, 3).join(', ')}`,
  );
}

// Check what validate-bv.ts classifies as def equip from crits
console.log('\n=== ALL CRIT SLOT DEF EQUIP NAMES (resolved + unresolved) ===');
const allDefEquip = new Map<
  string,
  { count: number; resolved: boolean; bv: number }
>();
for (const u of valid.slice(0, 200)) {
  // sample first 200
  const unit = loadUnit(u.unitId);
  if (!unit || !unit.criticalSlots) continue;
  for (const [loc, slots] of Object.entries(unit.criticalSlots)) {
...
const sortedAll = [...allDefEquip.entries()].sort(
  (a, b) => b[1].count - a[1].count,
);
for (const [name, info] of sortedAll.slice(0, 40)) {
  const status = info.resolved ? `bv=${info.bv}` : 'UNRESOLVED';
  console.log(
    `  ${name.padEnd(50)} count=${String(info.count).padStart(4)} ${status}`,
  );
}
