...
if (archer) {
  ...
  console.log(
    `Gap:      ${archer.difference} (${archer.percentDiff.toFixed(2)}%)`,
  );
  ...
  console.log(`  AmmoBV:     ${archer.breakdown.ammoBV}`);
  console.log(
    `  Remainder:  ${(preSF - archer.breakdown.weaponBV - archer.breakdown.ammoBV).toFixed(2)} (should be weightBonus + physicalBV + offEquipBV)`,
  );
}
...
for (const u of simpleUnits) {
  ...
  console.log(
    `${u.unitId.padEnd(40)} idx=${u.indexBV} calc=${u.calculatedBV} gap=${u.difference} (${u.percentDiff.toFixed(2)}%) ton=${u.tonnage} SF=${sf} wBV=${u.breakdown.weaponBV} aBV=${u.breakdown.ammoBV} remain=${remainder.toFixed(1)}`,
  );
}
