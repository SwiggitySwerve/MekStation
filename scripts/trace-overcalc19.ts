...
for (const d of exactFive) {
  ...
  try {
    ...
    if (critsLo.some((s) => s.includes('c3master') || s.includes('c3 master')))
      hasC3Master++;
    if (
      critsLo.some(
        (s) =>
          s.includes('c3slave') || s.includes('c3 slave') || s.includes('c3s'),
      )
    )
      hasC3Slave++;
    ...
    if (
      critsLo.some(
        (s) =>
          s.includes('narc') && !s.includes('inarc') && !s.includes('ammo'),
      )
    )
      hasNarc++;
    if (critsLo.some((s) => s.includes('inarc') && !s.includes('ammo')))
      hasINarc++;
    ...
    if (
      critsLo.some(
        (s) =>
          ...
          s.includes('activeprobe') ||
          s.includes('lightactiveprobe'),
      )
    )
      hasBAP++;
    ...
    if (critsLo.some((s) => s.includes('tsm') || s.includes('triplestrength')))
      hasTSM++;
    if (
      critsLo.some(
        (s) =>
          s.includes('targeting computer') ||
          s.includes('istargeting') ||
          s.includes('cltargeting'),
      )
    )
      hasTC++;
    if (
      critsLo.some(
        (s) =>
          s.includes('anti-missile') ||
          s.includes('antimissile') ||
          s.includes('ams'),
      )
    )
      hasAMS++;
...
for (const d of exactFive.slice(0, 10)) {
  const neededDefFactor =
    (d.indexBV - d.breakdown.offensiveBV) / d.breakdown.defensiveBV;
  console.log(
    `  ${d.unitId}: defFactor=${neededDefFactor.toFixed(4)} (defBV=${d.breakdown.defensiveBV.toFixed(0)} offBV=${d.breakdown.offensiveBV.toFixed(0)})`,
  );
}
...
console.log('\nIf factor only on offBV:');
for (const d of exactFive.slice(0, 10)) {
  const neededOffFactor =
    (d.indexBV - d.breakdown.defensiveBV) / d.breakdown.offensiveBV;
  console.log(
    `  ${d.unitId}: offFactor=${neededOffFactor.toFixed(4)} (defBV=${d.breakdown.defensiveBV.toFixed(0)} offBV=${d.breakdown.offensiveBV.toFixed(0)})`,
  );
}
