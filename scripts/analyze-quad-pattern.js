...
console.log(
  `Within 1%: ${quads.filter((u) => Math.abs(u.percentDiff) <= 1).length}`,
);
...
for (const u of quads.filter((x) => Math.abs(x.percentDiff) > 1)) {
  ...
  console.log(
    `  ${u.chassis} ${u.model}: ${u.percentDiff.toFixed(2)}% ref=${u.indexBV} calc=${u.calculatedBV} gap=${u.difference} cockpit=${cockpit} tech=${b.techBase}`,
  );
}

// Check: what % of overcalculated units have high ammo BV?
console.log('\n=== Ammo BV analysis for overcalculated outside-1% ===');
const over = r.allResults
...
for (const u of over.slice(0, 20)) {
  const b = u.breakdown || {};
  const ammoPct =
    b.ammoBV > 0 && b.rawWeaponBV > 0
      ? ((b.ammoBV / b.rawWeaponBV) * 100).toFixed(0)
      : '0';
  const ammoInfo = b.ammoBV > 0 ? ` ammo=${b.ammoBV} (${ammoPct}% of wBV)` : '';
  if (b.ammoBV > b.rawWeaponBV * 0.3) highAmmoCnt++;
  console.log(
    `  ${u.chassis} ${u.model}: +${u.percentDiff.toFixed(2)}%${ammoInfo} wBV=${b.rawWeaponBV} tech=${b.techBase}`,
  );
}
console.log(
  `High ammo (>30% of weapon BV): ${highAmmoCnt}/${Math.min(20, over.length)}`,
);
