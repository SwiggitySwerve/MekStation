...
for (const [id, data] of unitMap) {
  ...
  const hasMag = data.equipment.some((e) =>
    e.id.toLowerCase().includes('magshot'),
  );
  if (!hasMag) continue;
...
for (const m of outside.sort((a, b) => b.u.percentDiff - a.u.percentDiff)) {
  ...
  const crits = Object.values(m.data.criticalSlots || {})
    .flat()
    .filter(Boolean);
  const flags = [];
  if (crits.some((s) => s.toLowerCase().includes('light machine gun')))
    flags.push('LMG');
  if (crits.some((s) => s.toLowerCase().includes('heavy machine gun')))
    flags.push('HMG');
  if (
    crits.some(
      (s) =>
        s.toLowerCase().includes('machine gun') &&
  ...
  console.log(
    `+${m.u.percentDiff.toFixed(2)}%  ${m.u.chassis} ${m.u.model} (diff=+${m.u.difference}) [${flags.join(',')}]`,
  );
...
console.log('\n--- Magshot BV check ---');
// In catalog: magshot = ?
const weaponFiles = fs
  .readdirSync('public/data/equipment/official/weapons')
  .filter((f) => f.endsWith('.json'));
for (const wf of weaponFiles) {
  const data = JSON.parse(
    fs.readFileSync('public/data/equipment/official/weapons/' + wf, 'utf8'),
  );
  if (data.items) {
    const mag = data.items.find(
      (i) => i.id && i.id.toLowerCase().includes('magshot'),
    );
    if (mag)
      console.log(
        'Catalog:',
