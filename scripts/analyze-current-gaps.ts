...
// Distribution
const under1to2 = outside1.filter(
  (x: any) => x.percentDiff < -1 && x.percentDiff >= -2,
);
const under2to5 = outside1.filter(
  (x: any) => x.percentDiff < -2 && x.percentDiff >= -5,
);
const under5plus = outside1.filter((x: any) => x.percentDiff < -5);
const over1to2 = outside1.filter(
  (x: any) => x.percentDiff > 1 && x.percentDiff <= 2,
);
const over2to5 = outside1.filter(
  (x: any) => x.percentDiff > 2 && x.percentDiff <= 5,
);
const over5plus = outside1.filter((x: any) => x.percentDiff > 5);

console.log(
  `Outside 1%: ${outside1.length} (need ~${outside1.length - 148} fixed for 95%)`,
);
...
for (const u of over1to2) {
      ...
      (slots: any) =>
        ...
        slots.some(
          (s: any) =>
            s &&
            typeof s === 'string' &&
            s.toLowerCase().includes('supercharger'),
        ),
    );
  ...
  if (unit.armor?.type && unit.armor.type.toLowerCase() !== 'standard')
    tags.push('special-armor:' + unit.armor.type);
  if (unit.structure?.type && unit.structure.type.toLowerCase() !== 'standard')
    tags.push('special-struct:' + unit.structure.type);
...
for (const [k, v] of Object.entries(underPatterns).sort(
  (a, b) => b[1] - a[1],
)) {
  console.log(`  ${k}: ${v}`);
}
...
for (const [k, v] of Object.entries(unresolvedCounts)
  .sort((a, b) => b[1] - a[1])
  .slice(0, 20)) {
...
for (const [k, v] of Object.entries(unresolvedBig)
  .sort((a, b) => b[1] - a[1])
  .slice(0, 20)) {
  console.log(`  ${k}: ${v} units`);
}
...
for (const u of over2to5.slice(0, 15)) {
  ...
  console.log(
    `  ${u.unitId.padEnd(45)} ${u.percentDiff > 0 ? '+' : ''}${u.percentDiff.toFixed(1)}%  diff=${u.difference}  tsm=${hasTSM ? 'Y' : 'N'}  smallCk=${hasSmallCockpit ? 'Y' : 'N'}  eng=${unit.engine?.type || '?'}`,
  );
}
