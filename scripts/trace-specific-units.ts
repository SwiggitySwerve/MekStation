...
function isClanEquipAtLocation(
  equipId: string,
  location: string,
  critSlots?: Record<string, (string | null)[]>,
): boolean {
...
function resolveWeaponForUnit(
  ...
  techBase: string,
  isClanEquip?: boolean,
): { battleValue: number; heat: number; resolved: boolean; source: string } {
    ...
    for (const cid of candidates) {
      ...
      if (cr.resolved && cr.battleValue > 0) {
        if (!isResult.resolved || cr.battleValue > isResult.battleValue)
          return { ...cr, source: `clan(${cid})` };
        if (isResult.battleValue === cr.battleValue)
          return { ...cr, source: `clan-same(${cid})` };
      }
    }
  }

  if (isResult.resolved && isResult.battleValue > 0)
    return { ...isResult, source: 'direct' };
  ...
  if (
    ...
    for (const cid of clanCandidates) {
      const cr = resolveEquipmentBV(cid);
      if (cr.resolved && cr.battleValue > 0)
        return { ...cr, source: `mixed-fallback(${cid})` };
    }
  }
...
for (const uid of targetUnits) {
  ...
  console.log(
    `  Reference: ${result.indexBV}  Calculated: ${result.calculatedBV}  Gap: ${result.difference} (${result.percentDiff?.toFixed(1)}%)`,
  );
  console.log(
    `  DefBV: ${b?.defensiveBV?.toFixed(0)}  OffBV: ${b?.offensiveBV?.toFixed(0)}  SF: ${b?.speedFactor}  CockpitMod: ${b?.cockpitModifier}`,
  );
  console.log(
    `  WeaponBV: ${b?.weaponBV?.toFixed(0)} (raw=${b?.rawWeaponBV?.toFixed(0)}, halved=${b?.halvedWeaponBV?.toFixed(0)}, ${b?.halvedWeaponCount}/${b?.weaponCount} halved)`,
  );
  console.log(
    `  AmmoBV: ${b?.ammoBV}  WeightBonus: ${b?.weightBonus?.toFixed(0)}  PhysBV: ${b?.physicalWeaponBV?.toFixed(0)}  OffEqBV: ${b?.offEquipBV}`,
  );
  console.log(
  ...
  for (const eq of unit.equipment) {
    ...
    const clanDetected =
      unit.techBase === 'MIXED' &&
      isClanEquipAtLocation(eq.id, eq.location, unit.criticalSlots);
    ...
    console.log(
      `    ${eq.id} @${eq.location} x${qty} → bv=${res.battleValue} heat=${res.heat} [${res.source}]${marker}`,
    );
  }
  console.log(
    `  Total from equip list: BV=${totalEquipBV} Heat=${totalEquipHeat}`,
  );
  ...
  if (unit.criticalSlots) {
    ...
    for (const [loc, slots] of Object.entries(unit.criticalSlots)) {
      ...
      for (const s of slots) {
        ...
        // Skip non-weapons
        if (
          slo.includes('heat sink') ||
          slo.includes('engine') ||
          slo.includes('gyro') ||
          slo.includes('cockpit') ||
          slo.includes('life support') ||
          slo.includes('sensor') ||
          slo.includes('shoulder') ||
          slo.includes('actuator') ||
          slo.includes('endo') ||
          slo.includes('ferro') ||
          slo.includes('case') ||
          slo.includes('ammo') ||
          slo.includes('jump jet') ||
          slo.includes('targeting') ||
          slo.includes('masc') ||
          slo.includes('supercharger') ||
          slo.includes('tsm') ||
          slo.includes('ecm') ||
          slo.includes('probe') ||
          slo.includes('shield') ||
          slo.includes('null sig') ||
          slo.includes('void sig') ||
          slo.includes('chameleon') ||
          slo.includes('stealth') ||
          slo.includes('artemis') ||
          slo.includes('apollo') ||
          slo.includes('coolant') ||
          slo.includes('partial wing') ||
          slo.includes('pod') ||
          slo.includes('lift hoist') ||
          slo.includes('armor') ||
          slo.includes('structure') ||
          slo.includes('command console') ||
          slo.includes('aes') ||
          slo.includes('c3') ||
          slo.includes('narc') ||
          slo.includes('tag') ||
          slo.includes('(armored)') ||
          slo.includes('(r)')
        )
          continue;
