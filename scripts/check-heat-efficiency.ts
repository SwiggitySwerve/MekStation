...
for (const r of report.allResults) {
  ...
  try {
    ...
    const isDHS =
      u.heatSinks?.type?.toUpperCase()?.includes('DOUBLE') ||
...
for (const [type, data] of Object.entries(byHSType).sort()) {
  ...
  console.log(
    `  ${type.padEnd(15)} n=${String(data.count).padStart(5)} avgPct=${avg.toFixed(2).padStart(7)}% within1%=${String(within1).padStart(5)} (${((within1 / data.count) * 100).toFixed(1)}%)`,
  );
}
...
for (const bin of heBins) {
  const pool = heatInfos.filter(
    (h) => h.heatEfficiency >= bin.min && h.heatEfficiency < bin.max,
  );
  if (pool.length > 0) {
    const avgGap = pool.reduce((s, h) => s + h.pctDiff, 0) / pool.length;
    const within1 = pool.filter((h) => Math.abs(h.pctDiff) <= 1).length;
    console.log(
      `  ${bin.label.padEnd(12)} n=${String(pool.length).padStart(5)} avgPct=${avgGap.toFixed(2).padStart(7)}% within1%=${String(within1).padStart(5)} (${((within1 / pool.length) * 100).toFixed(1)}%)`,
    );
  }
}
...
const shsMinorDisc = heatInfos.filter(
  (h) =>
    !h.hsType.toUpperCase().includes('DOUBLE') &&
    !h.hsType.toUpperCase().includes('LASER') &&
    !h.hsType.toUpperCase().includes('COMPACT') &&
    Math.abs(h.pctDiff) > 1 &&
    Math.abs(h.pctDiff) <= 5,
);
console.log(`Count: ${shsMinorDisc.length}`);
for (const h of shsMinorDisc.slice(0, 15)) {
  console.log(
    `  ${h.id.padEnd(45)} gap=${String(h.gap).padStart(5)} (${h.pctDiff.toFixed(2).padStart(6)}%) hsCount=${h.hsCount} HE=${h.heatEfficiency} wBV=${h.weaponBV}`,
  );
}

// Check Compact HS
console.log('\n=== COMPACT HEAT SINK UNITS ===');
const compactHS = heatInfos.filter((h) =>
  h.hsType.toUpperCase().includes('COMPACT'),
);
console.log(`Count: ${compactHS.length}`);
for (const h of compactHS.slice(0, 10)) {
  console.log(
    `  ${h.id.padEnd(45)} gap=${String(h.gap).padStart(5)} (${h.pctDiff.toFixed(2).padStart(6)}%) hsCount=${h.hsCount} HE=${h.heatEfficiency}`,
  );
}
