...
console.log(
  `Outside 1%: ${outside1.length} (under: ${under.length}, over: ${over.length})\n`,
);
...
for (const u of outside1) {
  ...
  if (
    ...
    addFeature('Quad', u);
  if (
    crits.includes('oneshot') ||
    crits.includes('one-shot') ||
    crits.includes('(os)')
  )
    addFeature('OneShot', u);

  // Check for unresolved weapons
  if ((u.breakdown?.issues || []).some((i) => i.includes('unresolved')))
    addFeature('Unresolved', u);
}
...
const sorted = Object.entries(features).sort(
  (a, b) =>
    b[1].under.length +
    b[1].over.length -
    (a[1].under.length + a[1].over.length),
);
for (const [name, data] of sorted) {
  const total = data.under.length + data.over.length;
  const pct = ((total / outside1.length) * 100).toFixed(0);
  console.log(
    `  ${name}: ${data.under.length} under / ${data.over.length} over / ${total} total (${pct}%)`,
  );
}
...
const avgOverTC =
  overTC.length > 0
    ? overTC.reduce((s, u) => s + u.percentDiff, 0) / overTC.length
    : 0;
const avgOverNoTC =
  overNoTC.length > 0
    ? overNoTC.reduce((s, u) => s + u.percentDiff, 0) / overNoTC.length
    : 0;
console.log(
  `\nOvercalc with TC: ${overTC.length} (avg +${avgOverTC.toFixed(2)}%)`,
);
console.log(
  `Overcalc no TC: ${overNoTC.length} (avg +${avgOverNoTC.toFixed(2)}%)`,
);
...
for (const u of interfaceUnits) {
  console.log(
    `  ${u.chassis} ${u.model}: ${u.percentDiff.toFixed(2)}% ref=${u.indexBV} calc=${u.calculatedBV}`,
  );
}
console.log(
  `Avg: ${(interfaceUnits.reduce((s, u) => s + u.percentDiff, 0) / interfaceUnits.length).toFixed(2)}%`,
);
