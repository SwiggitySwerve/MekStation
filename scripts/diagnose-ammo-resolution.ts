...
for (const a of missileAmmo) {
  console.log(
    `  ${a.id.padEnd(35)} BV=${String(a.battleValue).padStart(3)} shots=${String(a.shotsPerTon).padStart(4)} compatible=[${(a.compatibleWeaponIds || []).join(', ')}]`,
  );
}
...
const underCalc = allResults.filter(
  (r: any) => r.difference < 0 && r.percentDiff < -1,
);
...
for (const r of allResults) {
  if (r.breakdown?.ammoBV > 0) {
    unitsWithAmmo++;
    totalAmmoBV += r.breakdown.ammoBV;
  }
...
console.log(
  `Average ammoBV (where > 0): ${(totalAmmoBV / unitsWithAmmo).toFixed(1)}`,
);
...
const bins: Record<
  string,
  { count: number; totalGap: number; totalPct: number }
> = {
...
for (const [bin, data] of Object.entries(bins)) {
  if (data.count > 0) {
    console.log(
      `  ${bin.padEnd(18)} n=${String(data.count).padStart(4)} avgGap=${(data.totalGap / data.count).toFixed(1).padStart(7)} avgPct=${(data.totalPct / data.count).toFixed(2).padStart(7)}%`,
    );
  }
}
...
for (const r of sampleUnits) {
  ...
  try {
    ...
    console.log(
      `  WeaponBV: ${r.breakdown?.weaponBV}  DefBV: ${r.breakdown?.defensiveBV}`,
    );
