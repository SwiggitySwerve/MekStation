...
console.log(
  '=== FINDING 1: Speed Factor Rounding Comparison (4dp vs 2dp) ===\n',
);
...
for (let mp = 0; mp <= 20; mp++) {
  ...
  if (diff !== 0) diffCount++;
  console.log(
    `${String(mp).padStart(3)} | ${ours.toFixed(4).padStart(10)} | ${mm.toFixed(2).padStart(10)} | ${diffStr.padStart(9)} | ${raw.toFixed(8)}`,
  );
}
...
const systematicUnder = report.allResults.filter(
  (u: any) => u.percentDiff < -1.5 && u.percentDiff > -5,
);
console.log(
  `Units in -1.5% to -5% undercalculation range: ${systematicUnder.length}`,
);
...
console.log(
  `Units where SF rounding differs: ${sfDiffImpactCount} / ${analyzedCount}`,
);
console.log(`Total BV impact from SF rounding: ${totalSFBVImpact.toFixed(1)}`);
console.log(
  `Average BV impact per affected unit: ${sfDiffImpactCount > 0 ? (totalSFBVImpact / sfDiffImpactCount).toFixed(1) : 'N/A'}`,
);

console.log('\n--- Top 20 by SF BV Impact ---\n');
console.log(
  'Unit ID                          | Ton | W | Run | J | MP | OurSF   | MM SF | SFDiff  | OffBV   | SF Impact | TotalDiff | %Diff',
);
console.log(
  '---------------------------------|-----|---|-----|---|----|---------|-------|---------|---------|-----------|-----------|------',
);
...
console.log(
  `Percentage of gap explained by SF: ${totalUndercalcBV !== 0 ? ((totalSFBVImpact / totalUndercalcBV) * 100).toFixed(1) : 'N/A'}%`,
);
...
console.log(
  `Speed factor values that ARE 2dp-compatible: ${count2dp} (these happen to round the same)`,
);
...
console.log(
  'Note: The DEFENSIVE factor uses TMM (1 + TMM/10) = always clean 1-decimal values.',
);
console.log(
  'Only the OFFENSIVE speed factor uses the pow() formula that creates rounding differences.',
);
...
for (const [mp, count] of sortedMPs.slice(0, 15)) {
  ...
  console.log(
    `${String(mp).padStart(2)} | ${String(count).padStart(5)} | ${ourSF.toFixed(4)} | ${mmSF.toFixed(2).padStart(5)} | ${diff.toFixed(4)}`,
  );
}
