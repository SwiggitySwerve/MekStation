...
const units = r.allResults.filter(
  (u) => Math.abs(u.percentDiff) > 1 && Math.abs(u.percentDiff) <= 5,
);
...
console.log(
  ...
  over.length,
  'avg',
  (over.reduce((s, u) => s + u.percentDiff, 0) / over.length).toFixed(1) + '%',
);
console.log(
  '  Undercalculated:',
  under.length,
  'avg',
  (under.reduce((s, u) => s + u.percentDiff, 0) / under.length).toFixed(1) +
    '%',
);
...
const halvUnder = under.filter(
  (u) => (u.breakdown || {}).halvedWeaponCount > 0,
);
console.log(
  'Halved weapons: over=' + halvOver.length + ', under=' + halvUnder.length,
);
...
console.log(
  'ammoBV=0: over=' + ammo0Over.length + ', under=' + ammo0Under.length,
);

// Check tech base
const clanOver = over.filter((u) => (u.breakdown || {}).techBase === 'CLAN');
const clanUnder = under.filter((u) => (u.breakdown || {}).techBase === 'CLAN');
const isOver = over.filter(
  (u) => (u.breakdown || {}).techBase === 'INNER_SPHERE',
);
const isUnder = under.filter(
  (u) => (u.breakdown || {}).techBase === 'INNER_SPHERE',
);
...
const cockpitNon1Over = over.filter(
  (u) =>
    (u.breakdown || {}).cockpitModifier &&
    (u.breakdown || {}).cockpitModifier !== 1,
);
const cockpitNon1Under = under.filter(
  (u) =>
    (u.breakdown || {}).cockpitModifier &&
    (u.breakdown || {}).cockpitModifier !== 1,
);
console.log(
  'Non-standard cockpit: over=' +
    cockpitNon1Over.length +
    ', under=' +
    cockpitNon1Under.length,
);

// Check unresolved weapons
const unresolvedOver = over.filter(
  (u) =>
    (u.breakdown || {}).unresolvedWeapons &&
    (u.breakdown || {}).unresolvedWeapons.length > 0,
);
const unresolvedUnder = under.filter(
  (u) =>
    (u.breakdown || {}).unresolvedWeapons &&
    (u.breakdown || {}).unresolvedWeapons.length > 0,
);
console.log(
  'Unresolved weapons: over=' +
    unresolvedOver.length +
    ', under=' +
    unresolvedUnder.length,
);
  ...
  for (const u of unresolvedUnder.slice(0, 10)) {
    console.log(
      '    ' +
        ...
        '%): ' +
        (u.breakdown || {}).unresolvedWeapons.join(', '),
    );
  }
}
...
console.log(
  'Has issues: over=' + issueOver.length + ', under=' + issueUnder.length,
);
...
console.log(
  ...
  absGaps[0],
  'max:',
  absGaps[absGaps.length - 1],
  'median:',
  absGaps[Math.floor(absGaps.length / 2)],
);

// Find units very close to 1% that we might be able to fix
const nearBorder = units
  .filter((u) => Math.abs(u.percentDiff) <= 1.5)
