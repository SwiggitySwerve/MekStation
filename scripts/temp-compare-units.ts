...
const iu = indexData.units.find(
  (u: any) => `${u.chassis} ${u.model}` === unitName,
);
if (!iu) {
...
if (!result) {
  console.log(`No result for ${unitName}`);
  process.exit(1);
}
...
console.log(
  `Index BV: ${iu.bv}, Calculated: ${result.calculatedBV}, Diff: ${result.difference} (${result.percentDiff?.toFixed(1)}%)`,
);
console.log(
  `Breakdown: def=${result.breakdown?.defensiveBV} off=${result.breakdown?.offensiveBV}`,
);
console.log(
  `  weaponBV=${result.breakdown?.weaponBV} ammoBV=${result.breakdown?.ammoBV} sf=${result.breakdown?.speedFactor} expl=${result.breakdown?.explosivePenalty} defEq=${result.breakdown?.defensiveEquipBV}`,
);
...
for (const eq of ud.equipment || []) {
  ...
  console.log(
    `  ${eq.id.padEnd(30)} → norm=${norm.padEnd(25)} bv=${res.battleValue} heat=${res.heat} resolved=${res.resolved}`,
  );
  ...
  if (ud.techBase === 'CLAN' || ud.techBase === 'MIXED') {
    ...
    if (clanRes.resolved && clanRes.battleValue !== res.battleValue) {
      console.log(
        `    → clan: ${clanId.padEnd(25)} bv=${clanRes.battleValue} heat=${clanRes.heat}`,
      );
    }
  }
}
...
if (ud.criticalSlots) {
  for (const [loc, slots] of Object.entries(ud.criticalSlots)) {
    for (const s of slots as any[]) {
      ...
      console.log(
        `  ${s.padEnd(40)} @ ${loc.padEnd(12)} bv=${res.battleValue} resolved=${res.resolved}`,
      );
    }
  }
}
