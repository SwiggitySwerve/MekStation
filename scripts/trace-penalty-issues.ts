  ...
  try {
    ...
    for (const [loc, ammos] of Object.entries(ammoByLoc)) {
      const hasCase =
        caseByLoc[loc] ||
        (d.techBase === 'CLAN' &&
          (loc.includes('TORSO') || loc.includes('LT') || loc.includes('RT')));
      if (!hasCase) {
...
const minorUnder = r.allResults.filter(
  (x: any) => x.percentDiff < -1 && x.percentDiff > -5 && x.breakdown,
);
  ...
  try {
    ...
    for (const [loc, slots] of Object.entries(d.criticalSlots || {})) {
      ...
      for (const s of slots) {
        if (!s || typeof s !== 'string') continue;
        const lo = s.toLowerCase();
        if (lo.includes('case ii') || lo.includes('caseii'))
          caseByLoc[loc] = 'CASE_II';
        else if (lo.includes('case') && !caseByLoc[loc])
          caseByLoc[loc] = 'CASE';
      }
    }
    ...
    console.log(
      `  gap=${gap.toFixed(1)} penalty=${b.explosivePenalty} (gap-penalty=${(gap - b.explosivePenalty).toFixed(1)})`,
    );
