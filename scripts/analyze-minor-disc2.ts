...
const minor = report.allResults.filter(
  (r: any) =>
    r.percentDiff !== null &&
...
for (const bd of breakdownDiffs.slice(0, 20)) {
  console.log(
    `${bd.unit.padEnd(35)} ${bd.techBase.padEnd(6)} diff=${bd.diff} (${bd.percentDiff.toFixed(1)}%) weapBV=${bd.weaponBV} ammoBV=${bd.ammoBV} defEquip=${bd.defEquipBV} explPen=${bd.explosivePen} offBV=${bd.offBV.toFixed(0)} defBV=${bd.defBV.toFixed(0)} sf=${bd.sf.toFixed(3)}`,
  );
