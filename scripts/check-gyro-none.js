...
for (const u of noneGyro) {
  ...
  console.log(
    `  OFF: wBV=${b.rawWeaponBV} halved=${b.halvedWeaponBV} ammo=${b.ammoBV} wt=${b.weightBonus} sf=${b.speedFactor}`,
  );
  console.log(
    `  tonnage=${data.tonnage} engine=${data.engine?.type} structure=${data.structure?.type} cockpit=${data.cockpit}`,
  );
  ...
  if (b.gyroBV > 0) {
    ...
    console.log(
      `  If gyroBV=0: newCalc=${newTotal} newDiff=${newPct.toFixed(2)}% ${Math.abs(newPct) <= 1 ? '[WITHIN 1%]' : ''}`,
    );
  }
  console.log('');
}
...
for (const u of compStruct.filter((x) => Math.abs(x.percentDiff) > 1)) {
  ...
  console.log(
    `  DEF: struct=${b.structureBV} armor=${b.armorBV} gyro=${b.gyroBV} df=${b.defensiveFactor}`,
  );
