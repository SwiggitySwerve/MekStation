...
console.log(
  `=== ${boundary.length} undercalculated units between 1-2% off ===\n`,
);

for (const u of boundary.slice(0, 25)) {
  const entry = unitIdx.units.find((x) => x.id === u.unitId);
  if (!entry) {
    console.log(u.unitId + ': NOT FOUND IN INDEX');
    continue;
  }
  ...
  console.log(
    `  calc=${u.calculatedBV} exp=${u.indexBV} diff=${u.difference} (${u.percentDiff.toFixed(2)}%)`,
  );
  console.log(
    `  tonnage=${unit.tonnage} tech=${unit.techBase} engine=${unit.engine?.type}`,
  );
  console.log(
    `  walk=${unit.movement?.walk} run=${unit.movement?.run} jump=${unit.movement?.jump || 0}`,
  );
  ...
  if (b) {
    ...
    console.log(
      `  defBV=${b.defensiveBV?.toFixed(1)} offBV=${b.offensiveBV?.toFixed(1)} cockpitMod=${b.cockpitModifier}`,
    );
    console.log(
      `  weaponBV=${b.weaponBV?.toFixed(1)} rawWeaponBV=${b.rawWeaponBV?.toFixed(1)} halvedBV=${b.halvedWeaponBV?.toFixed(1)}`,
    );
    console.log(
      `  ammoBV=${b.ammoBV?.toFixed(1)} weightBonus=${b.weightBonus} physBV=${b.physicalWeaponBV}`,
    );
    console.log(
      `  heatEff=${b.heatEfficiency} heatDiss=${b.heatDissipation} moveHeat=${b.moveHeat}`,
    );
    console.log(
      `  speedFactor=${b.speedFactor} defFactor=${b.defensiveFactor}`,
    );
    ...
    if (b.speedFactor && b.cockpitModifier) {
      ...
      console.log(
        `  → needed weaponBV=${neededWeapon.toFixed(1)} (gap=${weaponGap.toFixed(1)})`,
      );
    }
  }
  ...
  if (unit.equipment) {
    ...
    if (ammo.length)
      console.log(`  ammo: ${ammo.map((a) => a.name || a.id).join(', ')}`);
  }
