...
for (const id of targets) {
  ...
  console.log(`\n${'='.repeat(70)}`);
  console.log(
    `${id}: ${d.tonnage}t ${d.techBase} ${d.engine.type} cockpit=${d.cockpit || 'STANDARD'}`,
  );
  console.log(
    `  idx=${res.indexBV} calc=${res.calculatedBV} diff=${res.difference} (${res.percentDiff.toFixed(1)}%)`,
  );
  console.log(`\nBreakdown:`);
  console.log(
  ...
  console.log(
    `  defEquipBV=${b.defensiveEquipBV?.toFixed(1)} explPen=${b.explosivePenalty}`,
  );
  console.log(`  defFactor=${b.defensiveFactor?.toFixed(3)}`);
  console.log(`  defensiveBV=${b.defensiveBV?.toFixed(1)}`);
  console.log(
    `  offensiveBV=${b.offensiveBV?.toFixed(1)} (weaponBV=${b.weaponBV?.toFixed(1)} ammoBV=${b.ammoBV?.toFixed(1)} speed=${b.speedFactor?.toFixed(3)})`,
  );
  ...
  console.log(
    `  defensiveBV recalc = baseDef * defFactor = ${baseDef.toFixed(1)} * ${b.defensiveFactor?.toFixed(3)} = ${(baseDef * (b.defensiveFactor || 1)).toFixed(1)}`,
  );
  ...
  console.log(`\nReverse engineering:`);
  console.log(
    `  cockpitMod=${cockpitMod} neededBaseBV=${neededBaseBV.toFixed(1)}`,
  );
  console.log(
    `  neededDefBV = ${neededBaseBV.toFixed(1)} - ${b.offensiveBV.toFixed(1)} = ${neededDefBV.toFixed(1)}`,
  );
  console.log(
    `  neededBaseDef = ${neededDefBV.toFixed(1)} / ${(b.defensiveFactor || 1).toFixed(3)} = ${neededBaseDef.toFixed(1)}`,
  );
  console.log(
    `  GAP = neededBaseDef - baseDef = ${neededBaseDef.toFixed(1)} - ${baseDef.toFixed(1)} = ${(neededBaseDef - baseDef).toFixed(1)}`,
  );
  ...
  console.log(
    `  Armor BV per point: ${(b.armorBV || 0) / Math.max(1, totalArmor)}`,
  );

  // Structure details
  console.log(`\nStructure: type=${d.structure?.type || 'STANDARD'}`);
  console.log(
    `  Movement: walk=${d.movement?.walk} jump=${d.movement?.jump || 0}`,
  );
