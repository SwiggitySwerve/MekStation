...
if (!u) {
  ...
  const partial = idx.units
    .filter((u: any) => `${u.chassis} ${u.model}`.includes(target))
    .slice(0, 5);
  for (const p of partial) console.log('  Did you mean:', p.chassis, p.model);
  process.exit(1);
}
const ud = JSON.parse(
  fs.readFileSync(path.join('public/data/units/battlemechs', u.path), 'utf-8'),
);
console.log(`\n=== ${u.chassis} ${u.model} ===`);
...
if (fs.existsSync(mulPath)) {
  ...
  if (mulEntry)
    console.log('MUL BV:', mulEntry.bv, '(match:', mulEntry.matchType + ')');
}
...
for (const eq of ud.equipment || []) {
  ...
  console.log(
    `  "${eq.id}" @ ${eq.location} -> norm="${norm}" BV=${res.battleValue} Heat=${res.heat} Resolved=${res.resolved}`,
  );
}
...
if (allSlotsLo.some((s) => s.includes('ferro-lamellor')))
  detectedArmor = 'ferro-lamellor';
else if (
  allSlotsLo.some(
    (s) =>
      s.includes('ballistic-reinforced') || s.includes('ballistic reinforced'),
  )
)
  detectedArmor = 'ballistic-reinforced';
else if (allSlotsLo.some((s) => s.includes('reactive') && !s.includes('ferro')))
  detectedArmor = 'reactive';
else if (
  allSlotsLo.some(
    (s) =>
