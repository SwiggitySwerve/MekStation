...
for (const r of underc.filter((r: any) => r.weapBV < 100).slice(0, 15)) {
  ...
  console.log(
    `  Equipment: ${ud.equipment.map((e: any) => `${e.id}@${e.location}`).join(', ')}`,
  );
  ...
  for (const eq of ud.equipment) {
    ...
    if (!res.resolved || res.battleValue === 0) {
      ...
      console.log(
        `  UNRESOLVED: ${eq.id} -> norm=${normalizeEquipmentId(eq.id)} (clan attempt: bv=${clanRes.battleValue})`,
      );
    }
  }
  ...
  if (ud.criticalSlots) {
    ...
    if (weapSlots.length > 0)
      console.log(`  Crit weapons: ${weapSlots.join('; ')}`);
  }
}
...
for (const r of ryokenIII) {
  ...
  console.log(
    `  weapBV=${r.weapBV} ammoBV=${r.ammoBV} sf=${r.sf} offBV=${r.offBV} defBV=${r.defBV}`,
  );
  ...
  for (const eq of ud.equipment) {
    ...
    if (!res.resolved || res.battleValue === 0) {
      console.log(
        `  UNRESOLVED: ${eq.id} (norm: ${normalizeEquipmentId(eq.id)})`,
      );
    }
  }
}
