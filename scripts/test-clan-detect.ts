...
for (const unitId of testUnits) {
  ...
  for (const eq of data.equipment) {
    const detected = isClanEquipAtLocation(
      eq.id,
      eq.location,
      data.criticalSlots,
    );
    console.log(
      `  ${eq.id.padEnd(30)} @${eq.location.padEnd(15)} clanDetected=${detected}`,
    );
    ...
    if (!detected && data.criticalSlots) {
      ...
      if (slots) {
        ...
        const clSlots = (slots as any[]).filter(
          (s: any) =>
            s &&
            typeof s === 'string' &&
            /^CL/i.test(s.replace(/\s*\(omnipod\)/gi, '').trim()),
        );
        if (clSlots.length > 0) {
          ...
          for (const s of clSlots) {
            ...
            const slotNorm = clean
              ...
              .replace(/[^a-z0-9]/g, '');
            console.log(
              `      "${s}" → clean="${clean}" → norm="${slotNorm}" match=${slotNorm === eqNorm || slotNorm.includes(eqNorm) || eqNorm.includes(slotNorm)}`,
            );
          }
        }
      }
