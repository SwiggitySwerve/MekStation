...
for (const w of wrong) {
  ...
  try {
    ...
    const hasTSM = (
      d.criticalSlots
        ? (Object.values(d.criticalSlots).flat().filter(Boolean) as string[])
        : []
    ).some(
      (s) =>
    ...
    const hasShield = d.equipment.some((e: any) =>
      e.id.toLowerCase().includes('shield'),
    );

    // Count unresolved weapons
    const issues = w.issues || [];
    const unresolvedWeapons = issues.filter((i: string) =>
      i.includes('Unresolved weapons'),
    );
    ...
    analysis.push({
      ...
      pen: b.explosivePenalty,
      hasTSM,
      hasShield,
      unresolvedWeapons: unresolvedWeapons.length > 0,
...
console.log(
  `  Over: ${tsmUnits.filter((a) => a.pct > 0).length} avg +${(tsmUnits.filter((a) => a.pct > 0).reduce((s: number, a: any) => s + a.gap, 0) / Math.max(1, tsmUnits.filter((a) => a.pct > 0).length)).toFixed(0)}`,
);
...
const byDefF: Record<
  string,
  { over: number; under: number; avgGap: number; count: number }
> = {};
...
for (const [df, stats] of Object.entries(byDefF).sort(
  (a, b) => parseFloat(a[0]) - parseFloat(b[0]),
)) {
  console.log(
    `  defF=${df}: ${stats.over} over, ${stats.under} under, avg gap=${(stats.avgGap / stats.count).toFixed(0)}`,
  );
}
...
for (const [sf, stats] of Object.entries(bySF)) {
  console.log(
    `  sf ${sf}: ${stats.over} over, ${stats.under} under (${stats.count} total)`,
  );
}
...
for (const a of analysis
  ...
  const flags = [
    a.hasTSM ? 'TSM' : '',
    a.hasShield ? 'SHIELD' : '',
    a.unresolvedWeapons ? 'UNRESOLVED' : '',
  ]
    .filter(Boolean)
    .join(',');
  console.log(
    `  ${a.id}: +${a.gap} (+${a.pct.toFixed(1)}%) ${a.techBase} ${a.engineType} sf=${a.speedFactor} defF=${a.defFactor} ${flags}`,
  );
}
...
for (const a of analysis
  ...
  const flags = [
    ...
    .join(',');
  console.log(
    `  ${a.id}: ${a.gap} (${a.pct.toFixed(1)}%) ${a.techBase} ${a.armorType} ${flags}`,
  );
}

// Check if unresolved weapons is a major driver
console.log('\n=== UNRESOLVED WEAPON UNITS ===');
for (const u of withUnresolved
  .sort((a: any, b: any) => Math.abs(b.pct) - Math.abs(a.pct))
  .slice(0, 15)) {
  const res = r.allResults.find((x: any) => x.unitId === u.id);
  const issues =
    res?.issues?.filter((i: string) => i.includes('Unresolved')) || [];
  console.log(
    `  ${u.id}: ${u.gap > 0 ? '+' : ''}${u.gap} (${u.pct.toFixed(1)}%) — ${issues.join('; ')}`,
  );
}
