...
const under = r.allResults.filter(
  (x: any) => x.percentDiff < -1 && x.breakdown,
);
...
console.log(
  `  Overcalculated (>1%): ${overNamed.named}/${overNamed.total} (${((overNamed.named / overNamed.total) * 100).toFixed(1)}%)`,
);
console.log(
  `  Undercalculated (<-1%): ${underNamed.named}/${underNamed.total} (${((underNamed.named / underNamed.total) * 100).toFixed(1)}%)`,
);
console.log(
  `  Correct (<=1%): ${correctNamed.named}/${correctNamed.total} (${((correctNamed.named / correctNamed.total) * 100).toFixed(1)}%)`,
);
...
for (const res of over) {
  ...
  if (entry && hasNamedPilot(entry.model)) {
    console.log(
      `  ${entry.chassis} ${entry.model}: diff=+${res.difference} (${res.percentDiff.toFixed(1)}%)`,
    );
  }
}
...
for (const res of over.slice(0, 20)) {
  ...
  console.log(
    `  ${chassis} ${model}: diff=+${res.difference} (${res.percentDiff.toFixed(1)}%) [${correctCount}/${allVariants.length} variants within 1%]`,
  );
}
