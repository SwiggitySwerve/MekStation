...
for (const u of outside.slice(0, 30)) {
  ...
  const crits = Object.values(data.criticalSlots || {})
    ...
    .filter(Boolean);
  const uniqueCrits = [
    ...new Set(crits.map((s) => s.replace(/\s*\(omnipod\)/gi, ''))),
  ];
  ...
  console.log(
    `  tech=${data.techBase} tonnage=${data.tonnage} engine=${data.engine?.type}/${data.engine?.rating}`,
  );
  console.log(
    `  gyro=${data.gyro?.type || 'STANDARD'} cockpit=${data.cockpit || 'STANDARD'}`,
  );
  console.log(`  walk=${b.walkMP} run=${b.runMP} jump=${b.jumpMP}`);
  console.log(
    `  defBV=${b.defensiveBV?.toFixed(1)} offBV=${b.offensiveBV?.toFixed(1)} cockMod=${b.cockpitModifier}`,
  );
  console.log(
    `  armorBV=${b.armorBV} structBV=${b.structureBV} gyroBV=${b.gyroBV}`,
  );
  console.log(
    `  defFactor=${b.defensiveFactor} defEquip=${b.defEquipBV} explosive=${b.explosivePenalty}`,
  );
  console.log(
    `  weapBV=${b.weaponBV} rawWpn=${b.rawWeaponBV} halved=${b.halvedWeaponBV}`,
  );
  console.log(
    `  ammoBV=${b.ammoBV} wgtBonus=${b.weightBonus} offEquip=${b.offEquipBV}`,
  );
  console.log(
