...
console.log(
  '=== INVESTIGATION: Structure BV, Gyro BV, Engine Multipliers ===\n',
);
...
console.log(
  '  none       → MISSING (defaults to 0.5, should be 0.0) *** BUG ***',
);
...
for (const valUnit of report.allResults) {
  ...
  try {
    ...
    if (cockpit.includes('INTERFACE') || gyro.includes('NONE')) {
      gyroNoneCount++;
      gyroNoneUnits.push(
        `${valUnit.unitId} (${unit.tonnage}t, cockpit: ${unit.cockpit}, gyro: ${unit.gyro?.type})`,
      );
    }
  } catch (e) {
    /* skip */
...
console.log(
  '  XXL        → 0.25 (IS only, Clan XXL handled via override in validate-bv.ts)',
);
...
console.log(
  '  But the EngineDefinition sideTorsoSlots=3 may affect CASE/explosive penalty logic!',
);
...
for (const valUnit of report.allResults) {
  ...
  try {
    const unit = JSON.parse(fs.readFileSync(unitPath, 'utf-8'));
    const engineStr = (unit.engine?.type || '')
      .toUpperCase()
      .replace(/[_\s-]+/g, '');
    if (engineStr === 'XXL' || engineStr === 'XXLENGINE') {
      if (unit.techBase === 'CLAN') {
        clanXXLCount++;
        clanXXLUnits.push(
          `${valUnit.unitId} (diff: ${valUnit.difference}, ${valUnit.percentDiff?.toFixed(1)}%)`,
        );
      } else {
        isXXLCount++;
      }
...
console.log('  standard    → 1.0 ✓');
console.log(
  '  endo-steel  → MISSING (defaults to 1.0 via ?? 1.0) → ✓ correct!',
);
...
for (const [st, count] of [...structureCounts.entries()].sort(
  (a, b) => b[1] - a[1],
)) {
...
console.log(
  '  With sideTorsoSlots=3, CASE in side torso does NOT eliminate penalty.',
);
console.log('');
console.log(
  'MegaMek IS XXL: 6 side torso crits → CASE does NOT help (≥3) → correct',
);
console.log(
  'MegaMek Clan XXL: 4 side torso crits → CASE does NOT help (≥3) → correct',
);
console.log(
  'Our code: XXL sideTorsoSlots=3 → CASE does NOT help (≥3) → CORRECT for both IS & Clan!',
);
console.log(
  '  (The threshold is ≥3, and Clan XXL has 4, IS XXL has 6, we have 3 → all ≥3)',
);
console.log(
  '  So the CASE logic is coincidentally correct even with wrong sideTorsoSlots.',
);
...
console.log(
  'MegaMek: structureBV = totalInternal * 1.5 * structureMultiplier * engineMultiplier',
);
console.log(
  'Our code: structureBV = totalStructurePoints * 1.5 * structureMultiplier * engineMultiplier ✓',
);
...
console.log(
  '  structureBV = totalStructurePoints * 1.5 * structureMultiplier * engineMultiplier ✓',
);
...
if (gyroNoneCount > 0) {
  console.log(
    '  Impact: Each affected unit gets tonnage * 0.5 extra defensive BV',
  );
...
console.log(
  '  Impact on BV multiplier: mitigated by override in validate-bv.ts',
);
...
console.log(
  "The gyro 'none' bug would cause overcalculation, not undercalculation.",
);
