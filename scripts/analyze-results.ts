...
const candidates = results
  .filter((u: any) => u.pct >= -5 && u.pct < -1 && u.ammoBV > 0 && u.weapBV > 0)
...
for (const u of candidates.slice(0, 30)) {
  ...
  console.log(
    `  ${u.name.padEnd(40)} ref=${String(u.ref).padStart(5)} calc=${String(u.calc).padStart(5)} pct=${u.pct}% defBV=${u.defBV?.toFixed(0)} offBV=${u.offBV?.toFixed(0)} sf=${u.sf}`,
  );
}

// Look at units that are very close to 1% off - 2-3% range
const twoThree = results.filter(
  (u: any) => u.pct >= -3 && u.pct < -2 && u.ammoBV > 0,
);
console.log(
...
for (const u of twoThree.slice(0, 15)) {
  console.log(
    `  ${u.name.padEnd(40)} ref=${String(u.ref).padStart(5)} calc=${String(u.calc).padStart(5)} pct=${u.pct}%`,
  );
}
