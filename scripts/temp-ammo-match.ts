...
for (const r of under1to3) {
  ...
  if (hasAmmo && r.breakdown!.ammoBV === 0) {
    ...
    if (zeroAmmoWithAmmo <= 10) {
      ...
      console.log(
        `ZERO AMMO: ${r.chassis} ${r.model} (${r.percentDiff.toFixed(1)}%) ammo=[${ammoSlots.join('; ')}]`,
      );
    }
  }
  ...
  if (hasAmmo && r.breakdown!.ammoBV > 0 && r.breakdown!.weaponBV > 0) {
    ...
    if (ratio < 0.05) {
      ...
      if (lowAmmoRatio <= 5) {
        console.log(
          `LOW AMMO RATIO: ${r.chassis} ${r.model} ammo=${r.breakdown!.ammoBV} weapon=${r.breakdown!.weaponBV} ratio=${ratio.toFixed(3)}`,
        );
      }
    }
  }
