...
function scanCritModifiers(unit: UnitData) {
  ...
  for (const [, slots] of Object.entries(unit.criticalSlots)) {
    ...
    for (const s of slots) {
      ...
      if (
        ...
        result.hasTSM = true;
      if (lo.includes('partial wing') || lo.includes('partialwing'))
        result.hasPartialWing = true;
  ...
  if (Array.isArray(headSlots)) {
    ...
    if (cockpitCount >= 2) {
      if (!allSlotsLo.some((s) => s.includes('gyro')))
        result.detectedInterfaceCockpit = true;
    }
  }
...
function main() {
  ...
  for (const result of selected) {
    ...
    console.log(
      `--- ${result.chassis} ${result.model} (${result.tonnage}t) [${result.percentDiff.toFixed(2)}%] ---`,
    );
    console.log(
      `  IndexBV=${result.indexBV}  CalcBV=${result.calculatedBV}  Gap=${result.difference}`,
    );
    console.log(
      `  Engine: ${unit.engine.type} -> ${engineType} (structMult=${engineBVMult})`,
    );
    console.log(
    ...
    console.log(
      `  Move: walk=${walkMP} run=${runMP} jump=${jumpMP}${mods.hasMASC ? ' MASC' : ''}${mods.hasSupercharger ? ' SC' : ''}${mods.hasTSM ? ' TSM' : ''}`,
    );
    ...
    console.log(
      `    explosivePenalty = ${reportExplosive.toFixed(2)} (from report)`,
    );
    ...
    console.log(
      `    Formula match report = ${defFormulaMatch ? 'YES' : 'NO (gap=' + defFormulaGap.toFixed(2) + ')'}`,
    );
    console.log(
      `    MegaMek expected Def = ${megamekDefBV.toFixed(2)} (indexBV/cockpit - offBV)`,
    );
    ...
    if (Math.abs(defGapFromMegamek) > Math.abs(offGapFromMegamek)) {
      console.log(
        `    => PRIMARY GAP: DEFENSIVE (MegaMek expects ${defGapFromMegamek.toFixed(1)} more defBV)`,
      );
    } else if (Math.abs(offGapFromMegamek) > Math.abs(defGapFromMegamek)) {
      console.log(
        `    => PRIMARY GAP: OFFENSIVE (MegaMek expects ${offGapFromMegamek.toFixed(1)} more offBV)`,
      );
    } else {
  ...
  console.log(
    `${'Unit'.padEnd(28)} ${'Ton'.padStart(3)} ${'Idx'.padStart(5)} ${'Calc'.padStart(5)} ${'%'.padStart(6)} | ${'OurDef'.padStart(8)} ${'RptDef'.padStart(8)} ${'Match'.padStart(5)} | ${'MMDef'.padStart(8)} ${'DefGap'.padStart(7)} ${'OffGap'.padStart(7)} | Side`,
  );
  console.log(`${'-'.repeat(120)}`);
  ...
  for (const r of summaryRows) {
    const side =
      Math.abs(r.defGapFromMegamek) > Math.abs(r.offGapFromMegamek)
        ? 'DEF'
        : 'OFF';
  ...
  console.log(`${'-'.repeat(120)}`);
  console.log(
    `${'TOTALS'.padEnd(28)} ${''.padStart(3)} ${''.padStart(5)} ${''.padStart(5)} ${''.padStart(6)} | ${''.padStart(8)} ${''.padStart(8)} ${''.padStart(5)} | ${''.padStart(8)} ${totalDefGap.toFixed(1).padStart(7)} ${totalOffGap.toFixed(1).padStart(7)} |`,
  );

  console.log(`\n=== KEY QUESTION ANSWERED ===`);
  const formulaMatchCount = summaryRows.filter((r) => r.defFormulaMatch).length;
  console.log(
    `Formula matches report: ${formulaMatchCount}/${summaryRows.length}`,
  );
  console.log(
    `Average DefGap from MegaMek: ${(totalDefGap / summaryRows.length).toFixed(2)}`,
  );
  console.log(
    `Average OffGap from MegaMek: ${(totalOffGap / summaryRows.length).toFixed(2)}`,
  );
  ...
  console.log(
    `and we derive MegaMek's expected def from: indexBV/cockpit - OUR offBV.`,
  );
  console.log(
    `The question is: is our defensiveBV too low or our offensiveBV too low?`,
  );
  console.log(
    `\nSince our formula exactly matches the report for ${formulaMatchCount}/${summaryRows.length} units,`,
  );
  console.log(
    `the report's defensiveBV IS our defensiveBV. The remaining question is`,
  );
  console.log(
    `whether MegaMek computes HIGHER defensiveBV or HIGHER offensiveBV.`,
  );
  console.log(
    `\nTo determine this, we'd need to compare against MegaMek's actual defensive`,
  );
  console.log(
    `breakdown (not just the total). Without that, we can only say the total is low.`,
  );
}
