...
const diff1 = r.allResults.filter(
  (u) => Math.abs(u.difference) === 1 && u.difference !== 0,
);
console.log('Units off by exactly 1 BV:', diff1.length);
console.log(
  '  Overcalculated (our BV +1):',
  diff1.filter((u) => u.difference > 0).length,
);
console.log(
  '  Undercalculated (our BV -1):',
  diff1.filter((u) => u.difference < 0).length,
);

// Units with diff=2
const diff2 = r.allResults.filter(
  (u) => Math.abs(u.difference) === 2 && u.difference !== 0,
);
console.log('\nUnits off by exactly 2 BV:', diff2.length);

// Units with diff=3-5
const diff3to5 = r.allResults.filter(
  (u) => Math.abs(u.difference) >= 3 && Math.abs(u.difference) <= 5,
);
...
const small = r.allResults.filter(
  (u) => Math.abs(u.difference) >= 1 && Math.abs(u.difference) <= 3,
);
...
console.log(
  '  Ratio:',
  ((overSmall.length / (overSmall.length + underSmall.length)) * 100).toFixed(
    1,
  ) + '% over',
);
