...
for (const u of under5plus.sort(
  ...
  for (const eq of unit.equipment || []) {
    ...
    if (
      !res.resolved &&
      !eq.id.includes('targeting-computer') &&
      !eq.id.includes('tsm')
    ) {
  ...
  if (b.weaponBV === 0 || (b.weaponBV && b.weaponBV < 10))
    tags.push('ZERO-WEAP');
  if (hasAmmoWeapons && b.ammoBV === 0 && !hasAmmoInCrits)
    tags.push('NO-AMMO-DATA');
  if (hasAmmoWeapons && b.ammoBV === 0 && hasAmmoInCrits)
    tags.push('AMMO-UNRESOLVED');
  if (unresolvedW.length > 0) tags.push(`UNRESOLVED(${unresolvedW.join(',')})`);

  console.log(
    `  ${u.unitId.padEnd(42)} ${u.percentDiff.toFixed(1)}% diff=${u.difference} weap=${b.weaponBV?.toFixed(0)} ammo=${b.ammoBV} defEq=${b.defEquipBV?.toFixed(0)} SF=${b.speedFactor} [${tags.join(',')}]`,
  );
}
