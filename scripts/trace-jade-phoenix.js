...
function findWeapon(pattern) {
  return allWeapons.filter((w) =>
    w.id.toLowerCase().includes(pattern.toLowerCase()),
  );
}

console.log('=== Clan ER PPC ===');
for (const w of findWeapon('er-ppc'))
  console.log('  ' + w.id + ' bv=' + w.battleValue + ' heat=' + w.heat);

console.log('\n=== Clan Gauss Rifle ===');
for (const w of findWeapon('gauss-rifle'))
  console.log('  ' + w.id + ' bv=' + w.battleValue + ' heat=' + w.heat);

console.log('\n=== Clan ER Medium Laser ===');
for (const w of findWeapon('er-medium-laser'))
  console.log('  ' + w.id + ' bv=' + w.battleValue + ' heat=' + w.heat);
...
for (const f of miscFiles) {
  try {
    ...
    const pw = (data.items || []).filter(
      (i) =>
        i.id.toLowerCase().includes('partial-wing') ||
        i.name.toLowerCase().includes('partial wing'),
    );
    if (pw.length > 0) {
      console.log('\n=== Partial Wing (from ' + f + ') ===');
      for (const w of pw)
        console.log(
          '  ' + w.id + ' bv=' + w.battleValue + ' heat=' + (w.heat || 0),
        );
    }
  } catch (_error) {
  ...
  for (const f of files2) {
    ...
    const pw = (data.items || []).filter(
      (i) =>
        ...
        i.name.toLowerCase().includes('partial'),
    );
    if (pw.length > 0) {
...
if (clanERPPC && clanGauss && clanERMed) {
  const base =
    2 * clanERPPC.battleValue + clanGauss.battleValue + clanERMed.battleValue;
  const withTC =
    2 * clanERPPC.battleValue * 1.25 +
    clanGauss.battleValue * 1.25 +
    clanERMed.battleValue * 1.25;
