...
const mixedOutside = r.allResults
  .filter(
    (u) => Math.abs(u.percentDiff) > 1 && u.breakdown?.techBase === 'MIXED',
  )
...
for (const u of mixedOutside) {
  ...
  console.log(
    `  OFF: wBV=${b.rawWeaponBV} halved=${b.halvedWeaponBV} ammo=${b.ammoBV} sf=${b.speedFactor}`,
  );
  console.log(
    `  DEF: armor=${b.armorBV} struct=${b.structureBV} gyro=${b.gyroBV} expl=${b.explosivePenalty} df=${b.defensiveFactor}`,
  );
...
console.log(
  `  Within 0.5%: ${mixedAll.filter((u) => Math.abs(u.percentDiff) <= 0.5).length}`,
);
console.log(
  `  Within 1%: ${mixedAll.filter((u) => Math.abs(u.percentDiff) <= 1).length}`,
);
console.log(
  `  Outside 1%: ${mixedAll.filter((u) => Math.abs(u.percentDiff) > 1).length}`,
);
