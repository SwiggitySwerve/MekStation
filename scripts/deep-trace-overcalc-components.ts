...
for (const u of big.slice(0, 20)) {
  ...
  console.log(
    `${u.unitId} ref=${u.indexBV} calc=${u.calculatedBV} diff=${u.difference} (+${u.percentDiff.toFixed(1)}%)`,
  );
  console.log(
    `  DEF=${b.defensiveBV?.toFixed(0)} (expectedDef=${expectedDef.toFixed(0)} excess=${defExcess.toFixed(0)})`,
  );
  console.log(
    `    armor=${b.armorBV?.toFixed(1)} struct=${b.structureBV?.toFixed(1)} gyro=${b.gyroBV?.toFixed(1)} defEq=${b.defEquipBV} expPen=${b.explosivePenalty} DF=${b.defensiveFactor}`,
  );
  console.log(
    `  OFF=${b.offensiveBV?.toFixed(0)} (expectedOff=${expectedOff.toFixed(0)} excess=${offExcess.toFixed(0)})`,
  );
  console.log(
    `    weap=${b.weaponBV?.toFixed(1)} ammo=${b.ammoBV} wt=${b.weightBonus?.toFixed(1)} phys=${b.physicalWeaponBV?.toFixed(1)} offEq=${b.offEquipBV} SF=${b.speedFactor} HE=${b.heatEfficiency} halved=${b.halvedWeaponCount}/${b.weaponCount}`,
  );
  console.log(
    `    run=${b.runMP} jump=${b.jumpMP || 0} walk=${b.walkMP || unit.movement?.walk} eng=${unit.engine?.type} tech=${unit.techBase}`,
  );
  ...
  for (const eq of unit.equipment || []) {
    ...
    console.log(
      `    ${eq.id} @ ${eq.location}: bv=${res.battleValue} heat=${res.heat}`,
    );
  }
  console.log('');
}
...
for (const u of big) {
  ...
  console.log(
    `  ${u.unitId.padEnd(42)} totalExcess=${totalExcess.toFixed(0)} off=${b.offensiveBV?.toFixed(0)} def=${b.defensiveBV?.toFixed(0)} offFrac=${(offFrac * 100).toFixed(0)}%`,
  );
}
