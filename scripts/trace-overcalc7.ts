...
for (const d of overCalc) {
  ...
  try {
    ...
    if (
      ...
      c3Count++;
    if (
      allCritsLo.some((s) => s.includes('inarc') || s.includes('improved narc'))
    )
      inarcCount++;
    if (
      allCritsLo.some(
    ...
    if (
      ['archangel', 'seraph', 'grigori', 'preta', 'deva', 'malak'].some((c) =>
        chassis.includes(c),
      )
    )
      celestialCount++;
...
const equipChecks: Array<[string, (unit: any, critsLo: string[]) => boolean]> =
  [
    ...
    [
      'Narc',
      (u, c) => c.some((s) => s.includes('narc') && !s.includes('inarc')),
    ],
    [
      'Celestial',
      (u, _) =>
        ['archangel', 'seraph', 'grigori', 'preta', 'deva', 'malak'].some((c) =>
          (u.chassis || '').toLowerCase().includes(c),
        ),
    ],
...
for (const [name, check] of equipChecks) {
  ...
  if (with_.length > 0) {
    const avgWith =
      with_.reduce((s: number, d: any) => s + d.percentDiff, 0) / with_.length;
    const avgWithout =
      without_.length > 0
        ? without_.reduce((s: number, d: any) => s + d.percentDiff, 0) /
          without_.length
        : 0;
    console.log(
      `  ${name}: with=${with_.length} (${avgWith.toFixed(1)}%) without=${without_.length} (${avgWithout.toFixed(1)}%)`,
    );
  }
}
...
const highDef = overCalc.filter(
  (d: any) => d.breakdown.defensiveBV / d.calculatedBV > 0.5,
);
const lowDef = overCalc.filter(
  (d: any) => d.breakdown.defensiveBV / d.calculatedBV <= 0.5,
);
console.log(
  `  High defense (>50% of total): ${highDef.length} units, avg ${(highDef.reduce((s: number, d: any) => s + d.percentDiff, 0) / highDef.length).toFixed(1)}%`,
);
console.log(
  `  Low defense (<=50% of total): ${lowDef.length} units, avg ${(lowDef.reduce((s: number, d: any) => s + d.percentDiff, 0) / lowDef.length).toFixed(1)}%`,
);
