...
const within1to5under = report.allResults.filter(
  (r: Result) =>
    r.status !== 'error' &&
    r.percentDiff !== null &&
    r.percentDiff < -1 &&
...
for (const r of within1to5under) {
  ...
  if (armorType !== 'STANDARD' && armorType !== '')
    addFeature(`armor:${armorType}`, r.percentDiff);
      ...
      (slots: any) =>
        ...
        slots.some(
          (s: any) =>
            s &&
            typeof s === 'string' &&
            s.toLowerCase().includes('coolant pod'),
        ),
    );
  if (hasCoolantPod) addFeature('hasCoolantPod', r.percentDiff);

  const hasStreakSRM = ud.equipment?.some((eq: any) =>
    eq.id.toLowerCase().includes('streak'),
  );
  if (hasStreakSRM) addFeature('hasStreak', r.percentDiff);

  const hasLRM = ud.equipment?.some((eq: any) =>
    eq.id.toLowerCase().includes('lrm'),
  );
  ...
  const hasATM = ud.equipment?.some((eq: any) =>
    eq.id.toLowerCase().includes('atm'),
  );
  if (hasATM) addFeature('hasATM', r.percentDiff);

  const hasGauss = ud.equipment?.some((eq: any) =>
    eq.id.toLowerCase().includes('gauss'),
  );
  if (hasGauss) addFeature('hasGauss', r.percentDiff);

  const hasPPC = ud.equipment?.some((eq: any) =>
    eq.id.toLowerCase().includes('ppc'),
  );
  ...
  const hasPulseLaser = ud.equipment?.some((eq: any) =>
    eq.id.toLowerCase().includes('pulse'),
  );
  ...
  const isOmni =
    ...
    Object.values(ud.criticalSlots).some(
      (slots: any) =>
        ...
        slots.some(
          (s: any) =>
            s && typeof s === 'string' && s.toLowerCase().includes('(omnipod)'),
        ),
    );
  if (isOmni) addFeature('isOmniMech', r.percentDiff);

  if (r.breakdown) {
    if (r.breakdown.ammoBV === 0 && hasAmmo)
      addFeature('zeroAmmoBV_withAmmo', r.percentDiff);
    if (r.breakdown.explosivePenalty === 0 && hasAmmo)
      addFeature('zeroExplosivePenalty_withAmmo', r.percentDiff);
  }
}
...
for (const [name, count] of sorted) {
  const avg =
    featureAvgDiff[name].reduce((s, v) => s + v, 0) /
    featureAvgDiff[name].length;
  console.log(
    `  ${name.padEnd(35)} ${String(count).padStart(4)} units  avg=${avg.toFixed(2)}%`,
  );
}
...
const topDiffs = Object.entries(exactDiffs)
  .sort((a, b) => b[1] - a[1])
  .slice(0, 20);
