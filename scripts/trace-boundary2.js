...
for (const u of under) {
  ...
  if (
    ...
    crits.includes('istc') ||
    crits.includes('cltargetingcomputer')
  )
    equips.push('TC');
  if (crits.includes('apollo') || crits.includes('fcs'))
    equips.push('Apollo/FCS');
  ...
  if (equips.length > 0) {
    console.log(
      '  ' +
        u.unitId.padEnd(40) +
        u.percentDiff.toFixed(2).padStart(7) +
        '% gap=' +
        u.difference.toString().padStart(4) +
        ' equip=' +
        equips.join(','),
    );
  } else {
...
console.log(
  '\nEquipment frequency in undercalculated boundary:',
  JSON.stringify(equipFreq, null, 2),
);
...
console.log(
  '\nEquipment frequency in overcalculated boundary:',
  JSON.stringify(overEquipFreq, null, 2),
);
