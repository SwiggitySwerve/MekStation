...
const goodUnits = report.allResults.filter(
  (r: any) => Math.abs(r.percentDiff) <= 1.0,
);
...
for (const e of enriched.slice(0, 30)) {
  console.log(
    `${e.weapon.padEnd(50)} ${String(e.badCount).padStart(4)} ${String(e.goodCount).padStart(5)}  ${e.ratio.toFixed(2).padStart(5)}  ${e.avgGap.toFixed(1).padStart(6)}`,
  );
}
...
for (const r of underCalc) {
  if (r.issues && r.issues.some((i: string) => i.includes('Unresolved'))) {
    ...
    if (unresolvedCount <= 10) {
      console.log(
        `  ${r.unitId}: ${r.issues.filter((i: string) => i.includes('Unresolved')).join('; ')}`,
      );
    }
  }
}
...
const resolved = underCalc.filter(
  (r: any) => !r.issues?.some((i: string) => i.includes('Unresolved')),
);
const avgGapResolved =
  resolved.reduce((s: number, r: any) => s + (r.indexBV - r.calculatedBV), 0) /
  resolved.length;
const avgPctResolved =
  resolved.reduce((s: number, r: any) => s + r.percentDiff, 0) /
  resolved.length;
console.log(`\nUnits without unresolved weapons: ${resolved.length}`);
console.log(
  `  Average gap: ${avgGapResolved.toFixed(1)} BV (${avgPctResolved.toFixed(2)}%)`,
);

const unresolved = underCalc.filter((r: any) =>
  r.issues?.some((i: string) => i.includes('Unresolved')),
);
if (unresolved.length > 0) {
  const avgGapUnresolved =
    unresolved.reduce(
      (s: number, r: any) => s + (r.indexBV - r.calculatedBV),
      0,
    ) / unresolved.length;
  const avgPctUnresolved =
    unresolved.reduce((s: number, r: any) => s + r.percentDiff, 0) /
    unresolved.length;
  console.log(`Units with unresolved weapons: ${unresolved.length}`);
  console.log(
    `  Average gap: ${avgGapUnresolved.toFixed(1)} BV (${avgPctUnresolved.toFixed(2)}%)`,
  );
}
