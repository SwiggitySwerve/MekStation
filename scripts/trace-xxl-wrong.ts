...
console.log(
  `XXL engine units: ${xxlWrong.length} wrong, ${xxlCorrect.length} correct`,
);
console.log(`\nWrong XXL units:`);

xxlWrong.sort(
  (a: any, b: any) => Math.abs(b.percentDiff) - Math.abs(a.percentDiff),
);
...
for (const res of xxlWrong.slice(0, 20)) {
  ...
  console.log(
    `    defBV: armor=${b.armorBV?.toFixed(1)} struct=${b.structureBV?.toFixed(1)} gyro=${b.gyroBV?.toFixed(1)} defEq=${b.defensiveEquipBV?.toFixed(1)} pen=${b.explosivePenalty} defF=${b.defensiveFactor?.toFixed(2)}`,
  );
  console.log(
    `    offBV: wep=${b.weaponBV?.toFixed(1)} ammo=${b.ammoBV?.toFixed(1)} speed=${b.speedFactor?.toFixed(3)}`,
  );
  ...
  const isISXXL =
    d.techBase === 'INNER_SPHERE' ||
    (d.techBase === 'MIXED' && !d.engine.type.toUpperCase().includes('CLAN'));
  ...
  console.log(
    `    expectedStructBV=${expectedStructBV.toFixed(1)} (IS=${totalIS} * 1.5 * structM=${structMult} * engM=${expectedMult})`,
  );
  if (Math.abs(expectedStructBV - (b.structureBV || 0)) > 1) {
    console.log(
      `    *** STRUCT BV MISMATCH: expected ${expectedStructBV.toFixed(1)} vs actual ${b.structureBV?.toFixed(1)}`,
    );
  }
}
...
for (const res of xxlCorrect.slice(0, 10)) {
  const d = res.unit;
  console.log(
    `  ${res.unitId}: ${d.tonnage}t ${d.techBase} ${d.engine.type} diff=${res.difference} (${res.percentDiff?.toFixed(1)}%)`,
  );
}
