...
for (const d of exactFive) {
  ...
  try {
    ...
    if (armorType.includes('REACTIVE')) reactiveCount++;
      ...
      armorType.includes('LASERREFLECTIVE')
    )
      reflectiveCount++;
...
for (const d of exactFive.sort((a: any, b: any) =>
  ...
  console.log(
    `  ${d.unitId}: calc=${d.calculatedBV} ref=${d.indexBV} ratio=${ratio} *0.95=${adj} (diff=${adjDiff})`,
  );
}
...
for (const d of notExactFive.slice(0, 30)) {
  ...
  const adj = Math.round(d.calculatedBV * 0.95);
  console.log(
    `  ${d.unitId}: calc=${d.calculatedBV} ref=${d.indexBV} ratio=${ratio} *0.95=${adj} (diff=${adj - d.indexBV})`,
  );
}
