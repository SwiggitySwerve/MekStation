...
for (const u of r.allResults) {
  ...
  const crits = Object.values(unit.criticalSlots)
    .flat()
...
for (const u of blueShieldInCrits) {
  console.log(
    `  ${u.id.padEnd(45)} diff=${String(u.diff).padStart(5)} (${(u.pct || 0).toFixed(2)}%) ${u.status}`,
  );
}
...
for (const u of commandConsoleUnits) {
  console.log(
    `  ${u.id.padEnd(45)} diff=${String(u.diff).padStart(5)} (${(u.pct || 0).toFixed(2)}%) ${u.status} cockpit=${u.cockpit}`,
  );
}
