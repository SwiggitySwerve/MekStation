...
for (const uid of underIds) {
  ...
  console.log(`\n${uid}:`);
  console.log(
    `  Index: chassis="${ie?.chassis}" model="${ie?.model}" bv=${ie?.bv}`,
  );
  if (entry) {
    console.log(
      `  MUL: name="${entry.mulName}" bv=${entry.mulBV} match=${entry.matchType}`,
    );
  } else {
  ...
  if (res) {
    console.log(
      `  Validation: calc=${res.calculatedBV} diff=${res.difference}`,
    );
...
for (const u of overMinor) {
  ...
  try {
    ...
    const allSlots = Object.values(d.criticalSlots)
      .flat()
      .filter(Boolean) as string[];
    const hasTSM = allSlots.some(
      (s) =>
    ...
    if (hasTSM) {
      console.log(
        `  ${u.unitId}: +${u.difference} (+${u.percentDiff.toFixed(1)}%) ${d.tonnage}t ${d.techBase} walk=${d.movement.walk}`,
      );
    }
  } catch (_error) {
