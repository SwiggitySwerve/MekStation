...
for (const target of targets) {
  const [chassis, model] = target.includes(' ')
    ? [
  ...
  if (!rr) {
    console.log(`NOT IN REPORT: ${target}`);
    continue;
  }

  console.log(`\n${'='.repeat(70)}`);
  console.log(
    `${target} (${ud.tonnage}t, ${ud.techBase}) ref=${rr.indexBV} calc=${rr.calculatedBV} diff=${rr.difference} (${rr.percentDiff.toFixed(2)}%)`,
  );
  ...
  if (rr.breakdown) {
    ...
    console.log(
      `Breakdown: defBV=${bd.defensiveBV.toFixed(1)} offBV=${bd.offensiveBV.toFixed(1)} weapBV=${bd.weaponBV.toFixed(1)} ammoBV=${bd.ammoBV.toFixed(1)} sf=${bd.speedFactor.toFixed(3)} explPen=${bd.explosivePenalty} defEquip=${bd.defensiveEquipBV || 0}`,
    );
  }
  ...
  for (const eq of ud.equipment) {
    ...
    const clanRes = !norm.startsWith('clan-')
      ? resolveEquipmentBV('clan-' + norm)
      : { resolved: false, battleValue: 0 };

    const clanNote =
      clanRes.resolved && clanRes.battleValue > res.battleValue
        ? ` (CLAN: ${clanRes.battleValue})`
        : '';
    console.log(
      `  ${eq.id.padEnd(35)} loc=${eq.location.padEnd(12)} BV=${res.battleValue} heat=${res.heat || 0} resolved=${res.resolved}${clanNote}`,
    );
  }
  ...
  console.log(
    `Has MASC: ${eqIds.includes('masc')}, Has Supercharger: ${eqIds.includes('supercharger')}, Has TSM: ${eqIds.includes('tsm')}`,
  );
}
