...
for (const r of under) {
  ...
  for (const eq of ud.equipment) {
    ...
    if (clanResolved && clanBV !== isResult.battleValue) {
      console.log(
        `  ${eq.id.padEnd(30)} IS_BV=${isResult.battleValue} Clan_BV=${clanBV} candidates=${JSON.stringify(candidates)} norm=${normalizedIS}`,
      );
    }
  }
...
for (const id of tests) {
  ...
  console.log(
    `${id}: IS resolved=${isResult.resolved} IS_BV=${isResult.battleValue}`,
  );
  for (const cid of candidates) {
    const cr = resolveEquipmentBV(cid);
    console.log(
      `  candidate '${cid}': resolved=${cr.resolved} BV=${cr.battleValue}`,
    );
    if (cr.resolved && cr.battleValue > 0) {
      console.log(
        `  CONDITION: cr.bv(${cr.battleValue}) > isResult.bv(${isResult.battleValue}) = ${cr.battleValue > isResult.battleValue}`,
      );
      console.log(
        `  Would return Clan: ${!isResult.resolved || cr.battleValue > isResult.battleValue}`,
      );
    }
  }
}
