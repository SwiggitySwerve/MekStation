...
const unresolvedAmmo = new Map<
  string,
  { count: number; units: string[]; pctDiffs: number[] }
>();
...
for (const ie of idx.units) {
  ...
  for (const [loc, slots] of Object.entries(unit.criticalSlots)) {
    ...
    for (const slot of slots as string[]) {
      ...
      if (!entry) {
        entry = { count: 0, units: [], pctDiffs: [] };
        unresolvedAmmo.set(key, entry);
      }
      entry.count++;
...
const sorted = [...unresolvedAmmo.entries()].sort(
  (a, b) => b[1].count - a[1].count,
);
for (const [name, info] of sorted) {
  const avgDiff =
    info.pctDiffs.length > 0
      ? (
          info.pctDiffs.reduce((a, b) => a + b, 0) / info.pctDiffs.length
        ).toFixed(1)
      : '?';
  console.log(
    `  ${String(info.count).padStart(4)}x  ${name.padEnd(55)} avgDiff=${avgDiff}%  [${info.units.join(', ')}]`,
  );
}
