...
for (const f of weapFiles) {
  const data = bvAnalysis.readJson(path.join(weapPath, f));
...
for (const u of mixedUnits.slice(0, 5)) {
    ...
    if (weapon) {
      console.log(
        '  ' +
          ...
          ' -> IS bv=' +
          weapon.battleValue +
          (clanWeap ? ' | Clan bv=' + clanWeap.battleValue : '') +
          ' @ ' +
          eq.location,
      );
    } else {
  ...
  if (clanWeapons.length > 0)
    console.log('  Clan crit items: ' + [...new Set(clanWeapons)].join(', '));
  if (isWeapons.length > 0)
    console.log('  IS crit items: ' + [...new Set(isWeapons)].join(', '));
}
