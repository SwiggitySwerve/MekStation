...
for (const b of boundary) {
  ...
  if (bd.runMP && bd.runMP !== expectedRun) {
    ...
    console.log(
      `  ${b.unitId.padEnd(40)} walk=${bd.walkMP} run=${bd.runMP} (expected ${expectedRun}) ${sign}${b.percentDiff.toFixed(2)}%`,
    );
    if (unit) {
      console.log(
        `    Engine: ${unit.engineType || '?'}  MASC: ${unit.hasMASC || false}  SC: ${unit.hasSupercharger || false}  Triple-Strength: ${unit.myomerType || 'Standard'}`,
      );
    }
  }
}
...
if (Object.keys(unresolvedCounts).length === 0) {
  ...
  const sortedUnresolved = Object.entries(unresolvedCounts).sort(
    (a, b) => b[1] - a[1],
  );
...
console.log(
  `  With defensive equip BV: ${withDefEquip} (${underWithDef} under, ${overWithDef} over)`,
);
...
for (const b of boundary) {
  ...
  const sign = b.difference > 0 ? '+' : '';
  console.log(
    `  ${b.unitId.padEnd(40)} armoredBV=${bd.armoredComponentBV}  ${sign}${b.percentDiff.toFixed(2)}%`,
  );
}
...
for (const b of boundary) {
  ...
  if (
    bd.heatEfficiency &&
    Math.abs(bd.heatEfficiency - expectedHeatEff) > 0.1
  ) {
    const sign = b.difference > 0 ? '+' : '';
    console.log(
      `  ${b.unitId.padEnd(40)} heatEff=${bd.heatEfficiency} expected=${expectedHeatEff} dissipation=${bd.heatDissipation} moveHeat=${bd.moveHeat} ${sign}${b.percentDiff.toFixed(2)}%`,
    );
    heatAnomalies++;
  }
}
...
console.log(
  `  Under with explosive penalty: ${underWithExp} (avg penalty: ${underWithExp ? (underExpTotal / underWithExp).toFixed(0) : 'n/a'})`,
);
console.log(
  `  Over with explosive penalty: ${overWithExp} (avg penalty: ${overWithExp ? (overExpTotal / overWithExp).toFixed(0) : 'n/a'})`,
);
...
const allSFs = new Set([
  ...Object.keys(sfBuckets.under),
  ...Object.keys(sfBuckets.over),
]);
for (const sf of [...allSFs].sort()) {
  const u = sfBuckets.under[sf] || 0;
  const o = sfBuckets.over[sf] || 0;
  console.log(
    `  ${sf.padEnd(14)} ${String(u).padStart(5)} ${String(o).padStart(5)}`,
  );
}
