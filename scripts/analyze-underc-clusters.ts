...
const clusters = [...byChassis.entries()]
  .filter(([, v]) => v.length >= 2)
  .sort((a, b) => b[1].length - a[1].length);
for (const [chassis, units] of clusters) {
  const avgPct =
    units.reduce((s: number, u: any) => s + u.percentDiff, 0) / units.length;
  console.log(
    `  ${chassis} (${units.length} units, avg ${avgPct.toFixed(1)}%): ${units.map((u: any) => u.model).join(', ')}`,
  );
}
  ...
  for (const eq of unit.equipment || []) {
    ...
    if (!res.resolved) {
      ...
      if (!unresolvedCounts.has(key))
        unresolvedCounts.set(key, { count: 0, units: [] });
...
// Sort by unit count
const sorted = [...unresolvedCounts.entries()].sort(
  (a, b) => b[1].units.length - a[1].units.length,
);
for (const [id, info] of sorted.slice(0, 30)) {
  console.log(
    `  ${id.padEnd(40)} ${info.units.length} units (${info.count} instances)`,
  );
}
...
for (const u of under) {
  ...
  if (hasAmmoWeapons && (b.ammoBV || 0) === 0) {
    ...
    if (zeroAmmoBV <= 10) {
      console.log(
        `  ${u.unitId.padEnd(40)} ${u.percentDiff.toFixed(1)}% ammoBV=${b.ammoBV} weapBV=${b.weaponBV?.toFixed(0)}`,
      );
    }
  }
}
...
for (const u of under.slice(0, 10)) {
  ...
  console.log(
    `  weap=${b.weaponBV?.toFixed(0)} rawWeap=${b.rawWeaponBV?.toFixed(0)} halved=${b.halvedWeaponCount}/${b.weaponCount} HE=${b.heatEfficiency} ammo=${b.ammoBV} wt=${b.weightBonus?.toFixed(0)} SF=${b.speedFactor}`,
  );
  ...
  console.log(
    `  Expected base=${total.toFixed(0)} deficit=${deficit.toFixed(0)}`,
  );
}
