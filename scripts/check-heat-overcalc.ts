...
for (const id of targets) {
  ...
  const isDHS =
    hsType.toUpperCase().includes('DOUBLE') ||
    hsType.toUpperCase().includes('LASER');
  ...
  console.log(
    `  Crit DHS slots=${dhsCritSlots} (${critDHS} DHS), Crit SHS slots=${shsCritSlots}`,
  );
  ...
  if (b) {
    ...
    console.log(
      `  weaponBV=${b.weaponBV} offBV=${b.offensiveBV} speed=${b.speedFactor}`,
    );
