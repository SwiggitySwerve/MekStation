...
// 1-2% under
const under12 = valid
  .filter((x: any) => x.percentDiff < -1 && x.percentDiff >= -2)
...
for (const u of under12.slice(0, 10)) {
  ...
  console.log(
    `${u.unitId.padEnd(42)} ref=${totalRef} calc=${totalCalc} diff=${diff} (${u.percentDiff.toFixed(1)}%)`,
  );
  console.log(
    `  DEF=${defBV.toFixed(0)} OFF=${offBV.toFixed(0)} cockpit=${cockMod}`,
  );
  console.log(
    `  armorBV=${b.armorBV?.toFixed(0)} structBV=${b.structureBV?.toFixed(0)} gyroBV=${b.gyroBV?.toFixed(0)} defEq=${b.defEquipBV?.toFixed(0)} expPen=${b.explosivePenalty?.toFixed(0)} DF=${b.defensiveFactor}`,
  );
  console.log(
    `  weapBV=${b.weaponBV?.toFixed(0)} ammoBV=${b.ammoBV} physBV=${b.physicalWeaponBV?.toFixed(0)} wtBonus=${b.weightBonus?.toFixed(0)} offEq=${b.offEquipBV?.toFixed(0)} SF=${b.speedFactor}`,
  );
  ...
  console.log(
    `  Need +${Math.abs(diff).toFixed(0)} BV total. If all in DEF: +${(Math.abs(diff) / cockMod / b.defensiveFactor).toFixed(0)} base-DEF. If all in OFF: +${(Math.abs(diff) / cockMod / b.speedFactor).toFixed(0)} base-OFF`,
  );
  console.log('');
}
...
for (const [label, group] of [
  ...
  const defRatio = group.map(
    (u: any) => (u.breakdown.defensiveBV || 0) / Math.max(u.calculatedBV, 1),
  );
  const offRatio = group.map(
    (u: any) => (u.breakdown.offensiveBV || 0) / Math.max(u.calculatedBV, 1),
  );

  console.log(`${label} (${group.length} units):`);
  console.log(
    `  HeatEff: mean=${stats(heatEffs).mean.toFixed(1)}, std=${stats(heatEffs).std.toFixed(1)}`,
  );
  console.log(
    `  SF: mean=${stats(sfs).mean.toFixed(2)}, std=${stats(sfs).std.toFixed(2)}`,
  );
  console.log(
    `  DF: mean=${stats(dfs).mean.toFixed(2)}, std=${stats(dfs).std.toFixed(2)}`,
  );
