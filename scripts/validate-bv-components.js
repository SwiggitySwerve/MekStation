...
function isQuadFromArmor(unit) {
  const armorKeys = Object.keys(unit.armor?.allocation || {}).map((k) =>
    k.toUpperCase(),
  );
  return armorKeys.some((k) =>
    [
      'FLL',
      'FRL',
      'RLL',
...
function scanSpecialCrits(unit) {
  const crits = unit.criticalSlots
    ? Object.values(unit.criticalSlots)
        .flat()
  ...
  return {
    hasTSM: lo.some(
      (s) =>
        s === 'tsm' ||
        s.includes('triple strength') ||
        s.includes('triplestrength'),
    ),
    hasMASC: lo.some((s) => s.includes('masc') && !s.includes('ammo')),
    hasSC: lo.some((s) => s.includes('supercharger')),
    hasTC: lo.some(
      (s) =>
        s.includes('targeting computer') || s.includes('targetingcomputer'),
    ),
    hasPartialWing: lo.some((s) => s.includes('partial') && s.includes('wing')),
    hasImprovedJJ: lo.some(
      (s) =>
        s.includes('improvedjumpjet') ||
        (s.includes('improved') && s.includes('jump')),
    ),
    hasNullSig: lo.some((s) => s.includes('null') && s.includes('sig')),
    hasVoidSig: lo.some((s) => s.includes('void') && s.includes('sig')),
    hasChameleon: lo.some(
      (s) =>
        s.includes('chameleon') &&
        (s.includes('shield') || s.includes('lps') || s.includes('polariz')),
    ),
    hasStealth: (unit.armorType || unit.armor?.type || '')
      .toLowerCase()
      .includes('stealth'),
    hasLargeShield: lo.some(
      (s) => s.includes('shield') && s.includes('large') && !s.includes('blue'),
    ),
    hasHardened: (unit.armorType || unit.armor?.type || '')
      .toLowerCase()
      .includes('hardened'),
    aesCount: lo.filter(
      (s) => s.includes('aes') || s.includes('actuator enhancement'),
    ).length,
  };
}
...
for (const u of all) {
  ...
  const isQuad =
    (unit.configuration || '').toLowerCase() === 'quad' ||
    isQuadFromArmor(unit);
  ...
  if (Math.abs(expectedStructBV - b.structureBV) > 1.0) {
    errors.push(
      `struct: expected=${expectedStructBV.toFixed(1)} got=${b.structureBV} (engineMult=${engineMult} structMult=${structMult} totalIS=${expectedTotalIS})`,
    );
  ...
  if (Math.abs(expectedGyro - b.gyroBV) > 0.5) {
    errors.push(
      `gyro: expected=${expectedGyro} got=${b.gyroBV} (gyroType=${gyroType})`,
    );
  ...
  const expectedArmorBV =
    Math.round(totalArmorPts * 2.5 * getArmorMult(armorType) * 10) / 10;
  if (Math.abs(expectedArmorBV - b.armorBV) > 2.0) {
    errors.push(
      `armor: expected=${expectedArmorBV.toFixed(1)} got=${b.armorBV} (pts=${totalArmorPts} mult=${getArmorMult(armorType)} type=${armorType})`,
    );
  ...
  if (spec.hasVoidSig) {
    if (expectedTMM < 3) expectedTMM = 3;
    else if (expectedTMM === 3) expectedTMM++;
  }
  const expectedDefFactor = 1 + expectedTMM / 10.0;
  if (Math.abs(expectedDefFactor - b.defensiveFactor) > 0.01) {
    errors.push(
      `defFactor: expected=${expectedDefFactor.toFixed(2)} got=${b.defensiveFactor} (runMP=${runMP} jumpMP=${jumpMP} TMM=${expectedTMM})`,
    );
  ...
  if (Math.abs(expectedSF - b.speedFactor) > 0.01) {
    errors.push(
      `speedFactor: expected=${expectedSF.toFixed(2)} got=${b.speedFactor} (runMP=${runMP} jumpMP=${jumpMP})`,
    );
  ...
  if (spec.hasTSM && b.weightBonus !== ton * 1.5) {
    errors.push(
      `weightBonus: expected=${ton * 1.5} (TSM) got=${b.weightBonus}`,
    );
    componentErrors.weightBonus++;
  } else if (
    !spec.hasTSM &&
    spec.aesCount === 0 &&
    Math.abs(b.weightBonus - expectedWeight) > 0.5
  ) {
  ...
  if (Math.abs(expectedHeatDiss - b.heatDissipation) > 1) {
    errors.push(
      `heatDiss: expected=${expectedHeatDiss} got=${b.heatDissipation} (hs=${hsCount}×${isDHS ? 'DHS' : 'SHS'})`,
    );
...
for (const [comp, count] of Object.entries(componentErrors).sort(
  (a, b) => b[1] - a[1],
)) {
  console.log(
    `  ${comp.padEnd(20)} ${count} errors (${((count / all.length) * 100).toFixed(1)}%)`,
  );
}
