...
for (const d of overCalc) {
  ...
  if (entry && entry.mulBV > 0 && entry.matchType === 'exact') {
    ...
    if (
      mulNameNorm.endsWith(modelNorm) ||
      mulNameNorm.endsWith(') ' + modelNorm)
    ) {
      mulBVUsed++;
    } else {
...
console.log(
  'MUL exact match units: avg overcalc =',
  (
    mulExact.reduce((s: number, d: any) => s + d.percentDiff, 0) /
    mulExact.length
  ).toFixed(1) + '%, count:',
  mulExact.length,
);
console.log(
  'No MUL / index units: avg overcalc =',
  (
    noMul.reduce((s: number, d: any) => s + d.percentDiff, 0) / noMul.length
  ).toFixed(1) + '%, count:',
  noMul.length,
);
console.log('');
...
console.log(
  '=== Top no-MUL overcalculated units (index BV may be outdated) ===',
);
...
for (const d of noMul.slice(0, 15)) {
  ...
  const entry = mulCache.entries?.[d.unitId];
  console.log(
    `  ${d.unitId}: indexBV=${d.indexBV} calc=${d.calculatedBV} diff=+${d.percentDiff.toFixed(1)}%`,
  );
  if (entry) {
    console.log(
      `    MUL: mulBV=${entry.mulBV} match=${entry.matchType} name="${entry.mulName}"`,
    );
  } else {
...
for (const d of mulExact.slice(0, 15)) {
  const entry = mulCache.entries?.[d.unitId];
  console.log(
    `  ${d.unitId}: mulBV=${d.indexBV} calc=${d.calculatedBV} diff=+${d.percentDiff.toFixed(1)}%`,
  );
  console.log(
    `    MUL name: "${entry?.mulName}" weapBV=${d.breakdown.weaponBV} ammoBV=${d.breakdown.ammoBV} SF=${d.breakdown.speedFactor} defBV=${d.breakdown.defensiveBV.toFixed(0)}`,
  );
}
