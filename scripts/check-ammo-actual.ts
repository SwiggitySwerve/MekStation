...
for (const uid of targets) {
  ...
  console.log(
    `${uid.padEnd(42)} ammoBV=${u.breakdown.ammoBV} diff=${u.percentDiff.toFixed(1)}% calcBV=${u.calculatedBV} refBV=${u.indexBV}`,
  );
}
...
console.log(
  `Units with ammoBV=0 and weapons: ${zeroAmmoCount} / ${outside1.length}`,
);

// Specifically check all units outside 1% and show their ammoBV
console.log('\n=== AMMO BV DISTRIBUTION (outside 1%) ===');
const ammoBVBuckets: Record<string, number> = {
  '0': 0,
  '1-50': 0,
  '51-100': 0,
  '100+': 0,
...
for (const u of lowAmmo
  .sort((a: any, b: any) => a.percentDiff - b.percentDiff)
  .slice(0, 20)) {
  console.log(
    `  ${u.unitId.padEnd(42)} ${u.percentDiff.toFixed(1)}% ammoBV=${u.breakdown?.ammoBV}`,
  );
}
