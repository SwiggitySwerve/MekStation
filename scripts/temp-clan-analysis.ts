...
const indexData = JSON.parse(
  fs.readFileSync(
    path.resolve(process.cwd(), 'public/data/units/battlemechs/index.json'),
    'utf-8',
  ),
);
...
const minorDisc = report.allResults.filter(
  (r: Result) => r.rootCause === 'minor-discrepancy',
);
...
console.log(
  `  Under-calculated: ${underCalc.length} (avg ${(underCalc.reduce((s: number, r: Result) => s + r.percentDiff, 0) / underCalc.length).toFixed(2)}%)`,
);
console.log(
  `  Over-calculated: ${overCalc.length} (avg ${(overCalc.reduce((s: number, r: Result) => s + r.percentDiff, 0) / overCalc.length).toFixed(2)}%)`,
);
...
for (const [b, items] of Object.entries(buckets).sort((a, b) => {
  ...
  const tbStr = Object.entries(techBases)
    .map(([t, c]) => `${t}:${c}`)
    .join(' ');
  console.log(
    `  ${b.padEnd(12)} ${String(items.length).padStart(4)} units  ${tbStr}`,
  );
}

const within5 = report.allResults.filter(
  (r: Result) =>
...
for (const r of underBy2to5.slice(0, 10)) {
  ...
  const hasAmmo =
    ud.criticalSlots &&
    Object.values(ud.criticalSlots).some(
      (slots: any) =>
        Array.isArray(slots) &&
        slots.some(
          (s: any) =>
  ...
  const hasMGA = ud.equipment?.some(
    (eq: any) =>
      eq.id.toLowerCase().includes('machine-gun-array') ||
      eq.id.toLowerCase().includes('mga'),
  );
  const hasAES =
    ud.criticalSlots &&
    Object.values(ud.criticalSlots).some(
      (slots: any) =>
        ...
        slots.some(
          (s: any) =>
            s && typeof s === 'string' && s.toLowerCase().includes('aes'),
        ),
    );
  console.log(
    `  ${(r.chassis + ' ' + r.model).padEnd(40)} ${iu.techBase.padEnd(6)} BV%=${r.percentDiff.toFixed(1)}% idx=${r.indexBV} calc=${r.calculatedBV} ammo=${hasAmmo} mga=${hasMGA} aes=${hasAES}`,
  );
}
...
for (const r of report.allResults) {
        ...
        (slots: any) =>
          ...
          slots.some(
            (s: any) =>
              s &&
              typeof s === 'string' &&
              (s.toLowerCase().includes('machine gun array') ||
                s.toLowerCase().includes('mga')),
          ),
      ));
  if (hasMGA && r.percentDiff !== null) {
    ...
    if (Math.abs(r.percentDiff) > 1) {
      console.log(
        `  ${(r.chassis + ' ' + r.model).padEnd(40)} ${iu.techBase.padEnd(6)} BV%=${r.percentDiff.toFixed(1)}% idx=${r.indexBV} calc=${r.calculatedBV}`,
      );
    }
  }
}
...
for (const r of report.allResults) {
  ...
  const hasAES =
    ...
    ud.equipment?.some((eq: any) => eq.id.toLowerCase().includes('aes'));
