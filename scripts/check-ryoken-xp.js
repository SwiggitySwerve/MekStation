...
for (const id of targets) {
  ...
  console.log(
    `  Equipment: ${data.equipment.map((e) => e.id + '@' + e.location).join(', ')}`,
  );
  console.log(
    `  OFF: wBV=${b.rawWeaponBV} halved=${b.halvedWeaponBV} aBV=${b.ammoBV} wt=${b.weightBonus} sf=${b.speedFactor}`,
  );
  console.log(
    `  DEF: armor=${b.armorBV} struct=${b.structureBV} gyro=${b.gyroBV} defEq=${b.defEquipBV} expl=${b.explosivePenalty} df=${b.defensiveFactor}`,
  );
  console.log(
    `  HEAT: eff=${b.heatEfficiency} diss=${b.heatDissipation} moveHeat=${b.moveHeat} cockpit=${b.cockpitType}`,
  );
  console.log(
    `  Tech: ${b.techBase}, tonnage=${data.tonnage}, movement=${JSON.stringify(data.movement)}`,
  );

  // Check for TC in crits
  const crits = JSON.stringify(data.criticalSlots || {}).toLowerCase();
  if (crits.includes('targeting') || crits.includes('targcomp'))
    console.log('  HAS TC');
...
for (const u of artUnder) {
  ...
  const b = u.breakdown || {};
  console.log(
    `${u.chassis} ${u.model}: ${u.percentDiff.toFixed(2)}% ref=${u.indexBV} calc=${u.calculatedBV} gap=${u.difference} tech=${b.techBase}`,
  );
  // Count artemis slots
  const crits = JSON.stringify(data.criticalSlots || {}).toLowerCase();
