...
for (const u of outside1) {
  ...
  if (expectedWalk !== actualWalk && expectedWalk > 0) {
    console.log(
      `WALK MISMATCH: ${u.chassis} ${u.model} (${u.percentDiff.toFixed(2)}%): data.walk=${actualWalk} expected=${expectedWalk} (rating=${data.engine?.rating} tonnage=${data.tonnage})`,
    );
  }
  ...
  if (jump > 0 && jump > actualWalk * 2) {
    console.log(
      `HIGH JUMP: ${u.chassis} ${u.model} (${u.percentDiff.toFixed(2)}%): jump=${jump} walk=${actualWalk}`,
    );
  }
  if (hasJJ && jump === 0) {
    console.log(
      `JJ BUT NO JUMP: ${u.chassis} ${u.model} (${u.percentDiff.toFixed(2)}%)`,
    );
  }
}
...
for (const u of outside1) {
  ...
  if (Math.abs(actualGyro - expectedGyro) > 0.1) {
    console.log(
      `GYRO MISMATCH: ${u.chassis} ${u.model} (${u.percentDiff.toFixed(2)}%): gyroBV=${actualGyro} expected=${expectedGyro} gyroType=${gyroType} tonnage=${data.tonnage}`,
    );
  }
}
...
for (const u of outside1) {
  ...
  if (actualDiss > 0 && Math.abs(actualDiss - expectedDiss) > 3) {
    console.log(
      `HS MISMATCH: ${u.chassis} ${u.model} (${u.percentDiff.toFixed(2)}%): diss=${actualDiss} expected=${expectedDiss} count=${hsCount} type=${data.heatSinks?.type}`,
    );
  }
}
