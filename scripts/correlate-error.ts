...
console.log(
  '(Positive correlation = component associated with overcalculation)',
);
for (const [name, values] of Object.entries(components)) {
  const r = pearson(values, errors);
  const rPct = pearson(values, pctErrors);
  console.log(
    `  ${name.padEnd(25)} r(abs)=${r.toFixed(3).padStart(7)}  r(pct)=${rPct.toFixed(3).padStart(7)}`,
  );
}
...
console.log(
  `r(pctError, totalBV) = ${pearson(pctErrors, totalBVs).toFixed(4)}`,
);

// Check: do units with more heat-halved weapons have larger errors?
console.log('\n=== HALVED WEAPONS IMPACT ===');
const noHalved = valid.filter(
  (x: any) => (x.breakdown.halvedWeaponCount || 0) === 0,
);
const hasHalved = valid.filter(
  (x: any) => (x.breakdown.halvedWeaponCount || 0) > 0,
);
const avgErrNoHalved =
  noHalved.reduce((s: number, x: any) => s + x.percentDiff, 0) /
  noHalved.length;
const avgErrHasHalved =
  hasHalved.reduce((s: number, x: any) => s + x.percentDiff, 0) /
  hasHalved.length;
const outlierRateNoHalved =
  noHalved.filter((x: any) => Math.abs(x.percentDiff) > 1).length /
  noHalved.length;
const outlierRateHasHalved =
  hasHalved.filter((x: any) => Math.abs(x.percentDiff) > 1).length /
  hasHalved.length;
console.log(
  `No halved: ${noHalved.length} units, avg err=${avgErrNoHalved.toFixed(3)}%, outlier rate=${(outlierRateNoHalved * 100).toFixed(1)}%`,
);
console.log(
  `Has halved: ${hasHalved.length} units, avg err=${avgErrHasHalved.toFixed(3)}%, outlier rate=${(outlierRateHasHalved * 100).toFixed(1)}%`,
);
...
for (const band of heBands) {
  const inBand = valid.filter(
    (x: any) =>
      (x.breakdown.heatEfficiency || 0) >= band.min &&
      (x.breakdown.heatEfficiency || 0) <= band.max,
  );
  if (inBand.length === 0) continue;
  const avgErr =
    inBand.reduce((s: number, x: any) => s + x.percentDiff, 0) / inBand.length;
  const outlierRate =
    inBand.filter((x: any) => Math.abs(x.percentDiff) > 1).length /
    inBand.length;
  console.log(
    `  ${band.label.padEnd(10)} ${inBand.length} units, avg err=${avgErr.toFixed(3).padStart(7)}%, outlier rate=${(outlierRate * 100).toFixed(1)}%`,
  );
}
...
const outliers = valid.filter(
  (x: any) => Math.abs(x.percentDiff) > 1 && Math.abs(x.percentDiff) <= 3,
);
for (const dir of ['over', 'under'] as const) {
  const subset = outliers.filter((x: any) =>
    dir === 'over' ? x.percentDiff > 0 : x.percentDiff < 0,
  );
  const avgDefFrac =
    subset.reduce((s: number, x: any) => {
      const d = x.breakdown.defensiveBV || 0;
      const o = x.breakdown.offensiveBV || 0;
      return s + (d + o > 0 ? d / (d + o) : 0);
    }, 0) / subset.length;
  console.log(
    `  ${dir}: ${subset.length} units, avg DEF/(DEF+OFF)=${avgDefFrac.toFixed(3)}`,
  );
}
