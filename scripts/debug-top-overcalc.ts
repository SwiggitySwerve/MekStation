...
for (const t of targets) {
  ...
  console.log(
    `  MUL BV: ${mulEntry?.mulBV} (match: ${mulEntry?.matchType}, name: ${mulEntry?.mulName})`,
  );
  ...
  const sameChassisVariants = index.units.filter(
    (u: any) => u.chassis === match.chassis,
  );
  ...
  console.log(
    `  Variants with same BV (${match.bv}): ${sharedBVCount} of ${sameChassisVariants.length} total`,
  );
}
