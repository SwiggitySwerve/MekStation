...
const underCalc = report.allResults
  .filter((r: any) => r.percentDiff < -1.0 && r.percentDiff > -5.0)
...
const avgGapPerSF =
  analyses.reduce((s, a) => s + a.gapPerSF, 0) / analyses.length;
console.log(`Average gap: ${avgGap.toFixed(1)} BV`);
console.log(`Average gap/speedFactor: ${avgGapPerSF.toFixed(1)} BV`);
console.log(
  `Average speedFactor: ${(analyses.reduce((s, a) => s + a.speedFactor, 0) / analyses.length).toFixed(3)}`,
);
...
for (const [bin, data] of Object.entries(tonBins).sort()) {
  ...
  const avgTon =
    data.tonnages.reduce((s, t) => s + t, 0) / data.tonnages.length;
  console.log(
    `  ${bin.padEnd(10)} n=${String(data.gaps.length).padStart(3)} avgGap=${avgGap.toFixed(1).padStart(7)} avgTon=${avgTon.toFixed(0).padStart(4)}`,
  );
}
...
for (const bin of wBins) {
  const pool = analyses.filter(
    (a) => a.weaponBV >= bin.min && a.weaponBV < bin.max,
  );
  if (pool.length > 0) {
    const avgGap = pool.reduce((s, a) => s + a.gap, 0) / pool.length;
    const avgPct = pool.reduce((s, a) => s + a.pctDiff, 0) / pool.length;
    console.log(
      `  ${bin.label.padEnd(18)} n=${String(pool.length).padStart(3)} avgGap=${avgGap.toFixed(1).padStart(7)} avgPct=${avgPct.toFixed(2).padStart(7)}%`,
    );
  }
}
...
const medianGapPerTon = gapPerTon.sort((a, b) => a - b)[
  Math.floor(gapPerTon.length / 2)
];
...
for (const bin of sfBins) {
  const pool = analyses.filter(
    (a) => a.speedFactor >= bin.min && a.speedFactor < bin.max,
  );
  if (pool.length > 0) {
    const avgRatio =
      pool.reduce((s, a) => s + a.gap / a.tonnage, 0) / pool.length;
    const avgGap = pool.reduce((s, a) => s + a.gap, 0) / pool.length;
    const avgSF = pool.reduce((s, a) => s + a.speedFactor, 0) / pool.length;
    console.log(
      `  ${bin.label.padEnd(14)} n=${String(pool.length).padStart(3)} avgGap/ton=${avgRatio.toFixed(3).padStart(7)} avgGap=${avgGap.toFixed(1).padStart(7)} avgSF=${avgSF.toFixed(3)}`,
    );
  }
}
...
const avgGapDefRatio =
  gapDefRatio.reduce((s, r) => s + r, 0) / gapDefRatio.length;
console.log(
  `  Avg gap/defBV: ${avgGapDefRatio.toFixed(4)} (${(avgGapDefRatio * 100).toFixed(2)}%)`,
);

// Check: is the gap exactly = some fraction of offBV?
const gapOffRatio = analyses.map((a) => a.gap / a.offBV);
const avgGapOffRatio =
  gapOffRatio.reduce((s, r) => s + r, 0) / gapOffRatio.length;
console.log(
  `  Avg gap/offBV: ${avgGapOffRatio.toFixed(4)} (${(avgGapOffRatio * 100).toFixed(2)}%)`,
);
...
for (const a of traceUnits) {
  ...
  console.log(`\n${'='.repeat(60)}`);
  console.log(
    `${a.unitId} (${u.techBase}, ${u.tonnage}t, gap=${a.gap}, ${a.pctDiff.toFixed(2)}%)`,
  );
  ...
  console.log(
    `  Movement: walk=${u.movement?.walk} jump=${u.movement?.jump || 0}`,
  );
  console.log(`  HeatSinks: ${u.heatSinks?.type} ${u.heatSinks?.count}`);
  console.log(`  defBV=${a.defBV.toFixed(2)} offBV=${a.offBV.toFixed(2)}`);
  console.log(
    `  weaponBV=${a.weaponBV.toFixed(2)} ammoBV=${a.ammoBV.toFixed(2)}`,
  );
  console.log(`  speedFactor=${a.speedFactor.toFixed(4)}`);
  console.log(
    `  defEquipBV=${a.defensiveEquipBV} explosivePen=${a.explosivePen}`,
  );
  ...
  for (const [loc, slots] of Object.entries(u.criticalSlots || {})) {
    ...
    const filled = (slots as any[]).filter(
  ...
  const allSlots = Object.values(allCrits)
    .flat()
    .map((s) => s.toLowerCase());
  const defEquip = allSlots.filter(
    (s) =>
      s.includes('ams') ||
      s.includes('ecm') ||
      s.includes('probe') ||
      ...
      s.includes('narc') ||
      s.includes('shield') ||
      s.includes('blue-shield') ||
      s.includes('armored'),
  );
  if (defEquip.length > 0) {
    console.log(
      `  Def equipment in crits: ${[...new Set(defEquip)].join(', ')}`,
    );
  }

  // Check for offensive equipment we might be missing
  const offEquip = allSlots.filter(
    (s) =>
      s.includes('targeting computer') ||
      s.includes('apollo') ||
      s.includes('artemis'),
  );
  if (offEquip.length > 0) {
    console.log(
      `  Off equipment in crits: ${[...new Set(offEquip)].join(', ')}`,
    );
  }
}
