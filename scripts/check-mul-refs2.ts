...
for (const uid of underIds) {
  ...
  if (entry) {
    console.log(
      `  Cache: name="${entry.mulName}" bv=${entry.mulBV} match=${entry.matchType}`,
    );
  } else {
  ...
  if (res) {
    console.log(
      `  Result: indexBV=${res.indexBV} calc=${res.calculatedBV} diff=${res.difference}`,
    );
  }
}
...
console.log(
  `  Total: ${exactMatch + fuzzyAccepted + fuzzyRejected + notFound + noEntry}`,
);
...
for (const v of validated) {
  ...
  if (
    ...
    (entry.matchType === 'exact' ||
      (entry.matchType === 'fuzzy' && entry.mulBV > 0))
  ) {
    withMul++;
  } else {
    withoutMul++;
