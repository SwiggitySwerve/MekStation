...
const under = results.filter(
  (r) => r.pct < -1 && r.pct >= -5 && (r.ammoBV || 0) === 0,
);
...
for (const r of under.slice(0, 25)) {
  ...
  const allCrits = Object.values(crits)
    .flat()
    .filter((s): s is string => !!s && typeof s === 'string');

  // Check for flamer (which is a weapon but some might be filtered)
  const hasFlamer = allCrits.some((s: string) =>
    s.toLowerCase().includes('flamer'),
  );
  const hasTag = allEquip.some((eq: any) =>
    eq.id.toLowerCase().includes('tag'),
  );
  const hasNarc = allEquip.some((eq: any) =>
  ...
  const weaponEquip = allEquip.filter((eq: any) => {
    ...
    return (
      ...
      !lo.includes('umu') &&
      !lo.includes('cargo') &&
      !lo.includes('searchlight') &&
      !lo.includes('improved-sensors') &&
  ...
  const critWeapons = allCrits.filter((s: string) => {
    ...
    return (
      ...
      !lo.includes('shield') &&
      !lo.includes('ballistic-reinforced') &&
  ...
  console.log(
    `    Tech: ${ud.techBase} | ${weaponEquip.length} equip weapons | totalWeapBV=${totalWeapBV}`,
  );
