...
const under2to5 = valid.filter(
  (x: any) => x.percentDiff < -2 && x.percentDiff >= -5 && x.breakdown,
);
...
for (const u of under1to2
  ...
  for (const eq of unit.equipment || []) {
    ...
    if (unit.techBase === 'MIXED' || unit.techBase === 'CLAN') {
      ...
      if (clanRes.resolved && clanRes.battleValue !== res.battleValue) {
        clanWeapons.push(
          `${eq.id}(is=${res.battleValue} clan=${clanRes.battleValue})`,
        );
      }
    }
  }
  ...
  for (const [loc, slots] of Object.entries(unit.criticalSlots || {})) {
    ...
    for (const s of slots as string[]) {
      ...
      if (lo.includes('ammo') && !lo.includes('case')) {
        ...
        if (!res.resolved)
          unresolvedAmmo.push(s.replace(/\s*\(omnipod\)/gi, '').trim());
      }
    }
  }
  ...
  console.log(
    `${u.unitId.padEnd(45)} ${u.percentDiff.toFixed(1)}% diff=${u.difference} ${side} defD=${defDiff.toFixed(0)} offD=${offDiff.toFixed(0)} tech=${unit.techBase} halved=${b.halvedWeaponCount}/${b.weaponCount}`,
  );
  if (unresolved.length > 0)
    console.log(`  UNRESOLVED: ${unresolved.join(', ')}`);
  if (clanWeapons.length > 0)
    console.log(`  CLAN-DIFF: ${clanWeapons.join(', ')}`);
  if (unresolvedAmmo.length > 0)
    console.log(`  UNRESOLVED-AMMO: ${unresolvedAmmo.join(', ')}`);
}
...
for (const u of under2to5
  ...
  const side = defDiff < offDiff ? 'DEF-LOW' : 'OFF-LOW';
  console.log(
    `${u.unitId.padEnd(45)} ${u.percentDiff.toFixed(1)}% diff=${u.difference} ${side} tech=${unit.techBase} halved=${b.halvedWeaponCount}/${b.weaponCount}`,
  );
  if (unresolved.length > 0)
    console.log(`  UNRESOLVED: ${unresolved.join(', ')}`);
  if (unresolvedAmmo.length > 0)
