...
for (const u of mixed.slice(0, 12)) {
  ...
  console.log(
    `  idx=${u.indexBV} calc=${u.calculatedBV} defBV=${b.defensiveBV.toFixed(1)} offBV=${b.offensiveBV.toFixed(1)} wBV=${b.weaponBV} ammoBV=${b.ammoBV} sf=${b.speedFactor}`,
  );
  ...
  for (const eq of data.equipment) {
    ...
    const marker =
      delta !== 0
        ? ` [CL:bv=${resClan.battleValue}/h=${resClan.heat} delta=${delta}]`
        : '';
    console.log(
      `  ${eq.id.padEnd(35)} @${eq.location.padEnd(15)} bv=${resBase.battleValue}/h=${resBase.heat}${marker}`,
    );
  }
    ...
    for (const [loc, slots] of Object.entries(data.criticalSlots)) {
      for (const s of slots as any[]) {
        ...
        if (
          (sl.startsWith('cl') &&
            !sl.includes('case') &&
            !sl.includes('double heat sink') &&
            !sl.includes('heat sink') &&
            !sl.includes('-empty-')) ||
          sl.includes('(clan)') ||
          sl.includes('clan ')
        ) {
    ...
    for (const [loc, slots] of Object.entries(data.criticalSlots)) {
      for (const s of slots as any[]) {
        ...
        if (
          ...
          sl.includes('engine') ||
          sl.includes('gyro') ||
          sl.includes('life support') ||
          sl.includes('sensors') ||
          sl.includes('cockpit') ||
          sl.includes('shoulder') ||
          sl.includes('upper arm') ||
          sl.includes('lower arm') ||
          sl.includes('hand') ||
          sl.includes('hip') ||
          sl.includes('upper leg') ||
          sl.includes('lower leg') ||
          sl.includes('foot') ||
          sl.includes('armor') ||
          sl.includes('structure') ||
          sl.includes('actuator')
        )
          continue;
