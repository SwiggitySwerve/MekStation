...
for (const r of pmp) {
  ...
  if (!mulEntry || mulEntry.matchType === 'not-found') {
    ...
    if (
      ...
      !mulEntry.mulName
        .toLowerCase()
        .includes(r.name.split(' ').pop()?.toLowerCase() || '')
    ) {
      fuzzyWrongName++;
    }
...
console.log(
  `  Exact match: ${exactMatch} (diff BV from ref: ${exactMatchWithDiffBV})`,
);
console.log(`  Fuzzy match: ${fuzzyMatch} (zero BV: ${fuzzyZeroBV})`);
console.log(`  Not found: ${notFound}`);
console.log(
  `  Total unreliable ref BV (fuzzy+not-found): ${fuzzyMatch + notFound}`,
);
console.log(
  `  % unreliable: ${(((fuzzyMatch + notFound) / pmp.length) * 100).toFixed(1)}%`,
);
...
for (const r of exactPMP) {
  const mulEntry = mulCache.entries?.[r.id];
  console.log(
    `  ${r.name.padEnd(40)} ref=${r.ref} calc=${r.calc} ${r.pct >= 0 ? '+' : ''}${r.pct}% mulBV=${mulEntry.mulBV} mulName=${mulEntry.mulName}`,
  );
}

// Also count all categories with exact MUL matches
console.log(`\n=== EXACT MUL MATCH breakdown by cause ===`);
const causeCounts: Record<
  string,
  { total: number; exactMul: number; fuzzy: number; notFound: number }
> = {};
for (const r of results) {
  const cause = r.cause || 'none';
  if (!causeCounts[cause])
    causeCounts[cause] = { total: 0, exactMul: 0, fuzzy: 0, notFound: 0 };
  causeCounts[cause].total++;
  const mulEntry = mulCache.entries?.[r.id];
  if (mulEntry?.matchType === 'exact' && mulEntry.mulBV > 0)
    causeCounts[cause].exactMul++;
  else if (mulEntry?.matchType === 'fuzzy') causeCounts[cause].fuzzy++;
  else causeCounts[cause].notFound++;
}
for (const [cause, counts] of Object.entries(causeCounts).sort(
  (a, b) => b[1].total - a[1].total,
)) {
  console.log(
    `  ${cause.padEnd(30)} total=${String(counts.total).padStart(5)} exactMul=${String(counts.exactMul).padStart(5)} fuzzy=${String(counts.fuzzy).padStart(5)} notFound=${String(counts.notFound).padStart(5)}`,
  );
}
