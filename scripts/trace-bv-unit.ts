...
for (const r of under.slice(0, 15)) {
  ...
  console.log(
    `${r.chassis} ${r.model}`.padEnd(35) +
      ` ref=${r.indexBV} calc=${r.calculatedBV} diff=${diff} (${r.percentDiff.toFixed(2)}%)`,
  );
  console.log(
    `  defBV=${bd.defensiveBV?.toFixed(0) || '?'} offBV=${bd.offensiveBV?.toFixed(0) || '?'} weapBV=${bd.weaponBV?.toFixed(0) || '?'} ammoBV=${bd.ammoBV?.toFixed(0) || '?'} sf=${bd.speedFactor?.toFixed(3) || '?'}`,
  );
  console.log(
    `  explPen=${bd.explosivePenalty?.toFixed(0) || '?'} defEquipBV=${bd.defensiveEquipBV?.toFixed(0) || '?'}`,
  );
}
...
console.log('\n--- SYSTEMATIC ANALYSIS ---');
const underWithBD = under.filter(
  (r: any) => r.breakdown?.defensiveBV !== undefined,
);
if (underWithBD.length > 0) {
  ...
  const avgDeficitFrac =
    deficitFractions.reduce((s, f) => s + f, 0) / deficitFractions.length;
  console.log(
    `Avg deficit as fraction of ref BV: ${(avgDeficitFrac * 100).toFixed(2)}%`,
  );

  // Check if deficit correlates with tonnage
  const lightUnder = underWithBD.filter((r: any) => r.tonnage <= 35);
  const medUnder = underWithBD.filter(
    (r: any) => r.tonnage > 35 && r.tonnage <= 55,
  );
  const heavyUnder = underWithBD.filter(
    (r: any) => r.tonnage > 55 && r.tonnage <= 75,
  );
  const assaultUnder = underWithBD.filter((r: any) => r.tonnage > 75);

  const avgPctByWeight = (units: any[]) =>
    units.length > 0
      ? (
          units.reduce((s: number, r: any) => s + r.percentDiff, 0) /
          units.length
        ).toFixed(2)
      : 'n/a';
  console.log(
    `Light (≤35t): ${lightUnder.length} units, avg ${avgPctByWeight(lightUnder)}%`,
  );
  console.log(
    `Medium (36-55t): ${medUnder.length} units, avg ${avgPctByWeight(medUnder)}%`,
  );
  console.log(
    `Heavy (56-75t): ${heavyUnder.length} units, avg ${avgPctByWeight(heavyUnder)}%`,
  );
  console.log(
    `Assault (76+t): ${assaultUnder.length} units, avg ${avgPctByWeight(assaultUnder)}%`,
  );
}
