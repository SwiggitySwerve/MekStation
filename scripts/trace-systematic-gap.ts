...
console.log(
  `Analyzing ${undercalc.length} undercalculated units (1-5% below MUL)\n`,
);
...
console.log(
  `  mean=${defStats.mean.toFixed(2)}% median=${defStats.median.toFixed(2)}% stddev=${defStats.stddev.toFixed(2)}%`,
);
...
console.log(
  `  mean=${offStats.mean.toFixed(2)}% median=${offStats.median.toFixed(2)}% stddev=${offStats.stddev.toFixed(2)}%`,
);
...
console.log(
  `  mean=${totStats.mean.toFixed(2)}% median=${totStats.median.toFixed(2)}% stddev=${totStats.stddev.toFixed(2)}%`,
);
...
function linearRegression(
  ...
  const ssRes = ys.reduce(
    (s, v, i) => s + (v - (slope * xs[i] + intercept)) ** 2,
    0,
  );
...
console.log(
  `gap = ${regDef.slope.toFixed(4)} * defBV + ${regDef.intercept.toFixed(1)}  R²=${regDef.r2.toFixed(4)}`,
);

const regOff = linearRegression(offs, gaps);
console.log(
  `gap = ${regOff.slope.toFixed(4)} * offBV + ${regOff.intercept.toFixed(1)}  R²=${regOff.r2.toFixed(4)}`,
);

const regTotal = linearRegression(totals, gaps);
console.log(
  `gap = ${regTotal.slope.toFixed(4)} * totalBV + ${regTotal.intercept.toFixed(1)}  R²=${regTotal.r2.toFixed(4)}`,
);

const regWeapon = linearRegression(weapons, gaps);
console.log(
  `gap = ${regWeapon.slope.toFixed(4)} * weaponBV + ${regWeapon.intercept.toFixed(1)}  R²=${regWeapon.r2.toFixed(4)}`,
);

const regIndex = linearRegression(indexes, gaps);
console.log(
  `gap = ${regIndex.slope.toFixed(4)} * indexBV + ${regIndex.intercept.toFixed(1)}  R²=${regIndex.r2.toFixed(4)}`,
);
...
console.log(
  `k: mean=${kStats.mean.toFixed(4)} median=${kStats.median.toFixed(4)} stddev=${kStats.stddev.toFixed(4)}`,
);
console.log(
  `If we add ${(kStats.mean * 100).toFixed(1)}% to each unit's BV, how many come within 1%?`,
);
let within1 = 0;
...
console.log(
  `  ${within1} of ${analyses.length} (${((within1 / analyses.length) * 100).toFixed(1)}%)`,
);
...
for (const k of [0.01, 0.015, 0.02, 0.025, 0.03]) {
  ...
  console.log(
    `  k=${k}: ${w1} within 1% (${((w1 / analyses.length) * 100).toFixed(1)}%)`,
  );
}
...
console.log(
  `gap = ${regWS.slope.toFixed(4)} * (weaponBV*speedFactor) + ${regWS.intercept.toFixed(1)}  R²=${regWS.r2.toFixed(4)}`,
);
...
console.log(
  `Gap absolute: mean=${stats(gaps).mean.toFixed(1)} median=${stats(gaps).median.toFixed(1)} stddev=${stats(gaps).stddev.toFixed(1)} min=${stats(gaps).min} max=${stats(gaps).max}`,
);
...
for (const ton of [...byTonnage.keys()].sort((a, b) => a - b)) {
  ...
  if (pcts.length >= 3) {
    const s = stats(pcts);
    console.log(
      `  ${ton}t (n=${pcts.length}): mean=${s.mean.toFixed(2)}% median=${s.median.toFixed(2)}%`,
    );
  }
}
