...
function scanForDefensiveEquip(unit: UnitData): {
  ...
  for (const eq of unit.equipment || []) {
    ...
    if (lo.includes('null') && lo.includes('sig')) {
      result.hasNullSig = true;
      result.defEquipNames.push(eq.id);
    }
    if (lo.includes('void') && lo.includes('sig')) {
    ...
    if (lo.includes('ecm') || lo.includes('guardian')) {
      result.hasECM = true;
      result.defEquipNames.push(eq.id);
    }
  }
  ...
  if (unit.criticalSlots) {
    for (const [loc, slots] of Object.entries(unit.criticalSlots)) {
      for (const slot of slots) {
        ...
        if (lo.includes('stealth')) {
          result.hasStealth = true;
        }
        if (lo.includes('ecm') || lo.includes('guardian')) {
          result.hasECM = true;
        }
...
function main() {
  ...
  const highDefUnits = report.allResults.filter(
    (r) =>
      r.breakdown &&
      r.breakdown.defensiveFactor !== undefined &&
      r.breakdown.defensiveFactor >= 1.7,
  );

  console.log('='.repeat(100));
  console.log(
    `TRACE: Units with defensiveFactor >= 1.7 (i.e., TMM >= 7 for defensive calc)`,
  );
  ...
  for (const result of highDefUnits) {
    ...
    console.log(
      `  Calculated BV: ${result.calculatedBV}  |  Reference BV: ${result.indexBV}  |  Gap: ${result.difference} (${result.percentDiff?.toFixed(2)}%)`,
    );
    ...
    console.log(
      `  Engine: ${unit!.engine.type} (rating ${unit!.engine.rating})`,
    );
    console.log(`  Gyro: ${unit!.gyro.type}`);
    console.log(`  Armor: ${unit!.armor.type}`);
    console.log(`  Structure: ${unit!.structure.type}`);
    console.log(
      `  Movement: walk=${unit!.movement.walk}, jump=${unit!.movement.jump}`,
    );
    console.log(
      `  Heat Sinks: ${unit!.heatSinks.count} ${unit!.heatSinks.type}`,
    );
    ...
    if (scan.hasStealth) {
      tmmAdj += 2;
      tmmMods.push('stealth: +2');
    }
    if (scan.hasNullSig) {
      tmmAdj += 2;
      tmmMods.push('null-sig: +2');
    }
    if (scan.hasChameleon) {
      tmmAdj += 2;
      tmmMods.push('chameleon: +2');
    }
    if (scan.hasVoidSig) {
      if (tmmAdj < 3) {
        tmmMods.push(`void-sig: set to 3 (was ${tmmAdj})`);
        tmmAdj = 3;
      } else if (tmmAdj === 3) {
        tmmAdj++;
        tmmMods.push('void-sig: +1 (was 3)');
      } else {
        tmmMods.push(`void-sig: no effect (TMM already ${tmmAdj})`);
      }
    }
    const expectedDefFactor = 1 + tmmAdj / 10.0;

    console.log(
      `  TMM modifiers: ${tmmMods.length > 0 ? tmmMods.join(', ') : 'none'}`,
    );
    ...
    console.log(
      `  Check: baseDef * defF = ${(baseDef * bd.defensiveFactor!).toFixed(2)}`,
    );
    ...
    console.log(
      `  Current gap: ${gap} (calc=${result.calculatedBV}, ref=${result.indexBV})`,
    );
    ...
    console.log(
      `  If no stealth/null/chameleon/void TMM bonus (defF=${defFactorWithoutSpecial.toFixed(1)}): defBV=${defBVWithoutSpecial.toFixed(1)}, total=${Math.round(totalBVWithoutSpecial)}`,
    );
    ...
    console.log(
      `  If defF=${defFactorMinus1.toFixed(1)}: defBV=${defBVMinus1.toFixed(1)}, total=${Math.round(totalBVMinus1)}, gap=${Math.round(totalBVMinus1) - result.indexBV}`,
    );
    ...
    console.log(
      `  If defF=${defFactorMinus2.toFixed(1)}: defBV=${defBVMinus2.toFixed(1)}, total=${Math.round(totalBVMinus2)}, gap=${Math.round(totalBVMinus2) - result.indexBV}`,
    );
    ...
    if (baseDef > 0) {
      ...
      console.log(
        `  Ideal defF to match reference BV: ${idealDefF.toFixed(4)} (TMM ~${((idealDefF - 1) * 10).toFixed(1)})`,
      );
    }
    ...
    if (Math.abs(checkDefBV - bd.defensiveBV) > 0.01) {
      console.log(
        `  WARNING: baseDef * defF = ${checkDefBV.toFixed(2)} != defensiveBV = ${bd.defensiveBV.toFixed(2)} (rounding or extra components?)`,
      );
    }
    ...
    console.log(
      `  If defensive is correct, ideal offensiveBV = ${idealOffBV.toFixed(1)} (actual: ${bd.offensiveBV.toFixed(1)}, diff: ${(idealOffBV - bd.offensiveBV).toFixed(1)})`,
    );
    ...
    if (result.calculatedBV && cockpitMod !== 1.0) {
      console.log(
        `  Pre-cockpit BV (calc): ${(result.calculatedBV / cockpitMod).toFixed(1)}`,
      );
      console.log(
        `  Pre-cockpit BV (ref): ${(result.indexBV / cockpitMod).toFixed(1)}`,
      );
    }
  }
  ...
  console.log('='.repeat(100));
  console.log(
    `Total units with defensiveFactor >= 1.7: ${highDefUnits.length}`,
  );
  ...
  if (undercalc.length > 0) {
    const avgGap =
      undercalc.reduce((s, r) => s + (r.difference || 0), 0) / undercalc.length;
    console.log(`  Average undercalc gap: ${avgGap.toFixed(1)}`);
  }

  // Aggregate patterns
  const defFactors = new Map<
    number,
    { count: number; avgGap: number; units: string[] }
  >();
  for (const r of highDefUnits) {
    const df = r.breakdown!.defensiveFactor!;
    if (!defFactors.has(df))
      defFactors.set(df, { count: 0, avgGap: 0, units: [] });
  ...
  console.log('\n  By defensiveFactor:');
  for (const [df, data] of [...defFactors.entries()].sort(
    (a, b) => a[0] - b[0],
  )) {
    console.log(
      `    defF=${df}: ${data.count} units, avg gap=${(data.avgGap / data.count).toFixed(1)}, units: ${data.units.join(', ')}`,
    );
  }
