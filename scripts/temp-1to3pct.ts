...
const target = report.allResults.filter(
  (r: Result) =>
    r.status !== 'error' &&
    r.percentDiff !== null &&
    Math.abs(r.percentDiff) > 1 &&
    Math.abs(r.percentDiff) <= 3,
);
...
for (const r of under) {
  ...
  for (const eq of ud.equipment || []) {
    ...
    if (!weaponCorrelation[lo])
      weaponCorrelation[lo] = { count: 0, totalDiff: 0 };
...
for (const [name, data] of weaponsSorted) {
  console.log(
    `  ${name.padEnd(40)} ${String(data.count).padStart(4)} units  avg=${(data.totalDiff / data.count).toFixed(2)}%`,
  );
}
...
for (const r of under) {
  ...
  const offPct =
    r.breakdown.offensiveBV /
    (r.breakdown.defensiveBV + r.breakdown.offensiveBV);
...
for (const [ton, diffs] of Object.entries(diffByTonnage).sort(
  (a, b) => Number(a[0]) - Number(b[0]),
)) {
  if (diffs.length < 3) continue;
  const avg = diffs.reduce((s, v) => s + v, 0) / diffs.length;
  console.log(
    `  ${ton}t: ${diffs.length} units, avg diff=${avg.toFixed(0)} BV`,
  );
}
