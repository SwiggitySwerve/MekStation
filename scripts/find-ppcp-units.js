...
for (const [id, data] of unitMap) {
  ...
  if (
    eqs.some((e) => e.includes('ppcp') || e.includes('primitive-prototype'))
  ) {
    ...
    console.log(
      id +
        ': ' +
        eqs
          .filter((e) => e.includes('ppcp') || e.includes('primitive'))
          .join(', ') +
        ' => ' +
        (u ? u.percentDiff.toFixed(2) + '%' : 'N/A'),
    );
  }
}
...
console.log(
  '\n--- Units at exactly 0% (exact match) count:',
  r.allResults.filter((u) => u.difference === 0).length,
);
console.log(
  '--- Units between 0-1% (within 1%):',
  r.allResults.filter(
    (u) => Math.abs(u.percentDiff) > 0 && Math.abs(u.percentDiff) <= 1,
  ).length,
);
console.log(
  '--- Units outside 1%:',
  r.allResults.filter((u) => Math.abs(u.percentDiff) > 1).length,
);
