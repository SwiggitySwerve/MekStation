...
const underc = results
  .filter((r: any) => r.cause === 'undercalculation')
  .sort((a: any, b: any) => a.pct - b.pct);
const missingAmmo = results
  .filter((r: any) => r.cause === 'missing-ammo-bv')
  .sort((a: any, b: any) => a.pct - b.pct);

console.log(`=== UNDERCALCULATION (${underc.length}) - top 20 ===`);
for (const r of underc.slice(0, 20)) {
  console.log(
    `  ${r.name.padEnd(40)} ref=${String(r.ref).padStart(5)} calc=${String(r.calc).padStart(5)} ${r.pct}% sf=${r.sf} weapBV=${r.weapBV} ammoBV=${r.ammoBV}`,
  );
}
...
const overc = results
  .filter((r: any) => r.cause === 'overcalculation')
  .sort((a: any, b: any) => b.pct - a.pct);
console.log(`\n=== OVERCALCULATION (${overc.length}) - top 15 ===`);
for (const r of overc.slice(0, 15)) {
  console.log(
    `  ${r.name.padEnd(40)} ref=${String(r.ref).padStart(5)} calc=${String(r.calc).padStart(5)} +${r.pct}% sf=${r.sf} weapBV=${r.weapBV} ammoBV=${r.ammoBV} defBV=${Math.round(r.defBV)}`,
  );
}
