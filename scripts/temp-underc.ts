...
const underCalc = report.allResults.filter(
  (r: any) => r.rootCause === 'undercalculation',
);
...
for (const r of sorted.slice(0, 20)) {
  ...
  console.log(
    `  ${(r.chassis + ' ' + r.model).padEnd(35)} idx=${r.indexBV} calc=${r.calculatedBV} diff=${r.difference} (${r.percentDiff.toFixed(1)}%) tech=${ud.techBase} ammo=${bd?.ammoBV?.toFixed(0)} hasAmmo=${hasAmmo} wep=${bd?.weaponBV?.toFixed(0)}`,
  );
}

console.log('\nMild undercalculation (-5% to -10%):');
const mild = sorted.filter(
  (r: any) => r.percentDiff >= -10 && r.percentDiff < -5,
);
...
for (const r of mild.slice(0, 15)) {
  ...
  const bd = r.breakdown;
  console.log(
    `  ${(r.chassis + ' ' + r.model).padEnd(35)} idx=${r.indexBV} calc=${r.calculatedBV} diff=${r.difference} (${r.percentDiff.toFixed(1)}%) tech=${ud.techBase} def=${bd?.defensiveBV?.toFixed(0)} off=${bd?.offensiveBV?.toFixed(0)} ammo=${bd?.ammoBV?.toFixed(0)}`,
  );
}
