...
const near = r.allResults.filter(
  (u) =>
    Math.abs(u.percentDiff) >= 1.0 &&
    Math.abs(u.percentDiff) < 2.0 &&
    u.breakdown,
);
...
console.log(
  ...
  under.length,
  '— Need avg',
  (under.reduce((s, u) => s - u.difference, 0) / under.length).toFixed(1),
  'more BV per unit',
);
console.log(
...
for (const u of under.sort((a, b) => a.percentDiff - b.percentDiff)) {
  ...
  console.log(
    u.unitId.padEnd(40) +
      ' need+' +
      String(needed).padStart(3) +
      'BV offDelta=+' +
      offDelta.toFixed(1).padStart(5) +
      ' defDelta=+' +
      defDelta.toFixed(1).padStart(5) +
      ' SF=' +
      sf +
      ' DF=' +
      df,
...
console.log(
  ...
  over.length,
  '— Need avg',
  (over.reduce((s, u) => s + u.difference, 0) / over.length).toFixed(1),
  'less BV per unit',
);
