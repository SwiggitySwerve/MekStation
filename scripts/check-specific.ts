...
for (const uid of targets) {
  ...
  if (r) {
    const b = r.breakdown;
    console.log(
      `${uid}: calc=${r.calculatedBV} ref=${r.indexBV} diff=${r.difference} pct=${r.percentDiff?.toFixed(1)}% weapBV=${b?.weaponBV?.toFixed(1)} halved=${b?.halvedWeaponCount}/${b?.weaponCount}`,
    );
  }
}
...
for (const [label, min, max] of [
  ...
  const uc = under.filter(
    (x: any) => Math.abs(x.percentDiff) >= min && Math.abs(x.percentDiff) < max,
  ).length;
...
console.log(
  `Fixed by removing all small cockpit detection: ${fixedByNoSmallCockpit}`,
);
...
console.log(
  `Overcalculated fixed by applying 0.95 cockpit: ${fixedByCockpit095}`,
);
