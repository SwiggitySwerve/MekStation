...
for (const id of targets) {
  ...
  console.log(
    `MUL=${u.indexBV} Calc=${u.calculatedBV} Gap=${u.difference} (${(u.percentDiff || 0).toFixed(2)}%)`,
  );
  ...
  console.log(
    `  totalIS=${tIS} armorPts=${totalArmorPts} tonnage=${unit.tonnage}`,
  );
  console.log(
    `  Reported: armorBV=${b.armorBV} structBV=${b.structureBV} gyroBV=${b.gyroBV} defEq=${b.defEquipBV}`,
  );
  console.log(
    `  explosive=${b.explosivePenalty} armoredComp=${b.armoredComponentBV} harjel=${b.harjelBonus} amsAmmo=${b.amsAmmoBV}`,
  );
  ...
  console.log(
    `  Recomputed defTotal=${defTotal.toFixed(1)} (reported=${b.defensiveBV})`,
  );

  // Verify offensive components
  console.log(`\n--- OFFENSIVE VERIFICATION ---`);
  console.log(
    `  weaponBV=${b.weaponBV} (raw=${b.rawWeaponBV} halved=${b.halvedWeaponBV} count=${b.halvedWeaponCount})`,
  );
  console.log(
    `  ammoBV=${b.ammoBV} physBV=${b.physicalWeaponBV} weightBonus=${b.weightBonus} offEq=${b.offEquipBV}`,
  );
  console.log(
    `  heatEff=${b.heatEfficiency} heatDiss=${b.heatDissipation} moveHeat=${b.moveHeat}`,
  );
  console.log(
    `  SF=${b.speedFactor} walk=${b.walkMP} run=${b.runMP} jump=${b.jumpMP}`,
  );
  ...
  console.log(
    `  Recomputed offTotal=${offTotal.toFixed(1)} (reported=${b.offensiveBV})`,
  );

  const recomputedBV = Math.round(
    (defTotal + offTotal) * (b.cockpitModifier || 1),
  );
  console.log(
  ...
  const crits = unit.criticalSlots
    ...
    : [];
  const special = crits.filter(
    (s) =>
      !/^(engine|endo|ferro|fusion|gyro|hip|upper|lower|hand|foot|shoulder|heat|life|sensors|cockpit|double|standard|xl|compact)/i.test(
        s.replace(/ /g, ''),
      ),
  );
