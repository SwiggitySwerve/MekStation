...
for (const [id, data] of unitMap) {
  ...
  const hasArt4 = crits.some(
    (s) =>
      s.toLowerCase().includes('artemis iv') ||
      s.toLowerCase().includes('artemisiv'),
  );
  const hasArt5 = crits.some(
    (s) =>
      s.toLowerCase().includes('artemis v') ||
      s.toLowerCase().includes('artemisv'),
  );
  if (hasArt4 || hasArt5) {
...
console.log(
  'Outside 1%:',
  outside.length,
  '(' + ((100 * outside.length) / artUnits.length).toFixed(1) + '%)',
);
console.log(
  'Overcalculated:',
  outside.filter((a) => a.u.percentDiff > 0).length,
);
console.log(
  'Undercalculated:',
  outside.filter((a) => a.u.percentDiff < 0).length,
...
for (const a of outside.sort((x, y) => x.u.percentDiff - y.u.percentDiff)) {
  ...
  const weapons = a.data.equipment
    .filter((e) => {
    ...
    .map((e) => e.id);
  console.log(
    `${a.u.percentDiff > 0 ? '+' : ''}${a.u.percentDiff.toFixed(2)}%  ${a.u.chassis} ${a.u.model} [Art-${a.type}]`,
  );
  console.log(
    `  calc=${a.u.calculatedBV} mul=${a.u.indexBV} diff=${a.u.difference}`,
  );
  console.log(`  missiles: ${weapons.join(', ')}`);
  console.log(
    `  weaponBV=${b.weaponBV} raw=${b.rawWeaponBV} ammoBV=${b.ammoBV}`,
  );
  console.log('');
}
...
for (const a of outside.filter((x) => x.u.percentDiff < -1)) {
  ...
  const ammo = a.data.equipment.filter((e) =>
