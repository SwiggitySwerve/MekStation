...
for (const r of outside10.sort(
  (a, b) => Math.abs(b.percentDiff) - Math.abs(a.percentDiff),
)) {
  const bd = r.breakdown;
  console.log(
    `  ${r.chassis} ${r.model} (${r.tonnage}t): ref=${r.indexBV} calc=${r.calculatedBV} (${r.percentDiff > 0 ? '+' : ''}${r.percentDiff.toFixed(1)}%)`,
  );
  console.log(
    `    cockpit=${bd.cockpitType} sf=${bd.speedFactor} explPen=${bd.explosivePenalty} defBV=${bd.defensiveBV?.toFixed(0)} offBV=${bd.offensiveBV?.toFixed(0)}`,
  );
  console.log(
    `    weapBV=${bd.weaponBV?.toFixed(0)} ammoBV=${bd.ammoBV?.toFixed(0)} walkMP=${bd.walkMP} runMP=${bd.runMP} jumpMP=${bd.jumpMP}`,
  );
...
for (const r of outside5.filter((r) => Math.abs(r.percentDiff) <= 10)) {
  ...
  console.log(
    `  ${r.chassis} ${r.model} (${r.tonnage}t): ref=${r.indexBV} calc=${r.calculatedBV} (${r.percentDiff > 0 ? '+' : ''}${r.percentDiff.toFixed(1)}%) ${dir}`,
  );
  console.log(
    `    cockpit=${bd.cockpitType} sf=${bd.speedFactor} explPen=${bd.explosivePenalty} defBV=${bd.defensiveBV?.toFixed(0)} offBV=${bd.offensiveBV?.toFixed(0)}`,
  );
...
for (const r of outside5) {
  const bd = r.breakdown;
  if (bd.cockpitType !== 'standard')
    patterns[`cockpit:${bd.cockpitType}`] =
      (patterns[`cockpit:${bd.cockpitType}`] || 0) + 1;
  if (bd.explosivePenalty > 0)
    patterns['has-explosive-penalty'] =
      (patterns['has-explosive-penalty'] || 0) + 1;
  if (r.percentDiff < 0)
    patterns['undercalc'] = (patterns['undercalc'] || 0) + 1;
  ...
  if (entry?.path) {
    const unit = JSON.parse(
      fs.readFileSync(
        path.resolve('public/data/units/battlemechs', entry.path),
        'utf-8',
      ),
    );
    if (unit.techBase === 'CLAN')
      patterns['clan'] = (patterns['clan'] || 0) + 1;
    if (unit.techBase === 'MIXED')
      patterns['mixed'] = (patterns['mixed'] || 0) + 1;

    // Check for torso-mounted cockpit
    if (unit.cockpit === 'TORSO_MOUNTED')
      patterns['torso-mounted'] = (patterns['torso-mounted'] || 0) + 1;
    ...
    if (
      allSlots.some(
        (s: any) =>
          s &&
          typeof s === 'string' &&
          s.toLowerCase().includes('shield') &&
          !s.toLowerCase().includes('blue-shield') &&
          !s.toLowerCase().includes('chameleon'),
      )
    ) {
...
for (const [k, v] of Object.entries(patterns).sort((a, b) => b[1] - a[1])) {
  console.log(
    `  ${k}: ${v}/${outside5.length} (${((v / outside5.length) * 100).toFixed(0)}%)`,
  );
}
