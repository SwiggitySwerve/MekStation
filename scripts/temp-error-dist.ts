...
const results = report.allResults.filter(
  (r: any) => r.status !== 'error' && r.percentDiff !== null,
);
...
const twoThreeUnder = results.filter(
  (r: any) => r.percentDiff >= -3.5 && r.percentDiff <= -1.5,
);
let clanCount = 0,
  isCount = 0,
  mixedCount = 0;
...
console.log(
  `  Total: ${twoThreeUnder.length}, Clan: ${clanCount}, Mixed: ${mixedCount}, IS: ${isCount}`,
);

console.log('\n\nUnits with exactly +2% to +3% error:');
const twoThreeOver = results.filter(
  (r: any) => r.percentDiff >= 1.5 && r.percentDiff <= 3.5,
);
clanCount = 0;
isCount = 0;
mixedCount = 0;
...
console.log(
  `  Total: ${twoThreeOver.length}, Clan: ${clanCount}, Mixed: ${mixedCount}, IS: ${isCount}`,
);

console.log('\n\nSample -2% to -3% Clan units:');
const clanUnder = twoThreeUnder
  .filter((r: any) => {
    const iu = unitIndex.get(r.unitId);
    if (!iu) return false;
    const ud = JSON.parse(
      fs.readFileSync(path.join(basePath, iu.path), 'utf-8'),
    );
...
for (const r of clanUnder) {
  ...
  console.log(
    `  ${(r.chassis + ' ' + r.model).padEnd(35)} idx=${r.indexBV} calc=${r.calculatedBV} diff=${r.difference} (${r.percentDiff.toFixed(1)}%) def=${bd?.defensiveBV?.toFixed(0)} off=${bd?.offensiveBV?.toFixed(0)} spd=${bd?.speedFactor?.toFixed(2)}`,
  );
}
