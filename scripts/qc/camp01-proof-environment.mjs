...
export async function prepareEnvironment({row,proofTarget},dependencies={}) {
  ...
  try {
    ...
    const bootstrapArgv=expandLogicalCommand(['@npm','ci','--fund=false','--audit=false'],tools), bootstrapEnvironment={...baseEnvironment}, before=JSON.stringify(bootstrapEnvironment), result=await runSpawn(bootstrapArgv,{cwd:targetRoot,env:bootstrapEnvironment},dependencies); if(result===undefined||result===null||result.status===null||result.status===undefined) fail('bootstrap omitted'); if(result.status!==0) fail(`bootstrap failed with exit code ${result.status}`); if(JSON.stringify(bootstrapEnvironment)!==before) fail('bootstrap environment drift'); await verifyInputs({tools,versions,digests,packageLock,packageLockSha256,baseEnvironment,proofTarget},dependencies);
