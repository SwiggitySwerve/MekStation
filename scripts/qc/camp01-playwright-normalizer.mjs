...
export function prepareCamp01PlaywrightCollection(environment,isolation,dependencies={}) {
  ...
  const collectionEnvironment=Object.freeze({PLAYWRIGHT_JSON_OUTPUT_DIR:rawDirectory,PLAYWRIGHT_JSON_OUTPUT_NAME:RAW_NAME}), collection=Object.freeze({environment:{...routed,...collectionEnvironment},hReport,io,isolation:validated,mode:proof?'proof02':'h',repoRoot,rawDirectory,reporter:proof?REPORTER:hReport.reporter}); return Object.freeze({active:true,environment:collectionEnvironment,normalize:()=>normalizeCollection(collection)});
}
...
// prettier-ignore
function readObserved(raw,reporter,knownFailures) { const observed=new Map(), sources=[]; for(const suite of raw.suites){const source=reporter.sourceIds.find((id)=>id===`e2e/${suite?.file}`);if(!source)fail('raw report source mismatch');sources.push(source);walkSuite(suite,source,[],observed,knownFailures,true);} if(JSON.stringify([...sources].sort())!==JSON.stringify([...reporter.sourceIds].sort()))fail('raw report source mismatch'); return observed; }
// prettier-ignore
function walkSuite(suite,source,titles,observed,knownFailures,root=false) { if(!suite||!Array.isArray(suite.specs)||suite.suites!==undefined&&!Array.isArray(suite.suites))fail('raw report malformed'); const next=root?titles:[...titles,needString(suite.title)]; for(const spec of suite.specs){const id=[source,...next,needString(spec?.title)].join('::');if(observed.has(id))fail('duplicate observed test');const status=resolveStatus(spec?.tests);observed.set(id,{id,status,knownFailureCode:knownFailures[id]??null});} for(const child of suite.suites??[])walkSuite(child,source,next,observed,knownFailures); }
// prettier-ignore
