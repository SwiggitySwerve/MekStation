...
import { prepareCamp01GReport } from './camp01-g-report-normalizer.mjs';
...
export function prepareCamp01PlaywrightCollection(environment,isolation,dependencies={}) {
  ...
  const collectionEnvironment=Object.freeze({PLAYWRIGHT_JSON_OUTPUT_DIR:rawDirectory,PLAYWRIGHT_JSON_OUTPUT_NAME:RAW_NAME}), collection=Object.freeze({environment:{...routed,...collectionEnvironment},gReport,hReport,io,isolation:validated,mode:proof?'proof02':hReport.active?'h':'g',repoRoot,rawDirectory,reporter:proof?REPORTER:hReport.active?hReport.reporter:gReport.reporter}); return Object.freeze({active:true,environment:collectionEnvironment,normalize:()=>normalizeCollection(collection)});
}
