...
async function exportReceipt(input, dependencies) {
  try {
    ...
    await dependencies.invokePublicValidator({entry:VALIDATOR_ENTRY,stage:'durable',wave:row.wave,mode:arguments_.mode,sha:arguments_.sha,runRoot:arguments_.runRoot,runId:receipt.runId}); const receiptDigest=digestBytes(dependencies.io.readFileSync(path.join(destination,'receipt-manifest.json'))); return {runId:receipt.runId,phase:receipt.phase,finalizedPaths:[...receipt.finalizedPaths],receiptDigest};
...
// prettier-ignore
function ensureDurableRoot(root,relative,io) { let current=root; for(const segment of relative.split('/')){current=path.join(current,segment); const stat=lstatIfPresent(current,io); if(stat===null) io.mkdirSync(current); const present=io.lstatSync(current); if(present.isSymbolicLink()) fail('reparse point present in durable path'); if(!present.isDirectory()) fail('durable path is not a directory');} assertNonReparse(current,'durable',io); return current; }
// prettier-ignore
