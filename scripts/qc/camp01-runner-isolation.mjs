...
export function createCamp01RunnerIsolation(environment,dependencies={}) {
  ...
  // Keep this at the worktree root. Nesting it under runRoot/stage/executionId
  // puts Next's webpack output past Windows MAX_PATH; server.js then never listens.
  const runtimeRoot=path.join(repositoryRoot,'.c1r'), paths=Object.freeze({browserTemp:path.join(runtimeRoot,'browser-temp'),browserProfile:path.join(runtimeRoot,'browser-profile'),browserStorage:path.join(runtimeRoot,'browser-storage'),browserStorageState:path.join(runtimeRoot,'browser-storage','state.json'),browserDownloads:path.join(runtimeRoot,'browser-downloads'),databases:path.join(runtimeRoot,'databases'),next:path.join(runtimeRoot,'next'),playwrightResults:path.join(runtimeRoot,'playwright-results'),playwrightHtml:path.join(runtimeRoot,'playwright-html'),playwrightSnapshots:path.join(runtimeRoot,'playwright-snapshots'),uxWalkthrough:path.join(runtimeRoot,'ux-walkthrough')}), directories=[runtimeRoot,paths.browserTemp,paths.browserProfile,paths.browserStorage,paths.browserDownloads,paths.databases,paths.next,paths.playwrightResults,paths.playwrightHtml,paths.playwrightSnapshots,paths.uxWalkthrough], existing=lstatIfPresent(runtimeRoot,io); if(existing!==null)return adopt(environment,repositoryRoot,runtimeRoot,paths,directories,io,realpath);
