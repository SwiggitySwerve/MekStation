...
const MISSING_WEAPON_BV: Record<
  string,
  { bv: number; heat: number; source: string }
> = {
  ...
  'medium-chem-laser': {
    bv: 37,
    heat: 2,
    source: 'catalog: clan-medium-chemical-laser',
  },
...
console.log(
  '=== IMPACT ANALYSIS: Unresolved Weapons on Undercalculating Units ===\n',
);

const undercalcUnits = data.allResults.filter(
...
console.log(
  `Units with unresolved weapons: ${unitsWithUnresolved} (${((unitsWithUnresolved / totalUnits) * 100).toFixed(1)}%)`,
);
...
for (const imp of impacts.slice(0, 15)) {
  console.log(
    `  ${imp.unitId}: index=${imp.indexBV}, calc=${imp.calcBV}, diff=${imp.pctDiff.toFixed(2)}%`,
  );
...
console.log(
  'These weapons exist in catalog with clan- prefix but units reference them without:',
);
const erPulseTests = [
...
for (const test of erPulseTests) {
  ...
  console.log(
    `  ${test.unit} -> should be ${test.catalog} (BV=${resolved.battleValue}, heat=${resolved.heat})`,
  );
  const badResolve = resolveEquipmentBV(test.unit);
  console.log(
    `    Resolves as: BV=${badResolve.battleValue}, resolved=${badResolve.resolved}`,
  );
}
