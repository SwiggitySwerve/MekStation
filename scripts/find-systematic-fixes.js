...
console.log(
  'Undercalculated, ammoBV=0 but have weapons:',
  noAmmoWithBallistic.length,
);
...
for (const [sf, group] of Object.entries(sfGroups).sort(
...
console.log(
  '  If we could fix these, we would gain',
  near1pct.length,
  'more within-1% units',
);
...
for (const u of units) {
  ...
  if (crits.includes('OMNIPOD')) {
    omniCount++;
    if (u.percentDiff > 0) omniOver++;
    else omniUnder++;
  }
}
console.log(
  '\nOmniMech units in 1-5%:',
  omniCount,
  '(over:',
  omniOver,
  'under:',
