...
const exactUnits = oldR.allResults.filter(
  (x: any) => x.status === 'exact' || x.status === 'within1',
);
...
for (const [at, count] of Object.entries(byArmorType).sort(
  (a, b) => b[1] - a[1],
)) {
...
for (const uid of ffExact.slice(0, 10)) {
  ...
  console.log(
    `  ${uid}: ${d.armor.type} armor=${totalArmor} defF=${defFactor} expected_overcalc=+${expectedOvercalc} ref=${u.indexBV}`,
  );
}
