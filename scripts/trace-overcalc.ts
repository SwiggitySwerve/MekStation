...
} from '../src/utils/construction/equipmentBVResolver.ts';
...
for (const id of eqIds) {
  ...
  console.log(
    `  ${id.padEnd(30)} -> norm="${normalizeEquipmentId(id)}" bv=${r.battleValue} resolved=${r.resolved}`,
  );
}
...
for (const unitId of ids) {
  ...
  console.log(`\n${'='.repeat(70)}`);
  console.log(
    `${unitId} — ref=${result.indexBV} calc=${result.calculatedBV} diff=${result.difference > 0 ? '+' : ''}${result.difference} (${result.percentDiff?.toFixed(2)}%)`,
  );
  console.log(
    `  ${unit.tonnage}t ${unit.techBase} engine=${unit.engine?.type}/${unit.engine?.rating} walk=${unit.movement?.walk} jump=${unit.movement?.jump || 0}`,
  );
  console.log(
    `  armor=${unit.armor?.type} struct=${unit.structure?.type || 'STANDARD'} cockpit=${unit.cockpit || '(none)'} gyro=${unit.gyro?.type || 'STANDARD'}`,
  );
  ...
  if (Array.isArray(headSlots)) {
    ...
    console.log(
      `  HEAD analysis: LS count=${lsCount}, slot4="${slot4}", isSensors=${slot4?.includes?.('Sensors')}`,
    );
    if (slot4?.includes?.('Sensors') && lsCount === 1) {
      console.log(
        `  ** SMALL COCKPIT HEURISTIC TRIGGERED (LS=1, slot4=Sensors) **`,
      );
    }
  }
  ...
  console.log(
    `  DEF: armor=${b.armorBV?.toFixed(1)} struct=${b.structureBV?.toFixed(3)} gyro=${b.gyroBV?.toFixed(1)} defEq=${b.defEquipBV?.toFixed(0)} armoredComp=${b.armoredComponentBV} harjel=${b.harjelBonus} exp=-${b.explosivePenalty?.toFixed(0)}`,
  );
  console.log(
    `       DF=${b.defensiveFactor} maxTMM=${b.maxTMM} → defBV=${b.defensiveBV?.toFixed(1)}`,
  );
  console.log(
    `  OFF: weap=${b.weaponBV?.toFixed(1)} (raw=${b.rawWeaponBV?.toFixed(1)}, halved=${b.halvedWeaponBV?.toFixed(1)}) ammo=${b.ammoBV} phys=${b.physicalWeaponBV?.toFixed(1)} wt=${b.weightBonus?.toFixed(1)} offEq=${b.offEquipBV?.toFixed(0)}`,
  );
  console.log(
    `       HE=${b.heatEfficiency} HD=${b.heatDissipation} MH=${b.moveHeat} SF=${b.speedFactor}`,
  );
  ...
  console.log(
    `  base=(${b.defensiveBV?.toFixed(1)} + ${b.offensiveBV?.toFixed(1)}) = ${((b.defensiveBV || 0) + (b.offensiveBV || 0)).toFixed(1)}`,
  );
  console.log(
    `  final=round(${((b.defensiveBV || 0) + (b.offensiveBV || 0)).toFixed(1)} * ${b.cockpitModifier}) = ${result.calculatedBV}`,
  );
