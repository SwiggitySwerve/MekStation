...
for (const id of targets) {
  ...
  console.log(
    `  OFF: wBV=${b.rawWeaponBV} halved=${b.halvedWeaponBV} aBV=${b.ammoBV} wt=${b.weightBonus} phys=${b.physicalWeaponBV} offEq=${b.offEquipBV} sf=${b.speedFactor}`,
  );
  console.log(
    `  DEF: armor=${b.armorBV} struct=${b.structureBV} gyro=${b.gyroBV} defEq=${b.defEquipBV} expl=${b.explosivePenalty} df=${b.defensiveFactor}`,
  );
  console.log(
    `  HEAT: eff=${b.heatEfficiency} diss=${b.heatDissipation} moveHeat=${b.moveHeat} cockpit=${b.cockpitType} tech=${b.techBase}`,
  );
