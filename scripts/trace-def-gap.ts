...
const avgGap =
  minor.reduce((s: number, r: any) => s + Math.abs(r.difference), 0) /
  minor.length;
...
for (const s of minor.slice(0, 20)) {
  console.log(
    s.unitId.substring(0, 35).padEnd(36) +
      ' idx=' +
      String(s.indexBV).padStart(5) +
      ' calc=' +
      String(s.calculatedBV).padStart(5) +
      ' diff=' +
      String(s.difference).padStart(5) +
      ' def=' +
      String(s.breakdown?.defensiveBV?.toFixed(0)).padStart(6) +
      ' off=' +
      String(s.breakdown?.offensiveBV?.toFixed(0)).padStart(6) +
      ' wBV=' +
      String(s.breakdown?.weaponBV?.toFixed(0)).padStart(5) +
      ' dEq=' +
      String(s.breakdown?.defensiveEquipBV?.toFixed(0)).padStart(4),
  );
}
...
const avgOverGap =
  overMinor.reduce((s: number, r: any) => s + Math.abs(r.difference), 0) /
  overMinor.length;
