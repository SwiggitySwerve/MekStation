...
for (const r of undercalc) {
  ...
  try {
    ...
    if (unit.engine.type !== 'FUSION') {
      score += 1;
      details.push(unit.engine.type);
    }
    if (unit.armor.type !== 'STANDARD') {
      score += 1;
      details.push(unit.armor.type);
    }
    if (unit.structure.type !== 'STANDARD') {
      score += 1;
      details.push(unit.structure.type);
    }
    if ((unit.gyro?.type || 'STANDARD') !== 'STANDARD') {
      score += 2;
      details.push(unit.gyro.type);
    }
    if (unit.heatSinks.type !== 'DOUBLE' && unit.heatSinks.type !== 'SINGLE') {
      score += 1;
      details.push(unit.heatSinks.type);
    }
    if (unit.movement.jump > 0) {
      score += 1;
      details.push(`JJ=${unit.movement.jump}`);
    }
    ...
    if (specialEquip.length > 0) {
      score += specialEquip.length * 2;
      details.push(specialEquip.join(','));
    }
...
for (const s of scored.slice(0, 30)) {
  console.log(
    `${s.id.padEnd(40).slice(0, 40)} ${s.tonnage}t  gap=${String(s.gap).padStart(4)} (${s.gapPct.toFixed(1)}%)  def=${s.defBV.toFixed(0)} off=${s.offBV.toFixed(0)} w=${s.weaponBV}  score=${s.score}  [${s.details}]`,
  );
}
