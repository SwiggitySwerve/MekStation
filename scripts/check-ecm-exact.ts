...
const ecmCritNames = new Map<
  string,
  { count: number; resolvedBV: number; exactCount: number; failCount: number }
>();
    ...
    for (const s of slots as string[]) {
      ...
      if (
        ...
        if (!ecmCritNames.has(key)) {
          ...
          ecmCritNames.set(key, {
            count: 0,
            resolvedBV: res.battleValue,
            exactCount: 0,
            failCount: 0,
          });
        }
...
const sortedEcm = [...ecmCritNames.entries()].sort(
  (a, b) => b[1].count - a[1].count,
);
for (const [name, info] of sortedEcm) {
  console.log(
    `  ${name.padEnd(40)} bv=${String(info.resolvedBV).padStart(3)} total=${String(info.count).padStart(4)} exact=${String(info.exactCount).padStart(4)} fail=${String(info.failCount).padStart(4)}`,
  );
}
    ...
    for (const s of slots as string[]) {
      ...
      if (
        ...
        if (ecmExactCount <= 10) {
          const b = u.breakdown;
          console.log(
            `  ${u.unitId.padEnd(40)} defEq=${b?.defEquipBV} defBV=${b?.defensiveBV?.toFixed(0)} ref=${u.indexBV} calc=${u.calculatedBV}`,
          );
        }
        break;
      }
...
console.log(
  `  Exact: ${ecmExact}, Within1%: ${ecmWithin1}, Over: ${ecmOver}, Under: ${ecmUnder}`,
);
