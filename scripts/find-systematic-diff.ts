...
const under = report.allResults.filter(
  (r: any) =>
    ...
    (!r.issues ||
      r.issues.length === 0 ||
      !r.issues.some((i: string) => i.includes('Unresolved'))),
);
...
console.log(
  `avg: ${(offRatios.reduce((s, v) => s + v, 0) / offRatios.length).toFixed(2)}%`,
);
console.log(
  `min: ${Math.min(...offRatios).toFixed(2)}%, max: ${Math.max(...offRatios).toFixed(2)}%`,
);

console.log('\n=== DIFF AS % OF DEFENSIVE BV ===');
const defRatios = diffs.map((d) => (d.diff / d.defBV) * 100);
console.log(
  `avg: ${(defRatios.reduce((s, v) => s + v, 0) / defRatios.length).toFixed(2)}%`,
);
console.log(
  `min: ${Math.min(...defRatios).toFixed(2)}%, max: ${Math.max(...defRatios).toFixed(2)}%`,
);

console.log('\n=== DIFF AS % OF WEAPON BV ===');
const weapRatios = diffs
  .filter((d) => d.weapBV > 0)
  .map((d) => (d.diff / d.weapBV) * 100);
console.log(
  `avg: ${(weapRatios.reduce((s, v) => s + v, 0) / weapRatios.length).toFixed(2)}%`,
);
...
for (const d of diffs) {
  if (!armorGroups[d.armorType])
    armorGroups[d.armorType] = { count: 0, totalDiff: 0 };
  armorGroups[d.armorType].count++;
  armorGroups[d.armorType].totalDiff += d.pct;
}
for (const [type, data] of Object.entries(armorGroups).sort(
  (a, b) => b[1].count - a[1].count,
)) {
  console.log(
    `  ${type.padEnd(25)} ${data.count} units, avg ${(data.totalDiff / data.count).toFixed(2)}%`,
  );
}
...
for (const d of diffs) {
  if (!engineGroups[d.engineType])
    engineGroups[d.engineType] = { count: 0, totalDiff: 0 };
  engineGroups[d.engineType].count++;
  engineGroups[d.engineType].totalDiff += d.pct;
}
for (const [type, data] of Object.entries(engineGroups).sort(
  (a, b) => b[1].count - a[1].count,
)) {
  console.log(
    `  ${type.padEnd(25)} ${data.count} units, avg ${(data.totalDiff / data.count).toFixed(2)}%`,
...
for (const d of diffs) {
  if (!structGroups[d.structType])
    structGroups[d.structType] = { count: 0, totalDiff: 0 };
  structGroups[d.structType].count++;
  structGroups[d.structType].totalDiff += d.pct;
}
for (const [type, data] of Object.entries(structGroups).sort(
  (a, b) => b[1].count - a[1].count,
)) {
  console.log(
    `  ${type.padEnd(25)} ${data.count} units, avg ${(data.totalDiff / data.count).toFixed(2)}%`,
...
for (const d of diffs) {
  ...
  if (!tonnageGroups[bucket])
    tonnageGroups[bucket] = { count: 0, totalDiff: 0 };
  tonnageGroups[bucket].count++;
  tonnageGroups[bucket].totalDiff += d.pct;
}
for (const [ton, data] of Object.entries(tonnageGroups).sort(
  (a, b) => Number(a) - Number(b),
)) {
  console.log(
    `  ${ton}t+: ${data.count} units, avg ${(data.totalDiff / data.count).toFixed(2)}%`,
  );
}
...
for (const d of closest) {
  console.log(
    `  ${d.name.padEnd(35)} ${d.techBase.padEnd(6)} diff=${d.diff} (${d.pct.toFixed(2)}%) off=${d.offBV.toFixed(0)} def=${d.defBV.toFixed(0)} wBV=${d.weapBV.toFixed(0)} sf=${d.sf} ton=${d.tonnage}`,
  );
}
