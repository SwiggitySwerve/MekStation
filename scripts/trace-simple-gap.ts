...
for (const unitId of traceIds) {
  ...
  console.log(
    `  idx=${r.indexBV} calc=${r.calculatedBV} diff=${r.difference} (${r.percentDiff.toFixed(1)}%)`,
  );
  console.log(
    `  defBV=${b.defensiveBV.toFixed(1)} offBV=${b.offensiveBV.toFixed(1)} wBV=${b.weaponBV} ammoBV=${b.ammoBV}`,
  );
  console.log(
    `  sf=${b.speedFactor} explP=${b.explosivePenalty} defEqBV=${b.defensiveEquipBV}`,
  );
  ...
  for (const eq of data.equipment) {
    ...
    const usedBV =
      data.techBase === 'CLAN'
        ...
        : res.battleValue;
    console.log(
      `    ${eq.id.padEnd(30)} @${eq.location.padEnd(15)} bv=${usedBV} h=${data.techBase === 'CLAN' ? (clanRes.resolved ? clanRes.heat : res.heat) : res.heat}`,
    );
  }
  ...
  console.log(
    `  Our offBV = ${b.offensiveBV.toFixed(1)}, rawOff = ${ourRawOff.toFixed(1)}`,
  );
  console.log(`  Raw gap = ${rawGap.toFixed(1)}`);
  console.log(`  Weight bonus = ${data.tonnage}`);
  console.log(
    `  Expected raw = wBV + weight + physBV + offEqBV = ${b.weaponBV} + ${data.tonnage} + 0 + 0 = ${b.weaponBV + data.tonnage}`,
  );
  console.log(
    `  ourRawOff = ${ourRawOff.toFixed(1)}, expected sum = ${b.weaponBV + data.tonnage}`,
  );
  ...
  console.log(
    `  Cockpit: ${data.cockpit || 'STANDARD'} Gyro: ${data.gyro?.type || 'STANDARD'}`,
  );
