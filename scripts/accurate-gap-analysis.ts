...
for (const r of underCalc.slice(0, 20)) {
  ...
  const cockpitMod =
    Math.abs(inferredCockpitMod - 0.95) < 0.01
      ? 0.95
      : Math.abs(inferredCockpitMod - 1.3) < 0.01
        ? 1.3
        : 1.0;
  ...
  console.log(
    `  idx=${r.indexBV} calc=${r.calculatedBV} gap=${r.difference} cockpitMod=${cockpitMod}`,
  );
  console.log(
    `  defBV=${b.defensiveBV.toFixed(1)} offBV=${b.offensiveBV.toFixed(1)} wBV=${b.weaponBV} sf=${b.speedFactor}`,
  );
  console.log(`  baseGap=${baseGap.toFixed(1)} rawOffGap=${rawGap.toFixed(1)}`);
  console.log(
    `  rawGap/tonnage=${tonnage > 0 ? (rawGap / tonnage).toFixed(3) : 'N/A'}`,
  );
  console.log('');
}

// Now do a statistical analysis across ALL undercalculated units (not just energy-only)
const allUnderCalc = report.allResults.filter(
...
for (const r of allUnderCalc) {
  ...
  const expectedBase = r.indexBV / cockpitMod;
  const rawGap =
    (expectedBase - b.defensiveBV) / b.speedFactor -
...
console.log(
  `Raw offensive gap: avg=${avgRawGap.toFixed(1)}, median=${medianRawGap.toFixed(1)}`,
);

rawGapsPerTon.sort((a, b) => a - b);
const avgRawGapPerTon =
  rawGapsPerTon.reduce((s, g) => s + g, 0) / rawGapsPerTon.length;
const medianRawGapPerTon = rawGapsPerTon[Math.floor(rawGapsPerTon.length / 2)];
console.log(
  `Raw gap/tonnage: avg=${avgRawGapPerTon.toFixed(3)}, median=${medianRawGapPerTon.toFixed(3)}`,
);
...
for (const [bin, count] of Object.entries(bins).sort(
  ...
  if (count > 0) {
    console.log(
      `  ${bin.padStart(5)}: ${'█'.repeat(Math.min(count, 60))} (${count})`,
    );
  }
}
