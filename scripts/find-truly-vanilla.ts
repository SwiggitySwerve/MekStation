...
const specialPatterns =
  /c3|tag|vibro|lance|retract|shield|spike|drill|saw|welder|flail|backhoe|combine|pile|rock.*cut|wrecking|blue.?shield|aes|partial.?wing|coolant|harjel|supercharger|masc|tsm|targeting.?comp|ecm|bap|probe|stealth|null.?sig|chameleon|drone|radical/i;
...
for (const r of undercalc) {
  ...
  try {
    ...
    if (
      !['FUSION', 'LIGHT', 'XL', 'CLAN_XL', 'COMPACT'].includes(
        unit.engine.type,
      )
    )
      continue;
    ...
    if (
      ![
        ...
        'FERRO_FIBROUS_CLAN',
        'LIGHT_FERRO_FIBROUS',
      ].includes(unit.armor.type)
    )
      continue;
    // Allow common structure types
    if (
      !['STANDARD', 'ENDO_STEEL', 'ENDO_STEEL_CLAN'].includes(
        unit.structure.type,
      )
    )
      continue;
    ...
    for (const [, slots] of Object.entries(unit.criticalSlots)) {
      ...
      for (const s of slots) {
        ...
        if (specialPatterns.test(s as string)) {
          hasSpecial = true;
          break;
        }
      }
    ...
    console.log(
      `${r.unitId.padEnd(40).slice(0, 40)} ${unit.tonnage}t  MUL=${r.indexBV}  calc=${r.calculatedBV}  gap=${gap} (${((gap / r.calculatedBV) * 100).toFixed(1)}%)`,
    );
    console.log(
      `  W=${unit.movement.walk} J=${unit.movement.jump || 0} HS=${unit.heatSinks.count}x${unit.heatSinks.type}`,
    );
    console.log(
      `  DefBV=${b.defensiveBV.toFixed(1)} OffBV=${b.offensiveBV.toFixed(1)} WBV=${b.weaponBV} SF=${b.speedFactor.toFixed(2)}`,
    );
    console.log(
      `  Equipment: [${unit.equipment.map((e: any) => e.id).join(', ')}]`,
    );
    console.log('');
  } catch (_error) {
