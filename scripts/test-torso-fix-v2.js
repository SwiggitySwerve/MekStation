...
for (const u of tmUnits) {
  ...
  const best =
    Math.abs(diffA) < Math.abs(diffB)
      ? 'A'
      : Math.abs(diffB) < Math.abs(u.percentDiff)
        ? 'B'
        : 'C';
  if (
    ...
    console.log(
      `${u.chassis} ${u.model}: CT=${ctTotal} armorM=${armorMult} df=${df}`,
    );
    console.log(
      `  C=${u.percentDiff.toFixed(2)}% A=${diffA.toFixed(2)}% B=${diffB.toFixed(2)}% best=${best}`,
    );
  }
}
