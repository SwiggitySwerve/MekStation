...
const offEquipPatterns = [
  {
    pattern: /c3i|c3 improved|isc3iunit|c3slave|c3master|c3boosted|c3m|c3s/i,
    name: 'C3',
  },
  ...
  {
    pattern:
      /backhoe|chainsaw|combine|dual\s*saw|flail|mining\s*drill|pile\s*driver|rock\s*cutter|spot\s*welder|wrecking\s*ball/i,
    name: 'Industrial',
  },
...
for (const [type, units] of [...byType.entries()].sort(
  ...
  for (const u of units.sort((a, b) => b.gap - a.gap).slice(0, 5)) {
    console.log(
      `  ${u.unitId.padEnd(40).slice(0, 40)} gap=${u.gap}  [${[...new Set(u.critSlots)].slice(0, 3).join(', ')}]`,
    );
  }
}

// How many undercalculated units have NO offensive equipment at all?
const unitIdsWithEquip = new Set(
  [...uniqueFindings.values()].map((f) => f.unitId),
);
const withoutEquip = undercalc.filter(
  (r: any) => !unitIdsWithEquip.has(r.unitId),
);
console.log(
