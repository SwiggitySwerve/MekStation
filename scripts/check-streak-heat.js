...
const streakOver = valid
  .filter((x) => x.percentDiff > 1 && x.percentDiff <= 2)
  .filter((x) => {
    const unit = loadUnit(x.unitId);
    if (!unit) return false;
    return JSON.stringify(unit.criticalSlots || {})
      .toLowerCase()
      .includes('streak');
  });
...
for (const u of streakOver) {
  ...
  if (b.weapons) {
    const streakWeapons = b.weapons.filter((w) =>
      (w.id || '').toLowerCase().includes('streak'),
    );
    const otherWeapons = b.weapons.filter(
      (w) => !(w.id || '').toLowerCase().includes('streak'),
    );
    ...
    console.log(
      'Total weapon heat:',
      b.weapons.reduce((s, w) => s + (w.heat || 0), 0),
    );
    console.log(
      'Streak weapon heat:',
      streakWeapons.reduce((s, w) => s + (w.heat || 0), 0),
    );
  }
}
