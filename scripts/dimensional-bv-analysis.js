...
const outliers = report.allResults.filter(
...
for (const r of records) {
  ...
  if (!byEngine[key])
    byEngine[key] = { count: 0, totalGap: 0, over: 0, under: 0 };
  byEngine[key].count++;
  byEngine[key].totalGap += r.diff;
  if (r.diff > 0) byEngine[key].over++;
  else byEngine[key].under++;
}
console.log('\nBy Engine Type:');
for (const [k, v] of Object.entries(byEngine).sort(
  (a, b) => b[1].count - a[1].count,
)) {
  const avg = (v.totalGap / v.count).toFixed(1);
  console.log(
    `  ${k.padEnd(15)} n=${String(v.count).padStart(3)} avg=${avg.padStart(7)} over=${v.over} under=${v.under}`,
  );
}
...
for (const r of records) {
  ...
  if (r.diff > 0) byTech[key].over++;
  else byTech[key].under++;
}
console.log('\nBy Tech Base:');
for (const [k, v] of Object.entries(byTech).sort(
  (a, b) => b[1].count - a[1].count,
)) {
  const avg = (v.totalGap / v.count).toFixed(1);
  console.log(
...
for (const r of records) {
  ...
  if (!bySystem[key])
    bySystem[key] = { count: 0, totalGap: 0, over: 0, under: 0, units: [] };
  bySystem[key].count++;
  bySystem[key].totalGap += r.diff;
  if (r.diff > 0) bySystem[key].over++;
  else bySystem[key].under++;
  bySystem[key].units.push(r.name);
}
console.log('\nBy System Combination (sorted by count):');
for (const [k, v] of Object.entries(bySystem).sort(
  (a, b) => b[1].count - a[1].count,
)) {
  const avg = (v.totalGap / v.count).toFixed(1);
  const bias =
    v.over > v.under
      ...
      : v.under > v.over
        ...
        : 'BALANCED';
  console.log(
    `  ${k.padEnd(25)} n=${String(v.count).padStart(3)} avg=${avg.padStart(7)} over=${v.over} under=${v.under} ${bias}`,
  );
}
...
for (const sys of systemNames) {
  ...
  const avgWithout =
    withoutSys.reduce((s, r) => s + r.diff, 0) / withoutSys.length;
  const overWith = withSys.filter((r) => r.diff > 0).length;
  const underWith = withSys.filter((r) => r.diff < 0).length;
  console.log(
    `  ${sys.padEnd(12)} with: n=${String(withSys.length).padStart(3)} avg=${avgWith.toFixed(1).padStart(7)} (${overWith}↑ ${underWith}↓)  without: n=${String(withoutSys.length).padStart(3)} avg=${avgWithout.toFixed(1).padStart(7)}`,
  );
}
...
const balanced = defRatios.filter(
  (r) => r.defRatio >= 0.4 && r.defRatio <= 0.6,
);
...
console.log(
  `  Def-heavy (>60%): n=${dh.n} avg=${dh.avg} over=${dh.over} under=${dh.under}`,
);
const bl = groupStats(balanced);
console.log(
  `  Balanced (40-60%): n=${bl.n} avg=${bl.avg} over=${bl.over} under=${bl.under}`,
);
const oh = groupStats(offHeavy);
console.log(
  `  Off-heavy (<40%): n=${oh.n} avg=${oh.avg} over=${oh.over} under=${oh.under}`,
);
...
for (const r of records) {
  ...
  if (!allGroups[key])
    allGroups[key] = { count: 0, totalGap: 0, over: 0, under: 0 };
  allGroups[key].count++;
  allGroups[key].totalGap += r.diff;
  if (r.diff > 0) allGroups[key].over++;
  else allGroups[key].under++;
}
...
const ranked = Object.entries(allGroups)
  ...
  .map(([k, v]) => {
    ...
    return {
      ...
      avg,
      biasRatio,
      impact: v.count * Math.abs(avg) * biasRatio,
    };
  })
...
for (const r of ranked.slice(0, 15)) {
  ...
  console.log(
    `  ${r.key.padEnd(35)} n=${String(r.count).padStart(3)} avg=${r.avg.toFixed(1).padStart(7)} bias=${(r.biasRatio * 100).toFixed(0)}% ${dir} impact=${r.impact.toFixed(0)}`,
  );
}
