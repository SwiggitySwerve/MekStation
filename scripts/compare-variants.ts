...
for (const { chassis, exact, wrong } of interesting.slice(0, 15)) {
  console.log(
    `${chassis}: ${exact.length} exact, ${wrong.length} undercalculated`,
  );
  ...
  for (const r of [ex, wr]) {
    ...
    try {
      ...
      const b = r.breakdown;
      console.log(
        `  [${tag}] ${r.unitId}: idx=${r.indexBV} calc=${r.calculatedBV} diff=${r.difference}`,
      );
      console.log(
        `    engine=${data.engine.type} struct=${data.structure.type} armor=${data.armor.type} hs=${data.heatSinks.type}(${data.heatSinks.count})`,
      );
      console.log(
        `    walk=${data.movement.walk} jump=${data.movement.jump || 0}`,
      );
      console.log(
        `    defBV=${b.defensiveBV.toFixed(1)} offBV=${b.offensiveBV.toFixed(1)} sf=${b.speedFactor} wBV=${b.weaponBV} ammoBV=${b.ammoBV} defEq=${b.defensiveEquipBV} expl=${b.explosivePenalty}`,
      );
      console.log(
        `    equip: ${data.equipment.map((e: any) => e.id).join(', ')}`,
      );
    } catch (_error) {
