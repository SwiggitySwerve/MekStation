...
for (const iu of indexData.units) {
  ...
  for (const [loc, slots] of Object.entries(ud.criticalSlots || {})) {
    for (const slot of slots as (string | null)[]) {
      if (!slot) continue;
      if (
        /endo|ferro|engine|gyro|cockpit|life support|sensors|shoulder|upper arm|lower arm|hand|hip|upper leg|lower leg|foot|actuator|heat sink|jump jet|^case$/i.test(
          slot,
        )
      )
        continue;
