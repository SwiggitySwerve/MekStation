...
for (const w of weapons) {
  ...
  console.log(
    `  ${w.padEnd(30)} IS: BV=${isRes.battleValue} heat=${isRes.heat}`,
  );
}
...
for (const w of weapons) {
  ...
  const isRes = resolveEquipmentBV(w);
  const diff =
    res.battleValue !== isRes.battleValue
      ? ` (IS was ${isRes.battleValue}, diff=${res.battleValue - isRes.battleValue})`
      : ' (SAME as IS)';
  console.log(
    `  ${w.padEnd(30)} Clan: BV=${res.battleValue} heat=${res.heat}${diff}`,
  );
}
...
for (const id of clanChecks) {
  ...
  console.log(
    `  ${id.padEnd(35)} BV=${res.battleValue} resolved=${res.resolved}`,
  );
}
