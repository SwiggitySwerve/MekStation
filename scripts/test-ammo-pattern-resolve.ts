...
function buildAmmoLookup(): Map<string, { bv: number; weaponType: string }> {
  ...
  const hc: Array<[string, number, string]> = [
    ...
    ['clan-improved-lrm-5-ammo', 7, 'lrm-5'],
    ['clan-improved-lrm-10-ammo', 14, 'lrm-10'],
    ['clan-improved-lrm-15-ammo', 21, 'lrm-15'],
    ['clan-improved-lrm-20-ammo', 27, 'lrm-20'],
    ...
    ['clanimprovedlrm5ammo', 7, 'lrm-5'],
    ['clanimprovedlrm10ammo', 14, 'lrm-10'],
    ['clanimprovedlrm15ammo', 21, 'lrm-15'],
    ['clanimprovedlrm20ammo', 27, 'lrm-20'],
...
for (const name of failingAmmoNames) {
  ...
  const status = matched
    ? `OK (bv=${matchedBV} via ${matchedId})`
    : matchedId
      ? `FAIL: ${matchedId}`
      : 'FAIL: no pattern matched';
  console.log(
    `  "${name.padEnd(52)}" stripped="${stripped.padEnd(45)}" → ${status}`,
  );
}
