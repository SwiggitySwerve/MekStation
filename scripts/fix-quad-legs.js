...
function isMtfQuad(content) {
  return /^Config:\s*Quad$/im.test(content);
}
...
const unitFiles = bvAnalysis.findJsonFiles(UNIT_DIR);
let fixed = 0,
  notFound = 0,
  alreadyHasLegs = 0,
  noEquip = 0;
  ...
  try {
    ...
    for (const [loc, items] of Object.entries(legs)) {
      for (const item of items) {
        if (
          !actuatorItems.has(item) &&
          !item.includes('Endo Steel') &&
          !item.includes('Ferro-Fibrous') &&
          !item.includes('Endo-Steel') &&
          !item.includes('Endo Composite')
        ) {
          hasEquipment = true;
        }
      }
    ...
    const equipNote = hasEquipment
      ? 'HAS EQUIPMENT'
      : 'actuators/structure only';
    fixLog.push(
