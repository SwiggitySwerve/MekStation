...
console.log(
  `Current: ${within1}/${totalValid} = ${((within1 / totalValid) * 100).toFixed(1)}%`,
);
console.log(
  `After excluding no-MUL outliers: ${within1}/${afterExclude} = ${((within1 / afterExclude) * 100).toFixed(1)}%`,
);
...
const within1WithMul = validWithMul.filter(
  (x: any) => Math.abs(x.percentDiff) <= 1,
);
console.log(
  `MUL-only accuracy: ${within1WithMul.length}/${validWithMul.length} = ${((within1WithMul.length / validWithMul.length) * 100).toFixed(1)}%`,
);

// Show the no-MUL outliers for reference
console.log(`\n--- NO-MUL OUTLIERS (${outlierNoMul.length} units) ---`);
for (const u of outlierNoMul
  ...
  console.log(
    `  ${u.unitId.padEnd(45)} diff=${u.percentDiff.toFixed(1).padStart(6)}% ref=${u.indexBV} mulMatch=${entry?.matchType || 'none'} mulBV=${entry?.mulBV || 0}`,
  );
}

// For MUL-confirmed 1-2% band: what's the breakdown?
const mul12 = outlierWithMul.filter((x: any) => Math.abs(x.percentDiff) <= 2);
const mul25 = outlierWithMul.filter(
  (x: any) => Math.abs(x.percentDiff) > 2 && Math.abs(x.percentDiff) <= 5,
);
const mul5 = outlierWithMul.filter((x: any) => Math.abs(x.percentDiff) > 5);
console.log(`\n=== MUL-CONFIRMED OUTLIER BANDS ===`);
console.log(
  `1-2%: ${mul12.length} (over: ${mul12.filter((x: any) => x.percentDiff > 0).length}, under: ${mul12.filter((x: any) => x.percentDiff < 0).length})`,
);
console.log(
  `2-5%: ${mul25.length} (over: ${mul25.filter((x: any) => x.percentDiff > 0).length}, under: ${mul25.filter((x: any) => x.percentDiff < 0).length})`,
);
console.log(
  `>5%:  ${mul5.length} (over: ${mul5.filter((x: any) => x.percentDiff > 0).length}, under: ${mul5.filter((x: any) => x.percentDiff < 0).length})`,
);
