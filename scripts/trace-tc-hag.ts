...
for (const uid of unitIds) {
  ...
  const equipTC = d.equipment.filter((e: any) =>
  ...
  const equipHAG = d.equipment.filter((e: any) =>
    e.id.toLowerCase().includes('hag'),
  );
  ...
  console.log(
    `  Equip TC: ${equipTC.length > 0 ? equipTC.map((e: any) => e.id).join(', ') : 'NONE'}`,
  );
  console.log(`  Crit TC: ${critTC.length > 0 ? critTC.join(', ') : 'NONE'}`);
  console.log(
    `  Equip HAG: ${equipHAG.length > 0 ? equipHAG.map((e: any) => `${e.id}@${e.location}`).join(', ') : 'NONE'}`,
  );
  console.log(
    `  Crit HAG: ${critHAG.length > 0 ? critHAG.join(', ') : 'NONE'}`,
  );
  ...
  if (res) {
    console.log(
      `  BV: calc=${res.calculatedBV} diff=${res.difference} (${res.percentDiff?.toFixed(1)}%)`,
    );
...
for (const u of overNoAmmo) {
  ...
  try {
    ...
    for (const eq of d.equipment) {
      overEquipCounts[eq.id.toLowerCase()] =
        (overEquipCounts[eq.id.toLowerCase()] || 0) + 1;
    }
    for (const [loc, slots] of Object.entries(d.criticalSlots || {})) {
      for (const s of slots as string[]) {
        if (!s) continue;
...
const overSorted = Object.entries(overCritEquip)
  .sort((a, b) => b[1] - a[1])
  .filter(([_, c]) => c >= 3);
