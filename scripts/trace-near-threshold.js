...
const nearThreshold = report.allResults
  .filter((x) => {
    return (
      x.status !== 'exact' &&
      x.status !== 'within1' &&
      x.breakdown &&
      x.difference < 0 &&
      Math.abs(x.percentDiff) < 3.0
    );
  })
  .sort((a, b) => Math.abs(a.percentDiff) - Math.abs(b.percentDiff));

console.log(
  `Found ${nearThreshold.length} undercalculated units within 3% of expected\n`,
);
...
for (const u of nearThreshold.slice(0, 15)) {
  ...
  const hasUMU = critStr.some(
    (s) => s.includes('umu') || s.includes('underwater'),
  );
  ...
  console.log(
    `  MUL BV: ${mulBV}  Calc BV: ${calcTotal}  Gap: ${gap} (${u.percentDiff.toFixed(2)}%)`,
  );
  console.log(
    `  Def: ${Math.round(b.defensiveBV)} (${defPct.toFixed(0)}%)  Off: ${Math.round(b.offensiveBV)} (${offPct.toFixed(0)}%)`,
  );
  console.log(
    `  armorBV=${b.armorBV} structBV=${b.structureBV} gyroBV=${b.gyroBV} defEquip=${b.defEquipBV || 0} explosive=${b.explosivePenalty || 0}`,
  );
  console.log(`  defFactor=${b.defensiveFactor} (TMM-based)`);
  console.log(
    `  weaponBV=${Math.round(b.weaponBV)} ammoBV=${b.ammoBV || 0} physBV=${b.physicalWeaponBV || 0} weightBonus=${b.weightBonus || 0}`,
  );
  console.log(
    `  speedFactor=${b.speedFactor} heatEff=${b.heatEfficiency} heatDiss=${b.heatDissipation} moveHeat=${b.moveHeat}`,
  );
  console.log(
    `  Engine: ${engine.type} rating=${engine.rating}  HS: ${hs.count}×${hs.type}`,
  );
  console.log(
    `  Movement: walk=${mov.walk} run=${mov.run} jump=${mov.jump || 0}`,
  );
  console.log(
    `  cockpitMod=${b.cockpitModifier} cockpit=${b.cockpitType || unit.cockpit || 'standard'}`,
  );
