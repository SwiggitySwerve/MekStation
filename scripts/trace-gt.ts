...
console.log(
  `${uid}: ref=${r.indexBV} calc=${r.calculatedBV} diff=${r.difference}`,
);
console.log(`\nBREAKDOWN:`);
console.log(
  `  Defensive: armorBV=${b.armorBV?.toFixed(1)} structBV=${b.structureBV?.toFixed(1)} gyroBV=${b.gyroBV?.toFixed(1)}`,
);
console.log(
  `  defEquipBV=${b.defEquipBV} amsAmmoBV=${b.amsAmmoBV} armoredBV=${b.armoredComponentBV} harjelBonus=${b.harjelBonus}`,
);
console.log(
  `  explosivePenalty=${b.explosivePenalty} defensiveFactor=${b.defensiveFactor} maxTMM=${b.maxTMM}`,
);
...
console.log(
  `  halvedBV=${b.halvedWeaponBV} ammoBV=${b.ammoBV} weightBonus=${b.weightBonus?.toFixed(1)}`,
);
...
console.log(
  `  movement: walk=${unit.movement?.walk} run=${unit.movement?.run} jump=${unit.movement?.jump || 0}`,
);
console.log(
  `  heatSinks: count=${unit.heatSinks?.count} type=${unit.heatSinks?.type}`,
);
...
for (const eq of unit.equipment || []) {
  ...
  console.log(
    `  ${eq.id.padEnd(35)} @${eq.location.padEnd(12)} → bv=${res.battleValue} heat=${res.heat} norm=${norm}`,
  );
}
...
console.log(
  `  neededOff = ${neededOff.toFixed(1)} (actual ${b.offensiveBV?.toFixed(1)})`,
);
console.log(`  offGap = ${(neededOff - b.offensiveBV).toFixed(1)}`);
console.log(`  neededBaseOff = ${(neededOff / b.speedFactor).toFixed(1)}`);
console.log(
  `  actualBaseOff = ${(b.weaponBV + b.ammoBV + (b.weightBonus || 0) + (b.physicalWeaponBV || 0) + (b.offEquipBV || 0)).toFixed(1)}`,
);
