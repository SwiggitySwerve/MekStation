...
for (const u of wrong.filter((x: any) => x.percentDiff < 0).slice(0, 15)) {
  ...
  const b = u.breakdown;
  if (!b) {
    console.log(`\n${u.unitId}: NO BREAKDOWN`);
    continue;
  }

  console.log(`\n${u.unitId} (${unit.techBase}, ${unit.tonnage}t)`);
  console.log(
    `  Reference BV: ${u.referenceBV}, Calculated: ${u.calculatedBV}, Gap: ${u.difference} (${u.percentDiff?.toFixed(1)}%)`,
  );
  ...
  console.log(
    `    (def + off) * cockpit = (${defBV} + ${offBV}) * ${cockpit} = ${total}`,
  );
  console.log(`    Actual calculated: ${u.calculatedBV}`);
  const gapFromFormula = u.calculatedBV - total;
  if (Math.abs(gapFromFormula) > 1)
    console.log(`    *** FORMULA MISMATCH: ${gapFromFormula} ***`);

  // Show what the reference implies for the offensive BV
  const impliedOff = Math.round(u.referenceBV / cockpit) - defBV;
  console.log(
    `    Implied offensiveBV for ref match: ${impliedOff} (actual: ${offBV}, delta: ${impliedOff - offBV})`,
  );
...
for (const u of wrong.filter((x: any) => x.percentDiff < 0)) {
  ...
  if (Math.abs(u.calculatedBV - total) > 2) {
    otherGap++;
    continue;
  }
