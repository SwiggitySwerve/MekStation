...
const MM_DATA_VERSION = process.argv.find(a => a.startsWith('--version='))?.split('=')[1] || 'v0.3.1';
...
function getCurrentAssets(): string[] {
  const result = execSync('find public/record-sheets -name "*.svg" -type f', { encoding: 'utf-8' });
  return result
    .trim()
    .split('\n')
...
async function main() {
  ...
  for (let i = 0; i < assets.length; i += BATCH_SIZE) {
    ...
    const results = await Promise.all(
      batch.map(async (asset) => {
        ...
        if (cdnResult.ok) {
          return { asset, success: true, cdnStatus: cdnResult.status, rawStatus: 0 };
        }

        // Fallback to GitHub raw
        const rawResult = await testUrl(rawUrl);
        if (rawResult.ok) {
          return { asset, success: true, cdnStatus: cdnResult.status, rawStatus: rawResult.status };
        }

        return { asset, success: false, cdnStatus: cdnResult.status, rawStatus: rawResult.status };
      })
    );
    ...
    for (const result of results) {
      if (result.success) {
        ...
        failures.push({ asset: result.asset, cdnStatus: result.cdnStatus, rawStatus: result.rawStatus });
      }
    }
  }
  ...
  console.log(`Results: ${passed} passed, ${failed} failed out of ${assets.length} total`);
  console.log('='.repeat(60));
