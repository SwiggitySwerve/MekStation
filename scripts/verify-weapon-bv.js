...
for (const id of targets) {
  ...
  for (const eq of weaponEqs) {
    ...
    if (direct) {
      ...
      console.log(
        `  ${eq.id.padEnd(35)} BV=${String(direct.bv).padStart(4)} (${direct.name}, ${direct.tb})`,
      );
    } else if (alt) {
      totalCatalogBV += alt.bv;
      console.log(
        `  ${eq.id.padEnd(35)} BV=${String(alt.bv).padStart(4)} (via ${stripped}: ${alt.name})`,
      );
    } else {
  ...
  console.log(
    `  Reported raw weapon BV: ${b.rawWeaponBV} (halved: ${b.halvedWeaponBV})`,
  );
  console.log(`  Reported final weapon BV: ${b.weaponBV}`);

  // Check ammo
  const ammoEqs = (unit.equipment || []).filter((e) =>
