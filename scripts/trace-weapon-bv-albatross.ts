...
for (const w of weapons) {
  ...
  console.log(
    `  Our BV: ${ourBV}, Expected: ${expectedBV}, Diff: ${diff > 0 ? '+' : ''}${diff}`,
  );
  console.log(
    `  Resolved: ${result.resolved}, Source: ${result.equipmentId || 'UNRESOLVED'}`,
  );
}
...
for (const ammo of ammoItems) {
  ...
  console.log(
    `${ammo}: BV=${result.battleValue}, resolved=${result.resolved}, id=${result.equipmentId || 'UNRESOLVED'}`,
  );
}
...
console.log(
  'Expected weight bonus (tonnage/2 rounded down): ' + Math.floor(95 / 2),
);
console.log('Formula: adjustedWeight = max(tonnage, unitBV/10)');
console.log('For BV ~1885, adjustedWeight = max(95, 188.5) = 188.5');
console.log(
  'Weight bonus = tonnage/2 (standard formula) = ' + Math.floor(95 / 2),
);
...
for (const id of alternateIds) {
  ...
  if (result.resolved) {
    console.log(
      `${id}: BV=${result.battleValue}, source=${result.equipmentId}`,
    );
  } else {
