...
for (const id of targets) {
  ...
  if (b) {
    console.log(
      `  DEFENSIVE: armor=${b.armorBV?.toFixed(1)} struct=${b.structureBV?.toFixed(1)} gyro=${b.gyroBV?.toFixed(1)} defEquip=${b.defEquipBV?.toFixed(1)} explosive=${b.explosivePenalty?.toFixed(1)}`,
    );
    console.log(
      `    factor=${b.defensiveFactor} tmm=${b.maxTMM} total=${b.defensiveBV?.toFixed(1)}`,
    );
    console.log(
      `  OFFENSIVE: weaponBV=${b.weaponBV?.toFixed(1)} raw=${b.rawWeaponBV?.toFixed(1)} halved=${b.halvedWeaponBV?.toFixed(1)}`,
    );
    console.log(
      `    ammo=${b.ammoBV?.toFixed(1)} weight=${b.weightBonus} phys=${b.physicalWeaponBV} offEquip=${b.offEquipBV}`,
    );
    console.log(
      `    heatEff=${b.heatEfficiency} heatDiss=${b.heatDissipation} moveHeat=${b.moveHeat}`,
    );
    console.log(
      `    speed=${b.speedFactor} total=${b.offensiveBV?.toFixed(1)}`,
    );
    ...
    console.log(
      `  GAP ANALYSIS: if defBV correct → offGap=${offGap.toFixed(1)} | if offBV correct → defGap=${defGap.toFixed(1)}`,
    );
  }
  console.log();
}
