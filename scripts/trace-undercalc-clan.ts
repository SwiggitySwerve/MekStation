...
const underClan = r.allResults
  .filter((x: any) => x.percentDiff < -1 && x.percentDiff >= -5 && x.breakdown)
  .map((res: any) => {
...
for (const u of underClan.slice(0, 10)) {
  ...
  console.log(
    `${u.unitId}: calc=${u.calculatedBV} indexBV=${u.indexBV} unitJsonBV=${unitBV} diff=${u.difference}`,
  );
...
const sorted = Object.entries(critEquip)
  .sort((a, b) => b[1] - a[1])
  .filter(([_, c]) => c >= 3);
...
const clanFF = underClan.filter(
  (u: any) =>
    u.unit.armor?.type?.toUpperCase().includes('FERRO') &&
    u.unit.techBase === 'CLAN',
);
console.log(`\n=== CLAN FF UNDERCALCULATED (${clanFF.length}) ===`);
let totalGap = 0;
for (const u of clanFF
  .sort((a: any, b: any) => a.difference - b.difference)
  .slice(0, 10)) {
  totalGap += u.difference;
  const b = u.breakdown;
  console.log(
    `${u.unitId}: gap=${u.difference} def=${b.defensiveBV.toFixed(0)} off=${b.offensiveBV.toFixed(0)} wep=${b.weaponBV} spd=${b.speedFactor}`,
  );
}
