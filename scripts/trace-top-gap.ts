...
for (const id of targets) {
  ...
  console.log(
    `  armorBV=${b.armorBV.toFixed(1)} structBV=${b.structureBV.toFixed(1)} gyroBV=${b.gyroBV.toFixed(1)} defEquip=${b.defensiveEquipBV.toFixed(1)} explPen=${b.explosivePenalty}`,
  );
  console.log(
    `  defFactor=${b.defensiveFactor.toFixed(2)} baseDef=${baseDef.toFixed(1)} gap=${gap.toFixed(1)}`,
  );
