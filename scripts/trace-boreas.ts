...
const boreas = report.allResults.filter((r: any) =>
  r.unitId.startsWith('boreas'),
);
...
for (const r of boreas) {
  ...
  if (b) {
    console.log(
      `  defBV=${b.defensiveBV?.toFixed(2)} offBV=${b.offensiveBV?.toFixed(2)}`,
    );
    console.log(
      `  weaponBV=${b.weaponBV?.toFixed(2)} ammoBV=${b.ammoBV?.toFixed(2)}`,
    );
    console.log(
      `  speedFactor=${b.speedFactor?.toFixed(4)} defEquipBV=${b.defensiveEquipBV} explosivePen=${b.explosivePenalty}`,
    );
  }
}
...
if (!boreEntry) {
  console.log('Boreas-D not found');
  process.exit(1);
}
const unit = JSON.parse(
  fs.readFileSync(path.join(unitsDir, boreEntry.path), 'utf8'),
);
...
// TMM
const tmmTable: Record<number, number> = {
  0: -4,
  1: -2,
  2: 0,
  3: 1,
...
if (boreResult) {
  ...
  console.log(
    `Our offBV: ${offBV.toFixed(2)} (raw: ${rawOff.toFixed(2)} × SF ${sf})`,
  );
  console.log(
    `Expected offBV: ${neededOff.toFixed(2)} (raw: ${neededRawOff.toFixed(2)} × SF ${sf})`,
  );
  ...
  console.log(
    `  known raw = weaponBV + ammoBV + physicalBV + weightBonus + offEquipBV`,
  );
  console.log(
    `  ${rawOff.toFixed(2)} = ${b.weaponBV} + ${b.ammoBV} + physBV + ${unit.tonnage} + offEquipBV`,
  );
  console.log(
    `  physBV + offEquipBV = ${(rawOff - b.weaponBV - b.ammoBV - unit.tonnage).toFixed(2)}`,
  );
  console.log(
    `  needed physBV + offEquipBV = ${(neededRawOff - b.weaponBV - b.ammoBV - unit.tonnage).toFixed(2)}`,
  );
}
...
console.log(
  `Engine rating: ${engineRating} → ${builtInHS} built-in HS capacity`,
);
...
console.log(
  `HS type: ${hsType} → ${dissPerHS} dissipation each → total ${totalDiss}`,
);
...
console.log(
  `heatEfficiency: 6 + ${totalDiss} - ${moveHeat} = ${heatEfficiency}`,
);
console.log(
...
console.log(
  `If heatDissipation = totalDiss = ${totalDiss}, then 6 + ${totalDiss} - ${moveHeat} = ${6 + totalDiss - moveHeat}`,
);
console.log(
  `MegaMek uses: heatEfficiency = 6 + entity.getHeatCapacity() - movementHeat`,
);
