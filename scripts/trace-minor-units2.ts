...
if (bkEntry?.path) {
  const d = JSON.parse(
    fs.readFileSync(
      path.join('public/data/units/battlemechs', bkEntry.path),
      'utf8',
    ),
  );
  ...
  console.log(
    'Equipment:',
    d.equipment.map((e: any) => `${e.id}@${e.location}`).join(', '),
  );
...
for (const u of minor) {
  ...
  try {
    ...
    const hasAmmo = d.equipment.some((e: any) =>
