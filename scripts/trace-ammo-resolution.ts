...
for (const uid of traceUnits) {
  ...
  const result =
    under.find((r: any) => r.unitId === uid) ||
    valid.find((r: any) => r.unitId === uid);
  if (!unit || !result) {
    console.log(`\n${uid}: NOT FOUND`);
    continue;
  }
  ...
  console.log(`\n${'='.repeat(60)}`);
  console.log(
    `${uid} (${unit.techBase}, ${unit.tonnage}t) gap=${result.difference} (${result.percentDiff?.toFixed(1)}%)`,
  );
  console.log(
    `  ammoBV=${b?.ammoBV ?? 'N/A'} weaponBV=${b?.weaponBV?.toFixed(0) ?? 'N/A'}`,
  );

  // List all ammo from equipment
  const equipAmmo = unit.equipment.filter((eq: any) =>
  ...
  if (equipAmmo.length > 0) {
    ...
    for (const eq of equipAmmo) {
      ...
      console.log(
        `    ${eq.id} @${eq.location} → bv=${res.battleValue} wt=${res.weaponType} ${res.resolved ? 'OK' : 'UNRESOLVED'}`,
      );
    }
  } else {
    ...
    for (const [loc, slots] of Object.entries(unit.criticalSlots)) {
      ...
      for (const s of slots) {
        ...
        const clean = (s as string)
          ...
          .replace(/\s*\(OMNIPOD\)/gi, '')
          .trim();
        const norm = normalizeEquipmentId(
          clean
            .toLowerCase()
            .replace(/\s+/g, '-')
            .replace(/[^a-z0-9-]/g, ''),
        ...
        console.log(
          `    [${loc}] "${clean}" → norm="${norm}" bv=${res.battleValue} wt=${res.weaponType} ${marker}`,
        );
        totalCritAmmo++;
      }
    }
...
console.log(
  `Resolved: ${totalResolved}, Failed: ${totalFailed} (${((totalFailed / (totalResolved + totalFailed)) * 100).toFixed(1)}%)`,
);
