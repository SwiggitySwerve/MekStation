...
const ARMOR_BV_PER_POINT: Record<string, number> = {
  HEAD: 3.5,
  CENTER_TORSO: 2.0,
  CENTER_TORSO_REAR: 2.5,
  LEFT_TORSO: 1.5,
  LEFT_TORSO_REAR: 2.0,
  RIGHT_TORSO: 1.5,
  RIGHT_TORSO_REAR: 2.0,
  LEFT_ARM: 1.5,
  RIGHT_ARM: 1.5,
  LEFT_LEG: 1.0,
  RIGHT_LEG: 1.0,
...
function calcStructBV(unit: any): number {
  ...
  const sp =
    STRUCTURE_POINTS_TABLE[tonnage as keyof typeof STRUCTURE_POINTS_TABLE];
  if (!sp) return 0;
  let total = 0;
  ...
  let structMult = 1.0;
  if (structType.includes('endo') && structType.includes('composite'))
    structMult = 1.0;
...
let armorDiffTotal = 0,
  armorDiffCount = 0;
let structDiffTotal = 0,
  structDiffCount = 0;
let gyroDiffCount = 0;
...
console.log(
  `Armor BV mismatches: ${armorDiffCount}/${over1to5.length} (avg diff: ${(armorDiffTotal / Math.max(armorDiffCount, 1)).toFixed(1)})`,
);
for (const d of armorDiffs
  .sort((a, b) => Math.abs(b.diff) - Math.abs(a.diff))
  .slice(0, 10)) {
  console.log(
    `  ${d.uid.padEnd(45)} +${d.pct.toFixed(1)}%  armor: our=${d.ourArmor.toFixed(1)} vs calc=${d.theirArmor.toFixed(1)} diff=${d.diff.toFixed(1)}`,
  );
}
...
for (const d of structDiffs
  ...
  .slice(0, 10)) {
  console.log(
    `  ${d.uid.padEnd(45)} +${d.pct.toFixed(1)}%  struct: our=${d.ourStruct.toFixed(1)} vs calc=${d.theirStruct.toFixed(1)} diff=${d.diff.toFixed(1)}`,
  );
}

// Also check undercalculated - are any components consistently wrong?
const under1to5 = valid.filter(
  (x: any) => x.percentDiff < -1 && x.percentDiff >= -5 && x.breakdown,
);
console.log(`\n=== UNDERCALCULATED 1-5% (${under1to5.length} units) ===`);

let underArmorDiffCount = 0,
  underStructDiffCount = 0;
...
console.log(
  `Struct BV mismatches: ${underStructDiffCount}/${under1to5.length}`,
);
for (const d of underArmorDiffs
  .sort((a, b) => Math.abs(b.diff) - Math.abs(a.diff))
  .slice(0, 10)) {
  console.log(
    `  ${d.uid.padEnd(45)} ${d.pct.toFixed(1)}%  armor: our=${d.ourArmor.toFixed(1)} vs calc=${d.theirArmor.toFixed(1)} diff=${d.diff.toFixed(1)}`,
...
console.log(
  `Rounding mismatches (rawBV != calcBV): ${totalRoundDiff}/${allWithBreakdown.length}`,
);
