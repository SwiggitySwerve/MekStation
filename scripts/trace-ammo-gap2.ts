...
function hasAmmoCritSlots(unitId: string): {
  has: boolean;
  count: number;
  names: string[];
} {
...
const accurate = report.allResults.filter(
...
for (const r of undercalc) {
  ...
  if (ammoBV === 0 && ammo.has) {
    ucAmmo0WithCrits++;
    ucAmmo0WithCritsList.push({
      id: r.unitId,
      gap: r.indexBV - r.calculatedBV,
      ammoSlots: ammo.count,
      ammoNames: ammo.names,
    });
...
console.log(
  `  ammoBV=0, has ammo crits:    ${ucAmmo0WithCrits}  <-- POTENTIAL BUG`,
);
console.log(
  `  ammoBV=0, no ammo crits:     ${ucAmmo0NoCrits}  (energy-only units, expected)`,
);
...
if (ucAmmo0WithCritsList.length > 0) {
  ...
  for (const u of ucAmmo0WithCritsList.slice(0, 30)) {
    console.log(
      `    ${u.id.padEnd(40).slice(0, 40)} gap=${String(u.gap).padStart(5)} slots=${u.ammoSlots} [${u.ammoNames.slice(0, 3).join(', ')}]`,
    );
  }
}
...
const ammoDistribution: { withAmmo: number[]; withoutAmmo: number[] } = {
  withAmmo: [],
  withoutAmmo: [],
};
for (const r of undercalc) {
...
console.log(
  `  WITH resolved ammoBV:  avg=${(ammoDistribution.withAmmo.reduce((s, v) => s + v, 0) / ammoDistribution.withAmmo.length).toFixed(1)} (n=${ammoDistribution.withAmmo.length})`,
);
console.log(
  `  WITHOUT ammoBV (all):  avg=${(ammoDistribution.withoutAmmo.reduce((s, v) => s + v, 0) / ammoDistribution.withoutAmmo.length).toFixed(1)} (n=${ammoDistribution.withoutAmmo.length})`,
);
...
if (ucAmmo0WithCritsList.length > 0) {
  ...
  console.log(
    `Estimated % of gap explained: ${((totalEstimatedMissing / totalGapForThose) * 100).toFixed(1)}%`,
  );
}
