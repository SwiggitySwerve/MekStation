...
console.log(
  `Units under-calculated by 1-5% with breakdown: ${within1to5under.length}`,
);
...
for (const [b, c] of Object.entries(sfBuckets).sort())
  console.log(`  ${b}: ${c}`);

console.log(`\n=== Checking specific units for patterns ===`);
const samples = within1to5under
  .sort((a: Result, b: Result) => a.percentDiff - b.percentDiff)
  .slice(0, 15);
for (const r of samples) {
  const b = r.breakdown!;
  console.log(
    `${(r.chassis + ' ' + r.model).padEnd(40)} idx=${r.indexBV} calc=${r.calculatedBV} diff=${r.difference} (${r.percentDiff.toFixed(1)}%)`,
  );
  console.log(
    `  def=${b.defensiveBV.toFixed(0)} off=${b.offensiveBV.toFixed(0)} wpn=${b.weaponBV.toFixed(0)} ammo=${b.ammoBV.toFixed(0)} sf=${b.speedFactor.toFixed(3)} expl=${b.explosivePenalty.toFixed(0)} defEq=${b.defensiveEquipBV.toFixed(0)}`,
  );
}

console.log(`\n=== Checking exact match units for comparison ===`);
const exactUnits = report.allResults
  .filter((r: Result) => r.status === 'exact' && r.breakdown)
  .slice(0, 5);
for (const r of exactUnits) {
  const b = r.breakdown!;
  console.log(
    `${(r.chassis + ' ' + r.model).padEnd(40)} idx=${r.indexBV} calc=${r.calculatedBV}`,
  );
  console.log(
    `  def=${b.defensiveBV.toFixed(0)} off=${b.offensiveBV.toFixed(0)} wpn=${b.weaponBV.toFixed(0)} ammo=${b.ammoBV.toFixed(0)} sf=${b.speedFactor.toFixed(3)} expl=${b.explosivePenalty.toFixed(0)} defEq=${b.defensiveEquipBV.toFixed(0)}`,
  );
...
for (const r of within1to5under.slice(0, 50)) {
  ...
  if (
    Math.abs(impliedMod - 1.0) > 0.001 ||
    cockpit.toUpperCase() !== 'STANDARD'
  ) {
    console.log(
      `  ${(r.chassis + ' ' + r.model).padEnd(40)} cockpit=${cockpit} impliedMod=${impliedMod.toFixed(4)} baseBV=${baseBV.toFixed(0)} calcBV=${r.calculatedBV}`,
    );
  }
}
