...
for (const u of outside) {
  ...
  console.log(
    'Equipment:',
    data.equipment.map((e) => `${e.id}@${e.location}`).join(', '),
  );
  ...
  // Count MGA
  const mgaSlots = allCrits.filter(
    (c) =>
      c.name.toLowerCase().includes('machine gun array') ||
      c.name.toLowerCase().includes('mga'),
  );
  if (mgaSlots.length > 0)
    console.log('  MGA:', mgaSlots.map((c) => c.name + '@' + c.loc).join(', '));

  // Count MGs
  const mgSlots = allCrits.filter((c) => {
    const lo = c.name.toLowerCase();
    return (
      (lo.includes('machine gun') ||
        lo === 'clmg' ||
        lo === 'ismg' ||
        lo === 'islightmg' ||
        lo === 'cllightmg' ||
  ...
  if (mgSlots.length > 0)
    console.log(
      ...
      mgSlots.length,
      'items:',
      [...new Set(mgSlots.map((c) => c.name))].join(', '),
    );

  // Check MASC/SC
  const masc = allCrits.filter(
    (c) =>
      c.name.toLowerCase().includes('masc') ||
      c.name.toLowerCase().includes('supercharger'),
  );
  if (masc.length > 0)
    console.log('  MASC/SC:', masc.map((c) => c.name).join(', '));
  ...
  const weapCrits = allCrits.filter((c) => {
    ...
    return (
      ...
      !lo.includes('clan ferro') &&
      !lo.includes('isenhancedsensor') &&
      !lo.includes('isangel') &&
      !lo.includes('bloodhound') &&
      !lo.includes('guardianecm') &&
      !lo.includes('angelecm') &&
