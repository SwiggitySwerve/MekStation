...
function parseRulesLevel(content, techBase) {
  ...
  if (
    upper.includes('.SETINTROLEVEL(TRUE)') ||
    upper.includes('INTRODUCTORY')
  ) {
    return 'INTRODUCTORY';
  }
...
function extractWeaponData(filePath, content) {
  ...
  if (classNameMatch) {
    ...
    if (className.startsWith('CL') || className.startsWith('Clan')) {
      ...
      className.startsWith('InnerSphere')
    ) {
  ...
  if (
    filePathLower.includes('laser') ||
    filePathLower.includes('ppc') ||
    filePathLower.includes('flamer') ||
    filePathLower.includes('plasma')
  ) {
    ...
    filePathLower.includes('autocannon') ||
    filePathLower.includes('gauss') ||
    filePathLower.includes('rifle') ||
    filePathLower.includes('machine') ||
    filePathLower.includes('mg')
  ) {
    weapon.category = 'Ballistic';
    if (filePathLower.includes('autocannon') || filePathLower.includes('ac'))
      weapon.subType = 'Autocannon';
    else if (filePathLower.includes('gauss')) weapon.subType = 'Gauss';
    else if (filePathLower.includes('rifle')) weapon.subType = 'Rifle';
    else if (filePathLower.includes('machine') || filePathLower.includes('mg'))
      weapon.subType = 'Machine Gun';
  } else if (
    filePathLower.includes('lrm') ||
    filePathLower.includes('srm') ||
    filePathLower.includes('mrm') ||
    filePathLower.includes('atm') ||
    filePathLower.includes('narc') ||
    filePathLower.includes('missile')
  ) {
...
function extractAmmunition(content) {
  ...
  const createMethodPattern =
    /private\s+static\s+AmmoType\s+create\w+\(\)\s*\{([^}]+(?:\{[^}]*\}[^}]*)*)\}/g;
  let match;
  ...
  while ((match = createMethodPattern.exec(content)) !== null) {
    ...
    const internalNameMatch = methodContent.match(
      /setInternalName\("([^"]+)"\)/,
    );
    if (internalNameMatch) {
      const internalName = internalNameMatch[1];
      ammo.id = generateId(
        internalName.replace(/^IS|^CL/, '').replace(/\s+Ammo$/, ''),
      );
    ...
    const ammoTypeMatch = methodContent.match(
      /ammo\.ammoType\s*=\s*AmmoTypeEnum\.(\w+)/,
    );
    if (ammoTypeMatch) {
      ...
      if (ammoTypeEnum.includes('AC') || ammoTypeEnum.includes('AUTOCANNON')) {
        ...
        ammoTypeEnum.includes('MG') ||
        ammoTypeEnum.includes('MACHINE')
      ) {
        ...
        ammoTypeEnum.includes('ARTILLERY') ||
        ammoTypeEnum.includes('ARROW') ||
        ammoTypeEnum.includes('SNIPER') ||
        ammoTypeEnum.includes('THUMPER') ||
        ammoTypeEnum.includes('LONG_TOM')
      ) {
    ...
    if (nameLower.includes('armor-piercing') || nameLower.includes('ap')) {
      ...
      nameLower.includes('fragmentation') ||
      ...
      nameLower.includes('high explosive') ||
      nameLower.includes('he')
    ) {
    ...
    if (
      nameLower.includes('non-explosive') ||
      nameLower.includes('nonexplosive')
    ) {
...
function extractMiscEquipment(content) {
  ...
  const createMethodPattern =
    /public\s+static\s+MiscType\s+create\w+\(\)\s*\{([^}]+(?:\{[^}]*\}[^}]*)*)\}/g;
  let match;
  ...
  while ((match = createMethodPattern.exec(content)) !== null) {
    ...
    const tonnageMatch = methodContent.match(
      /tonnage\s*=\s*([\d.]+|TONNAGE_VARIABLE)/,
    );
    if (tonnageMatch) {
    ...
    // Extract critical slots
    const critMatch = methodContent.match(
      /criticalSlots\s*=\s*(\d+|CRITICAL_SLOTS_VARIABLE)/,
    );
    if (critMatch) {
    ...
    if (methodContent.includes('F_TARGETING_COMPUTER')) {
      ...
      methodContent.includes('F_ECM') ||
      methodContent.includes('F_ANGEL_ECM')
    ) {
      misc.category = 'ECM';
    } else if (
      methodContent.includes('F_BAP') ||
      methodContent.includes('F_LIGHT_ACTIVE_PROBE')
    ) {
      misc.category = 'Active Probe';
    } else if (
      methodContent.includes('F_C3') ||
      methodContent.includes('F_C3I') ||
      methodContent.includes('F_C3S')
    ) {
...
function extractEquipment() {
  ...
  console.log(
    `  Ballistic Weapons: ${extractedEquipment.weapons.ballistic.length}`,
  );
  console.log(
    `  Missile Weapons: ${extractedEquipment.weapons.missile.length}`,
  );
  console.log(
    `  Physical Weapons: ${extractedEquipment.weapons.physical.length}`,
  );
      ...
      try {
        ...
        if (
          ...
          (content.includes('Weapon') ||
            content.includes('LaserWeapon') ||
            content.includes('PPCWeapon'))
        ) {
...
function writeOutputFiles() {
  ...
  const electronics = extractedEquipment.miscellaneous.filter(
  ...
  const miscOnly = extractedEquipment.miscellaneous.filter(
    (item) => item.category === 'Miscellaneous',
  );
