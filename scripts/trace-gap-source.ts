...
for (const r of sf1Units.slice(0, 10)) {
  ...
  console.log(
    `${r.unitId}: expected=${r.indexBV} calc=${r.calculatedBV} gap=${r.difference}`,
  );
  console.log(
    `  def=${b.defensiveBV.toFixed(0)} off=${b.offensiveBV.toFixed(0)} weapBV=${b.weaponBV} ammo=${b.ammoBV}`,
  );
  ...
  console.log(
    `  If off: needs ${offNeeded.toFixed(0)}, have ${b.offensiveBV.toFixed(0)}, gap=${offGap.toFixed(0)}`,
  );
  console.log(
    `  If def: needs ${defNeeded.toFixed(0)}, have ${b.defensiveBV.toFixed(0)}, gap=${defGap.toFixed(0)}`,
  );
...
console.log(
  `Avg defensive gap needed: ${(defGapSum / Math.min(sf1Units.length, 10)).toFixed(1)}`,
);
...
const bySF: Record<
  string,
  { count: number; avgGap: number; avgBaseGap: number; avgOff: number }
> = {};
...
for (const [sf, d] of Object.entries(bySF).sort(
  (a, b) => parseFloat(a[0]) - parseFloat(b[0]),
)) {
  console.log(
    `SF=${sf}: n=${d.count}, avgGap=${(d.avgGap / d.count).toFixed(1)}, avgBaseGap=${(d.avgBaseGap / d.count).toFixed(1)}, avgOff=${(d.avgOff / d.count).toFixed(0)}`,
  );
}
...
console.log(
  `If baseGap CV is lower, the gap is primarily in the offensive BV pipeline (amplified by SF)`,
);
console.log(
  `If rawGap CV is lower, the gap is primarily in the defensive BV pipeline (not affected by SF)`,
);
