...
const outside5 = report.allResults.filter(
  (r: any) => Math.abs(r.percentDiff) > 5,
);
...
for (const r of outside5
  ...
  console.log(
    `  cockpit=${bd.cockpitType} sf=${bd.speedFactor} defBV=${bd.defensiveBV?.toFixed(0)} offBV=${bd.offensiveBV?.toFixed(0)}`,
  );
  console.log(
    `  weapBV=${bd.weaponBV?.toFixed(0)} rawWeapBV=${bd.rawWeaponBV?.toFixed(0)} halved=${bd.halvedWeaponBV?.toFixed(0)} ammoBV=${bd.ammoBV?.toFixed(0)} explPen=${bd.explosivePenalty}`,
  );
