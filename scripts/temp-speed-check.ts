...
const under1to3 = report.allResults.filter(
  (r: Result) =>
    r.status !== 'error' &&
    r.percentDiff !== null &&
    r.percentDiff < -1 &&
...
for (const [sf, data] of Object.entries(sfGroups).sort(
...
for (const [sf, count] of Object.entries(exactSF).sort(
  (a, b) => Number(a[0]) - Number(b[0]),
)) {
  if (count < 3) continue;
      ...
      (slots: any) =>
        ...
        slots.some(
          (s: any) =>
            s &&
            typeof s === 'string' &&
            s.toLowerCase().includes('partial wing'),
        ),
    );
  if (hasPartialWing) {
    partialWingCount++;
    console.log(
      `  PW: ${r.chassis} ${r.model} sf=${r.breakdown!.speedFactor} walk=${ud.movement.walk} jump=${ud.movement.jump || 0} diff=${r.percentDiff.toFixed(1)}%`,
    );
  }
}
  ...
  const hasUMU =
    ...
    Object.values(ud.criticalSlots).some(
      (slots: any) =>
        ...
        slots.some(
          (s: any) =>
            s && typeof s === 'string' && s.toLowerCase().includes('umu'),
        ),
    );
  if (hasUMU) {
    umuCount++;
    console.log(
      `  UMU: ${r.chassis} ${r.model} sf=${r.breakdown!.speedFactor} diff=${r.percentDiff.toFixed(1)}%`,
    );
  }
}
