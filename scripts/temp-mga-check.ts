...
for (const iu of indexData.units) {
  ...
  if (hasMGAEquip || hasMGACrit) {
    ...
    const mgaEquips = (ud.equipment || [])
      .filter((eq: any) => {
        const lo = eq.id.toLowerCase();
        return (
          lo.includes('machine-gun-array') || lo === 'ismga' || lo === 'clmga'
        );
      })
      .map((eq: any) => `${eq.id}@${eq.location}`);

    const mgEquips = (ud.equipment || [])
      .filter((eq: any) => {
        const lo = eq.id.toLowerCase();
        return (
    ...
    console.log(
      `${iu.chassis} ${iu.model}: crits=[${mgaCrits.join('; ')}] equip=[${mgaEquips.join('; ')}] mgs=[${mgEquips.join('; ')}]`,
    );
  }
}
