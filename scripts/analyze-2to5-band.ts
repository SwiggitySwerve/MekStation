...
// Units in 2-5% band
const underBand = valid.filter(
  (x: any) => x.percentDiff >= -5 && x.percentDiff < -2,
);
...
console.log(
  `=== 2-5% BAND: ${underBand.length} under, ${overBand.length} over ===\n`,
);
...
for (const u of overBand.sort(
  ...
  if (allCrits.some((c) => c.includes('null') && c.includes('sig')))
    tags.push('null-sig');
  if (allCrits.some((c) => c.includes('void') && c.includes('sig')))
    tags.push('void-sig');
  ...
  if (overBand.indexOf(u) < 25)
    console.log(
      `  ${u.unitId.padEnd(42)} +${u.percentDiff.toFixed(1)}% diff=+${u.difference} ref=${u.indexBV} calc=${u.calculatedBV} SF=${b.speedFactor} [${tags.join(',')}]`,
    );
}
console.log(
...
for (const u of underBand.sort(
  ...
  if (underBand.indexOf(u) < 25)
    console.log(
      `  ${u.unitId.padEnd(42)} ${u.percentDiff.toFixed(1)}% diff=${u.difference} weap=${b.weaponBV?.toFixed(0)} ammo=${b.ammoBV} HE=${b.heatEfficiency} [${tags.join(',')}]`,
    );
...
for (const u of overBand) {
  ...
  for (const [, slots] of Object.entries(unit.criticalSlots)) {
    if (Array.isArray(slots))
      for (const s of slots as string[]) if (s) allCrits.push(s.toLowerCase());
  }
...
console.log(
  `  Stealth=${stealthCount}, VoidSig=${voidCount}, NullSig=${nullCount}, Chameleon=${chameleonCount}`,
);
