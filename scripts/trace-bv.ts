...
console.log(
  `Reference BV: ${refBV} (source: ${mulEntry?.matchType === 'exact' ? 'MUL' : 'index'})`,
);
...
const engineType = ud.engine.type.toUpperCase().includes('XL')
  ? ud.techBase === 'CLAN'
    ? EngineType.XL_CLAN
    : EngineType.XL_IS
  : EngineType.STANDARD;
...
console.log(
  `Armor BV: ${totalArmor} * 2.5 * ${armorMult} = ${totalArmor * 2.5 * armorMult}`,
);

// Structure
const structureMap: Record<number, number> = {
  10: 11,
  15: 16,
  20: 21,
  25: 26,
...
for (const eq of ud.equipment || []) {
  ...
  console.log(
    `  Weapon: "${eq.id}" @ ${eq.location} -> BV=${res.battleValue} Heat=${res.heat}`,
  );
  weapons.push({
    id: normalizeEquipmentId(eq.id),
    name: lo,
    heat: res.heat,
    bv: res.battleValue,
  });
...
const offResult = calculateOffensiveBVWithHeatTracking({
  ...
  engineType,
  heatSinkCount: ud.heatSinks.count,
  jumpHeatMP: jumpMP,
});
...
const totalBV = Math.round(
  (defResult.totalDefensiveBV + offResult.totalOffensiveBV) * cockpitMod,
);
console.log(
...
console.log(
  `Difference: ${totalBV - refBV} (${(((totalBV - refBV) / refBV) * 100).toFixed(1)}%)`,
);
