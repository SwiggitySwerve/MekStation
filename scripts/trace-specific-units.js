...
for (const id of targets) {
  ...
  if (!unit) {
    console.log('UNIT NOT LOADED: ' + id);
    continue;
  }
  const b = u.breakdown || {};
  ...
  console.log(
    `  MUL BV: ${u.indexBV}  Calc BV: ${u.calculatedBV}  Gap: ${u.difference} (${(u.percentDiff || 0).toFixed(2)}%)`,
  );
  ...
  console.log(
    `  defEquip=${b.defEquipBV} armoredComp=${b.armoredComponentBV} harjel=${b.harjelBonus} amsAmmo=${b.amsAmmoBV}`,
  );
  console.log(
    `  explosive=${b.explosivePenalty} defFactor=${b.defensiveFactor} maxTMM=${b.maxTMM}`,
  );
  console.log(`  TOTAL DEF: ${b.defensiveBV}`);
  console.log(`  --- OFFENSIVE ---`);
  console.log(
    `  weaponBV=${b.weaponBV} rawWeaponBV=${b.rawWeaponBV} halvedBV=${b.halvedWeaponBV} halvedCount=${b.halvedWeaponCount}`,
  );
  console.log(
    `  ammoBV=${b.ammoBV} physBV=${b.physicalWeaponBV} weightBonus=${b.weightBonus} offEquip=${b.offEquipBV}`,
  );
  console.log(
  ...
  console.log(
    `  Tech: ${unit.techBase} Config: ${unit.configuration} Tonnage: ${unit.tonnage}`,
  );
  console.log(
    `  Armor: ${unit.armorType || unit.armor?.type} Structure: ${unit.structureType || unit.structure?.type}`,
  );
  console.log(
    `  Cockpit: ${unit.cockpit} HS: ${unit.heatSinks?.count}×${unit.heatSinks?.type}`,
  );
  ...
  const crits = unit.criticalSlots
    ? Object.values(unit.criticalSlots).flat().filter(Boolean)
    : [];
  const specials = crits.filter(
    (s) =>
      typeof s === 'string' &&
      !/^(engine|endo|ferro|fusion|gyro|hip|upper|lower|hand|foot|shoulder|heat|life|sensors|cockpit)/i.test(
        s,
      ),
  );
  ...
  const verifyOff =
    ((b.weaponBV || 0) +
      ...
      (b.offEquipBV || 0)) *
    b.speedFactor;
  const verifyTotal = Math.round(
    (verifyDef + verifyOff) * (b.cockpitModifier || 1),
  );
  console.log(
    `  Verify: def=${verifyDef.toFixed(1)} off=${verifyOff.toFixed(1)} total=${verifyTotal} (calc=${u.calculatedBV})`,
  );

  // Back-calculate: what defensive or offensive would make it match?
  const neededTotal = u.indexBV;
  const neededDefGap =
    neededTotal / (b.cockpitModifier || 1) - verifyOff - verifyDef;
  console.log(
    `  Gap attribution: need +${neededDefGap.toFixed(1)} BV somewhere`,
  );
}
