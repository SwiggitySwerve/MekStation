...
for (const t of targets) {
  ...
  console.log(
    `  Ref BV: ${rr.indexBV}, Calc: ${rr.calculatedBV}, Diff: ${rr.difference} (${rr.percentDiff.toFixed(2)}%)`,
  );

  const bd = rr.breakdown;
  console.log(
    `  Breakdown: def=${bd.defensiveBV.toFixed(1)} off=${bd.offensiveBV.toFixed(1)}`,
  );
  console.log(
    `  WeapBV=${bd.weaponBV.toFixed(1)} AmmoBV=${bd.ammoBV.toFixed(1)} SF=${bd.speedFactor.toFixed(4)}`,
  );
  ...
  console.log(
    `  Missing total: ${missingTotal}, missing base (÷SF): ${missingBase.toFixed(1)}`,
  );
  ...
  if (rearWeaps.length > 0) {
    console.log(
      `  ** Rear weapons: ${rearWeaps.map((e: any) => e.id).join(', ')}`,
    );
  }
  ...
  if (ud.criticalSlots) {
    ...
    for (const slots of Object.values(ud.criticalSlots)) {
      if (Array.isArray(slots))
        for (const s of slots)
          if (s && typeof s === 'string') critItems.push(s);
    }
    ...
    if (interesting.length > 0) {
      console.log(
        `  ** Interesting crits: ${[...new Set(interesting)].join(', ')}`,
      );
    }
  ...
  for (const eq of ud.equipment) {
    const res = resolveEquipmentBV(eq.id);
    console.log(
      `    ${eq.id.padEnd(35)} BV=${String(res.battleValue).padStart(4)} heat=${res.heat} loc=${eq.location}${eq.rear ? ' [REAR]' : ''}`,
    );
  }
}
